...
// TestRollupOutputFirstSortedParity mirrors the rollup-output-first-sorted
// parity scenario (Java ResultSetOutputFirstSorted{join=false}): the same
// output-first rollup sequence with order by c0 then c1.
func TestRollupOutputFirstSortedParity(t *testing.T) {
	env := NewEnvironment()
