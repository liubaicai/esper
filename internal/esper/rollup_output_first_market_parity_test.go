...
// TestRollupOutputFirstMarketParity mirrors the rollup-output-first-market
// parity scenario (Java ResultSet5OutputLimitFirst): a 5.5s SupportMarketDataBean
// window with rollup(symbol) and output first every second.
func TestRollupOutputFirstMarketParity(t *testing.T) {
	env := NewEnvironment()
	...
	if len(batches) != 11 {
		t.Fatalf("batches = %d, want 11", len(batches))
	}
	want := [][][][2]any{
		{
			{{"IBM", 25.0}, {nil, 25.0}},
