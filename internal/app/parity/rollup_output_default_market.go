...
const rollupOutputDefaultMarketJavaCommit = "9e1b9f1cc9117fea4bf33ab043762c045d73839c"

var rollupOutputDefaultMarketJavaSources = []string{
...
var (
	rollupOutputDefaultMarketJavaRuntimeIDs = []string{
		"java-runtime-16db4dbf1209b932f029",
	}
	rollupOutputDefaultMarketJavaExecutions = []string{
		"ResultSet2OutputLimitDefault",
	}
)

func runRollupOutputDefaultMarketScenario(ctx context.Context, scenario compat.Scenario) (compat.Trace, error) {
	if err := scenario.Validate(); err != nil {
		return compat.Trace{}, err
	}
	caseName := "default-market"
	if !scenarioHasCase(scenario, caseName) {
		return compat.Trace{}, fmt.Errorf("rollup-output-default-market scenario %q has no supported cases", scenario.ID)
	}
	...
	return compat.ReplayWithStatements(ctx, engine, statement, caseScenario, decodeResultSetGroupedTimeWindowPayload, func(name string) (*esper.Statement, error) {
		if name != statement.Name() {
			return nil, fmt.Errorf("unknown rollup-output-default-market statement %q", name)
		}
