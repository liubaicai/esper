...
const rollupOutputAllJavaCommit = "9e1b9f1cc9117fea4bf33ab043762c045d73839c"

var rollupOutputAllJavaSources = []string{
...
var (
	rollupOutputAllJavaRuntimeIDs = []string{
		"java-runtime-8bc3878f4730fbf2fd21",
	}
	rollupOutputAllJavaExecutions = []string{
		"ResultSetOutputAll{join=false}",
	}
)

func runRollupOutputAllScenario(ctx context.Context, scenario compat.Scenario) (compat.Trace, error) {
	if err := scenario.Validate(); err != nil {
		return compat.Trace{}, err
	}
	caseName := "all"
	if !scenarioHasCase(scenario, caseName) {
		return compat.Trace{}, fmt.Errorf("rollup-output-all scenario %q has no supported cases", scenario.ID)
	}
	...
	return compat.ReplayWithStatements(ctx, engine, statement, caseScenario, decodeRollupPayload, func(name string) (*esper.Statement, error) {
		if name != statement.Name() {
			return nil, fmt.Errorf("unknown rollup-output-all statement %q", name)
		}
