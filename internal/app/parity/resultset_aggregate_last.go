...
const resultsetAggregateLastJavaCommit = "9e1b9f1cc9117fea4bf33ab043762c045d73839c"

var resultsetAggregateLastJavaSources = []string{
...
var (
	resultsetAggregateLastJavaRuntimeIDs = []string{
		"java-runtime-1debb0e930f955641300",
	}
	resultsetAggregateLastJavaExecutions = []string{
		"ResultSetNoJoinLast",
	}
)

func runResultSetAggregateLastScenario(ctx context.Context, scenario compat.Scenario) (compat.Trace, error) {
	if err := scenario.Validate(); err != nil {
		return compat.Trace{}, err
	}
	caseName := "last-aggregate"
	if !scenarioHasCase(scenario, caseName) {
		return compat.Trace{}, fmt.Errorf("resultset-aggregate-last scenario %q has no supported cases", scenario.ID)
	}
	...
	return compat.ReplayWithStatements(ctx, engine, statement, caseScenario, decodeResultSetGroupedTimeWindowPayload, func(name string) (*esper.Statement, error) {
		if name != statement.Name() {
			return nil, fmt.Errorf("unknown resultset-aggregate-last statement %q", name)
		}
