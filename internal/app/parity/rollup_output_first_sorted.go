...
const rollupOutputFirstSortedJavaCommit = "9e1b9f1cc9117fea4bf33ab043762c045d73839c"

var rollupOutputFirstSortedJavaSources = []string{
...
var (
	rollupOutputFirstSortedJavaRuntimeIDs = []string{
		"java-runtime-b46b070a7b151e7b4ecc",
	}
	rollupOutputFirstSortedJavaExecutions = []string{
		"ResultSetOutputFirstSorted{join=false}",
	}
)

func runRollupOutputFirstSortedScenario(ctx context.Context, scenario compat.Scenario) (compat.Trace, error) {
	if err := scenario.Validate(); err != nil {
		return compat.Trace{}, err
	}
	caseName := "first-sorted"
	if !scenarioHasCase(scenario, caseName) {
		return compat.Trace{}, fmt.Errorf("rollup-output-first-sorted scenario %q has no supported cases", scenario.ID)
	}
	...
	return compat.ReplayWithStatements(ctx, engine, statement, caseScenario, decodeRollupPayload, func(name string) (*esper.Statement, error) {
		if name != statement.Name() {
			return nil, fmt.Errorf("unknown rollup-output-first-sorted statement %q", name)
		}
