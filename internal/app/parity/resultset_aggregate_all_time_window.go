...
const resultsetAggregateAllTimeWindowJavaCommit = "9e1b9f1cc9117fea4bf33ab043762c045d73839c"

var resultsetAggregateAllTimeWindowJavaSources = []string{
...
var (
	resultsetAggregateAllTimeWindowJavaRuntimeIDs = []string{
		"java-runtime-21f1ac2a389796ce96ea",
	}
	resultsetAggregateAllTimeWindowJavaExecutions = []string{
		"ResultSet9AllNoHavingNoJoin",
	}
)

// runResultSetAggregateAllTimeWindowScenario replays the grouped time-window
// sum with output all every 1 seconds ordered by symbol, including accumulated
// event rows, representative re-emission and removal old rows, matching
// ResultSet9AllNoHavingNoJoin.
func runResultSetAggregateAllTimeWindowScenario(ctx context.Context, scenario compat.Scenario) (compat.Trace, error) {
	if err := scenario.Validate(); err != nil {
		return compat.Trace{}, err
	}
	caseName := "all-time"
	if !scenarioHasCase(scenario, caseName) {
		return compat.Trace{}, fmt.Errorf("resultset-aggregate-all-time-window scenario %q has no supported cases", scenario.ID)
	}
	...
	return compat.ReplayWithStatements(ctx, engine, statement, caseScenario, decodeResultSetGroupedTimeWindowPayload, func(name string) (*esper.Statement, error) {
		if name != statement.Name() {
			return nil, fmt.Errorf("unknown resultset-aggregate-all-time-window statement %q", name)
		}
