...
const rollupOutputAllSortedJavaCommit = "9e1b9f1cc9117fea4bf33ab043762c045d73839c"

var rollupOutputAllSortedJavaSources = []string{
...
var (
	rollupOutputAllSortedJavaRuntimeIDs = []string{
		"java-runtime-569b2b4848133878bfdc",
	}
	rollupOutputAllSortedJavaExecutions = []string{
		"ResultSetOutputAllSorted{join=false}",
	}
)

func runRollupOutputAllSortedScenario(ctx context.Context, scenario compat.Scenario) (compat.Trace, error) {
	if err := scenario.Validate(); err != nil {
		return compat.Trace{}, err
	}
	caseName := "all-sorted"
	if !scenarioHasCase(scenario, caseName) {
		return compat.Trace{}, fmt.Errorf("rollup-output-all-sorted scenario %q has no supported cases", scenario.ID)
	}
	...
	return compat.ReplayWithStatements(ctx, engine, statement, caseScenario, decodeRollupPayload, func(name string) (*esper.Statement, error) {
		if name != statement.Name() {
			return nil, fmt.Errorf("unknown rollup-output-all-sorted statement %q", name)
		}
