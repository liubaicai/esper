...
## 0. 实时状态入口
> 最新补充：Draft 4.290（2026-08-31），新增 `resultset.aggregate-dimensional` 的 `rollup-grouping-funcs-dedicated` differential-verified 场景，对照固定 Java `ResultSetQueryTypeRollupGroupingFuncs.java` ordinal 0 的 `ResultSetQueryTypeDocSampleCarEventAndGroupingFunc`（runtime `java-runtime-52a4f853b6052fb47437`；static `java-1ae66c9985dc53282af0`；无 flags）：三个 fresh deployment compilation paths（plain、`@Audit`、`eplToModelCompileDeploy`）各发送两个 `SupportCarEvent`，Java/Go 各 6 条 listener records、0 differences。覆盖 `grouping sets((name, place), name, place, ())` 的 detail→name subtotal→place subtotal→overall 四行顺序、grouping bits、`grouping_id` 和 Null 投影；typed Go 使用 `GroupByGroupingSets`、`Sum`、`Grouping`、`GroupingID`，严格 scenario/oracle validator 固定 metadata、payload、values/order/time/record-shape，并拒绝 malformed fields、duplicate keys、payload errors 与 trace mutations。manifest 更新为 581 cases、194 个 differential-verified case、721 个 differential runtime IDs、3342 条 associations（referenced 3125）；capability 119 个（34 DV）。下一优先级为同一 Java suite ordinal 2 的 FAF named-window grouping snapshot。`
