...
- 最新进展：`rollup-output-snapshot-order-limit` 登记为 differential-verified（`ResultSetOutputSnapshotOrderWLimit`，1 条 record/0 differences）；`rollup-output-first-sorted`、`rollup-output-first`（同时修复 grouped irstream 纯时间淘汰的 New/Old 语义）、`rollup-output-every-sorted`、`rollup-output-last` 与 `rollup-output-last-sorted` 亦已登记（上一轮 Java oracle 误加 order-by 已修正；grouped `OutputLastEveryTime` 现在按输出 key 缓存上一周期行、新 group 首次 old 行输出 null 聚合占位，并按 Java rollup 层级输出 leaf → subtotal → grand total）。
- 最新已提交：`b7e8fb8a1`（Add rollup-output-snapshot-order-limit differential scenario）
...
| Capability | 110 个 |
| Case | 416 个 |
| Case implemented（verification） | 416 个（其中 29 个 differential-verified、18 个 intentionally-different） |
| Case differential-verified | 29 个（39 个 runtime） |
...
| 未关联 runtime | 1,506 个 |
| 关联覆盖率 | 63.6% |
| Representative scenario | 29/29 通过 |
| NFR | 0 个已验证 |
