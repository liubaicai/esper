...
- 最新进展：`resultset-aggregate-default` 登记为 differential-verified（`ResultSetNoJoinDefault`，2 条 records/0 differences）；`rollup-output-first-having`（grouped output-first-with-having 的 new/old 都交付当前聚合值）、`rollup-output-all-sorted`、`rollup-output-all`、`rollup-output-default-market`、`rollup-output-no-limit-market`、`rollup-output-first-market`、`rollup-output-last-market`、`rollup-output-snapshot`、`rollup-output-snapshot-order-limit`、`rollup-output-first-sorted`、`rollup-output-first`、`rollup-output-every-sorted`、`rollup-output-last` 与 `rollup-output-last-sorted` 亦已登记。
- 最新已提交：`8cf34f28b`（Add resultset-aggregate-default differential scenario）
...
| Capability | 110 个 |
| Case | 425 个 |
| Case implemented（verification） | 425 个（其中 38 个 differential-verified、18 个 intentionally-different） |
| Case differential-verified | 38 个（48 个 runtime） |
...
| 未关联 runtime | 1,506 个 |
| 关联覆盖率 | 63.6% |
| Representative scenario | 38/38 通过 |
| NFR | 0 个已验证 |
