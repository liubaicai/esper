...
- 最新进展：`rollup-output-first-having` 登记为 differential-verified（`ResultSetOutputFirstHaving{join=false}`，7 条 records/0 differences；grouped output-first-with-having 的 new/old 都交付当前聚合值）；`rollup-output-all-sorted`、`rollup-output-all`（新增 `OutputAllEveryTime` 公共 API）、`rollup-output-default-market`、`rollup-output-no-limit-market`、`rollup-output-first-market`、`rollup-output-last-market`、`rollup-output-snapshot`、`rollup-output-snapshot-order-limit`、`rollup-output-first-sorted`、`rollup-output-first`、`rollup-output-every-sorted`、`rollup-output-last` 与 `rollup-output-last-sorted` 亦已登记。
...
| Capability | 110 个 |
| Case | 424 个 |
| Case implemented（verification） | 424 个（其中 37 个 differential-verified、18 个 intentionally-different） |
| Case differential-verified | 37 个（47 个 runtime） |
...
| 未关联 runtime | 1,506 个 |
| 关联覆盖率 | 63.6% |
| Representative scenario | 37/37 通过 |
| NFR | 0 个已验证 |
