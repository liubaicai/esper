...
- 最新进展：`rollup-output-snapshot` 登记为 differential-verified（`ResultSet6OutputLimitSnapshot{join=false}`，7 条 records/0 differences；同时修复时间型 output 调度锚点与 rollup 快照排序、统一三位毫秒 trace 格式）；`rollup-output-snapshot-order-limit`、`rollup-output-first-sorted`、`rollup-output-first`（同时修复 grouped irstream 纯时间淘汰的 New/Old 语义）、`rollup-output-every-sorted`、`rollup-output-last` 与 `rollup-output-last-sorted` 亦已登记。
...
| Capability | 110 个 |
| Case | 417 个 |
| Case implemented（verification） | 417 个（其中 30 个 differential-verified、18 个 intentionally-different） |
| Case differential-verified | 30 个（40 个 runtime） |
...
| 未关联 runtime | 1,506 个 |
| 关联覆盖率 | 63.6% |
| Representative scenario | 30/30 通过 |
| NFR | 0 个已验证 |
