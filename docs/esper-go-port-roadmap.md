...
## 0. 实时状态入口
> 最新补充：Draft 4.272（2026-08-28），新增 `resultset-aggregate-sorted-minmax-by` differential-verified 场景，对照固定 Java `ResultSetAggregateSortedMinMaxBy.java` 的 `ResultSetAggregateGroupedSortedMinMax`、`ResultSetAggregateMultipleOverlappingCategories` 与 `ResultSetAggregateMinByMaxByOverWindow`（3 个 runtime）：Java/Go 各 25 条 records、0 differences。场景覆盖 grouped length(3) 的 window/sorted/minby/maxby/ever 访问、keepall overlapping current/ever 分类，以及 length(5) current-vs-ever min/max-by 投影与 FIFO 淘汰；Go runner 拒绝缺失/额外字段、非整数 payload 和意外 case marker，oracle validator 与 Java `parseInt`/`parseLong` 词法及范围一致。manifest 更新为 565 cases、177 个 differential-verified case、692 个 differential runtime IDs、3315 条 associations（referenced 3103）；capability 118 个（33 DV）。
