...
| Case differential-verified | 4 个（11 个 runtime） |
...
| 未关联 runtime | 1,856 个 |
| 关联覆盖率 | 55.1% |
| Representative scenario | 4/4 通过 |
| NFR | 0 个已验证 |
...
2. 扩展 Java/Go persisted differential evidence；当前已有 4 个代表性场景（`context-hash-segmented`、`filter-window-aggregate-output`、`join-length-window`、`output-policy-iterator`）。
