...
> 最新补充：Draft 4.393（2026-09-12），database 子域第三个差分切片：新增 `epl-database-restart` 链覆盖 `EPLDatabaseJoin.java` 的 EPLDatabaseRestartStatement execution（`java-runtime-0d41625df368897ca97b`，ordinal 18；Java commit `9e1b9f1cc9117fea4bf33ab043762c045d73839c`）。Java/Go trace 各 100 条 listener records、0 differences——单 case 100 循环的部署/卸载/重部署连接泄漏探针：每循环 undeploy 后的无监听发送零投递（进程内守卫）、重部署 + 重订阅后恰好一次 {mychar:Z} 投递（逐循环断言）；100/100 循环全部投递证明无 'Too many connections' 连接耗尽（Java 套件注释的探针目的）。mychar 以 JSON 字符串记录。同单元顺带修正两处陈旧 docstring（database_join_class_parity_test.go：views-on-historical 的'构建器接受'分歧表述与 TimeBatch 测试位置）。`case.inventory.epl-database-join` dv 列表 11→12。Go 侧零引擎改动（FromHistoricalOn + Undeploy/Deploy 重部署 + Subscribe 重挂）。manifest 更新为 648 cases、271 个 differential-verified case、1013 个 differential runtime IDs（并行 OMP 单元已推进计数至 1012，本单元 +1）、3610 条 associations（referenced 3293、unreferenced 843 不变——0d41625d 已由本案例预关联，dv 列表 11→12）。
