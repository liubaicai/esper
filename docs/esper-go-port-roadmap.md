...
- 最新进展：`rollup-output-default-market` 登记为 differential-verified（`ResultSet2OutputLimitDefault`，6 条 records/0 differences，直接受益于 affected 顺序与空 tick 调度修复）；`rollup-output-no-limit-market`、`rollup-output-first-market`、`rollup-output-last-market`、`rollup-output-snapshot`、`rollup-output-snapshot-order-limit`、`rollup-output-first-sorted`、`rollup-output-first`、`rollup-output-every-sorted`、`rollup-output-last` 与 `rollup-output-last-sorted` 亦已登记。
- 最新已提交：`b29a09e43`（Add rollup-output-no-limit-market differential scenario）
...
| Capability | 110 个 |
| Case | 421 个 |
| Case implemented（verification） | 421 个（其中 34 个 differential-verified、18 个 intentionally-different） |
| Case differential-verified | 34 个（44 个 runtime） |
...
| 未关联 runtime | 1,506 个 |
| 关联覆盖率 | 63.6% |
| Representative scenario | 34/34 通过 |
| NFR | 0 个已验证 |
