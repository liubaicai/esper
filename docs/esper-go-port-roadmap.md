...
- 最新进展：`rollup-output-every-sorted` 登记为 differential-verified（`ResultSetOutputDefaultSorted{join=false}`）；`rollup-output-last` 与 `rollup-output-last-sorted` 亦已登记（上一轮 Java oracle 误加 order-by 已修正；grouped `OutputLastEveryTime` 现在按输出 key 缓存上一周期行、新 group 首次 old 行输出 null 聚合占位，并按 Java rollup 层级输出 leaf → subtotal → grand total；三个 evidence 均 3 条 records/0 differences）。
- 最新已提交：`ea4b00a9c`（Correct rollup-output-last oracle and add sorted scenario）
...
| Capability | 110 个 |
| Case | 413 个 |
| Case implemented（verification） | 413 个（其中 26 个 differential-verified、18 个 intentionally-different） |
| Case differential-verified | 26 个（36 个 runtime） |
...
| 未关联 runtime | 1,506 个 |
| 关联覆盖率 | 63.6% |
| Representative scenario | 26/26 通过 |
| NFR | 0 个已验证 |
