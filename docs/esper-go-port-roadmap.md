...
- 最新进展：`rollup-output-no-limit-market` 登记为 differential-verified（`ResultSet1NoOutputLimit`，12 条 records/0 differences；同时修复 rollup remove affected 顺序与 `OutputEveryTime` 空 tick 调度推进）；`rollup-output-first-market`、`rollup-output-last-market`、`rollup-output-snapshot`、`rollup-output-snapshot-order-limit`、`rollup-output-first-sorted`、`rollup-output-first`、`rollup-output-every-sorted`、`rollup-output-last` 与 `rollup-output-last-sorted` 亦已登记。
...
| Capability | 110 个 |
| Case | 420 个 |
| Case implemented（verification） | 420 个（其中 33 个 differential-verified、18 个 intentionally-different） |
| Case differential-verified | 33 个（43 个 runtime） |
...
| 未关联 runtime | 1,506 个 |
| 关联覆盖率 | 63.6% |
| Representative scenario | 33/33 通过 |
| NFR | 0 个已验证 |
