...
## 0. 实时状态入口
> 最新补充：Draft 4.278（2026-08-28），新增 `resultset-aggregate-minmax-groupby-join-select-having` differential-verified 场景，对照固定 Java `ResultSetAggregateMaxMinGroupBy.java` ordinals 3 与 5 的 `ResultSetAggregateMinMaxJoin`（runtime `java-runtime-802aec9425dc772e3aa5`，static candidate `java-6b177406a733a140a453`）和 `ResultSetAggregateMinNoGroupSelectHaving`（runtime `java-runtime-060f5af73edf420ef45a`，static candidate `java-89cead06d92ef2fbf53c`）：Java/Go 各 14 条 records、0 differences。场景覆盖 SupportBeanString DELL/IBM 种子驱动的过滤 `length(3)` 分组 `irstream` min/max all/distinct join、全局 FIFO 淘汰、重复极值、null 跳过与双组 callback 行序，以及无分组 `length(5)` 新行 HAVING `volume > min(volume) * 1.3` 的静默/命中边界；runner/oracle 严格拒绝缺失/额外字段、非整数与错误 seed/case 顺序，以及 trace value/order/time/record-count mutation。manifest 更新为 570 cases、183 个 differential-verified case、705 个 differential runtime IDs、3326 条 associations（referenced 3113）；capability 119 个（34 DV）。
> 最新补充：Draft 4.274（2026-08-28），新增 `resultset-aggregate-sorted-multi-criteria` differential-verified 场景，对照固定 Java `ResultSetAggregationMethodSorted.java` 的 `ResultSetAggregateSortedMultiCriteria`（ordinal 10，runtime `java-runtime-dd79ba5aba4eb4ec0a1a`，static candidate `java-427c23ee540c6e2e7cf9`）：Java/Go 各 1 条 record、0 differences。场景覆盖 public table `MyTable.sortcol` 的 `sorted(*)` 双键 `(theString, intPrimitive)` 字典序物化，七个 SupportBean 填充后由 `SupportBean_S0(-1)` 触发 `firstKey`/`lastKey`/`lowerKey`/`higherKey` 导航；runner/oracle 严格拒绝缺失/额外/重复字段、非整数与 int32 越界 payload 及 trace value/order/time/record-count mutation。manifest 更新为 567 cases、179 个 differential-verified case、694 个 differential runtime IDs、3317 条 associations（referenced 3104）；capability 118 个（33 DV）。
