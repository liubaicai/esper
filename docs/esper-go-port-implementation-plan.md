...
| 当前修订 | Draft 2.31（2026-08-07）：在 `InfraOnMergeInsertStream` action-chain 之后补齐 `InfraMultipleInsert` 的四个 not-matched 分支（A/B 源字段投影、C 常量投影、D 拼接与取负），并补齐 `InfraFlow` 的过滤 insert、DeleteAll、matched update/reset/delete、not-matched insert、merge plan redeploy 与 wildcard insert 子集；两块均覆盖 Table/Named Window 并登记到 manifest。表示转换、rollback/transaction、split-stream、持久化、EPL/SODA 重部署差异及完整 Java trace 仍按 partial 处理。 |
