...
> 最新补充：Draft 3.19（2026-08-09），`ClientInstrumentMetricsReportingStmtMetrics`、`StmtGroups`、`DisableStatement`、`DisableRuntime` 与 `NW` 5/5 execution 已 mapped，MetricsReporting 现为 6/6。新增 typed statement metric groups、snapshot/subscription、6s/7s/8s/10s virtual-time schedule、inactive report、动态 interval、单 statement/全局停启、部署生命周期与 Named Window A/W/B/C/D/M 输入计数；Linux 用 process CPU clock，非 Linux 无 portable sampler 时 CPU 为 0。Go 不把 metric 注册成普通 event stream，Named Window declaration 以 environment metric entry 表达。当前 213 mapped / 0 partial / 6 approved-difference。Maven 当前不可用，未执行 Java suite；本轮不需要 MySQL Docker。
...
| 项目 | 内容 |
|---|---|
| 文档状态 | Draft 3.19，MetricsReporting 6/6 execution 已 mapped；runtime/statement virtual-time interval、group、snapshot、subscription、enable/disable 与 Named Window accounting 已落地，下一步继续 Audit 或 protected deployment-local namespaces |
...
- structured audit、日志与异常回调；instrumentation 的固定 disabled execution 已处置，runtime/statement metrics 已有 typed management surface，剩余内部 callback/handle 精细度继续按 observable case 扩展。
