...
> 最新补充：Draft 2.89（2026-08-08），方法源 OuterNStream 切片：对照 Java `EPLFromClauseMethodOuterNStream`，新增 `Stream.EvaluateOnce()` 链式标记——常量参数方法源在 Deploy 时按 Esper statement-start 语义单次求值（`seedEvaluateOnceJoinSidesLocked`），像只读数据窗口一样保留且不发 listener 批次，触发周期不再清理或重查该侧；`validateNode` 拒绝 `EvaluateOnce` 与 trigger/`DependingOn` 组合。修正 outer join 占位语义贴合 Esper N-way 装配：依赖绑定（subordinate）方法行不再锚定占位元组（`joinStoredTupleHasAnchor`，`joinChainedTuples`/`joinOuterTuples` 的 outer 保留分支统一跳过），后续 outer 边无法求值时（累积元组中不含该边条件引用的真实事件）部分元组原样透传（`joinChainedEdgeAnchored`）。新增 `fromclausemethod_outer_parity_test.go` 五个对照：Inner（s0-first/h0-first 两变体）、LeftRight（LEFT×2、h1-first RIGHT+LEFT、h0-first RIGHT+LEFT、FULL×2 四变体，逐事件断言 lastNew 与迭代器全量）、2Stream1Hist（LEFT+RIGHT、LEFT×2、RIGHT×2 三变体，`fetchVal(s0.id||'H0',s0.p00)` 依赖读取）、NoSubordinate（`EvaluateOnce` 部署即 4 行占位、占位随匹配蒸发；Java 第二文本变体 h1 LEFT s0 RIGHT h0 在 Java 断言同一矩阵，按等价 s0 锚定 RIGHT×2 链建模并在 manifest difference 登记）、Invalid（两种无法满足的 outer 依赖诊断）。新增 4 个 Java runtime（`java-runtime-5e4e5ccb4cfd44bb9383`、`java-runtime-fef68112ee74271db7c7`、`java-runtime-ea3e68f3641325942d20`、`java-runtime-e13d949b611999c2af33`）已登记到 `case.epl-fromclausemethod-basic` 与 capability `epl.fromclausemethod`。定向 Go 测试通过；Java `TestSuiteEPLFromClauseMethod` 保持 3/3 通过；outer forward/3-hist subordinate 精确矩阵、cache、context、multikey/array-key、performance 与完整 Java 诊断仍为后续切片；本轮不需要 MySQL Docker。
...
| 项目 | 内容 |
|---|---|
| 文档状态 | Draft 2.89，方法源 OuterNStream 切片：Go 新增 `Stream.EvaluateOnce()` 链式标记，常量参数方法源 Deploy 时单次求值并保留；outer join 占位语义修正为依赖绑定方法行不锚定占位元组、无法求值的后续 outer 边原样透传部分元组。本轮对照 `EPLFromClauseMethodOuterNStream` 的 Inner/LeftRight/2Stream1Hist/NoSubordinate/Invalid 五组矩阵；outer forward/3-hist subordinate 精确矩阵、cache、context、multikey/array-key、performance 与完整 Java 诊断仍为后续项 |
