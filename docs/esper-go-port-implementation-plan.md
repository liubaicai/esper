...
> 最新补充：Draft 3.25（2026-08-09），`ClientCompileModuleUses` 4/4 execution 已 mapped。新增 generic `ModuleOrderItem[T]`/`OrderModules`、缺失 uses/循环验证与 functional opt-out，并逐项固定 Java order matrix、anonymous/duplicate/self-use/deployed dependency；`ModulePath.UsesNames` 支持 ignorable external module names 进入 Plan identity 而不污染 local catalog。当前 220 mapped / 0 partial / 7 approved-difference。Maven 当前不可用，未执行 Java suite；本轮不需要 MySQL Docker。


## 1. 文档信息

当前切片（Draft 3.25，2026-08-09）对照 Java `ClientCompileModuleUses`。Go 用 `ModuleOrderItem[T]` 将 name/uses 与任意 deployment artifact 组合，`OrderModules` 默认检查 proposed/deployed dependency 与 circular graph，并复现 Java reverse-root ordering 的稳定结果；blank name 表示 anonymous module，duplicate names 各自保留，self-use 不形成边。`AllowUnresolvedModuleUses` 和 `AllowCircularModuleUses` 取代 mutable `ModuleOrderOptions`，循环关闭时仍给出确定性顺序。`ModulePath.UsesNames` 允许依赖声明指向外部或尚未注册 module：名称进入 Plan canonical 与 ordering，但 symbol resolver 只使用本地已注册 public module，因此 Java `uses dummy; select MYVAR...` 对应 global typed variable 仍可运行。Go 不返回 Java ModuleOrder wrapper、不解析 module/EPL text；跨仓库 artifact loader、自动批量 deploy/rollback 与 versioned module package format 仍是后续项。

| 项目 | 内容 |
|---|---|
| 文档状态 | Draft 3.25，ClientCompileModuleUses 4/4 execution 已 mapped；generic module order、missing/cycle policy 和 ignorable named uses 已落地，manifest 当前 1,732 条关联、1,706/4,140 唯一 runtime（41.21%）、220 mapped / 0 partial / 7 approved-difference；下一步继续 compile engine path/path cache/SPI/syntax 或其他未映射 suite |
