...
> 最新补充：Draft 3.54（2026-08-10），ViewTimeWindowSceneOne/SceneTwo 关闭（虚拟时钟 #time(10 sec)：精确边界到期、同时刻到达合并驱逐批次、Iterator 到达序），TimeWindow+AdvanceTime 无需引擎改动即对齐。当前 276 mapped / 1 partial / 17 approved-difference。本轮不需要 MySQL Docker。
...
| 项目 | 内容 |
|---|---|
| 文档状态 | Draft 3.54，Client domain 100% 关闭（281/281）；View domain 已处置 16 个唯一 runtime（KeepAll 3/3、FirstUnique 2/2、FirstEvent/LastEvent/FirstLength、LengthWindow 3、Sort 6/6、TimeWindow 2），ExprFilter 23、ExprEnum 27、Pattern 3、ResultSet 2、EPLOther 2、EPLInsertInto 1；manifest 当前 1,935 条关联、1,900/4,140 唯一 runtime（45.89%）、276 mapped / 1 partial / 17 approved-difference；下一步 ViewTimeBatch/LengthBatch/Expression/Accum 系列、ExprFilter 剩余 suite 与 Pattern 批量登记 |
