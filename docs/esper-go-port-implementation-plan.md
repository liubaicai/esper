...
> 最新补充：Draft 3.05（2026-08-09），`ClientRuntimeTimeControl` 2/2 execution 已映射。新增 `AdvanceTimeSpan`、`NextScheduledTime` 与 `StatementNearestSchedules`：精确跨度推进保留逐到期点 listener batch，固定 resolution 采样把逾期 recurring timer 重锚到实际采样时刻；typed `TimerAt(...).And(...)` 对照 Java one-shot interval 的 statement 最近调度与 undeploy 生命周期。新增 2 个 runtime 和 `client.runtime.time-control` capability；当前 200 mapped / 0 partial / 4 approved-difference。本轮不需要 MySQL Docker。
...
| 项目 | 内容 |
|---|---|
| 文档状态 | Draft 3.05，`ClientRuntimeTimeControl` 2/2 execution 已映射；此前 `ClientRuntimeListener` 1/1、`ClientRuntimeUnmatchedListener` 3/3、`ClientRuntimeEPStatement` 2/2、`client/basic` 7/7 与 `EPLOtherSplitStream` 12/12 保持 mapped，下一步继续 client compile/deploy/runtime 管理面 |
