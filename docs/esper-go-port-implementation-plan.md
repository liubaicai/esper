...
这四个 case 已写入 `compat/capability-manifest.json`，并保留 `status=mapped` 的含义：它只表示 Java 来源、Go 测试和能力入口已关联；尚未有共享 Java/Go 中立 trace，因此不能改成 `passing`。本轮新增的 `plan.go` 遍历也必须收集 merge insert projection 的表达式依赖，避免参数依赖或 canonical/hash 漏失。
