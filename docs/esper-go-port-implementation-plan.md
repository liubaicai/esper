...
> 最新补充：Draft 3.00（2026-08-09），`EPLOtherSplitStream` 全部 12/12 execution 已关闭，`case.epl-other-split-stream` 转为 mapped。最后的 SingleInsert 由 initiated context + context-bound split projection 对照：partition id、initiating criteria 和 trigger event 均正确投影，两个底层 Go 指针保持对象 identity。新增 1 个 runtime；当前 195 mapped / 0 partial / 4 approved-difference。本轮不需要 MySQL Docker。
...
| 项目 | 内容 |
|---|---|
| 文档状态 | Draft 3.00，`EPLOtherSplitStream` 12/12 execution 全部映射；基础分流、subquery、contained-event branch source、preemptive Named Window 与 context single-insert 均已覆盖 |
