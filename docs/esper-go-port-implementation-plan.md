...
> 最新补充：Draft 2.87（2026-08-08），方法源变量驱动 3-historical 切片：对照 Java `EPLFromClauseMethod3HistPureNoSubordinate`、`EPLFromClauseMethod3Hist1Subordinate` 与 `EPLFromClauseMethod3Hist2SubordinateChain`，新增 3 个 Go 链式对照测试。Go 用 `FromMethod` + `RegisterVariable` + `OnEvent(...).SetVariables(...)` 表达 Java `on SupportBeanInt set var1=p00...` 的变量更新，方法源从 `request.Variables` 读取 var1/var2/var3 行数，依赖方法源通过 `DependingOn` 从 `request.Dependencies` 读取前一方法事件；`Statement.Snapshot` 对照 Java `assertPropsPerRowIteratorAnyOrder`。新增 3 个 Java runtime（`java-runtime-ef932b432431cc12b207`、`java-runtime-a518167e14ddf5f3870b`、`java-runtime-ad1d2f4d0edc546eb20e`）已登记到 `case.epl-fromclausemethod-basic` 与 capability `epl.fromclausemethod`。定向 Go 测试通过；Java `TestSuiteEPLFromClauseMethod` 保持 3/3 通过。NW-twice、outer joins、cache、context、multikey/array-key、performance 与完整 Java 诊断仍为后续切片；本轮不需要 MySQL Docker。
...
| 项目 | 内容 |
|---|---|
| 文档状态 | Draft 2.87，方法源变量驱动 3-historical 切片：Go 用 `FromMethod` + `RegisterVariable` + `OnEvent(...).SetVariables(...)` 复现 Java 变量更新与方法源计算；依赖方法源通过 `DependingOn` 从 `request.Dependencies` 读取前一方法事件，`Statement.Snapshot` 对照 Java iterator 语义。本轮对照 `EPLFromClauseMethod3HistPureNoSubordinate`、`EPLFromClauseMethod3Hist1Subordinate`、`EPLFromClauseMethod3Hist2SubordinateChain`；NW-twice、outer joins、cache、context、multikey/array-key、performance 与完整 Java 诊断仍为后续项 |
