...
本轮补充了枚举表达式的 Go 类型/精度边界：新增 `EnumCollect[T](Expr)`，将数组、slice 和命名 slice 归一化为可链式枚举输入；新增公开 `EnumOrdered`/`EnumNumeric` 约束，`EnumSum`/`EnumSumOf`/`EnumMin`/`EnumMax`/`EnumMinBy`/`EnumMaxBy`/`EnumOrderBy`/`EnumAverageOf` 可处理 `math/big.Int` 与 `math/big.Rat`，求和和排序不再经过 float64；新增 `EnumAverageExact`/`EnumAverageExactOf` 返回精确 `big.Rat`。`TestEnumerableSupportsExactBigNumbersAndArrayCollections` 固定大整数、分数、字段选择器、数组和命名 slice。Java 28/28 基线通过；Go 仍需补 map/iterator-backed collection metadata、嵌套/子查询/访问聚合组合以及完整无效规则诊断，不能将本切片解释为全量 ExprEnum parity。
...
本轮继续对照 Java `EPLInsertIntoLenientPropCount` 与 `EPLInsertIntoEventRepresentationsSimple`：Go 路由允许链式 projection 只提供目标 schema 的部分列，缺失字段在 Map/ObjectArray/JSON/XML/Avro 目标中物化为显式 null；移除 ObjectArray 路由必须“全列投影”的过严限制，并保留目标字段顺序和额外动态字段。`TestInsertIntoPartialColumnsMaterializesNullAcrossRepresentations` 已登记到 `case.insert-into-chain`。Java 的 wildcard 列语法、mixed-stream ordering、wrapper/recast 和完整多表示 insert-into trace 仍保持开放。
