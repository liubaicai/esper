...
| 项目 | 内容 |
|---|---|
| 文档状态 | Draft 2.53，补充 Java `InfraNWTableFAFSubstitutionParams` 的 typed named/positional PreparedQuery 矩阵、混用/跳号/缺参/类型错误校验；前序已补充 `InfraNWTableFAFInsertMultirow` 的 Go-native `InsertRows(InsertValues(...))`、1000 行校验、子查询值与 Named Window/Table 原子回滚及严格唯一索引约束。Variant 的单列 method-backed member conversion 与 Java application/type-class metadata 仍未关闭；其余历史增量与未完成项见下文 |
...
| 当前修订 | Draft 2.53（2026-08-07）：在 `InfraNWTableFAF` 的只读/单行/Context mutation/多行 insert 对照基础上，继续关闭 Java `InfraNWTableFAFSubstitutionParams` 的 7 个 execution：新增 1-based `ParameterAt[T]`/`PositionalParameter[T]`、`Engine.ExecuteFireAndForgetWithPositionalParameters`、selector/route 变体和 `PreparedQuery.ExecuteWithPositionalParameters`；具名 `Parameter[T]` 保持 map 绑定。Build 阶段拒绝 named/positional 混用、重复参数类型冲突、positional 跳号和非法下标；执行阶段校验缺参、额外参数和静态类型。Go 新增 Named Window/Table 正常单值/双值/slice-IN、具名重复参数及 invalid 对照测试；Java SODA 路径以 immutable fluent Plan 对应，EPL 保留关键字解析和精确 Java 诊断不作为 Go API 契约。Java `TestSuiteInfraNWTable` 26/26 通过；本轮不需要 MySQL Docker。 |
...
- 当前 capability manifest 已关联 1,283/4,136 个唯一 Java runtime（约 31.02% 的 Java runtime 对账/处置进度）；185 个 case 中 180 个 mapped、1 个 partial、4 个 approved-difference。该比例不是 Java/Go 行为 parity 通过率，也不是全量移植完成度。
