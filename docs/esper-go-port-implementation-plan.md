...
本轮继续对照 Java `EPLDataflowAPIExceptions`（`java-runtime-76175156292855f373cf`）：Go `DataflowError` 增加 `OperatorNum`/`OperatorPrettyPrint` 与原始错误的 `Unwrap`，pretty-print 采用 Go 链式端口定义生成；Fail 策略下 operator/source 异常只调用一次 handler、保留 `LastError`，并将实例终态置为 `Complete`，避免把 Java 的异常完成态误映射为 `Canceled`；Continue 策略继续丢弃失败 work item。`TestDataflowOperatorExceptionCompletesWithEsperContext`、`TestDataflowSourceExceptionContextMatchesEsper` 与已有 source/Continue 用例固定上述行为。Java 的精确 wrapper 文本、异步 handler 调度、内建 source/operator 背压及更宽恢复矩阵仍保持开放。
