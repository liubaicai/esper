...
> 最新补充：Draft 3.29（2026-08-09），`ClientCompileEventTypeAutoName` 2/2 execution 已 mapped。新增 application-owned、zero-value-safe、thread-safe 的 typed `EventTypeAutoNameRegistry`，用显式 Go type 注册替代 Java package/classpath scanning；唯一短名可通过 `RegisterAutoNamedStruct[T]` 注册 alias schema，跨 namespace 重名返回稳定 `ErrorAmbiguous`。当前 224 mapped / 0 partial / 7 approved-difference。Maven 当前不可用，未执行 Java suite；本轮不需要 MySQL Docker。


## 1. 文档信息

当前切片（Draft 3.29，2026-08-09）对照 Java `ClientCompileEventTypeAutoName`。Go 用 `RegisterAutoNameType[T]` 显式登记 named struct，默认以 `reflect.Type.Name`/`PkgPath` 作为 short name/namespace，必要时通过 functional option 覆盖，从而在一个 Go package 中复现 Java 两个 package 的同名 class ambiguity；`Resolve` 对 unknown、blank 与多 namespace 分别返回结构化错误，歧义 namespace 稳定排序。`RegisterAutoNamedStruct[T]` 校验 registry 解析结果与 T 精确一致，再调用普通 `RegisterStruct` 注册别名；`MANE` fluent statement 已对照发送 `MyAutoNameEvent` 并得到 `p0=test`。Go 不扫描 package/classpath、不使用 process-global registry、不加载 JVM class，也不接受 EPL create-schema string；完整 import/classloader/compiler text 继续作为明确差异。

| 项目 | 内容 |
|---|---|
| 文档状态 | Draft 3.29，ClientCompileEventTypeAutoName 2/2 execution 已 mapped；explicit typed auto-name registry、alias schema registration、deterministic namespace ambiguity 与 invalid/concurrency 边界已落地，manifest 当前 1,744 条关联、1,718/4,140 唯一 runtime（41.50%）、224 mapped / 0 partial / 7 approved-difference；下一步继续 compile syntax/tool compiler 或其他未映射 suite |
