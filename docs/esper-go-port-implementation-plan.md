...
> 最新补充：Draft 3.18（2026-08-09），`ClientInstrumentMetricsReportingRuntimeMetrics` 1/1 execution 已 mapped。新增 virtual-time runtime metric interval、typed subscription、current snapshot、enable/disable、累计/delta input 和 pending schedule depth；11s/21s 得到 1/4、1/3、1/0 对照，metric record 进入下一周期计数。Go 不复制 RuntimeMetric event stream、JMX/ThreadMXBean；statement metrics/groups 仍待后续。当前 212 mapped / 0 partial / 6 approved-difference。Maven 当前不可用，未执行 Java suite；本轮不需要 MySQL Docker。
...
| 项目 | 内容 |
|---|---|
| 文档状态 | Draft 3.18，runtime metrics 1/1 execution 已 mapped；virtual-time interval/input/schedule/subscription 已落地，下一步继续 statement metrics/groups、Audit 或 protected deployment-local namespaces |
