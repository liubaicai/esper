...
> 最新补充：Draft 3.17（2026-08-09），`ClientInstrumentInstrumentation` 1/1 execution 已按固定 Java 源码处置。`InstrumentationHelper.ENABLED/ASSERTIONENABLED` 均为 false，该 execution 只验证普通 compile/deploy/undeploy，无可观察 trace contract；Go 以 typed false expression 固定同一生命周期，N 级 approved difference，不复制 dormant JVM static/bytecode hooks。Audit/Metrics 仍未映射。当前 211 mapped / 0 partial / 6 approved-difference。Maven 当前不可用，未执行 Java suite；本轮不需要 MySQL Docker。
...
| 项目 | 内容 |
|---|---|
| 文档状态 | Draft 3.17，disabled Java instrumentation execution 已 approved-difference；Audit/Metrics 仍在范围内，下一步实现 client instrument 可观察能力或 protected deployment-local namespaces |
