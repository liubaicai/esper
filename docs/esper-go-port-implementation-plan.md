...
> 最新补充：Draft 3.58（2026-08-10），PatternOperatorEvery suite 2/6 关闭（PatternEverySimple/PatternOp）：嵌套 every 匹配倍数对齐 Esper（every^2 触发 B2×2/B3×4、every^3 B2×3/B3×9、every^4 B2×4/B3×16），every 节点完成转移拆分 continued（保留非退出已完成子节点）+ sibling（派生全新子节点）双转移并新增过滤器 armed 活跃标记。当前 278 mapped / 1 partial / 17 approved-difference。本轮不需要 MySQL Docker。
...
| 项目 | 内容 |
|---|---|
| 文档状态 | Draft 3.58，Client domain 100% 关闭（281/281）；View domain 22 个唯一 runtime（KeepAll/FirstUnique/FirstEvent/LastEvent/FirstLength/LengthWindow/Sort/TimeWindow/TimeBatch 6/10），Pattern domain 7 个（And/Or 3、CompositeSelect 2/2、Every 2/6）；ExprFilter 23、ExprEnum 27、ResultSet 2、EPLOther 2、EPLInsertInto 1；manifest 当前 1,944 条关联、1,909/4,140 唯一 runtime（46.11%）、278 mapped / 1 partial / 17 approved-difference；下一步 PatternOperatorEvery 剩余（WithAnd/FollowedByWithin/AndNot/FollowedBy）与 Pattern 基础 suite（Not/FollowedBy 批量）、View Expression/Accum 系列与 ExprFilter 剩余 |
