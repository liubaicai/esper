...
> 最新补充：Draft 2.93（2026-08-09），关闭 EPLFromClauseMethod 主类全部 20 个 execution：新增 2JoinEventItselfProvidesMethod（EvaluateOnce 区间界 + join-where 过滤）、StreamNameWContext 与 WithMethodResultParam（invocation context 语句名；嵌套方法结果参数折叠进 provider）、EventBeanArray（Java 三种 EventBean 返回形态统一为 Go []Event 契约，@type 登记差异）、InvocationTargetEx（provider 错误作为 SendEvent 错误）与 UDFAndScriptReturningEvents（UDF/JS 折叠进 provider，JS 引擎登记差异）。case.epl-fromclausemethod-basic 由 partial 转为 mapped。6 个 Java runtime（java-runtime-e358488a6167eed202fa、java-runtime-4432f22b8a5f345bb6af、java-runtime-815e9a0a9306829bb402、java-runtime-54dce3a9e46215efd1c7、java-runtime-b5af6ec95cf804a6e29a、java-runtime-f6c9835a034ef580d8c8）已登记。Java TestSuiteEPLFromClauseMethod 保持 3/3 通过；CacheLRU/CacheExpiry/JoinPerformance/MultikeyWArray/Variable 五个类、context 绑定、unidirectional-historical、多事件流无依赖方法绑定与完整 Java 诊断仍为后续切片；本轮不需要 MySQL Docker。
...
| 项目 | 内容 |
|---|---|
| 文档状态 | Draft 2.93，关闭 EPLFromClauseMethod 主类全部 20 个 execution（event-bean/UDF/script/context/异常 六对照），case.epl-fromclausemethod-basic 转为 mapped；CacheLRU/CacheExpiry/JoinPerformance/MultikeyWArray/Variable、context 绑定、unidirectional-historical、多事件流无依赖方法绑定与完整 Java 诊断仍为后续项 |
