...
| 项目 | 内容 |
|---|---|
| 文档状态 | Draft 2.46，继续补充 typed JSON 递归嵌套 fragment schema、schema-bound metadata/getter 和 null-versus-empty 渲染的 Go/Java 对照证据；此前已补充 JSON sender、显式 field adapter、Core typed、VM-class、provided-underlying、lax conversion、malformed JSON 和静态/动态未声明字段策略；其余历史增量与未完成项见下文 |
...
| 当前修订 | Draft 2.46（2026-08-07）：继续补齐 typed JSON 递归嵌套 fragment。Go 新增反射驱动的有限 schema graph：struct-valued scalar/indexed 字段自动生成稳定 `NestedSchema`，递归指针复用同一 schema identity，提供 `NestedSchemaNames`/`FragmentSchema`、深层 `Schema.Getter` 和 `Event.GetFragment`/`GetFragments`；nested typed collection 的 null 不再渲染为空数组。显式 `WithNestedPropertySchema` 保持优先，新增 `TestJSONProvidedNestedSchemasExposeRecursiveFragmentsAndMetadata` 与 `TestJSONMapSchemaInfersStructFragmentAndPreservesExplicitNestedSchema`，对照 `EventJsonTypingNestedRecursive`、`EventJsonTypingClassWArrayAndColl` 和 provided-class pattern-array runtime；manifest 进度仍为 1,211/4,136（约 29.28%），更新为 168 个 case（164 mapped、4 approved-difference），因为本轮 runtime 均已在既有 core/provided case 中登记。Java class-loader/module visibility、跨表示 fragment metadata、named nested-schema catalog 的完整注册生命周期、schema 演进和共享 Java/Go parse-write trace 仍为 partial；本轮核心验证不需要 MySQL Docker。 |
...
本轮只关闭上述 class/list/VM/nested parse-write slice，不等同于 JSON 能力全量完成。后续仍需对照 independently registered nested fragments、provided-class serialization conventions、recursive schema inheritance/metadata、dynamic strict-lax 全矩阵、BigDecimal/BigInteger 在所有 nested/dynamic 路径的格式规则、schema evolution，以及共享 Java/Go trace；JSON capability 继续保持 `partial`。
