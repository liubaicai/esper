...
> 最新补充：Draft 3.28（2026-08-09），`ClientCompileSPIExpression` 1/1 execution 已 mapped。新增 generic `CompileExpression[T]`/`CompiledExpression[T]`，以 validated typed AST、immutable source-less Plan metadata 和 detached evaluation context 替代 Java internal ExprNode forge/evaluator；multiply、concat、typed list、firstOf 与 five-second duration 均有对照测试。当前 223 mapped / 0 partial / 7 approved-difference。Maven 当前不可用，未执行 Java suite；本轮不需要 MySQL Docker。


## 1. 文档信息

当前切片（Draft 3.28，2026-08-09）对照 Java `ClientCompileSPIExpression`。Go 用 `CompileExpression[T](env, typedExpr)` 编译 standalone analyzable AST，返回 immutable `CompiledExpression[T]`，其 Type/Description/Plan 进入普通 source-less compiler canonical，并由 Evaluate/EvaluateWith 保留 Missing/Null/Present state；可选 `CompiledExpressionContext` 显式提供 Event、Engine、logical time 与 variables。Java 的 `1*1`、`'a'||'y'`、`Arrays.asList`、`firstOf`、`5 seconds` 分别映射到 typed Multiply、Concat、ArrayOf、EnumFirstOf 与 `time.Duration`。Go 不接受 standalone EPL string，也不暴露 internal ExprNode/forge/evaluator；source-less validator 本轮补齐 declared-expression、method、script 与 subquery traversal。raw syntax parser、bytecode SPI hooks、portable artifacts 与精确 EPCompileException text 继续作为后续切片。

| 项目 | 内容 |
|---|---|
| 文档状态 | Draft 3.28，ClientCompileSPIExpression 1/1 execution 已 mapped；generic standalone typed-expression compile/evaluate、source-less full AST validation 与 immutable Plan metadata 已落地，manifest 当前 1,742 条关联、1,716/4,140 唯一 runtime（41.45%）、223 mapped / 0 partial / 7 approved-difference；下一步继续 compile syntax/tool compiler/event auto-name 或其他未映射 suite |
