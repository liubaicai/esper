...
> 最新补充：Draft 3.47（2026-08-10），`ClientDeployUndeploy` 11/11 全部处置：Script 资源前置条件 mapped（Java `NameAndParamNum` 身份渲染），Java create-index 独立部署与 application-inlined class JVM 工件登记 approved-difference（Go 索引为注册期定义选项、无运行时编译 class）。当前 242 mapped / 0 partial / 10 approved-difference。Maven 当前不可用，未执行 Java suite；本轮不需要 MySQL Docker。


## 1. 文档信息

当前切片（Draft 3.47，2026-08-10）收官 Java `ClientDeployUndeploy`：Script 资源加入部署时引用收集，错误文本按 `NameAndParamNum` 渲染参数元数；create-index 独立部署与 application-inlined class 经处置测试登记 approved-difference——Go 索引是注册期 Table/Named Window 定义选项（可观察依赖停留在属主 infra），Go 无运行时编译 class 工件（共享可调用 provider 经 script registry 覆盖同类依赖）。suite 11/11 处置完成；下一步继续 `ClientDeploy`/`ClientStage` 剩余 suite。

| 项目 | 内容 |
|---|---|
| 文档状态 | Draft 3.47，ClientDeployUndeploy 11/11 全部处置；unknown ID、active module-use dependency precondition、Named Window/Table/变量/Context/事件类型/声明表达式/Script 资源级 undeploy 前置条件已落地，Index/Class 为 approved-difference；manifest 当前 1,798 条关联、1,772/4,140 唯一 runtime（42.80%）、242 mapped / 0 partial / 10 approved-difference；下一步继续 `ClientDeploy`/`ClientStage` 剩余 suite |
