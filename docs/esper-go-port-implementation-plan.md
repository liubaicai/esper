...
> 最新补充：Draft 2.86（2026-08-08），方法源多流别名切片：对照 Java `EPLFromClauseMethod2Stream2HistStarSubordinate` 与 `EPLFromClauseMethod3Stream1HistSubordinate`，新增 `FromAs[T]` 使同一事件类型在单条 Join 中以 s0/s1/s2 不同别名多次出现，并通过 `streamNode.isAlias` 限制别名解析仅对显式别名节点生效；`plan.go` 的 `validateNode`、`sourceSchema` 对 `isAlias` 节点做条件式 schema 回退，避免无条件回退破坏既有 regression。新增 2 个 Go 链式对照测试：`TestFromClauseMethodTwoStreamTwoHistStarSubordinateParity` 与 `TestFromClauseMethodThreeStreamOneHistSubordinateParity`，方法源通过 `DependingOn("s0")` 从 `request.Dependencies` 读取驱动流事件，`fcmMakeSenderMatchFields` 按 ids0/ids1/ids2 过滤 keepall 全量重放行，保留 Java `assertPropsPerRowLastNew` 语义。新增 2 个 Java runtime（`java-runtime-70ff0c7b491131d96598`、`java-runtime-8767976387656ddb4ced`）已登记到 `case.epl-fromclausemethod-basic` 与 capability `epl.fromclausemethod`。`go vet ./...`、`go test ./... -count=1`、`go test ./compat -count=1`、`go test -race . -count=1` 均通过；Java `TestSuiteEPLFromClauseMethod` 保持 3/3 通过。NW-twice、pure-no-subordinate、outer joins、cache、variable/context、multikey/array-key、performance 与完整 Java 诊断仍为后续切片；本轮不需要 MySQL Docker。
...
| 项目 | 内容 |
|---|---|
| 文档状态 | Draft 2.86，方法源多流别名切片：新增 `FromAs[T]` 与 `streamNode.isAlias`，支持同一事件类型在单条 Join 中多次出现并带不同别名；`plan.go` 对显式别名节点做条件式 schema 回退，保持其他流源不变。Go 用 `JoinMany` + `OnSourcesCompare`/`OnSourcesEqual` + `DependingOn` 表达方法源依赖拓扑，`fcmMakeSenderMatchFields` 按 ids0/ids1/ids2 过滤 keepall 全量重放行。本轮对照 Java `EPLFromClauseMethod2Stream2HistStarSubordinate` 与 `EPLFromClauseMethod3Stream1HistSubordinate`；NW-twice、pure-no-subordinate、outer joins、cache、variable/context、multikey/array-key、performance 与完整 Java 诊断仍为后续项 |
