...
> 最新补充：Draft 3.04（2026-08-09），`ClientRuntimeListener` 1/1 execution 已映射。statement listener 内同步重入 Route/representation sender，覆盖 struct、Map、ObjectArray、XML、Avro、JSON 六种事件表示及各自 consumer；复用既有 runtime，无生产代码修改。新增 1 个 runtime 和 `client.runtime.listener-route` capability；当前 199 mapped / 0 partial / 4 approved-difference。本轮不需要 MySQL Docker。
...
| 项目 | 内容 |
|---|---|
| 文档状态 | Draft 3.04，`ClientRuntimeListener` 1/1 execution 已映射；此前 `ClientRuntimeUnmatchedListener` 3/3、`ClientRuntimeEPStatement` 2/2、`client/basic` 7/7 与 `EPLOtherSplitStream` 12/12 保持 mapped，下一步继续 client compile/deploy/runtime 管理面 |
