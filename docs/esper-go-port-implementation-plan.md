...
| 项目 | 内容 |
|---|---|
| 文档状态 | Draft 2.58，补充 Java `InfraNWTableFAFIndex` 的 B-tree range 物理执行验证；Go 已将单源 Table/Named Window 的完整 hash equality/IN candidate lookup 与 B-tree ordered candidate path 接入 FAF，并覆盖 prepared named parameter、插入顺序、范围边界、反向操作数、复合 equality-prefix、Null 边界安全回退和变更后索引维护。Java `@Hint`/JVM query-plan hook 不复制为 Go API 外形；Join 候选、B-tree 二分游标、Context 物理路径、成本模型和性能阈值仍明确列为后续项 |
...
| 当前修订 | Draft 2.58（2026-08-08）：在 `InfraNWTableFAFResolve` module name-resolution 对照基础上，继续对照 `InfraNWTableFAFIndex` 的物理范围执行。Go 使用 `IndexHash`/`IndexBTree` 声明索引、`UseIndex`/`UseIndexOn` 结构化 hint、`Plan.IndexPlan` 稳定逻辑摘要和 `NamedWindow.Lookup`；当前已验证单源 Table/Named Window 完整 equality/IN、prepared named parameter、B-tree ordered candidate path、inclusive/exclusive 与反向范围、反向比较操作数、复合 equality-prefix + range、prepared range、Null 边界回退和 update/delete/clear 后索引维护。Join 候选、真正的 B-tree 二分游标、Context FAF、selectivity/cost/performance 仍开放。Java 的 `@Hint` 字符串、`SupportQueryPlanIndexHook` backing class 不作为 Go 契约；Java `TestSuiteInfraNWTable` 26/26 通过，本轮不需要 MySQL Docker。 |
...
本轮索引复核对照 Java `InfraNWTableFAFIndex` 的 12 个运行态 execution：新增 `faf_index_parity_test.go`，用 Go-native `IndexHash`/`IndexBTree`、`SecondaryIndex`/`UniqueIndex`、`NamedWindowIndex`/`NamedWindowBTreeIndex` 和 `UseIndex`/`UseIndexOn` 复现单源自动选择、Join 两侧选择、unique/non-unique backing、IN/equality/range、单数组/双数组/复合 slice key、null key 和错误 hint。`Plan.IndexPlan()` 提供可复制的 source/index/access/backing/matched-columns 摘要；`NamedWindow.Lookup` 维护声明索引并覆盖插入、更新、删除、Unique replacement、窗口淘汰和多行回滚恢复。随后把单源 Table/Named Window 的完整 hash equality/IN 接入物理 candidate lookup，并把 B-tree `Between`/单边比较接入 ordered candidate path；测试固定 inclusive/exclusive、反向操作数、反向 Between、复合 equality-prefix、prepared bounds、Null 边界 snapshot fallback 以及 update/delete/clear 后不残留陈旧成员。该路径保留最终谓词求值和稳定插入顺序；尚未宣称真正的二分游标、Join/Context/subquery 物理候选、成本选择、JVM backing class 或性能阈值，这些仍是后续优化/组合切片。本轮不需要 MySQL Docker。
...
当前 FAF 索引实施口径必须单独看待逻辑计划与物理执行：`IndexHash`/`IndexBTree`、`UseIndex`/`UseIndexOn` 和 `Plan.IndexPlan()` 先提供稳定的 Go-native 计划摘要；`index_runtime.go` 已把普通单源 Table/Named Window 的完整 hash equality/IN key（literal、变量、prepared named parameter、slice-IN）和 B-tree `Between`/单边范围（含 equality-prefix）接入物理 ordered candidate path，并由状态层按插入顺序合并命中。索引探针无法安全提取（UDF、算术/属性链、Null/Missing 或超大 IN）时必须回退完整 snapshot，不能猜测值；当前 Join 两侧、真正的 B-tree 二分游标、Context FAF、FAF subquery、成本/selectivity 和 Java backing-class/query-plan hook 仍是后续实施项。
...
本轮新增的物理索引验收口径如下：单源 hash equality/IN 与 B-tree range 的逻辑 `IndexPlan` 必须分别与实际 `indexLookups` 计数同时验证，不能只断言计划对象；Table 与 Named Window 都要覆盖重复 key、IN 探针顺序与最终插入顺序、prepared named parameter、范围边界、反向操作数、复合 equality-prefix 和变更后的索引状态。对 UDF 等值、Null/Missing range bound 和不安全动态表达式，要同时断言结果正确且索引计数不增长，证明运行时走了安全 snapshot fallback。该切片不关闭 Java `InfraNWTableFAFIndex` 的 Join、Context、真正的 B-tree 二分 cursor、cost/selectivity、性能阈值或 JVM query-plan hook 差异。
