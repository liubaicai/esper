...
> 最新补充：Draft 2.90（2026-08-08），关闭 OuterNStream 剩余三个精确矩阵：对照 tryAssertionThree/Four/Five 补齐 #lastevent 锚定的 outer forward subordinate——1Stream2HistForwardSubordinate（LEFT×2 链，fetchVal('H0',p00)/fetchVal('H1',p01) 常量前缀+依赖行计数 provider）、1Stream3HistForwardSubordinate（LEFT×3 链与 h0-first RIGHT+LEFT+FULL 混合变体均按声明建模，共享同一事件矩阵）、1Stream3HistForwardSubordinateChain（fetchVal(s0.id||'-H0')/fetchVal(h0.val||'-H1')/fetchVal(h1.val||'-H2') 链式前缀，lastevent 下 trigger 即当前 s0 行）。三个 runtime（java-runtime-b2283f546f03c3d53930、java-runtime-51307ad5fea63359ec49、java-runtime-9cc6163d0e611f7d3020）此前已随 NStream 同名类登记，本轮补齐精确对照后 epl.fromclausemethod 的 OuterNStream 覆盖完整。定向 Go 测试通过；Java TestSuiteEPLFromClauseMethod 保持 3/3 通过；cache、variable/context 绑定、multikey/array-key、performance 与完整 Java 诊断仍为后续切片；本轮不需要 MySQL Docker。
...
| 项目 | 内容 |
|---|---|
| 文档状态 | Draft 2.90，关闭 OuterNStream 剩余三个精确矩阵：#lastevent 锚定的 outer forward subordinate（2-hist LEFT×2、3-hist LEFT×3 与 h0-first RIGHT+LEFT+FULL 混合变体、3-hist 链式前缀变体）均按声明建模并通过；epl.fromclausemethod 的 OuterNStream 覆盖完整；cache、variable/context 绑定、multikey/array-key、performance 与完整 Java 诊断仍为后续项 |
