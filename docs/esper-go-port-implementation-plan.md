...
| 项目 | 内容 |
|---|---|
| 文档状态 | Draft 2.33，补充非分组聚合子查询 `SubqueryHaving`、相关 outer-field、`SubqueryExistsValue` 与带 options 的 IN/ANY/SOME/ALL fluent API 及 Go/Java 对照；并记录枚举 method footprint/component metadata、`EnumerationMetadata`、Func3/Func4 UDF 与 Build/panic 边界，并记录 Go 全量/竞态/vet 门禁；此前已补充参数化声明表达式的 Go AST/API、Java `ExprDefineValueParameter` 对照、Concat AST 子节点完整性、动态 Named Window 注册与单事件 access-aggregate 边界；此前已补充命名表达式声明/引用、脚本 provider 边界、Java execution 对照与 MySQL 按需启用规则；此前已补充 contained split/type 复核遗漏的 Java `EPLContainedScriptContextValue` runtime 登记、Go 全量/竞态/vet 门禁结果和 MySQL 仅按需启用的外部依赖口径；此前已补充 contained `UnnestAs`/`UnnestEvents` 的 Go-native `@type` 替代设计、Event/Variant 具体成员保留、map/JSON raw materialization、目标类型诊断及 Java `EPLContainedSplitExpr` 对照；此前已补充 contained child/parent/grandparent 的 Event-valued fragment alias 选择对照；此前已补充 contained 子查询结果数组经 typed event 路由后再次 `Unnest` 的 Java 对照映射；此前已补充 contained 字符串 `Split` 表达式、split 后 scalar child 展开及父字段投影的 Java 对照映射；此前已补充 contained scalar string-array 的父事件作用域与 `ContainedParentField`/`ContainedAncestor*` 链式表达式、父链元数据校验及 Java 对照测试映射；此前已补充 Dataflow parameter/operator/source provider 的 typed factory metadata 与 canonical option-name 校验，以及 schema-bound Go `AvroRecord` 原生 underlying、typed Set/Get/Lookup、schema identity、SendAvro、map/JSON ingestion normalization、Beacon/EventBus/recast representation propagation；并继续覆盖事件 indexed/mapped/nested 属性访问、JavaBean getter/setter 元数据发现、显式 field/path/method accessor 与 typed callback/method setter、nested explicit schema、有深度上限的 JSON/XML renderer、schema-directed typed JSON parse/write、XML tree/attribute/repeated-element parse、显式 parent schema inheritance、derived/statistical view 链式聚合、multi-key/array-key unique 与 rank window 的稳定快照/历史，以及 aggregate/join/current-state/event-count snapshot output；Context 已补充 multi-key key/hash/initiated-terminated API、flat periodic、fixed daily 与 static single-start/single-end cron temporal Context、`TimeOfDay`/`NewDailyTimeContext`/`NewCronTimeContext`、`ContextStartTime`/`ContextEndTime`、built-in name/id/keyN/label/hash 属性、initiated/terminated boundary-event 表达式、descriptor 的 ID/key/properties 快照、key/ID/category/hash/segmented/nested/descriptor-filter selector、Context created/destroyed replay、statement added/removed、activated/deactivated 生命周期事件、共享分区 ID 与 allocation/deallocation listener、partition-scoped context variable 注册/Get/Set/SetContextVariables/SetContextVariablesByID 原子批量更新、VariableValues/ContextVariableStates 一致快照与 selector 读取、VariableChangeListener old/new 回调、内置 selector-kind mismatch 校验、Statement.SnapshotWithSelector 迭代器式分区过滤、initiated context 的 termination-time pending/snapshot output、事件驱动 pattern-init/pattern-term 的 start-tag/end-tag 关联与 `Every` overlapping 分区、on-trigger/lifecycle、null/array tuple identity 与隔离测试，并登记首批分区/selector/lifecycle/temporal/output/pattern/dataflow representation case；纯 TimerAt/TimerInterval/TimerSchedule/TimerCron pattern-root Context、基础 timer-plus-event observer、事件级 Pattern `Consume(level ...int)`（含裸消费/未标注 filter 排除语义）、查询级 `DiscardPartialsOnMatch`/`SuppressOverlappingMatches`、动态/日历 `Within` guard、动态/组件化/日历 `TimerInterval` observer，以及动态 MatchUntil 上下界（变量、部署参数、null 开闭界和外层 tag）仍为部分对等；temporal Context termination snapshot、keyed initiated child nested Context、Dataflow、子查询的 fragment/iterator/error/Context-Pattern-Dataflow 组合及 EsperIO Kafka/AMQP/Spring JMS bridge contract 仍为部分对等 |
...
13. **矩阵完整性与漏域**：`compat/capability-manifest.json` 当前是首批纵向 capability/case 映射，不是 4,136 个 Java runtime execution、726 个源测试文件的全量处置矩阵；当前已扩展到 33 个 capability，仍不能用这些 `mapped` 状态代表全量完成。必须另行登记 client configuration、Module/PathCache/Stage、Filter Service、完整 view/statistical view、spatial、script/UDF、serde/render、metrics/instrumentation、multithread、transaction/qos、compiler/runtimeconfig、benchmark、example 和剩余 Event/Expr/Infra/EsperIO 入口，并为每项给出 Go test、replacement 或 N 级理由。
...
| `InfraInsertOtherStream` 的 12 个 Table/Named Window × representation execution（`java-runtime-c91b3df37512cfac454f`、`java-runtime-b54b14998ff59e65f2fc`、`java-runtime-945ba0790a1cfa2b1033`、`java-runtime-fef4b3a46f9b7da65a72`、`java-runtime-d8cb24fc9e29421236e2`、`java-runtime-e22957dc02c3ac4b8bb7`、`java-runtime-284497e8a9729bb76ac2`、`java-runtime-f6718d2a96a3e3668239`、`java-runtime-51773f11823a58156447`、`java-runtime-b891f1f54423ccb8a628`、`java-runtime-dd95faed8e31c95bec9f`、`java-runtime-6e68e972f23cef8dde85`） | matched `ThenInsertInto` + `ThenUpdate`；`TestTriggerMatchedMergeInsertStreamReadsTargetRow`、`TestTriggerMatchedMergeInsertStreamReadsTargetRowTypedSource` | `mapped`：side-stream projection 可读取当前 target row；side-stream-only 不产生伪造目标 old/new，后续 update 才提交目标变化；Go 已把完整 Java representation/target 矩阵登记为 source coverage，但当前测试实际覆盖 schema-bound map-equivalent Table/Named Window 语义；其它 representation conversion 与完整 insert/delete flow 仍为 `partial` |

这五个 case 已写入 `compat/capability-manifest.json`，并保留 `status=mapped` 的含义：它只表示 Java 来源、Go 测试和能力入口已关联；尚未有共享 Java/Go 中立 trace，因此不能改成 `passing`。本轮新增的 `plan.go` 遍历也必须收集 merge insert projection 的表达式依赖，避免参数依赖或 canonical/hash 漏失。
...
仍未关闭的遗漏必须单独建 case，而不能复用上面测试的名字：Java `InfraFlow` 除本节已覆盖的过滤 insert、DeleteAll、merge 生命周期、plan redeploy 和 wildcard insert 子集之外的 EPL/SODA 语义、歧义列诊断、模块依赖、表示转换、事务/持久化、split-stream 与完整 Java trace，`InfraMultipleInsert` 除本节四个 not-matched 分支之外的表示转换、rollback/transaction、split-stream 与完整 Java trace，以及 `InfraInsertOtherStream` 的 ObjectArray/JSON/Avro/XML representation 矩阵、InnerType/Variable、invalid/rollback、事务/持久化、索引计划和精确 Java 诊断仍属于 `partial`。其中 `InfraFlow` 与 `InfraOnMergeInsertStream` 都涉及多 action，但前者还包含既有 `insert into`、delete、重部署和 full lifecycle，不能仅凭 action-chain 测试宣称覆盖。

验证记录：

- Java：`mvn -pl regression-run -Dtest=TestSuiteInfraNWTable -DfailIfNoTests=false -Dgpg.skip=true test`；PowerShell 调用 `mvn.cmd` 时将每个 `-D...` 参数整体加引号，JDK 17.0.20、Maven 3.9.11，`26/26` 通过。
...
本轮验证：Java 使用 JDK 17/Maven 3.9.11 执行 `mvn -pl regression-run '-Dtest=TestSuiteInfraNWTable' '-DfailIfNoTests=false' '-Dgpg.skip=true' test`，26/26 通过；Go 的 `go test ./...`、`go vet ./...`、`go test ./compat`、`go test -run '^TestTriggerInfraInvalidBuildCases$' .`、`go test -race -run '^TestTriggerInfraInvalidBuildCases$' .`、`go test -race -run '^TestTriggerPatternMultimatchMatchesInfraPatternMultimatch$' .` 与 `go test -race -run 'TestTrigger|TestSubquery' .` 均通过，manifest 当前为 152 cases/152 mappings 且 runtime/case/test 引用无缺失。本轮核心与对照测试均不需要启动 MySQL。
