...
> 最新补充：Draft 3.09（2026-08-09），`ClientRuntimePriorityAndDropInstructions` 6/6 execution 已处置。新增 typed `StatementPriority`/`StatementDrop`，普通 continuous statement 在 event/timer/Named Window/routed dispatch 中按 priority 降序、同级 Drop 优先且命中后 preempt；management traversal 保持 deployment order，update-istream 继续使用独立的升序 `UpdatePriority`/`UpdateDrop`。动态 undeploy/redeploy 与 add/remove 轨迹均已覆盖。新增 6 个 runtime 和 `client.runtime.priority-drop` capability；当前 204 mapped / 0 partial / 5 approved-difference。本轮不需要 MySQL Docker。
...
| 项目 | 内容 |
|---|---|
| 文档状态 | Draft 3.09，`ClientRuntimePriorityAndDropInstructions` 6/6 execution 已 mapped；此前 `ClientRuntimeLockLogging` 1/1 与 `ClientRuntimeItself` 5/5 已处置，下一步继续 client runtime 剩余管理面 |
