...
> 最新补充：Draft 3.44（2026-08-10），`ClientDeployRollout` 3/3 与 `ClientDeployStateListener` 1/1 已 mapped：多 deployment 原子激活/逆序回滚、跨 item module dependency、statement sequence、零基错误/事件索引和 listener 管理已落地。当前 240 mapped / 0 partial / 9 approved-difference。Maven 当前不可用，未执行 Java suite；本轮不需要 MySQL Docker。


## 1. 文档信息

当前切片（Draft 3.44，2026-08-10）对照 Java `ClientDeployRollout` 与 `ClientDeployStateListener`。Go 的 `RolloutPlans(...).WithOptions(...)` 构造 typed item，`Engine.Rollout` 在单一 Engine mutex transaction 内按序激活，任一 item 失败会逆序撤销已激活 deployment、恢复 sequence 并禁止 lifecycle dispatch；成功后按 item 0..N-1 通知。普通 deploy/undeploy 的 lifecycle index 为 -1。

| 项目 | 内容 |
|---|---|
| 文档状态 | Draft 3.44，ClientDeployRollout 3/3 与 ClientDeployStateListener 1/1 已 mapped；atomic multi-deployment activation/rollback、typed item options、cross-item module dependency、statement sequence、indexed error/event 和 listener management 已落地；manifest 当前 1,787 条关联、1,761/4,140 唯一 runtime（42.54%）、240 mapped / 0 partial / 9 approved-difference；下一步继续 undeploy dependency precondition/redefinition 等 client/deploy inventory |
