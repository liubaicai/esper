...
> 最新补充：Draft 3.11（2026-08-09），`ClientRuntimeSolutionPatternPortScan` 3/3 execution 已处置。用 typed `UniqueBy + TimeWindow + GroupBy + IntoTable`、KeepAll situation window、`TimerCron` 与 reentrant listener orchestration 复现 DETECTED/UPDATE/DONE/EXPIRED 生命周期；ObjectArray 输入、contributors、29/59 秒刷新与分钟阈值清理均有对照测试。新增 3 个 runtime 和 `client.runtime.solution-port-scan` capability；当前 206 mapped / 0 partial / 5 approved-difference。本轮不需要 MySQL Docker。
...
| 项目 | 内容 |
|---|---|
| 文档状态 | Draft 3.11，`ClientRuntimeSolutionPatternPortScan` 3/3 execution 已 mapped；此前 `ClientRuntimeRuntimeProvider` 1/1、Priority/Drop 6/6 与 LockLogging 1/1 已 mapped，下一步继续 statement annotation/name/subscriber 组 |
