...
| 项目 | 内容 |
|---|---|
| 文档状态 | Draft 2.52，补充 Java `InfraNWTableFAFInsertMultirow` 的 Go-native `InsertRows(InsertValues(...))`、1000 行校验、子查询值与 Named Window/Table 原子回滚；Named Window 新增严格唯一索引约束。Variant 的单列 method-backed member conversion 与 Java application/type-class metadata 仍未关闭；其余历史增量与未完成项见下文 |
...
| 当前修订 | Draft 2.52（2026-08-07）：在 `InfraNWTableFAF` 的只读/单行/Context mutation 对照基础上，继续关闭 Java `InfraNWTableFAFInsertMultirow` 的 5 个 execution：新增 `OnDemandStream.InsertRows` 与 `InsertValues`，按目标 schema 声明顺序映射 positional values，支持 Literal/Variable/Parameter/Subquery 表达式，Build 阶段拒绝空值行、列数不匹配和超过 1000 行；执行阶段先完整求值，再在 engine 锁内一次性提交 Named Window delta 或 Table rows，Named Window/Table 的 unique/primary-key 失败均恢复整批状态。Named Window 新增 `NamedWindowUniqueIndex`，与 `Unique` retention 的替换语义区分。Go 新增多行成功、子查询、1000 行、invalid、rollback 对照测试；Java SODA 路径以 immutable fluent Plan 对应，精确 Java 诊断文本不作为 Go API 契约。Java `TestSuiteInfraNWTable` 26/26 通过；本轮核心验证不需要 MySQL Docker。 |
...
- 当前 capability manifest 已关联 1,276/4,136 个唯一 Java runtime（约 30.85% 的 Java runtime 对账/处置进度）；184 个 case 中 179 个 mapped、1 个 partial、4 个 approved-difference。该比例不是 Java/Go 行为 parity 通过率，也不是全量移植完成度。
