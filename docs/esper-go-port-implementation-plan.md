...
> 最新补充：Draft 3.15（2026-08-09），`ClientRuntimeThreadedConfigInbound`、InboundFastShutdown、Outbound、Route、Timer 共 5/5 execution 已处置，`suite/client/runtime` 的 52/52 runtime 均已有 manifest disposition。新增显式 bounded worker pools、waitable/reusable `AsyncTask`、transitive idle barrier、stats、error/panic reporting 与 fast shutdown；同步 Engine API 不变，规则状态仍由 transaction mutex 串行化，route/timer 把完整 cycle 移出调用方，显式 virtual time 取代 autonomous internal timer。当前 210 mapped / 0 partial / 5 approved-difference。Maven 当前不可用，未执行 Java suite；本轮不需要 MySQL Docker。
...
| 项目 | 内容 |
|---|---|
| 文档状态 | Draft 3.15，`suite/client/runtime` 52/52 runtime 已处置；threading 5/5 execution 已 mapped，bounded async pools、task wait/idle/stats/error/shutdown 已落地，下一步按 inventory 继续其他 client 与 multithread 域 |
