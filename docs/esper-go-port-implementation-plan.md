...
| 项目 | 内容 |
|---|---|
| 文档状态 | Draft 2.47，继续补充 typed JSON 命名 nested fragment catalog、跨表示 fragment 读取/渲染、schema-bound metadata/getter 和 null-versus-empty 渲染的 Go/Java 对照证据；此前已补充 JSON sender、显式 field adapter、Core typed、VM-class、provided-underlying、lax conversion、malformed JSON 和静态/动态未声明字段策略；其余历史增量与未完成项见下文 |
...
| 当前修订 | Draft 2.47（2026-08-07）：继续补齐 typed JSON 命名 nested fragment catalog。Go 新增 `WithNestedPropertySchemaFrom`，在 schema 构造阶段从显式 `Environment` catalog 解析独立注册的 JSON/Map fragment，缺失、nil 环境和跨环境引用立即失败；新增 Users/Friend、User/Map nested fragment 的 metadata、indexed `GetFragments`、深层 getter、RenderJSON 和 Plan catalog 证据。既有反射驱动有限 schema graph、递归指针 identity、`NestedSchemaNames`/`FragmentSchema`、`Event.GetFragment`/`GetFragments` 与 null-versus-empty 保持不变。新增 `TestJSONNamedNestedSchemaCatalogMatchesJavaCreateSchemaFragments`、`TestJSONNamedNestedSchemaCatalogRejectsMissingAndCrossEnvironmentReferences`、`TestJSONNamedNestedMapSchemaCatalogSupportsCrossRepresentationFragments`，对照 `EventJsonProvidedClassUsersEventWithCreateSchema`、`EventJsonProvidedClassClientsEventWithCreateSchema`、`EventJsonTypingNestedRecursive` 和 `EventJsonTypingClassWArrayAndColl`；manifest 进度仍为 1,211/4,136（约 29.28%），case 数保持 168（164 mapped、4 approved-difference），本轮复用既有 Java runtime 对账。完整 catalog 生命周期、所有表示的 fragment metadata/identity、Java module/path visibility、schema evolution 和共享 Java/Go parse-write trace 仍为 partial；本轮核心验证不需要 MySQL Docker。 |
