...
本轮继续对照 Java `ExprEnumDocSamples.ExprEnumSubquery`/`ExprEnumNamedWindow` 与 `ExprEnumDataSources.ExprEnumUDFStaticMethod`：Go 增加 `SubqueryValues`/`SubqueryEvents`，将 named-window 多行快照（含 inner predicate）转换成可继续 `EnumSelect` 的 typed slice；同时增加 `Func0`，使零参数 UDF 返回的集合可以直接进入 `EnumSum` 等枚举链。`TestEnumerableSubqueryAndZeroArgumentUDFFSources` 固定 E1/E2 named-window 过滤、投影求和及 UDF 求和。事件流 subquery、访问聚合/Context/Table/Pattern source、声明表达式、generic component metadata 和完整 invalid 矩阵仍保持开放；该切片登记为 `case.expr-enum-subquery-udf`。
