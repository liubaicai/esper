...
> 最新补充：Draft 3.24（2026-08-09），`ClientCompileStatementName`、`ClientCompileUserObject`、`ClientCompileOutput` 共 4/4 execution 已 mapped。新增 typed Build-time statement-name/user-object resolver、`StatementCompileContext`、`Statement.UserObject()` 和 `Plan.Manifest()`；resolver error/blank name 在 Plan 产生前失败，compiler version/provider 使用稳定 Go contract。当前 219 mapped / 0 partial / 7 approved-difference。Maven 当前不可用，未执行 Java suite；本轮不需要 MySQL Docker。


## 1. 文档信息

当前切片（Draft 3.24，2026-08-09）对照 Java `ClientCompileStatementName`、`ClientCompileUserObject` 与 `ClientCompileOutput`。Go 在 `Environment.Build` 上提供 typed `CompileOption`，resolver context 直接携带 immutable Query、typed description、statement/module identity 和 detached metadata，不提供 raw EPL supplier；name resolver 先运行，user-object resolver 随后观察解析后的名称。opaque user object 接受 native Go 值且不要求 Serializable，部署后由 `Statement.UserObject()` 读取；resolver callback 不持有 build mutex，错误与 blank name 不产生 Plan。`Plan.Manifest()` 报告 `esper-go-compiler/v1` 和稳定 fluent builder provider，取代 Java bytecode compiler version/generated module-provider class 的具体形式。当前 4 个 Java runtime 都是单 statement，statement number 为 0；多 query 一次 compile、portable user-object serialization、generated class inspection 与 raw annotation/EPL reflection 不在本切片宣称范围。

| 项目 | 内容 |
|---|---|
| 文档状态 | Draft 3.24，ClientCompileStatementName/UserObject/Output 4/4 execution 已 mapped；typed Build resolver context、opaque statement user object 与 Plan compiler manifest 已落地，manifest 当前 1,728 条关联、1,702/4,140 唯一 runtime（41.11%）、219 mapped / 0 partial / 7 approved-difference；下一步继续 compile module/path/SPI/syntax 或其他未映射 suite |
