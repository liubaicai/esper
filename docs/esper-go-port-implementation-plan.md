...
> 最新补充：Draft 3.57（2026-08-10），PatternCompositeSelect suite 2/2 关闭（followed-by 模式 insert-into 标签片段保留、MatchUntil(2,2) 重复标签 []Event 索引片段对照 Java indexed fragment）；TagEvents 显式投影取代 Java wildcard fragment 描述符。当前 278 mapped / 1 partial / 17 approved-difference。本轮不需要 MySQL Docker。
...
| 项目 | 内容 |
|---|---|
| 文档状态 | Draft 3.57，Client domain 100% 关闭（281/281）；View domain 22 个唯一 runtime（KeepAll/FirstUnique/FirstEvent/LastEvent/FirstLength/LengthWindow/Sort/TimeWindow/TimeBatch 6/10），Pattern domain 5 个（And/Or 3、CompositeSelect 2/2）；ExprFilter 23、ExprEnum 27、ResultSet 2、EPLOther 2、EPLInsertInto 1；manifest 当前 1,943 条关联、1,908/4,140 唯一 runtime（46.09%）、278 mapped / 1 partial / 17 approved-difference；下一步 Pattern 基础 suite（Every/Not/FollowedBy 批量）、View Expression/Accum 系列与 ExprFilter 剩余 |
