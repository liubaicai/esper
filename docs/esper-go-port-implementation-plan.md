...
> 最新补充：Draft 3.32（2026-08-09），`ClientCompileExceptionItems` 3/3 execution 已 mapped。新增 typed `CompileItem`/`CompileBatchError` 与 all-or-nothing `CompileBatch`，完整收集 ordered item causes、应用 line metadata 和稳定 single-line diagnostic text；失败批次不返回部分 Plan。当前 227 mapped / 0 partial / 7 approved-difference。Maven 当前不可用，未执行 Java suite；本轮不需要 MySQL Docker。


## 1. 文档信息

当前切片（Draft 3.32，2026-08-09）对照 Java `ClientCompileExceptionItems`。`CompileBatch(env, NewCompileItem(query).AtLine(...).WithDiagnosticText(...), ...)` 对所有 typed Query 执行 Build 并按输入顺序收集 `CompileErrorItem`；item 保存 Index/Line/Expression/Cause，diagnostic text 仅作应用 metadata，折叠 whitespace 后稳定输出且从不解析。批次只有全成功才返回全部 Plan，任何失败均返回 nil Plans 和 detached ordered error list；empty batch 合法，invalid item metadata 也参与聚合。Java 的 EPL parser/source line 自动发现由显式应用 line/text 取代，unknown schema type/parser message 则映射到真实 typed Build cause。

| 项目 | 内容 |
|---|---|
| 文档状态 | Draft 3.32，ClientCompileExceptionItems 3/3 execution 已 mapped；typed batch all-or-nothing compilation、ordered multi-error items、line/text metadata 与 detached diagnostics 已落地，manifest 当前 1,750 条关联、1,724/4,140 唯一 runtime（41.64%）、227 mapped / 0 partial / 7 approved-difference；下一步继续 statement object model、large compiler boundaries 或其他未映射 suite |
