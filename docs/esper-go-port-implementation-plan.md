...
当前切片（Draft 2.72，2026-08-08）继续补齐 Context Table 物理分区语义：相同 primary key 可在不同 Context partition 独立存在，live `InsertIntoTable`/`UpsertIntoTable`/merge、分区内 select/update/delete、Context FAF snapshot 与 aggregate `IntoTable` scoped replacement 均已覆盖；temporal/initiated partition 释放时回收 Table scoped state 与 row ownership。Context-scoped Table 现支持相关子查询的当前分区 primary/secondary hash 与 equality-prefix + B-tree candidate；普通 FAF/Join 在没有当前分区变量时遍历 root/scoped index states，根状态有潜在 legacy rows 时相关子查询回退 ownership-aware snapshot。完整跨 statement transaction、跨目标 routed side effect、listener/external resource rollback 和 transaction/concurrency trace 仍是后续项。

| 项目 | 内容 |
|---|---|
| 文档状态 | Draft 2.72，补充 Context-scoped Table 的 scope-aware hash/primary/B-tree candidate：相关 FAF 子查询在当前 partition 直接查对应物理 state；普通 Context FAF/Join 在无当前 partition 变量时合并 root/scoped candidate，根状态存在 legacy rows 时保留 ownership-aware snapshot fallback。此前已完成的 live `InsertIntoTable`/`UpsertIntoTable`/merge ownership、duplicate primary-key isolation、aggregate scoped replacement、生命周期回收、单源 Table/Named Window equality/range candidate、普通内连接及两流 outer optional side、Context candidate 与 B-tree ordered candidate，以及 `InfraUpdate` 全量更新/re-key 对照继续有效。Java `@Hint`/JVM query-plan hook 不复制为 Go API 外形；Context right/full-preserving/non-adjacent chain、成本模型、subquery index-sharing 和性能阈值仍明确列为后续项 |
...
| 当前修订（最新） | Draft 2.72（2026-08-08）：继续对照 Java `ContextKeyedSegmentedTable` 与 `InfraNWTableFAFSubquery/FAFIndex`，新增 Context-scoped Table 的 equality/range 子查询 candidate 与单源 FAF all-state candidate。`TestInfraFAFSubqueryContextScopedTableIndexCandidateParity` 验证相同 primary key 在 A/B partition 命中不同值并在 root legacy row 出现时回退 snapshot；`TestInfraFAFSubqueryContextScopedTableRangeIndexCandidateParity` 验证分区内 equality-prefix + B-tree range；`TestInfraFAFContextScopedTableIndexCandidateParity` 验证普通 Context FAF 合并 root/A/B 三组物理 state。此前的 live ownership、aggregate replacement、lifecycle cleanup、FAF mutation rollback 和 Java `TestSuiteContext` 17/17 继续通过；完整跨 statement transaction、跨目标 routed side effect、listener/external resource rollback、更广 Context Table mutation、Context right/full chain candidate、subquery index-sharing、成本/性能模型和 transaction/concurrency trace 仍列后续项，本轮不需要 MySQL Docker。 |
