...
> 最新补充：Draft 3.48（2026-08-10），`ClientDeployPreconditionDependency` 12/12 登记完成（8 mapped + preconfig/index/class 4 个 approved-difference），`ClientDeployPreconditionDuplicate` 9/9 关闭：Java deploy 期 duplicate precondition 在 Go 注册边界强制执行（`DuplicateModuleObjectError` 保留 Java 消息文本与 `NameAndParamNum` script 身份），Index/Class 登记 approved-difference；manifest 笔误 runtime ID 修正。当前 244 mapped / 0 partial / 12 approved-difference。JDK 17 + Maven 3.9.9 下 Java `TestSuiteClientDeploy` 11/11 通过；本轮不需要 MySQL Docker。


## 1. 文档信息

当前切片（Draft 3.48，2026-08-10）登记并关闭 Java `ClientDeployPreconditionDependency` 与 `ClientDeployPreconditionDuplicate`：deploy-time path dependency 前置条件的 12 个 execution 全部登记 manifest（preconfig 事件类型/变量在 Go 编译边界固定同一保证，index/class 为 approved-difference）；duplicate 前置条件的 7 类对象（NamedWindow/Table/EventType/Variable/ExprDecl/Script/Context）经 `registration_duplicate.go` 在注册期拒绝重复 catalog identity，消息文本与 unnamed module 渲染对齐 Java，Index/Class 经处置测试登记 approved-difference。下一步继续 `ClientDeployListDependencies`/`ClientDeployRedefinition` 与 `ClientStage` 剩余 suite。

| 项目 | 内容 |
|---|---|
| 文档状态 | Draft 3.48，ClientDeployUndeploy 11/11、ClientDeployPreconditionDependency 12/12、ClientDeployPreconditionDuplicate 9/9 全部处置；deploy-time path dependency 前置条件与注册期 duplicate 前置条件（Named Window/Table/变量/Context/事件类型/声明表达式/Script）已落地，Index/Class 与 preconfig 偏移交 approved-difference；manifest 当前 1,819 条关联、1,793/4,140 唯一 runtime（43.31%）、244 mapped / 0 partial / 12 approved-difference；下一步继续 `ClientDeployListDependencies`/`ClientDeployRedefinition` 与 `ClientStage` 剩余 suite |
