...
| 项目 | 内容 |
|---|---|
| 文档状态 | Draft 2.42，补充 JSON VM-class 标量（UUID、时间、URL、URI）及数组/集合形状、数值 lax conversion、非法值诊断、malformed JSON 和静态/动态未声明字段策略的 Go/Java 对照；此前已补充 JSON provided-underlying 的 Go-native 泛型结构体适配、schema invalid、constructor-style primitive default、null projection route、pattern typed-array，以及与 Java `EventJsonProvidedUnderlyingClass` 的 9 个选定 runtime 对账；其余历史增量与未完成项见下文 |
...
| 当前修订 | Draft 2.42（2026-08-07）：完成 Java `EventJsonTypingVMClass` 与 `EventJsonParserLaxness` 的 Go 对照切片，新增 `URL`/`URI` 标量类型及解析/渲染，修正时间、UUID、精确数值非法标量必须报错而非静默 Null 的语义，并覆盖一维/二维数组、集合、null、shape mismatch、malformed JSON、undeclared static/dynamic policy。核心 JSON 定向测试通过；全量 Java JSON 13/13 作为同一回归基线通过；完整 dynamic JSON matrix、递归 schema/metadata、所有嵌套 BigDecimal/BigInteger write policy 与共享 trace 仍保持 partial；本轮不需要 MySQL Docker。 |
