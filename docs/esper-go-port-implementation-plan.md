...
> 最新补充：Draft 3.36（2026-08-09），`ClientCompileLargeWConfig` MAP/ObjectArray select-column 2/12 execution 已 mapped。两种 representation 均覆盖 5,000 字段 schema、5,000 typed concat projections、完整 property/value 验证和真实 map/ordered-slice underlying；suite 累计关闭 4/12。当前 231 mapped / 0 partial / 7 approved-difference。Maven 当前不可用，未执行 Java suite；本轮不需要 MySQL Docker。


## 1. 文档信息

当前切片（Draft 3.36，2026-08-09）对照 Java `ClientCompileLargeWConfig.ClientCompileLargeSelectCol` 的 MAP 与 OBJECTARRAY execution。Go 为每种 representation 注册 5,000 字段 target schema，以 `Select(...).InsertInto(target)` 显式表达 output representation；下游 statement 接收 materialized Event，逐项验证 `cN=xN`、schema kind/field count，并固定 MAP 的 `map[string]any` 与 ObjectArray 的 ordered `[]any` underlying。相比 Java `@EventRepresentation` annotation，多一个显式 typed route，但实际 metadata/value/underlying 契约保持一致；suite 剩余 8 个 aggregation/table/create-schema execution 继续迁移。

| 项目 | 内容 |
|---|---|
| 文档状态 | Draft 3.36，ClientCompileLargeWConfig 累计 4/12 execution 已 mapped；MAP/ObjectArray 5,000-column projection、完整字段值和真实 underlying representation 已落地，manifest 当前 1,760 条关联、1,734/4,140 唯一 runtime（41.88%）、231 mapped / 0 partial / 7 approved-difference；下一步继续 large create-schema/insert 或 aggregation/table 分组 |
