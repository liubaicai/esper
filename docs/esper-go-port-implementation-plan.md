...
> 最新补充：Draft 2.98（2026-08-09），`EPLOtherSplitStream` 推进至 10/12 execution。新增 `SplitFrom(source).Into/IntoWhen` branch-local 链式 API，contained-event 分支按声明顺序执行并保持 Begin/body/End 路由时序，output-first 选择首个产出行的分支；父事件字段/事件投影与文档示例的 initiated-terminated context 终止计数均已对照。新增 1 个 runtime 登记到 partial `case.epl-other-split-stream`；SplitPremptiveNamedWindow、SingleInsert 两项仍待关闭。本轮不需要 MySQL Docker。
...
| 项目 | 内容 |
|---|---|
| 文档状态 | Draft 2.98，`EPLOtherSplitStream` 完成 10/12 execution；基础分流、subquery 与 contained-event branch source 已覆盖，剩余 context single-insert、preemptive named-window 两项继续迁移 |
