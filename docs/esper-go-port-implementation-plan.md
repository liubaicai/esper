...
> 最新补充：Draft 2.88（2026-08-08），方法源 NW-twice 切片：对照 Java `EPLFromClauseMethod3Stream1HistStreamNWTwice`，新增 `Stream.As(alias)` 链式别名。同一个 keepall Named Window 在单条 Join 中以 us/them 两个逻辑名出现两次，方法源通过 `DependingOn("us","them")` 按别名做依赖拓扑排序与 invocation 笛卡尔展开，provider 经 `request.Dependency` 按别名读取两侧窗口事件；窗口目录查找与事件匹配仍使用注册名（`methodJoinEvaluationOrder` 与 `methodDependencyInvocations` 统一走 `logicalName`）。Go 侧用 `InsertNamedWindow` 替代 Java 的 insert-into 语句填充窗口，`SelectSourceEvent` 投影 us/them 完整事件并对照 Java 的两行 `{one,two,1}/{two,one,1}` 结果。新增 1 个 Java runtime（`java-runtime-c33c309561a570b93373`）已登记到 `case.epl-fromclausemethod-basic` 与 capability `epl.fromclausemethod`。定向 Go 测试通过；Java `TestSuiteEPLFromClauseMethod` 保持 3/3 通过。outer joins、cache、context、multikey/array-key、performance 与完整 Java 诊断仍为后续切片；本轮不需要 MySQL Docker。
...
| 项目 | 内容 |
|---|---|
| 文档状态 | Draft 2.88，方法源 NW-twice 切片：Go 新增 `Stream.As(alias)` 链式别名，同一 keepall Named Window 在单条 Join 中以 us/them 两个逻辑名出现两次；方法源 `DependingOn("us","them")` 按别名拓扑排序并做 invocation 笛卡尔展开，窗口目录查找与事件匹配仍用注册名。本轮对照 `EPLFromClauseMethod3Stream1HistStreamNWTwice`；outer joins、cache、context、multikey/array-key、performance 与完整 Java 诊断仍为后续项 |
