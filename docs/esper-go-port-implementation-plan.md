...
> 最新补充：Draft 3.56（2026-08-10），ViewTimeBatch suite 累计 6/10（补 Multirow/MultiBatch/NoRefPoint/Longer：多事件累积、连续批次 IR-pair 链、首事件锚点精确到期、20 边界稳定性遍历）；剩余 RefPoint/StartEagerForceUpdate/MonthScoped/WSystemTime 待 reference-point/start-eager/force-update/日历选项切片。当前 277 mapped / 1 partial / 17 approved-difference。本轮不需要 MySQL Docker。
...
| 项目 | 内容 |
|---|---|
| 文档状态 | Draft 3.56，Client domain 100% 关闭（281/281）；View domain 已处置 22 个唯一 runtime（KeepAll 3/3、FirstUnique 2/2、FirstEvent/LastEvent/FirstLength、LengthWindow 3、Sort 6/6、TimeWindow 2、TimeBatch 6/10），time_batch old-only flush 与批式窗口迭代器语义已对齐 Java；manifest 当前 1,941 条关联、1,906/4,140 唯一 runtime（46.04%）、277 mapped / 1 partial / 17 approved-difference；下一步 View Expression/Accum/FirstTime 系列、TimeBatch 选项切片（refpoint/start-eager/force-update）与 ExprFilter/Pattern 批量登记 |
