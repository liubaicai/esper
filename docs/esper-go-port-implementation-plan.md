...
> 最新补充：Draft 2.95（2026-08-09），对照 `EPLOtherUnaryMinus` 的 1 个 execution。新增 `TestEPLOtherUnaryMinusMatchesEsper`，以 Go 链式 `Negate(Field(...))`/`Negate(VariableRef(...))` 固定事件整数字段和运行时浮点变量的负号投影（`-10`/`-1.0`）以及变量无副作用；Java runtime `java-runtime-8c3dcb3623deea547561` 已登记到 `case.epl-other-unary-minus` 并映射至 `expr.core`。Go 使用类型化、可分析的表达式构造器，不复制 EPL 前缀语法。本轮不需要 MySQL Docker。
...
| 项目 | 内容 |
|---|---|
| 文档状态 | Draft 2.95，对照 `EPLOtherUnaryMinus` 的 1 个 execution；Go 链式 `Negate` 覆盖事件字段/运行时变量负号投影与变量无副作用，runtime 已登记到 `case.epl-other-unary-minus`；TestSuiteEPLOther 的其余未登记类继续按 execution 分段迁移 |
