...
> 最新补充：Draft 3.62（2026-08-10），PatternOperatorAnd/Or 两 suite 剩余 4 个 execution 关闭（And 4/4、Or 3/3）：and 改写为 eventsPerChild 双侧匹配列表语义（新鲜完成与对侧缓存笛卡尔组合逐条回调、fireOnly 仅发射不入保留集、every 按回调逐次派生），新鲜性判定排除早前已满足侧修复定时器回调双触发；or 保持永久完成分支杀死整体、every/not 支存活语义；timer:interval(0) 放行且部署时钟即到期；模式匹配流接入无分组 count(*) 聚合。当前 281 mapped / 1 partial / 17 approved-difference。本轮不需要 MySQL Docker。
>
...
| 项目 | 内容 |
|---|---|
| 文档状态 | Draft 3.62，Client domain 100% 关闭（281/281）；View domain 22 个唯一 runtime，Pattern domain 31 个（And/Or 7、CompositeSelect 2/2、Every 6/6、Not 6/6、FollowedBy 10/10）；ExprFilter 23、ExprEnum 27、ResultSet 2、EPLOther 2、EPLInsertInto 1；manifest 当前 1,967 条关联、1,932/4,140 唯一 runtime（46.67%）、281 mapped / 1 partial / 17 approved-difference；下一步 Pattern 基础 suite（EveryDistinct/MatchUntil/ConsumingPattern/Observer 批量）与 View Expression/Accum 系列、ExprFilter 剩余 |
