...
> 最新补充：Draft 3.07（2026-08-09），`ClientRuntimeItself` 5/5 execution 已处置。新增 immutable named runtime service、ordered typed statement traversal/selection 与 deployment metadata；anonymous bean type 用未注册 `StructSchema` 对照，reflective EPL/SODA compile 用 typed Build/Plan/FAF/Snapshot 对照。Java module-vs-FAF `EPCompiled` kind mismatch 在 Go 单一 immutable Plan 中不存在，单列为 approved difference。新增 5 个 runtime 和 `client.runtime.self-management` capability；当前 202 mapped / 0 partial / 5 approved-difference。本轮不需要 MySQL Docker。
...
| 项目 | 内容 |
|---|---|
| 文档状态 | Draft 3.07，`ClientRuntimeItself` 5/5 execution 已处置（4 mapped + 1 approved difference）；此前 `ClientRuntimeExceptionHandler` 1/1、`ClientRuntimeTimeControl` 2/2、`ClientRuntimeListener` 1/1 与 `ClientRuntimeUnmatchedListener` 3/3 保持 mapped，下一步继续 client runtime 管理面 |
