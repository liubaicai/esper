...
> 最新补充：Draft 3.03（2026-08-09），`ClientRuntimeUnmatchedListener` 3/3 execution 已映射。新增 engine-wide `SetUnmatchedListener`，覆盖外部事件、insert-into 派生 route、nil disable、undeploy/redeploy 与 callback 内重入 Deploy；判定基于 active input stream filter 而非后置 having/output。新增 3 个 runtime 和 `client.runtime.unmatched-listener` capability；当前 198 mapped / 0 partial / 4 approved-difference。本轮不需要 MySQL Docker。
...
| 项目 | 内容 |
|---|---|
| 文档状态 | Draft 3.03，`ClientRuntimeUnmatchedListener` 3/3 execution 已映射；此前 `ClientRuntimeEPStatement` 2/2、`client/basic` 7/7 与 `EPLOtherSplitStream` 12/12 保持 mapped，下一步继续 client compile/deploy/runtime 管理面 |
