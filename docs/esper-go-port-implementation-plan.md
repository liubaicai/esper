...
> 最新补充：Draft 3.40（2026-08-09），`ClientCompileModule` 7/7 execution 已处置，`ClientCompile*` inventory 无未映射 execution。typed module metadata/two-module deployment 已 mapped；5 条 EPL module parser/text/comment execution 按无 EPL 输入原则 approved-difference。当前 235 mapped / 0 partial / 8 approved-difference。Maven 当前不可用，未执行 Java suite；本轮不需要 MySQL Docker。


## 1. 文档信息

当前切片（Draft 3.40，2026-08-09）对照 Java `ClientCompileModule` 7 个 execution。Go 为 typed Module/Deployment 增加 detached URI/archive/user-object/uses/import metadata 与 activation time；同一 module Plan 两次部署固定 `+1` 行为，两个各含 2 个 Plan 的 module 可同时部署并保持 metadata/order。EPL module parser、comment/line span、classpath resource diagnostics 与 MODULETEXT 不进入 Go API，相关 5 条以 typed construction/rejection 测试登记 approved-difference。

| 项目 | 内容 |
|---|---|
| 文档状态 | Draft 3.40，ClientCompileModule 7/7 execution 已处置且 ClientCompile* inventory 已闭环；typed module metadata/multi-plan deployment 已落地，EPL module text/parser 维持明确 approved difference；manifest 当前 1,775 条关联、1,749/4,140 唯一 runtime（42.25%）、235 mapped / 0 partial / 8 approved-difference；下一步继续其他 client/runtime inventory |
