...
> 最新补充：Draft 3.23（2026-08-09），`ClientCompileSubstitutionParams` 25/25 execution 已处置。新增 live `DeployWithPositionalParameters`、statement-local `WithDeploymentParameterResolver`、custom deployment ID、resolver metadata 与部署前原子参数验证；覆盖 named/positional、Pattern、Subquery、inheritance、multi-plan、array/generic 和 invalid matrix。24 个 runtime mapped；JVM-only arbitrary Object constant 限制作为 1 个 approved difference，Go native literal 使用确定性 canonical。当前 217 mapped / 0 partial / 7 approved-difference。Maven 当前不可用，未执行 Java suite；本轮不需要 MySQL Docker。


## 1. 文档信息

当前切片（Draft 3.23，2026-08-09）对照 Java `ClientCompileSubstitutionParams`。Go 继续使用 analyzable typed expression，而不是 EPL placeholder：named `Parameter[T]`、1-based positional `ParameterAt[T]` 可由单 Plan deploy API 或 multi-plan `WithDeploymentParameterResolver` 绑定；resolver context 暴露 immutable Plan、custom deployment/statement identity、metadata、稳定声明类型与 named ordinal。所有 statement 的参数、mode、类型和 resolver error 在 deployment 可见前验证，Pattern 与 stream Subquery 的 retained state 按 deployment 隔离；普通 stream Subquery source/window/historical-provider 的参数收集遗漏已修复。Java untyped 参数用 `Parameter[any]` + `Cast`，SODA/EPL 双入口合并为一个 fluent Plan。Java 因 JVM bytecode portable representation 拒绝 arbitrary Object constant，而 Go native literal 可确定性 canonical，因此作为 approved difference 保留。EPL parser 关键字、mutable `setObject`、portable JVM deployment serialization 与精确 Java diagnostic text 不复制；更广 substitution-parameter suite 继续逐项对账。

| 项目 | 内容 |
|---|---|
| 文档状态 | Draft 3.23，ClientCompileSubstitutionParams 25/25 execution 已处置；24 个 runtime mapped、1 个 JVM constant-representation runtime approved-difference，manifest 当前 1,724 条关联、1,698/4,140 唯一 runtime（41.01%）、217 mapped / 0 partial / 7 approved-difference；下一步继续 compile define/per-definition visibility 或其他未映射 suite |
