...
> 最新补充：Draft 3.27（2026-08-09），`ClientCompilePathCache` 5/5 execution 已 mapped。新增 application-owned、线程安全的 typed `CompilePathCache`/`CachedModulePath`，以原子 catalog snapshot、SHA-256 fingerprint、catalog-change invalidation 与 deterministic stats 取代 JVM ModuleProvider classloading cache；object matrix、protected module、fill/reuse、10 层 event type chain 和 duplicate path 均有对照测试。当前 222 mapped / 0 partial / 7 approved-difference。Maven 当前不可用，未执行 Java suite；本轮不需要 MySQL Docker。


## 1. 文档信息

当前切片（Draft 3.27，2026-08-09）对照 Java `ClientCompilePathCache`。Go 的 `CompilePathCache.Prepare(ModulePath)` 在同一 Environment read snapshot 中枚举 visible catalog，计算 stable in-process SHA-256 fingerprint，并生成 event type、variable、context、Named Window、Table、declared expression 与 script 的 immutable resolver matrix；并发重复 Prepare 仅产生一个 miss，其余命中同一 entry。explicit selected module 目录变化会生成新 entry，无关 private module 不改变 fingerprint；protected module 的 same-module path 保持可见，duplicate public export 在有/无 cache 时均返回 `ErrorAmbiguous`。`CachedModulePath.Build` 保留 module/uses Plan identity。Go 不生成 EPCompiled/ModuleProvider bytecode，也不观察 ClassLoaderProvider；application-owned cache、detached statistics 和 Clear 取代 Java singleton/classloader detector。跨进程 persistent cache、portable artifact 与 compiler SPI/syntax 继续作为后续切片。

| 项目 | 内容 |
|---|---|
| 文档状态 | Draft 3.27，ClientCompilePathCache 5/5 execution 已 mapped；atomic typed catalog snapshot cache、fingerprint invalidation、protected same-module lookup、event-type chain 与 duplicate-path validation 已落地，manifest 当前 1,741 条关联、1,715/4,140 唯一 runtime（41.43%）、222 mapped / 0 partial / 7 approved-difference；下一步继续 compile SPI/syntax/tool compiler 或其他未映射 suite |
