...
> 最新补充：Draft 3.43（2026-08-10），`ClientDeployResult` 非 rollout 的 3/4 execution 已 mapped：active deployment 管理、双键 statement lookup、module dependency snapshot、name trim 与 duplicate deployment ID rejection 已落地；state listener 留待 rollout。当前 238 mapped / 0 partial / 9 approved-difference。Maven 当前不可用，未执行 Java suite；本轮不需要 MySQL Docker。


## 1. 文档信息

当前切片（Draft 3.43，2026-08-10）对照 Java `ClientDeployResult` 的部署结果与管理入口。Go 新增 active deployment enumeration/lookup、deployment-scoped statement lookup 和 detached module dependency snapshot；statement name 在 Build/lookup 边界 trim，重复 deployment ID 不改变 active set。Java EPL source property 由 typed Query description 替代。依赖 rollout item event 的 state-listener execution 仍开放。

| 项目 | 内容 |
|---|---|
| 文档状态 | Draft 3.43，ClientDeployResult 非 rollout 3/4 已 mapped；active deployment/result/statement management、module dependency snapshot、name trim 和 duplicate ID rejection 已落地，state listener 留待 rollout；manifest 当前 1,783 条关联、1,757/4,140 唯一 runtime（42.44%）、238 mapped / 0 partial / 9 approved-difference；下一步继续 rollout/state listener/dependency precondition |
