...
> 最新补充：Draft 3.42（2026-08-10），`ClientDeployStatementName` 1/1 与 `ClientDeployUserObject` 2/2 已 mapped，部署期 name/user-object resolver 具备 typed context 与 runtime propagation；`ClientDeployClassLoaderOption` 1/1 按无 JVM generated class 原则 approved-difference。当前 237 mapped / 0 partial / 9 approved-difference。Maven 当前不可用，未执行 Java suite；本轮不需要 MySQL Docker。


## 1. 文档信息

当前切片（Draft 3.42，2026-08-10）对照 Java deployment runtime statement options。Go 的 name resolver context 公开 Plan/original name/显式 deployment ID/typed description/detached metadata；user-object resolver 在 runtime name 之后运行，并把 nil/string/native struct override 传播到 Statement 与表达式 evaluation context。JVM-only deployment ClassLoader callback 不进入 Go API。三个 statement option execution 已 mapped，一个 class-loader execution 已 approved-difference。

| 项目 | 内容 |
|---|---|
| 文档状态 | Draft 3.42，ClientDeployStatementName/UserObject 3/3 已 mapped，ClassLoaderOption 1/1 approved-difference；部署期 typed name/user-object context 与 runtime propagation 已落地；manifest 当前 1,780 条关联、1,754/4,140 唯一 runtime（42.37%）、237 mapped / 0 partial / 9 approved-difference；下一步继续 rollout/dependency/result 等 client/deploy inventory |
