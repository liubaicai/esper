...
> 最新补充：Draft 3.10（2026-08-09），`ClientRuntimeRuntimeProvider.ClientRuntimeObtainEngineWideRWLock` 1/1 execution 已处置。新增 callback-based `WithRuntimeWriteLock`，在 Engine transaction mutex 下提供可自动释放的独占管理边界，并用并发 send 阻塞/释放与 count snapshot 固定行为；Java execution 未使用 readLock，Go 不暴露裸 mutex handle。新增 1 个 runtime 和 `client.runtime.instance-lock` capability；当前 205 mapped / 0 partial / 5 approved-difference。本轮不需要 MySQL Docker。
...
| 项目 | 内容 |
|---|---|
| 文档状态 | Draft 3.10，`ClientRuntimeRuntimeProvider` direct inventory 1/1 execution 已 mapped；此前 `ClientRuntimePriorityAndDropInstructions` 6/6、`ClientRuntimeLockLogging` 1/1 已 mapped，下一步继续 client runtime solution/annotation/name/subscriber 组 |
