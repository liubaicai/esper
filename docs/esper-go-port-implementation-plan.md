...
本轮继续对照 Java `EPLContainedEventSplitExpr.EPLContainedSplitExprReturnsEventBean` 与 `EPLContainedSingleRowSplitAndType`：新增 `UnnestAs[T,V]`/`UnnestEvents[T]`，以显式注册的 target schema 替代 EPL `@type(...)`，并在 Build 阶段校验 target 存在、property element 类型和 source schema。Event-valued split 通过 `NewEvent` 创建 A/B concrete member，`UnnestEvents(..., "ContainedSplitVariant")` 保留具体 `TypeName`、member-only 字段和 immediate parent；raw `map[string]any`、JSON 文本则分别走目标 Struct/JSON schema 的 materialization，未知、空 target 和不兼容 concrete Event 有明确 Build/runtime 错误。相关证据为 `TestContainedSplitEventArrayPreservesVariantMembersMatchesEsper`、`TestContainedTypeMaterializesMapAndJSONElementsMatchesEsper`、`TestContainedTypeRejectsUnknownAndIncompatibleTargets`，并登记为 `case.event-contained-split-type`。Java 的 ObjectArray/Avro/XML representation loop、脚本/UDF 执行、wildcard method adapter、多级 splitter、精确 EventBean metadata、contained Table/FAF 和完整 split/type trace 仍保持 partial；因此 `event.contained` 仍不是全量完成。

本次复核从 `compat/java-execution-inventory.jsonl` 补出同一 Java 类的第三个 execution：`EPLContainedScriptContextValue`（`java-runtime-e5e8e7be2d9e28ca1fcf`）。该 runtime 已加入 `case.event-contained-split-type`，但当前没有 Go 脚本执行器或等价 ScriptProvider 测试，因此只作为已发现、未实现的 Java 资产登记，不能把该 case 或 `event.contained` 标为完整。`go test ./...`、`go test -race ./...`、`go vet ./...` 和受影响文件的 gofmt 检查均已通过；本轮 contained 语义不需要 MySQL，数据库 Docker 仍只在 SQL/connector 门禁显式启用时启动。
