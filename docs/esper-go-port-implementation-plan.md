...
> 最新补充：Draft 3.41（2026-08-10），`ClientDeployVersion` 1/1 execution 已 mapped。immutable Plan/PlanArtifact 现在同时携带 schema/compiler version，Deploy、DeployPlans 与 FAF 共享结构化兼容性 preflight；单项 batch 错误报告 plan index 0。完整原子 Engine.Rollout 仍归 `ClientDeployRollout` 后续切片。当前 236 mapped / 0 partial / 8 approved-difference。Maven 当前不可用，未执行 Java suite；本轮不需要 MySQL Docker。


## 1. 文档信息

当前切片（Draft 3.41，2026-08-10）对照 Java `ClientDeployVersionMinorCheck`。Go 不读取 Java serialized JAR，而让每个 typed immutable Plan 与 PlanArtifact 显式保存 schema/compiler provenance；普通部署、多计划部署和 FAF 在运行前统一拒绝旧 contract，多计划错误保留 zero-based plan index 0，artifact loader 同时校验两种版本。该 execution 已闭环；完整 rollout 原子性仍由 `ClientDeployRollout` 独立对照。

| 项目 | 内容 |
|---|---|
| 文档状态 | Draft 3.41，ClientDeployVersion 1/1 execution 已 mapped；Plan/PlanArtifact schema/compiler compatibility preflight 已覆盖 Deploy、DeployPlans plan index 与 FAF，完整原子 rollout 仍待 ClientDeployRollout；manifest 当前 1,776 条关联、1,750/4,140 唯一 runtime（42.27%）、236 mapped / 0 partial / 8 approved-difference；下一步继续其他 client/deploy inventory |
