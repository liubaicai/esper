...
> 最新补充：Draft 2.92（2026-08-09），补齐方法源 with-arg 四切片：ArrayWithArg（两文本顺序的精确 irstream 矩阵）、ObjectWithArg（空串建模 Java null 参数）、2StreamMaxAggregation（unique 窗 × 100 行网格 max={9}）与 NoJoinIterateVariables（变量驱动单方法源迭代器矩阵）。为此修正 join 运行时：事件驱动 join 中无依赖方法侧按接受触发侧的保留行逐行重求值（joinCycleOrder），轮询行携带 trigger 行 lineage、只在所属 trigger 元组内成组并随其过期；无触发单方法源语句 Snapshot 用当前变量重轮询。4 个 Java runtime（java-runtime-cfde81aa634ebedcc0b5、java-runtime-54ad4ad0b6bc90c10c01、java-runtime-ac44abecab3831a90772、java-runtime-85ea9a7b33a60b5e9851）已登记到 case.epl-fromclausemethod-basic 与 epl.fromclausemethod。Java TestSuiteEPLFromClauseMethod 保持 3/3 通过；EventBeanArray、UDF/script、StreamNameWContext、WithMethodResultParam、InvocationTargetEx、context 绑定、multikey/array-key、cache、unidirectional-historical、多事件流无依赖方法绑定与完整 Java 诊断仍为后续切片；本轮不需要 MySQL Docker。
...
| 项目 | 内容 |
|---|---|
| 文档状态 | Draft 2.92，补齐方法源 with-arg 四切片（ArrayWithArg 精确 irstream、ObjectWithArg、unique 网格 max 聚合、NoJoinIterateVariables）；无依赖方法侧按触发侧保留行逐行重求值并携带 trigger lineage，无触发单方法源 Snapshot 重轮询；EventBeanArray、UDF/script、context 绑定、multikey/array-key、cache、unidirectional-historical、多事件流无依赖方法绑定与完整 Java 诊断仍为后续项 |
