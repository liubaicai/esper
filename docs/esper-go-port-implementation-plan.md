...
> 最新补充：Draft 2.91（2026-08-08），补齐方法源 2JoinHistorical 五兄弟：IndependentOuter 以两个 EvaluateOnce 常量方法源覆盖 LEFT/RIGHT/FULL/FULL-reversed 四变体迭代器状态；SubordinateOuter 的 s1 经 DependingOn 绑定，部署期种子按拓扑序级联到无触发 subordinate 方法源（seedEvaluateOnceJoinSidesLocked），依赖行携带 lineage 不锚定占位；SubordinateOuterMultiField 覆盖 keep-all 事件流双字段等值 LEFT OUTER JOIN 100 行网格；OnlyDependent/OnlyIndependent 以 RegisterVariable(name, nil) 对应 Java null 变量，on-set 后重轮询，并固定无触发纯方法语句（queryIteratorOnlyMethodSources）只刷新状态、永不投递 listener/route 的 Java 语义。5 个 Java runtime（java-runtime-272e0404d73a30430a71、java-runtime-e60fa876a0a24d3df1ed、java-runtime-ec8f38e0c7a4571d5fe2、java-runtime-b5dbc9fc403804d2ec4b、java-runtime-8f12d418e3800f70fd80）已登记到 case.epl-fromclausemethod-basic 与 epl.fromclausemethod。Java TestSuiteEPLFromClauseMethod 保持 3/3 通过；ArrayWithArg/ObjectWithArg、2StreamMaxAggregation、NoJoinIterateVariables、EventBeanArray、UDF/script、context 绑定、multikey/array-key、cache、performance 与完整 Java 诊断仍为后续切片；本轮不需要 MySQL Docker。
...
| 项目 | 内容 |
|---|---|
| 文档状态 | Draft 2.91，补齐方法源 2JoinHistorical 五兄弟（independent/subordinate outer、多字段等值、变量驱动 dependent/independent）；部署期种子级联到无触发 subordinate 方法源，无触发纯方法语句固定为 iterator-only 不投递输出；ArrayWithArg/ObjectWithArg、2StreamMaxAggregation、NoJoinIterateVariables、EventBeanArray、UDF/script、context 绑定、multikey/array-key、cache、performance 与完整 Java 诊断仍为后续项 |
