...
> 最新补充：Draft 3.33（2026-08-09），`ClientCompileStatementObjectModel` 5/5 execution 已 mapped。Go 以 immutable fluent `Query`/`PatternStream` 作为唯一 statement object model，新增 detached expression/pattern typed AST inspection；完整 clause model、runtime deployment、Plan→Query→Plan identity 与 Java precedence tree 均有对照。当前 228 mapped / 0 partial / 7 approved-difference。Maven 当前不可用，未执行 Java suite；本轮不需要 MySQL Docker。


## 1. 文档信息

当前切片（Draft 3.33，2026-08-09）对照 Java `ClientCompileStatementObjectModel`。`Query`/`PatternStream` 直接承担 executable immutable object model；`InspectExpression`、`PatternStream.Model`/`PatternQuery.Model` 和 `Query.PatternModel` 返回 detached recursive typed tree，公开稳定 kind/description/type/tag/predicate/children。wildcard model Build/Deploy 后保留 underlying，完整 model 覆盖 insert target、projection、filter/window、where/group-by/having、output/order-by，`Plan.Query()` 再 Build 保持 canonical/hash/name；16 组 expression 与 15 组 pattern precedence 由 Go 函数嵌套树逐项固定。Go 不接受 EPL string，也不复制 `eplToModel`/`toEPL` parser normalization、mutable setters 或 Java serialization。

| 项目 | 内容 |
|---|---|
| 文档状态 | Draft 3.33，ClientCompileStatementObjectModel 5/5 execution 已 mapped；immutable fluent object model、detached expression/pattern AST inspection、完整 clause 构造和 Plan round-trip 已落地，manifest 当前 1,755 条关联、1,729/4,140 唯一 runtime（41.76%）、228 mapped / 0 partial / 7 approved-difference；下一步继续 large compiler boundaries 或其他未映射 suite |
