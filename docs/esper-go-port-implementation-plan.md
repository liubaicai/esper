...
> 最新补充：Draft 3.30（2026-08-09），`ClientCompileToolCompiler` 1/1 execution 已 mapped。新增 typed `CompilerProvider`/`CompilerRequest`/`NativeCompilerProvider`，让应用包装 fluent Query 编译并安全委托普通 `Environment.Build`；provider error、empty Plan 与 foreign-Environment Plan 均有结构化边界。当前 225 mapped / 0 partial / 7 approved-difference。Maven 当前不可用，未执行 Java suite；本轮不需要 MySQL Docker。


## 1. 文档信息

当前切片（Draft 3.30，2026-08-09）对照 Java `ClientCompileToolCompiler`。Java 通过 compiler hook 注入 `CompilerAbstractionToolProvider`，使用 `javax.tools.JavaCompiler` 编译生成 source；Go 的等价扩展点是 `CompileWithProvider(env, typedQuery, provider)`。`CompilerRequest` 保留 Environment、Query、stable typed description 与 detached Build options，自定义 provider 可观察/路由/缓存后调用 `CompileDefault`，`NativeCompilerProvider` 直接进入普通 fluent compiler。两条路径产生相同 Plan hash/manifest，provider 只能返回请求 Environment 所属的非空 Plan。Go 不生成源码、不 shell out 到外部 Go toolchain、不接受 EPL；JVM bytecode abstraction 与精确 EPCompileException 文本作为平台差异保留。

| 项目 | 内容 |
|---|---|
| 文档状态 | Draft 3.30，ClientCompileToolCompiler 1/1 execution 已 mapped；typed compiler-provider request/delegation、native provider、Plan ownership 与 invalid provider result 边界已落地，manifest 当前 1,745 条关联、1,719/4,140 唯一 runtime（41.52%）、225 mapped / 0 partial / 7 approved-difference；下一步继续 compile syntax validation、statement object model 或其他未映射 suite |
