...
> 最新补充：Draft 3.61（2026-08-10），PatternOperatorFollowedBy suite 10/10 关闭：已完成序列在右支未退出时保留并经保留序列新鲜完成再触发（对齐 EvalFollowedByStateNode nodes 映射），派生右支起始即满足（not/or-not）立即以 null 右侧完成，外层 every 按血统顺序 continued/sibling 派生使 every (every a -> every b) B3 精确 4 触发（经 Java 引擎实测核实），新增 ThenMax 有界复合右支与 PatternQuery.Where（from pattern 后语句级 where）。当前 280 mapped / 1 partial / 17 approved-difference。本轮不需要 MySQL Docker。
>
...
| 项目 | 内容 |
|---|---|
| 文档状态 | Draft 3.61，Client domain 100% 关闭（281/281）；View domain 22 个唯一 runtime，Pattern domain 27 个（And/Or 3、CompositeSelect 2/2、Every 6/6、Not 6/6、FollowedBy 10/10）；ExprFilter 23、ExprEnum 27、ResultSet 2、EPLOther 2、EPLInsertInto 1；manifest 当前 1,963 条关联、1,928/4,140 唯一 runtime（46.57%）、280 mapped / 1 partial / 17 approved-difference；下一步 Pattern 基础 suite（EveryDistinct/MatchUntil/ConsumingPattern/Observer 批量）与 View Expression/Accum 系列、ExprFilter 剩余 |
