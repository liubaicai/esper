...
> 最新补充：Draft 3.22（2026-08-09），`ClientCompileVisibility` 12/12 execution 已 mapped。新增默认 private 与显式 private/protected/public module visibility、immutable `ModulePath`、`module.Uses(...)`/`env.Uses(...)`、七类 catalog resolver、结构化 ambiguity diagnostics、bus-visible public schema 和 `SendBusRecord`；public Named Window 跨部署固定 length(2) sum 轨迹，uses/module identity 进入 Plan canonical。当前 216 mapped / 0 partial / 6 approved-difference。Maven 当前不可用，未执行 Java suite；本轮不需要 MySQL Docker。


## 1. 文档信息

当前切片（Draft 3.22，2026-08-09）对照 Java `ClientCompileVisibility`。Go 以 typed Module/ModulePath 取代 EPL annotation、CompilerPath 和 `uses` 文本：模块默认 private，可显式选择 private/protected/public；同 module 可读取自身目录，外部仅解析 public，重复 public export 及 preconfigured/module collision 返回 `ErrorAmbiguous`，`module.Uses(...)`/`env.Uses(...)` 对 event type、variable、context、Named Window、Table、declared expression 和 script 做确定性选择并写入 immutable Plan identity。`BusEventType()` 只允许 public module schema，`SendBusRecord` 拒绝内部 schema。JVM application-inlined class 使用 module-local declared expression/typed Go UDF 迁移，不复制 classloader。mixed per-definition visibility、其余 representation module constructor、binary path/cache 与精确 Java diagnostic 仍是后续项。

| 项目 | 内容 |
|---|---|
| 文档状态 | Draft 3.22，ClientCompileVisibility 12/12 execution 已 mapped；typed private/protected/public module path、uses disambiguation、bus event type 与 public Named Window 跨部署轨迹已落地，下一步继续 per-definition mixed visibility、compile substitution params/define suites 或其他未映射 suite |
