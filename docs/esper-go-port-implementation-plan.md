...
| 项目 | 内容 |
|---|---|
| 文档状态 | Draft 2.69，补充 FAF 目标 mutation 失败回滚：Table/Named Window 目标存储、Context partition 状态、Context Table row ownership 和 pending mutation queues 在中途冲突或 routed processing 失败时恢复；完整跨 statement transaction、跨目标 routed side effect、listener/external resource rollback、live Context Table insert ownership 和更广 transaction/concurrency trace 仍列为后续项。此前已完成的单源 Table/Named Window equality/range candidate、普通内连接及两流 outer optional side、Context candidate 与 B-tree ordered candidate，以及 `InfraUpdate` 全量更新/re-key 对照继续有效。Java `@Hint`/JVM query-plan hook 不复制为 Go API 外形；Context Join/subquery 物理路径、成本模型和性能阈值仍明确列为后续项 |
...
| 当前修订 | Draft 2.69（2026-08-08）：继续对照 `InfraNWTableFAF`/`InfraUpdate` 的失败路径，`ExecuteFireAndForget` 现在在目标 mutation 或 pending routed processing 失败时恢复 Table/Named Window 根及 Context 分区状态、Context Table row ownership 和 pending queues；`TestOnDemandTableMutationRollsBackAfterMidBatchFailure`、`TestOnDemandNamedWindowMutationRollsBackAfterAssignmentFailure`、`TestOnDemandMutationRollsBackAfterRoutedProcessingFailure` 与 `TestOnDemandContextTableMutationRollsBackAcrossPartitions` 固定全局 Table、Named Window 及 category Context Table 的跨行/跨分区 assignment/type、route cycle limit 与 primary-key collision rollback。此前的单源 Table/Named Window equality/IN、普通内连接及两流 outer optional side、Context candidate 与 B-tree ordered candidate、`OnDemand().UpdateAll(...)`/Table re-key 对照保持有效；完整跨 statement transaction、跨目标 routed side effect、listener/external resource rollback、完整 Context Table live-insert ownership 和更广 transaction/concurrency trace 仍列后续项。Java `@Hint`/`SupportQueryPlanIndexHook` 不作为 Go 契约；Java `TestSuiteInfraNWTable` 26/26 通过，本轮不需要 MySQL Docker。 |
