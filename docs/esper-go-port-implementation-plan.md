...
> 最新补充：Draft 3.85（2026-08-12），ExprFilterExpressions remaining 全部关闭：PromoteIndexToSetNotIn、ShortCircuitEvalAndOverspecified、RewriteWhere、Invalid。新增 `expr_filter_remaining_parity_test.go`；`And`/`Or` 改为 Esper 三值逻辑下的惰性求值，pattern filter Build 阶段拒绝 aggregate 与 previous/prior，测试覆盖 JavaBean 异常 getter short-circuit、普通/禁 rewrite hint/Named Window、typed invalid diagnostics。Coverage 2,076/4,140（50.14%），333 case 中 315 mapped / 1 partial / 17 approved-difference。下一步 ViewGroup/ViewIntersect 剩余矩阵及其他 capability 域。本轮不需要 MySQL Docker。
>
...
当前切片（Draft 3.85，2026-08-12）关闭 ExprFilterExpressions 的最后 4 个 execution：PromoteIndexToSetNotIn、ShortCircuitEvalAndOverspecified、RewriteWhere、Invalid；同步修复 `And`/`Or` 惰性短路和 pattern filter invalid 边界。下一步 ViewGroup/ViewIntersect 剩余矩阵及其他 capability 域。

| 项目 | 内容 |
|---|---|
| 文档状态 | Draft 3.85，Client domain 100% 关闭（281/281）；View domain 88 个唯一 runtime（TimeAccum 10/10、TimeLengthBatch 9/9、FirstTime 3/3、ExternallyTimedWin 5/5、ExternallyTimedBatched 5/5、ExpressionWindow 13/13、ExpressionBatch 13/13、LengthBatch 3/8、Union 1 及既有 KeepAll/Length/First/Last/Time/TimeBatch/Sort/FirstUnique/MarketData 子集），Pattern domain 107 个；ExprFilter 32、ExprEnum 27、ResultSet 2、EPLOther 2、EPLInsertInto 1；manifest 当前 2,076 条关联、2,076/4,140 唯一 runtime（50.14%）、315 mapped / 1 partial / 17 approved-difference；下一步 ViewGroup/ViewIntersect 剩余矩阵及其他 capability 域 |
