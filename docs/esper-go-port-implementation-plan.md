...
> 最新补充：Draft 3.52（2026-08-10），View domain 继续：FirstEvent/LastEvent/FirstLength 的 MarketData wildcard 变体 3 个 execution 与 LengthWindow Iterator/WPrevPrior 2 个 execution 关闭（length(2) 迭代器驱逐后到达序、prev/prior/prevtail/prevcount/prevwindow 窗口历史投影、驱逐事件 old 行 null 历史）；记录 Go-style 映射：Java prior(1, x) 对应 Go Prior(0, x)。当前 272 mapped / 1 partial / 17 approved-difference。本轮不需要 MySQL Docker。
...
| 项目 | 内容 |
|---|---|
| 文档状态 | Draft 3.52，Client domain 100% 关闭（281/281）；View domain 已处置 14 个 runtime（KeepAll 3/3、FirstUnique 2/2、FirstEvent 2、LastEvent 2、FirstLength 2、LengthWindow 3），ExprFilter 23、ExprEnum 27、Pattern 3、ResultSet 2、EPLOther 2、EPLInsertInto 1；group-by irstream null-prior 聚合空组求值语义已对齐 Java；manifest 当前 1,927 条关联、1,898/4,140 唯一 runtime（45.85%）、272 mapped / 1 partial / 17 approved-difference；下一步 View 剩余 suite（TimeWindow/TimeBatch、Sort、LengthBatch、Expression 系列）与 ExprFilter 剩余 suite |
