...
> 最新补充：Draft 3.35（2026-08-09），`ClientCompileLargeWConfig` substitution-parameter 2/12 execution 已 mapped。live deployment 与 prepared FAF 均覆盖 5,000 个 named typed parameters、ordered result columns 和 binding snapshot；suite 其余 10 个 aggregation/table/representation execution 保持后续。当前 230 mapped / 0 partial / 7 approved-difference。Maven 当前不可用，未执行 Java suite；本轮不需要 MySQL Docker。


## 1. 文档信息

当前切片（Draft 3.35，2026-08-09）对照 Java `ClientCompileLargeWConfig` 的 `ClientCompileLargeSubstitutionParams` 与 `ClientCompileLargeSubstitutionParamsFAF`。live Plan 构造 5,000 个 `Parameter[string](pN)` projection，通过 `DeployWithParameters` 固定 `cN=vN` 并验证调用方 map 后续修改不会反向影响部署；prepared FAF 从 `LastEvent` Named Window 的 `p0=x` 出发，对同一 5,000 bindings 逐列 typed Concat，固定 `cN=xvN`。Go 不解析 `?:name:type` EPL token，使用已有 typed parameter declaration/validation/binding contract；剩余 10 个 large aggregation/table/Map/ObjectArray execution 继续按组迁移。

| 项目 | 内容 |
|---|---|
| 文档状态 | Draft 3.35，ClientCompileLargeWConfig substitution-parameter 2/12 execution 已 mapped；5,000 named parameters 的 live deployment、prepared FAF、ordered projection 与 binding snapshot 已落地，manifest 当前 1,758 条关联、1,732/4,140 唯一 runtime（41.84%）、230 mapped / 0 partial / 7 approved-difference；下一步继续 large aggregation/table/representation 分组或其他未映射 suite |
