...
本轮把前文仍写作“wildcard/multi-action projection”的两个已落地子集拆成可追踪 case，避免把一个粗粒度的 `trigger.table-named-window` 状态误读为完整 on-merge parity：
...
这三个 case 已写入 `compat/capability-manifest.json`，并保留 `status=mapped` 的含义：它只表示 Java 来源、Go 测试和能力入口已关联；尚未有共享 Java/Go 中立 trace，因此不能改成 `passing`。本轮新增的 `plan.go` 遍历也必须收集 merge insert projection 的表达式依赖，避免参数依赖或 canonical/hash 漏失。
...
仍未关闭的遗漏必须单独建 case，而不能复用上面两个测试的名字：Java `InfraFlow` 的 insert/filter/delete/update/redeploy 完整生命周期（`java-runtime-403bba8c6b29e32b1a8f`、`java-runtime-ae05c015767242106de7`）、`InfraMultipleInsert` 的多重 insert 变体（`java-runtime-2c4851ea026cc835d39b`、`java-runtime-f075aa28d64b1ae78d2e`）、`InfraInsertOtherStream` 的 ObjectArray/JSON/Avro/XML representation 矩阵、InnerType/Variable、invalid/rollback、split-stream、事务/持久化、索引计划和精确 Java 诊断仍属于 `partial`。其中 `InfraFlow` 与 `InfraOnMergeInsertStream` 都涉及多 action，但前者还包含既有 `insert into`、delete、重部署和 full lifecycle，不能仅凭 action-chain 测试宣称覆盖。
...
- Go：`go test ./...` 通过；本轮定向 merge 与 trigger race 测试应继续作为提交门禁，并在下一次收口时重新跑 `go vet ./...`、全量 `go test -race` 分片和 manifest 校验。
