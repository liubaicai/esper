...
> 最新补充：Draft 3.64（2026-08-10），PatternOperatorMatchUntil suite 9/9 关闭（7 个新增 runtime ID，ExpressionBounds/BoundRepeatWithNot 此前已关联）：紧约束 [N:N]+until 达上限即触发不等终结符（对齐 EvalMatchUntilStateNode.isTightlyBound），宽松区间达上限仅停收集仍由 until 决定触发；已完成 match-until 释放子节点后改为保留满足持续上报（修复 and/or 父节点配对循环零转移丢触发）；MatchUntil 静态 bounds 放行 minimum=0（[:M]/[0:M] 形式）；52 个 W-harness 子用例按时间戳事件集回放，定时器触发归入下一事件桶。当前 283 mapped / 1 partial / 17 approved-difference。本轮不需要 MySQL Docker。
>
...
| 项目 | 内容 |
|---|---|
| 文档状态 | Draft 3.64，Client domain 100% 关闭（281/281）；View domain 22 个唯一 runtime，Pattern domain 53 个（And/Or 7、EveryDistinct 17/17、MatchUntil 9/9、CompositeSelect 2/2、Every 6/6、Not 6/6、FollowedBy 10/10）；ExprFilter 23、ExprEnum 27、ResultSet 2、EPLOther 2、EPLInsertInto 1；manifest 当前 1,989 条关联、1,954/4,140 唯一 runtime（47.20%）、283 mapped / 1 partial / 17 approved-difference；下一步 Pattern 基础 suite（ConsumingPattern/Observer 批量）与 View Expression/Accum 系列、ExprFilter 剩余 |
