...
| 项目 | 内容 |
|---|---|
| 文档状态 | Draft 2.55，补充 Java `InfraNWTableFAFResolve` 的 module-aware Named Window/Table 名称解析与同名隔离对照；Go 增加显式 `Environment.RegisterModule`/`Module` 命名空间和 module-bound source/FAF 链。Java `ModuleUsesOption`、EPL module 文本与 protected/public class-loader 可见性不复制为 Go API 外形；其余历史增量与未完成项见下文 |
...
| 当前修订 | Draft 2.55（2026-08-07）：在前述 `InfraNWTableFAFSubquery` 15 个功能 execution 对照基础上，新增 `faf_resolve_parity_test.go` 对照 `InfraNWTableFAFResolve` 的 2 个 runtime。Go 使用显式 `RegisterModule`/`Module.NamedWindow`/`Module.Table` 与 `OnDemand()` 构造 A/B 两个 module 中同名 `MyInfra` 的独立 Named Window/Table，验证 insert、wildcard snapshot、module-bound runtime lookup、Plan hash/canonical 隔离和未知 module Build 失败；补齐 `faf.go`、`subquery.go`、`trigger.go` 的 module-aware catalog 查找。Java `ModuleUsesOption`、EPL module 声明和 protected/public visibility 不作为 Go fluent API 契约，差异已登记；本轮不需要 MySQL Docker。 |
...
- 当前 capability manifest 已关联 1,305/4,136 个唯一 Java runtime（约 31.55% 的 Java runtime 对账/处置进度）；187 个 case 中 181 个 mapped、2 个 partial、4 个 approved-difference。该比例不是 Java/Go 行为 parity 通过率，也不是全量移植完成度。
