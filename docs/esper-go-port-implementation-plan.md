...
> 最新补充：Draft 3.55（2026-08-10），ViewTimeBatch 10Sec/SceneOne 关闭（首事件锚点、边界整批 flush、old-only flush、Iterator 累积批次）；修复 time_batch 空当前批次不交付前批 old 的差异与批式窗口 Snapshot 读取已 flush 批次的差异（新增 windowIteratorEvents）。当前 277 mapped / 1 partial / 17 approved-difference。本轮不需要 MySQL Docker。
...
| 项目 | 内容 |
|---|---|
| 文档状态 | Draft 3.55，Client domain 100% 关闭（281/281）；View domain 已处置 18 个唯一 runtime（KeepAll 3/3、FirstUnique 2/2、FirstEvent/LastEvent/FirstLength、LengthWindow 3、Sort 6/6、TimeWindow 2、TimeBatch 2），time_batch old-only flush 与批式窗口迭代器语义已对齐 Java；manifest 当前 1,937 条关联、1,902/4,140 唯一 runtime（45.94%）、277 mapped / 1 partial / 17 approved-difference；下一步 View 剩余（TimeBatch RefPoint/Longer/Multirow、Expression 系列、Accum、FirstTime）与 ExprFilter/Pattern 批量登记 |
