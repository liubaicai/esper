...
当前切片（Draft 2.73，2026-08-08）继续补齐 Context Table 物理分区和子查询索引共享语义：相同 primary key 可在不同 Context partition 独立存在，live `InsertIntoTable`/`UpsertIntoTable`/merge、分区内 select/update/delete、Context FAF snapshot 与 aggregate `IntoTable` scoped replacement 均已覆盖；temporal/initiated partition 释放时回收 Table scoped state 与 row ownership。Context-scoped Table 现支持相关子查询的当前分区 primary/secondary hash 与 equality-prefix + B-tree candidate；Named Window equality/hash 子集支持自动 shared index、显式 index、consumer disable/no-index 和更新删除后的索引重建；普通 FAF/Join 在没有当前分区变量时遍历 root/scoped index states，根状态有潜在 legacy rows 时相关子查询回退 ownership-aware snapshot。完整跨 statement transaction、跨目标 routed side effect、listener/external resource rollback 和 transaction/concurrency trace 仍是后续项。

| 项目 | 内容 |
|---|---|
| 文档状态 | Draft 2.73，补充 Context-scoped Table 的 scope-aware hash/primary/B-tree candidate 和 Named Window equality/hash subquery shared-index 子集：相关 FAF 子查询在当前 partition 直接查对应物理 state；普通 Context FAF/Join 在无当前 partition 变量时合并 root/scoped candidate，根状态存在 legacy rows 时保留 ownership-aware snapshot fallback。此前已完成的 live `InsertIntoTable`/`UpsertIntoTable`/merge ownership、duplicate primary-key isolation、aggregate scoped replacement、生命周期回收、单源 Table/Named Window equality/range candidate、普通内连接及两流 outer optional side、Context candidate 与 B-tree ordered candidate，以及 `InfraUpdate` 全量更新/re-key 对照继续有效。Go 用 `NamedWindowSubqueryIndexSharing`、`SubqueryUseIndex`、`SubqueryNoIndex` 和 `SubqueryDisableIndexSharing` 表达规则，不复制 Java `@Hint`/JVM query-plan hook 的 API 外形；Context right/full-preserving/non-adjacent chain、B-tree/multikey/multiple-hint shared index、成本模型和性能阈值仍明确列为后续项 |
