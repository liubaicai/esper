...
> 最新补充：Draft 3.06（2026-08-09），`ClientRuntimeExceptionHandler` 的唯一 direct execution 已映射。用户聚合扩展在 factory/state 计算中 panic 时，由类型化 plugin 边界转换成包含 plugin、statement 与原始原因的 `ErrorState`；仅恢复该边界标记，内部 panic 继续显式暴露。新增 1 个 runtime 和 `client.runtime.exception-boundary` capability；当前 201 mapped / 0 partial / 4 approved-difference。本轮不需要 MySQL Docker。
...
| 项目 | 内容 |
|---|---|
| 文档状态 | Draft 3.06，`ClientRuntimeExceptionHandler` 1/1 direct execution 已映射；此前 `ClientRuntimeTimeControl` 2/2、`ClientRuntimeListener` 1/1、`ClientRuntimeUnmatchedListener` 3/3、`ClientRuntimeEPStatement` 2/2 与 `client/basic` 7/7 保持 mapped，下一步继续 client runtime 管理面 |
