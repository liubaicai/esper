...
> 最新补充：Draft 3.14（2026-08-09），`ClientRuntimeSubscriber` 11/11 与 `ClientRuntimeSubscriberDisallowed` 1/1 execution 已处置。新增单一可替换 typed `Subscriber`、immutable `SubscriberUpdate`/`SubscriberRow`、subscriber error/panic isolation handler 与 `DisallowSubscriber` canonical policy；ordered projection、map、wildcard underlying、join、IR stream、listener coexistence、Named Window、variable set、生命周期和性能计数均有对照。Java 反射方法名/重载/static/vararg footprint 改为编译期 Go callback。新增 12 个 runtime 和 `client.runtime.subscriber` capability；当前 209 mapped / 0 partial / 5 approved-difference。Maven 当前不可用，未执行 Java suite；本轮不需要 MySQL Docker。
...
| 项目 | 内容 |
|---|---|
| 文档状态 | Draft 3.14，`ClientRuntimeSubscriber` 12/12 execution 已 mapped；typed subscriber delivery、error isolation、variable assignment row、lifecycle/disallow policy 已落地，下一步继续 runtime threading/configuration 组 |
