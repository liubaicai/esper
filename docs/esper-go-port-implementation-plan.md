...
| 项目 | 内容 |
|---|---|
| 文档状态 | Draft 2.41，补充 JSON provided-underlying 的 Go-native 泛型结构体适配、JSON tag、嵌套 struct/slice、UUID、`DateOnly`、`time.Time`、`big.Rat`、schema invalid、constructor-style primitive default、null projection route、pattern typed-array，以及与 Java `EventJsonProvidedUnderlyingClass` 的 9 个选定 runtime 对账；并记录 JVM public-class/default-constructor/non-static-inner-class 诊断作为 Go 零值结构体模型的 API 差异。此前已补充非分组聚合子查询 `SubqueryHaving`、相关 outer-field、`SubqueryExistsValue` 与带 options 的 IN/ANY/SOME/ALL fluent API 及 Go/Java 对照；其余历史增量与未完成项见下文 |
...
| 当前修订 | Draft 2.41（2026-08-07）：完成 `EventJsonProvidedUnderlyingClass` 选定 9 个 runtime 的 Go 对账切片，覆盖 typed JSON schema invalid、constructor-style primitive default 的直接解析与 null projection route、pattern typed-array materialization、UUID/日期/时间/精确 rational 转换、`Engine.SendJSON`/`RenderJSON` 边界；修复 MatchUntil 重启分支的外层 tag 继承和 tag-history 前缀合并。JVM public-class/default-constructor/non-static-inner-class 反射诊断按 Go 零值结构体模型记录为 API 差异；核心、本轮 JSON、compat 与 Java JSON 基线门禁通过，不需要 MySQL Docker。 |
