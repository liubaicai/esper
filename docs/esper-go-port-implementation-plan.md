...
> 最新补充：Draft 3.08（2026-08-09），`ClientRuntimeLockLogging` 1/1 execution 已处置。新增 opt-in Engine mutex activity trace，精确记录 attempt/acquired/released 并通过独立内存快照、clear 与 lifecycle 测试固定行为；Java internal timer disabled 对应 Go 默认显式虚拟时钟且无后台 timer，Java 未断言 logger 文本，因此不复制全局日志副作用。新增 1 个 runtime 和 `client.runtime.lock-activity` capability；当前 203 mapped / 0 partial / 5 approved-difference。本轮不需要 MySQL Docker。
...
| 项目 | 内容 |
|---|---|
| 文档状态 | Draft 3.08，`ClientRuntimeLockLogging` 1/1 execution 已 mapped；此前 `ClientRuntimeItself` 5/5 已处置（4 mapped + 1 approved difference），下一步继续 `ClientRuntimePriorityAndDropInstructions` 等 client runtime 管理面 |
