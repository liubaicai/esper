...
| 当前修订 | Draft 2.34（2026-08-07）：本轮补齐 `InfraInsertOtherStream` 的 ObjectArray/Map/Avro/JSON/JSON-class-provided/DEFAULT × Table/Named Window 表示矩阵，对应 `TestTriggerMergeInsertOtherStreamRepresentationMatrix`；新增 `TestTriggerInfraInsertOtherStreamBootstrapMatchesInfraInsertOtherStream`，覆盖 struct/Map/ObjectArray 的 insert-into bootstrap 与 deployment order；同时保留 `InfraInvalid` 的 merge Build 负例与结构化诊断。XML、跨表示 shared trace、完整 invalid 矩阵、直接 OnPattern trigger、事务/回滚、split-stream、持久化和完整 Java/Go trace 仍按 partial 处理。 |
...
- 当前 capability manifest 已关联 1,185/4,136 个唯一 Java runtime（约 28.65% 的 Java runtime 对账/处置进度）；160 个 case 中 157 个 mapped、3 个 approved-difference。该比例不是 Java/Go 行为 parity 通过率，也不是全量移植完成度。
