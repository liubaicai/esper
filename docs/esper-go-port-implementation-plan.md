...
| 项目 | 内容 |
|---|---|
| 文档状态 | Draft 2.48，继续补充 PREDEFINED/ANY Variant 的 Java 回归对照：boxed/numeric 类型合并、JavaBean getter cache 分派、indexed/mapped/nested metadata、Pattern/Subquery、LengthWindow old/new、Named Window Unique、wrapper/derived stream、通配 Join、late schema 和 Plan identity；单列 method-backed member conversion 与 Java application/type-class metadata 仍未关闭；其余历史增量与未完成项见下文 |
...
| 当前修订 | Draft 2.48（2026-08-07）：在 typed JSON 命名 nested fragment catalog 基础上，继续对照 Java `EventVariantStream` 全部 17 个 execution。Go 新增 Variant boxed/value 与 numeric width common metadata widening；PREDEFINED Variant 为共有字段建立成员 schema getter cache，按 routed Event 或 underlying Go type 分派 JavaBean/method-backed indexed、mapped、nested getter；Named Window merge 保留 Variant member Event；新增 Pattern/Subquery、LengthWindow old/new、Named Window Unique replacement、ANY dynamic getter/metadata、staggered wrapper/derived routes、wildcard Join、late Map/ObjectArray schema 和 Plan canonical identity 对照。新增 14 个 Variant case，15 个运行态 Java ID，其中单列 method-backed member conversion 保持 partial；Java application/type-class metadata 以 Go `Schema.Kind`/`VariantMode`/Plan canonical 替代。manifest 进度为 1,226/4,136（约 29.64%），case 数为 182（177 mapped、1 partial、4 approved-difference）；本轮核心验证不需要 MySQL Docker。 |
...
- 当前 capability manifest 已关联 1,226/4,136 个唯一 Java runtime（约 29.64% 的 Java runtime 对账/处置进度）；182 个 case 中 177 个 mapped、1 个 partial、4 个 approved-difference。该比例不是 Java/Go 行为 parity 通过率，也不是全量移植完成度。
