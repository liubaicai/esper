...
当前切片（Draft 2.75，2026-08-08）继续补齐 Context Table 物理分区和子查询索引共享语义：相同 primary key 可在不同 Context partition 独立存在，live `InsertIntoTable`/`UpsertIntoTable`/merge、分区内 select/update/delete、Context FAF snapshot 与 aggregate `IntoTable` scoped replacement 均已覆盖；temporal/initiated partition 释放时回收 Table scoped state 与 row ownership。Context-scoped Table 现支持相关子查询的当前分区 primary/secondary hash 与 equality-prefix + B-tree candidate；Named Window equality/hash 与 equality-prefix + range B-tree 子集支持自动 shared index、显式 index、consumer disable/no-index、multiple-index hint、确定性 index choice、multikey/array key 和更新删除后的索引重建；普通 FAF/Join 在没有当前分区变量时遍历 root/scoped index states，根状态有潜在 legacy rows 时相关子查询回退 ownership-aware snapshot。完整跨 statement transaction、跨目标 routed side effect、listener/external resource rollback 和 transaction/concurrency trace 仍是后续项。

| 项目 | 内容 |
|---|---|
| 文档状态 | Draft 2.75，补充 Context-scoped Table 的 scope-aware hash/primary/B-tree candidate 和 Named Window equality/hash、equality-prefix + range B-tree subquery shared-index 子集：相关 FAF 子查询在当前 partition 直接查对应物理 state；普通 Context FAF/Join 在无当前 partition 变量时合并 root/scoped candidate，根状态存在 legacy rows 时保留 ownership-aware snapshot fallback。此前已完成的 live `InsertIntoTable`/`UpsertIntoTable`/merge ownership、duplicate primary-key isolation、aggregate scoped replacement、生命周期回收、单源 Table/Named Window equality/range candidate、普通内连接及两流 outer optional side、Context candidate 与 B-tree ordered candidate，以及 `InfraUpdate` 全量更新/re-key 对照继续有效。Go 用 `NamedWindowSubqueryIndexSharing`、`SubqueryUseIndex`、`SubqueryNoIndex` 和 `SubqueryDisableIndexSharing` 表达规则，不复制 Java `@Hint`/JVM query-plan hook 的 API 外形；已新增 multiple-index hint、结构化 index choice、显式/自动 multikey-array shared-index 对照；Context right/full-preserving/non-adjacent chain、JVM query-plan hook、Esper 完整成本/选择性模型和性能阈值仍列为后续项 |
...
- 当前 capability manifest 已建立 1,364 条关联、覆盖 1,338/4,136 个唯一 Java runtime（约 32.35% 的 Java runtime 对账/处置进度）；190 个 case 中 183 个 mapped、3 个 partial、4 个 approved-difference。该比例不是 Java/Go 行为 parity 通过率，也不是全量移植完成度。
