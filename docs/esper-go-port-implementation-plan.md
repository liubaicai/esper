...
> 最新补充：Draft 3.34（2026-08-09），`ClientCompileLarge` 1/1 execution 已 mapped。新增 1,000 列、每列 26 addends 的 typed large-plan compile/rebuild stress，对 result schema、canonical 与 hash 做确定性验证；Go 内存 AST 不复制 JVM constant-pool/generated-method-size 限制。当前 229 mapped / 0 partial / 7 approved-difference。Maven 当前不可用，未执行 Java suite；本轮不需要 MySQL Docker。


## 1. 文档信息

当前切片（Draft 3.34，2026-08-09）对照 Java `ClientCompileLarge`。Go 一次构造并编译 1,000 个 typed projection，每列为 26 个整数字面量组成的左深 Add AST，总 expression node 数超过 26,000；Build 后固定 `z0`…`z999` 的 ordered `int` result schema、canonical/hash，并验证 `Plan.Query()` 再 Build identity 不变。Java execution 的目标是避免生成 JVM class 时触碰 constant pool/method-size 边界；Go 不生成规则 bytecode，因此以同等规模的 analyzable in-memory AST 编译成功作为可观察 parity。更大的 `ClientCompileLargeWConfig` aggregation/table/substitution/Map/ObjectArray 矩阵继续作为后续独立切片。

| 项目 | 内容 |
|---|---|
| 文档状态 | Draft 3.34，ClientCompileLarge 1/1 execution 已 mapped；1,000-column/26-addend typed AST stress、ordered result schema 与 Plan round-trip 已落地，manifest 当前 1,756 条关联、1,730/4,140 唯一 runtime（41.79%）、229 mapped / 0 partial / 7 approved-difference；下一步继续 ClientCompileLargeWConfig 或其他未映射 suite |
