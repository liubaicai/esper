> 文档职责（2026-08-17）：本文保存完整范围、架构、语义决策和历史补充，不再承担当前进度与逐切片日志。当前优先级见 [执行路线图](esper-go-port-roadmap.md)，日常流程见 [执行手册](esper-go-port-runbook.md)，质量口径见 [质量策略](esper-go-port-quality-strategy.md)，逐切片历史见 [CHANGELOG](../CHANGELOG.md)。后续不要在本文顶部继续追加迁移进度；只在稳定规范或架构决策发生变化时修改相关正文或 ADR。


> 最新补充：Draft 4.166（2026-08-18），新增 `epl-insert-into-transpose-stream` differential-verified 场景，对照 Java `EPLInsertIntoTransposeStream` 全部 10 个 execution（8 个 observable `CreateSchemaPOJO`/`MapAndObjectArrayAndOthers`/`FunctionToStreamWithProps`/`FunctionToStream`/`SingleColumnInsert`/`EventJoinMap`/`EventJoinPOJO`/`POJOPropertyStream`，2 个 INVALIDITY `SingleColumnInsertInvalid`/`InvalidTranspose`），1 个 case、8 条 differential 关联、25 条 records、0 differences。Go 侧 `Transpose[T]`（phantom 类型参数，因 Go 无法从 `(Event, any)` 推导出目标 underlying 类型，需显式 `Transpose[*X](...)`）与 `IsNull[Event](...)` 显式传参；类型强制错误（wrong-bean、addl props、twice、wrong-Map-target、null、OA-undefined-target、exec-9 mapped-property）在 Build 阶段经 `validateRoute`/`validateTransposeExpression`/`validateTransposeSelectShape` 拒绝，`TestEPLInsertIntoTransposeSingleColumnInsertInvalidBuild`/`TestEPLInsertIntoInvalidTransposeBuild` 登记为 Go-unit build-error（implemented，非 differential）。Java oracle（`EPLInsertIntoTransposeStreamScenarioOracle`，SupportBean/SupportBeanComplexProps/A/EventTE/BEventTE 使用 esper-common classpath 真实 bean + 与套件同款的 5 个 UDF `generateMap/OA/Avro/Json`/`custom/customOne/customTwo`）生成 25 条 trace，12 个 case 全部与 Go 逐字段一致；exec-3 `FunctionToStream` 修正为 Java 订阅时序（`second` producer 在 I2 发送前订阅，断言 OI2 而非 OI3）。Manifest v2 更新为 517 case、115 differential-verified、348 differential runtime、2,885/4,136 referenced runtime（69.8%）。

> 最新补充：Draft 4.165（2026-08-17），新增 `insertinto-eventcol-subquery-nonbean` differential-verified 场景，对照 `EPLInsertIntoColNonBeanFromSubquerySingle`（4 个 execution）、`EPLInsertIntoColNonBeanFromSubqueryMulti`（2 个）、`EPLInsertIntoColNonBeanFromSubqueryMultiFilter`（2 个），1 个 case、8 条关联、0 differences。Go 新增 `SubqueryRowAsEvent`/`SubqueryRowsAsEvent` 公共 API（含 `WithOptions` 变体），将多列 subquery 投影行物化为命名 schema Event；objectarray schema 需 `subqueryRowToUnderlying` 将 map 行转 positional slice。Java oracle 生成 16 条 trace，8 个 case 全部一致。

> 最新补充：Draft 4.163（2026-08-17），新增 `epl-insert-into-populate-underlying` differential-verified 场景，对照 `EPLInsertIntoPopulateUnderlying` 的 4 个 execution（`EPLInsertIntoPopulateBeanSimple`、`EPLInsertIntoBeanWildcard`、`EPLInsertIntoPopulateBeanObjects`、`EPLInsertIntoPopulateUnderlyingSimple`），1 个 case、4 条关联、0 differences。Java oracle（`EPLInsertIntoPopulateUnderlyingScenarioOracle`，SupportBean/SupportBeanComplexProps 使用 esper-common classpath 真实 bean 类，SupportEnum 经 `addImport` 引入）生成 11 条规范化 trace 记录，10 个 Go parity 测试（`epl_insert_into_populate_underlying_parity_test.go`）逐字段一致；simple 两 case 覆盖原始全部 12 列投影与断言，wildcard 覆盖原始 4 属性（含 boolPrimitive）。parity-reviewer 独立审查发现 3 处缺口（MySupportMap 缺 boolPrimitive 声明、simple 投影只覆盖 4/12 列、缺 evidence.json），均已修复并重新验证。运行时修复（parity 驱动）：`validateRoute` 复用 `fieldExpressionTypesCompatible`（修复 `*int` 字段与 int 表达式的构建期拒绝）；`assignReflectValue` 支持指针值到值类型字段的解引用赋值（匹配 Esper unboxing）；`projectMapToSchema` 对 map 投影值执行数值转换（修复 `*float64` 未解引用）。Manifest v2 更新为 494 case、2,907 条关联、2,765/4,136 referenced runtime（66.8%）、92 个 differential-verified case、211 个 differential runtime。

> 最新补充：Draft 4.162（2026-08-17），新增 `expr-filter-optimizable-or-rewrite` differential-verified 场景，对照 `ExprFilterOptimizableOrRewrite` 全部 21 个 execution，1 个 case、21 条关联、0 differences。Java oracle 生成 34 条 trace，11 个 case 全部与 Go 逐字段一致。Manifest v2 更新为 493 case、2,903 条关联、2,761/4,136 referenced runtime（66.7%）、91 个 differential-verified case。

> 最新补充：Draft 4.160（2026-08-16），新增 `context-key-segmented-pattern-filter` 差分场景，对照 `ContextKeySegmentedPatternFilter`（runtime `88b756bf…`），1 个 case、0 条 records、0 differences。覆盖 keyed 上下文内的永匹配 pattern 负例：`partition by theString` + `every (event1=SupportBean(stringContainsX=false) -> event2=SupportBean(stringContainsX=true))`——分区 key 就是 theString，event1（不含 X）与 event2（含 X）不可能出现在同一分区，pattern 永不触发，4 事件全部静默（F1 分区只有 F1、X1 分区只有 X1）。Java oracle 把回归的 stringContainsX UDF 归一化为等价的 `like '%X%'` 谓词（oracle 类路径无 regressionlib）；Go 侧 `Contains(theString, "X")`。无新增运行时改动。Manifest v2 更新为 479 case、2,869 条关联、90 个 differential-verified、186 个 differential runtime、92 个 representative-verified、92/92 场景通过；`context.partition` 新增 1 个执行证据。

> 最新补充：Draft 4.159（2026-08-16），新增 `context-key-segmented-join-remove-stream` 差分场景，对照 `ContextKeySegmentedJoinRemoveStream`（runtime `28299eb4…`），1 个 case、1 条 record、0 differences。覆盖 keyed 上下文 + 3 流 full outer join + rstream + time(30) 窗口 + where：`partition by sessionId from SupportWebEvent` + `select rstream A.pageName, A.sessionId, B.pageName, C.pageName from Start#time(30) A full outer join Middle#time(30) B full outer join End#time(30) C where A.pageName is not null and (B.pageName is null or C.pageName is null)`——完整会话（Start+Middle+End）与不完整会话的插入阶段全部静默，仅会话 3 的 Start 在 30s 过期时输出一条移除 {Start, 3, null, End}。运行时修复（Java rstream oracle 对账驱动）：outer join 的事件进入路径不再发出中间 null-padded 元组被替换的 old 行（`updateJoin` 对纯 insert 且 outer join（含链式边级 outer）丢弃 diff 的 oldTuples，即时窗口淘汰的真实移除保留）——此前 rstream 会把中间替换当移除输出；同步修正 8 个 Go 测试的匹配转换 oldCount 期望（1→0，Java 回归只断言 new、rstream oracle 证明 insert 阶段无移除）。Manifest v2 更新为 478 case、2,868 条关联、89 个 differential-verified、185 个 differential runtime、91 个 representative-verified、91/91 场景通过；`context.partition` 新增 1 个执行证据。

> 最新补充：Draft 4.158（2026-08-16），新增 `context-key-segmented-join-where-clause-on-partition-key` 差分场景，对照 `ContextKeySegmentedJoinWhereClauseOnPartitionKey`（runtime `10cb439d…`），1 个 case、1 条 record、0 differences。覆盖 keyed 上下文 join 的 where 子句过滤：`partition by theString from SupportBean` + `select * from SupportBean#lastevent as sb, SupportBean_S0#lastevent as s0 where theString is 'Test'`——S0 伙伴事件无 key 扇出到所有分区 join 状态，where 只放行 Test 分区的 join 结果（SB(Test,10)+S0(1) → 输出；E2 分区被过滤）。Go `JoinQuery.Where(JoinField[...])` 直接支持，复用 join fan-out（Draft 4.153），无新增运行时改动。Manifest v2 更新为 477 case、2,867 条关联、88 个 differential-verified、184 个 differential runtime、90 个 representative-verified、90/90 场景通过；`context.partition` 新增 1 个执行证据。

> 最新补充：Draft 4.157（2026-08-16），新增 `context-key-segmented-pattern-scene-two` 差分场景，对照 `ContextKeySegmentedPatternSceneTwo`（runtime `b85c4527…`），1 个 case、2 条 records、0 differences。覆盖双流 keyed 上下文内的跨流相关 pattern：`partition by theString from SupportBean, p00 from SupportBean_S0` + `every a=SupportBean -> b=SupportBean_S0(id=a.intPrimitive)`——S0 事件按 p00 路由到与 SB 同 key 的分区（共享 key 空间），每分区 a-等待只被本分区匹配 S0 完成（S0(20,G2) → {G2,20,20,G2}；S0(10,G1) → {G1,10,10,G1}；S0(0,G1)/S0(10,G2)/重复 S0(20,G2) 不匹配，已消费 a 不重触发）。复用多流 keyed + 左腿 every + `Then` 跨流分支（Draft 4.149/4.154），无新增运行时改动。Manifest v2 更新为 476 case、2,866 条关联、87 个 differential-verified、183 个 differential runtime、89 个 representative-verified、89/89 场景通过；`context.partition` 新增 1 个执行证据。

> 最新补充：Draft 4.156（2026-08-16），新增 `context-key-segmented-join-multitype-multifield` 差分场景，对照 `ContextKeySegmentedJoinMultitypeMultifield`（runtime `75cacdf0…`），1 个 case、4 条 records、0 differences。覆盖多流双字段 keyed 上下文的跨 join 与 key 投影：`partition by theString and intPrimitive from SupportBean, p00 and id from SupportBean_S0` + `select theString, intPrimitive, id, p00, context.key1, context.key2 from SupportBean#lastevent, SupportBean_S0#lastevent`（无 on 条件的每分区 cross join）——单侧事件只建分区不输出（前 4 事件静默），双侧齐备后每分区 join 自身 lastevent 对并投影本分区 key1/key2（{G2,1,1,G2,G2,1} 等 4 条）。复用多流 keyed + key 投影 + join fan-out 机制（Draft 4.153/4.154/4.155），无新增运行时改动。Manifest v2 更新为 475 case、2,865 条关联、86 个 differential-verified、182 个 differential runtime、88 个 representative-verified、88/88 场景通过；`context.partition` 新增 1 个执行证据。

> 最新补充：Draft 4.155（2026-08-16），新增 `context-key-segmented-multi-statement-filter-count` 差分场景，对照 `ContextKeySegmentedMultiStatementFilterCount`（runtime `7c4cef4f…`，双语句共存阶段），1 个 case、7 条 records、0 differences。覆盖多流 keyed 上下文被两条语句共享：`partition by theString from SupportBean, p00 from SupportBean_S0` + s0（`sum(id) from SupportBean_S0` 按 p00 分区：10/8/14/21）与 s1（`sum(intPrimitive) from SupportBean` 按 theString 分区：5/6/14）——每语句只收本流事件、分区空间互相独立、同一上下文下双语句并发。复用多流 keyed 机制（Draft 4.154），无新增运行时改动；oracle 双 listener 各自独立 sequence（对齐 Go 按语句计数）。Java 回归的 filter-service-count 断言为内部插桩不可差分，前置部署/卸载生命周期留待生命周期切片。Manifest v2 更新为 474 case、2,864 条关联、85 个 differential-verified、181 个 differential runtime、87 个 representative-verified、87/87 场景通过；`context.partition` 新增 1 个执行证据。

> 最新补充：Draft 4.154（2026-08-16），新增 `context-key-segmented-additional-filters` 差分场景，对照 `ContextKeySegmentedAdditionalFilters`（runtime `1bf346cf…`），1 个 case、4 条 records、0 differences。覆盖多流 keyed 上下文 + 每流 filter + pattern 聚合：`partition by theString from SupportBean(intPrimitive>0), p00 from SupportBean_S0(id > 0)` + `select sum(sb.intPrimitive) as col1, sum(s0.id) as col2 from pattern [every (s0=SupportBean_S0 or sb=SupportBean)]`——不满足本流 filter 的事件不建分区（B1/-1、S0(-2) 等 5 事件静默），pattern match 按 match 自身捕获 tag 逐行计入分区聚合（S0(2) → {null,2}；SB(S1,10) → {10,null}；S0(3,S1) → {10,3}；SB(S0,9) → {9,2}）。运行时新增（共享机制，解锁 ~5 个 execution）：(1) 多流 keyed 上下文 `NewKeyContextByStreams`/`CreateKeyContextByStreams`（`KeyContextStream{Type, Keys}`，等键数校验、类型查重，对齐 Java 多类型 partition 声明），`contextKeysForEvent`/`contextKeysForNode` 按事件类型分派 key；(2) pattern 聚合 group 并行 tags（`EvalContext.GroupTags` + `groupEventContext`——sum(tag) 类输入按每个 match 行自身的 tags 求值，对齐 Esper 对 pattern match 流的聚合）。公共 facade 重新生成。Manifest v2 更新为 473 case、2,863 条关联、84 个 differential-verified、180 个 differential runtime、86 个 representative-verified、86/86 场景通过；`context.partition` 新增 1 个执行证据。

> 最新补充：Draft 4.153（2026-08-16），新增 `context-key-segmented-join` 差分场景，对照 `ContextKeySegmentedJoin`（runtime `3418bdab…`），1 个 case、3 条 records、0 differences。覆盖 keyed 上下文内的 join 伙伴流扇出：`partition by theString` + `select sb.theString, sb.intPrimitive, s0.id from SupportBean#keepall as sb, SupportBean_S0#keepall as s0 where sb.intPrimitive = s0.id`——SupportBean_S0 无分区 key，其事件扇出到所有既有分区的 join 状态（G2 分区 join S0(20) → {G2,20,20}；S0(30) 后 G3 分区新建时不回填、G1/30 与 G2/30 各自 join → {G1,30,30}/{G2,30,30}）。运行时修复 2 处（Java 探针对账驱动）：(1) `partitionRuntime` 对 keyed/hash 上下文检测 Missing key（事件类型不在分区声明中）→ 返回 fan-out 标记；(2) `processContextFanOut` 按分区序把事件派发到所有既有分区（不新建），`process` 接入。校验放宽：keyed/hash 上下文的 key 表达式只需在 ≥1 个 join 源上可解析（对齐 Java "segmented context 要求声明类型出现在语句过滤器中"，仅 S0 的语句仍拒绝）。该 runtime 已被 `case.context-partitioning` inventoried 引用（referenced 唯一数不变，differential +1）。Manifest v2 更新为 472 case、2,862 条关联、83 个 differential-verified、179 个 differential runtime、85 个 representative-verified、85/85 场景通过；`context.partition` 新增 1 个执行证据。

> 最新补充：Draft 4.152（2026-08-16），新增 `context-key-segmented-selector` 差分场景，对照 `ContextKeySegmentedSelector`（runtime `b571b059…`），1 个 case、4 条 records（3 listener + 1 snapshot）、0 differences。覆盖 keyed 上下文的 key 属性投影与管理面：`partition by theString` + `select context.key1 as c0, sum(intPrimitive) as c1 from SupportBean#length(5)`——每个 listener 行携带分区 key（E1/10、E2/20、E2/41），跨分区 statement snapshot 每分区一行（E1/10 与 E2/41）+ keyed 分区描述符（key:E1/key:E2）。Go `ContextField[string]("key1")` 复用 `contextPropertyValues` 已物化的 key1 属性，无新增运行时改动。该 runtime 已被 `case.context-partitioning` inventoried 引用（referenced 唯一数不变，differential +1）。Manifest v2 更新为 471 case、2,861 条关联、82 个 differential-verified、178 个 differential runtime、84 个 representative-verified、84/84 场景通过；`context.partition` 新增 1 个执行证据。

> 最新补充：Draft 4.151（2026-08-16），新增 `context-key-segmented-prior` 差分场景，对照 `ContextKeySegmentedPrior`（runtime `9853fdd3…`），1 个 case、6 条 records、0 differences。覆盖 keyed 上下文内的每分区 prior 历史：`partition by theString` + `select intPrimitive as val0, prior(1, intPrimitive) as val1 from SupportBean`——每个分区独立 arrival history（G1/10→{10,null}、G1/11→{11,10}、G1/12→{12,11} 与 G2 流交错互不污染），分区首事件 prior 为 null。Go `Prior(0, …)`（0 基偏移 = Java prior(1)）复用 prev-prior 切片约定，无新增运行时改动。Manifest v2 更新为 470 case、2,860 条关联、81 个 differential-verified、177 个 differential runtime、83 个 representative-verified、83/83 场景通过；`context.partition` 新增 1 个执行证据。

> 最新补充：Draft 4.150（2026-08-16），JSON/事件格式并行轨道切片（代码随 Draft 4.149 提交落地，此处补记）：修复 2 处 JSON 渲染/适配器缺口。(1) `JSONFieldAdapter` 对可空（指针声明）字段的 round-trip：`ParseJSON` 存 `*T`，渲染时 `adapter.Write` 收到指针导致 typed adapter 失败（Java 契约把存储的可空对象直接交给 adapter）；新增 `jsonUnboxAdapterValue`（adapter 声明值类型非指针时解包；指针型 adapter 保留指针）+ `jsonFieldAdapterTyped[T].Write` 指针入参解引用。对应 Java `JsonWriteForgeProvidedStringAdapter` 与 `SupportJsonFieldAdapterStringDate` 的 null-check 语义。(2) 程序化事件的 Character(rune) 字段渲染为 JSON 字符串（此前输出数字码点）：`jsonCharacterValue` 在 raw-tree 之后回退，声明为 rune 的字段渲染 `"x"` 而非 120，对齐 Java `JsonForgeFactoryBuiltinClassTyped` CHARBOXED → `writeNullableStringToString`（`EventJsonTypingWriteBasicType` 断言 `"c1":"x"`）。新增 `json_render_parity.go` + 5 个测试（nullable 指针 adapter round-trip、typed struct、Funcs 双形态、直接 Write、rune 特殊值/指针/1D/2D 数组）。61 个 json/avro 测试全绿。遗留（需共享核心）：insert-into 到指针 JSON 目标的 value→pointer 强转、NaN/Infinity 渲染决策、LocalDateTime/ZonedDateTime 独立类型、动态 map Character。

> 最新补充：Draft 4.149（2026-08-16），新增 `context-key-segmented-pattern` 差分场景，对照 `ContextKeySegmentedPattern`（runtime `854addf9…`，第一语句形态，不含 @Consume 变体），1 个 case、6 条 records、0 differences。覆盖 keyed 上下文内的左腿 every 相关 pattern：`partition by theString` + `every a=SupportBean -> b=SupportBean(intPrimitive=a.intPrimitive+1)`——every 绑定左侧事件表达式，每个 a 事件在各自分区内派生独立并发 b-等待（G1/10 等 11、G2/20 等 21 互不跨越），匹配后 pattern 存活并链式延续（G2 21→22→23、G1 11→12→13）。运行时修复 2 处（Java 探针对账驱动）：(1) `PatternStream.Every()` 对事件根也物化为 AST every 节点（此前 def.every 每事件开新根且无法作为 followed-by 左腿组合）——`Every().FollowedBy()` 现在生成 sequence(every(a), b)，phase-0 续生逻辑按左腿 every 语义为每次 a 触发派生新 b-分支；(2) `patternCompletionPermanent` 对已完成的 sequence 同时考虑左腿重复性（`!repeatingLegAlive(left) && permanent(right)`）——左腿 every 时匹配不终止 pattern（修复 `TestPatternDiscardPartialsOnMatchClearsOverlappingStartsMatchesEsper` 在物化后的 patternStopped 误停）。Manifest v2 更新为 469 case、2,859 条关联、80 个 differential-verified、176 个 differential runtime、82 个 representative-verified、82/82 场景通过；`context.partition` 新增 1 个执行证据。

> 最新补充：Draft 4.148（2026-08-16），connectors 并行轨道（qianyi 第十三节）首个切片：CSV input adapter Java/Go 差分。对照固定 commit `9e1b9f1c…` 的 `com.espertech.esperio.csv.CSVInputAdapter`（esperio-csv，`/tmp/esper-cp.txt` + `esperio/esperio-csv/target/classes` 可运行），同一份 scenario（`testdata/parity/csv-input-adapter.json`，内嵌 CSV 文本 + propertyOrder + propertyTypes）分别经 Java oracle（`CsvInputAdapterScenarioOracle` + `run-csv-input-adapter.sh`，DirectSender 同步模式无 pacing）与 Go `connectors/csv.Source`（HasTitleLine+PropertyOrder+PropertyTypes）重放，规范化 trace 4 条 records 完全一致（重生成两次均相等）。CSV 覆盖适配器边界：`#` 注释行、空行、含逗号引号值、双引号转义、尾部空字段、中间空字段、跨行引号值。Go 侧断言测试 `TestCsvInputAdapterDifferential` 内嵌精确期望 trace（= Java oracle trace），任何 Go 行为偏差即失败（mutation 敏感性）。esperio-csv 类不在 regression execution inventory（4140 行无对应 runtime ID），故本切片不进 manifest case/evidence 管道，以 connectors 域差分工件记录（scenario+oracle+runner+断言测试）。已记录 Java/Go 解析器差异（尾随空格剥离、空白行、空 boolean 列、未闭合引号）与 Docker 门禁命令（kafka/amqp/db）。connectors 全包测试通过。

> 最新补充：Draft 4.147（2026-08-16），新增 `context-key-segmented-null-keys` 差分场景，对照 `ContextKeySegmentedNullSingleKey`（runtime `66e2c18a…`）与 `ContextKeySegmentedNullKeyMultiKey`（runtime `4449ff62…`），2 个 case、6 条 records、0 differences。覆盖 keyed 上下文的 null partition key：单 key `partition by theString`（null key 两事件共享分区 cnt 1→2，A 事件独立分区 cnt 1）与三字段 key `partition by theString, intBoxed, intPrimitive`（intBoxed=null 的 (A,null,1) 两次共享分区 cnt 1→2，(A,10,1) 独立分区 cnt 1）。Go 侧 null 指针 key 经 `encodeKey` 稳定编码（此前指针解引用修复的配套语义），无新增运行时改动。Manifest v2 更新为 468 case、2,858 条关联、79 个 differential-verified、175 个 differential runtime、81 个 representative-verified、81/81 场景通过；`context.partition` 新增 2 个执行证据（两 runtime 均已 inventoried 引用，referenced 唯一数不变）。

> 最新补充：Draft 4.146（2026-08-16），新增 `context-key-segmented-match-recognize` 差分场景，对照 `ContextKeySegmentedMatchRecognize`（runtime `3e24f25a…`），1 个 case、3 条 records、0 differences。覆盖 keyed 上下文内的每分区 match_recognize：`partition by theString` + `match_recognize (measures A.longPrimitive as a, B.longPrimitive as b pattern (A B) define A as A.intPrimitive=1, B as B.intPrimitive=2)`——A 分区（A/1/10→A/2/20 完成 {10,20}）与 B 分区（B/1/30→B/2/40 完成 {30,40}）识别状态互不跨越；A 分区第二周期（50,60）全新匹配。Go 侧每个 context 分区持有独立 `rowRecogRuntimeState`（partitionRuntime 自带 rowRecog 状态），无新增运行时改动。Manifest v2 更新为 467 case、2,856 条关联、78 个 differential-verified、173 个 differential runtime、80 个 representative-verified、80/80 场景通过；`context.partition` 新增 1 个执行证据。

> 最新补充：Draft 4.145（2026-08-16），新增 `context-key-segmented-multikey-w-array-two-field` 差分场景，对照 `ContextKeySegmentedMultikeyWArrayTwoField`（runtime `c6bd2f3b…`），1 个 case、6 条 records、0 differences。覆盖双字段（string + int[]）数组 partition key 的 keyed 上下文：`partition by id, array from SupportEventWithIntArray` + 每分区 `sum(value)`——等数组不同 id 独立分区（G1[1,2] 与 G2[1,2] 各自累积 1→16 与 2→12）、同 id 不同数组独立累积（G1[1] 3→21）。Go 侧多 key 编码 `encodeKey` 按值拼接，无新增运行时改动。Manifest v2 更新为 466 case、2,855 条关联、77 个 differential-verified、172 个 differential runtime、79 个 representative-verified、79/79 场景通过；`context.partition` 新增 1 个执行证据。

> 最新补充：Draft 4.144（2026-08-16），新增 `context-key-segmented-multikey-w-array-of-primitive` 差分场景，对照 `ContextKeySegmentedMultikeyWArrayOfPrimitive`（runtime `3e54e161…`），1 个 case、9 条 records、0 differences。覆盖 int[] 数组 partition key 的 keyed 上下文：`partition by array from SupportEventWithIntArray` + 每分区 `sum(value)`——相同数组内容共享分区（E1/E2 [1,2] 10→21，E11 续 42）、空数组 `[]` 与 null 数组是不同 key（E4=13/E13=36 与 E5=14/E10=34 各自独立累积）、单元素数组独立分区（E3=12/E12=34）。Go 侧数组 key 直接经 `encodeKey` 内容编码（`[]int(nil)` 与 `[]int{}` 区分），无新增运行时改动。Manifest v2 更新为 465 case、2,854 条关联、76 个 differential-verified、171 个 differential runtime、78 个 representative-verified、78/78 场景通过；`context.partition` 新增 1 个执行证据。

> 最新补充：Draft 4.143（2026-08-16），新增 `context-key-segmented-w-init-term-pattern-as-name` 差分场景，对照 `ContextKeySegmentedWInitTermPatternAsName`（runtime `589238c9…`），1 个 case、2 条 records、0 differences。覆盖 keyed 上下文的命名 pattern-end 投影：`partition by theString initiated by SupportBean(intPrimitive=1) as startevent terminated by pattern[s=SupportBean(intPrimitive=2)] as endpattern` + `select context.startevent.intBoxed as c0, context.endpattern.s.intBoxed as c1 from SupportBean#firstevent output snapshot when terminated`——快照行读 firstevent 窗口（起始事件）的 intBoxed 作为 c0，pattern match 的 tag 事件 intBoxed 作为 c1（c0=10、c1=20）；中间事件（intPrimitive=0）不产生输出；分区独立。Go 复用既有 `CreatePatternTerminatedContext` + `ContextPatternField`（endPattern tag 经 `applyContextPatternProperties` 落入 `pattern.s` 上下文属性）与 firstevent 快照路径，无新增运行时改动。Manifest v2 更新为 464 case、2,853 条关联、75 个 differential-verified、170 个 differential runtime、77 个 representative-verified、77/77 场景通过；`context.partition` 新增 1 个执行证据。

> 最新补充：Draft 4.142（2026-08-16），新增 `context-key-segmented-term-event-select` 差分场景，对照 `ContextKeySegmentedTermEventSelect`（runtime `0fe0eaef…`），1 个 case、2 条 records、0 differences。覆盖 keyed 上下文的 term 事件投影与终止前快照：`partition by userId initiated by UserEvent(alert='A') terminated by UserEvent(alert='B') as termEvent` + `select *, context.termEvent as term from UserEvent#firstevent output snapshot when terminated`——终止事件关闭分区但不进入 firstevent 窗口（快照行 alert=A 仍为起始事件），term 列携带终止事件（alert=B）；中间 null-alert 事件不产生输出；U1/U2 分区独立。运行时修复：`encodeKey` 对指针类型的 key 值解引用（`*string` 等可空字段此前按指针地址编码导致每个事件一个分区——`partition by` 一个 `*string` 字段时 U1/A 与 U1/B 落入不同分区、永远无法终止；现按值编码与 Java boxed 值语义一致）。差分协议无改动。Manifest v2 更新为 463 case、2,852 条关联、74 个 differential-verified、169 个 differential runtime、76 个 representative-verified、76/76 场景通过；`context.partition` 新增 1 个执行证据。

> 最新补充：Draft 4.141（2026-08-16），新增 `context-key-segmented-w-init-term-end-event` 差分场景，对照 `ContextKeySegmentedWInitTermEndEvent`（runtime `e5577cf3…`），1 个 case、2 条 records（每分区一条，各含 2 行）、0 differences。覆盖 keyed 上下文的边界事件投影：`partition by theString initiated by SupportBean(intPrimitive=1) as startevent terminated by SupportBean(intPrimitive=0) as endevent` + `select context.startevent as c0, context.endevent as c1 from SupportBean output all when terminated`。终止事件关闭分区但不进入语句（快照/批量不包含它），而终止时刻会把累积批次用当时的上下文属性重投影——每个缓冲行（含中间事件行）都看到 c1=终止事件（Java 探针对账：A/1、A/2 两行 c0=1、c1=0）。运行时修复：`outputAtTermination` 的 unbounded 行输入 flush 路径从"直接倾倒到达时投影的 pending 行"改为 `reprojectPendingBatch`（用终止时刻的 `r.variables` 对缓冲事件的源事件重投影，`Result` 新增 `RowEvent` 访问器；无选择列时保持原事件行）。差分协议无改动；`normalizeValue` 单事件列（Event）归一化新增（此前只有 `[]Event`）。Manifest v2 更新为 462 case、2,851 条关联、73 个 differential-verified、168 个 differential runtime、75 个 representative-verified、75/75 场景通过；`context.partition` 新增 1 个执行证据。

> 最新补充：Draft 4.140（2026-08-16），新增 `context-key-segmented-term-by-filter` 差分场景，对照 `ContextKeySegmentedTermByFilter`（runtime `820bb6f7…`），1 个 case、7 条 records、0 differences。覆盖 keyed 上下文的 filter 终止与分区重启：`partition by theString terminated by SupportBean(intPrimitive<0)` + `select theString, count(*) as cnt from SupportBean(intPrimitive>=0)`——A 分区两次非负事件 cnt 1→2，A/-1 终止分区（无输出），下一 A/0 重新起始从 1 累积；B 分区独立（1、-1 终止、1→2、-1 终止、1）；C/-1 无分区不输出。Java 的 `partition by X terminated by <filter>` 映射为 Go 的 keyed `CreateInitiatedTerminatedContext`（start=任意 SupportBean、end=intPrimitive<0），复用既有 keyed 生命周期机制，无新增运行时改动。Manifest v2 更新为 461 case、2,850 条关联、72 个 differential-verified、167 个 differential runtime、74 个 representative-verified、74/74 场景通过；`context.partition` 新增 1 个执行证据。剩余：`ContextKeySegmented` 其余 17 个 execution 与 context 域其余用例继续开放。

> 最新补充：Draft 4.139（2026-08-16），新增 `context-key-segmented-view` 差分场景，对照 `ContextKeySegmented` 的 2 个 execution（runtime `0b8a6173…`、`2a987a05…`），2 个 case、11 条 records、0 differences。覆盖 keyed 上下文 per-partition view：`partition by theString` 下每 key 独立 length(2) 窗口 + prevwindow（newest-to-oldest、淘汰行 pw null、跨分区 iterator）与 lastevent 替换 new/old 对。差分协议快照新增 keyed 分区描述符（`ContextPartitionIdentifierPartitioned` keys → `key:<value>`）。无新增运行时改动。Manifest v2 更新为 460 case、2,849 条关联、71 个 differential-verified、166 个 differential runtime、73 个 representative-verified、73/73 场景通过；`context.partition` 新增 2 个执行证据。剩余：`ContextKeySegmented` 其余 18 个 execution 与 context 域其余用例继续开放。

> 最新补充：Draft 4.138（2026-08-16），新增 `context-init-term-prev-prior` 差分场景，对照 `ContextInitTermPrevPrior`（runtime `962cedd0…`），1 个 case、6 条 records、0 differences。覆盖 daily cron 上下文（9:00–17:00 日窗口）内 keepall 窗口函数：prev/prevwindow（newest-to-oldest 事件数组）/prevtail/prior(1) + 运行 sum；17:00 分区关闭、次日 09:00 全新分区从空状态重启。差分协议新增 `[]Event` 字段归一化（prevwindow 嵌套 row 数组）；`Prior` offset 约定 0=前一事件映射 Java prior(1)。无新增运行时改动。Manifest v2 更新为 459 case、2,847 条关联、70 个 differential-verified、164 个 differential runtime、72 个 representative-verified、72/72 场景通过；`context.partition` 新增 1 个执行证据。剩余：`ContextInitTerm` 仅剩 ScheduleFilterResources（内部资源计数，属 NFR 面）与 context 域其余用例继续开放。

> 最新补充：Draft 4.137（2026-08-16），新增 `context-init-term-partition-selection` 差分场景，对照 `ContextInitTermContextPartitionSelection`（runtime `128472d2…`），1 个 case、11 条 records、0 differences。覆盖重叠 filter-initiated 上下文的迭代器/选择器管理面：上下文属性投影（context.id/context.s0.p00）+ keepall group-by 聚合、按 partition ID 与按起始事件属性的 `SnapshotWithSelector`、恒假过滤仍遍历全部分区（descriptor 快照含 startTime/initiating.p00）、segmented selector 拒绝（`invalid-context-partition-selector` 类别）。差分协议新增 `ids`/`filtered`/`segmented` 选择器与 `expectError` 步骤；initiated 分区 descriptor 新增 `startTime` 属性（epoch 毫秒 trace 归一化对齐 Java identifier）；错误文案对齐 `Invalid context partition selector`。Manifest v2 更新为 458 case、2,846 条关联、69 个 differential-verified、163 个 differential runtime、71 个 representative-verified、71/71 场景通过；`context.partition` 新增 1 个执行证据。剩余：`ContextInitTerm` 其余 2 个 execution（PrevPrior 属 prev/prior 能力域、ScheduleFilterResources 属内部资源计数）与 context 域其余用例继续开放。

> 最新补充：Draft 4.136（2026-08-16），新增 `context-init-term-inclusive-equals` 差分场景，对照 `ContextInitTermPatternInclusion`（runtime `b00a0be1…`）与 `ContextInitTermFilterInitiatedStraightEquals`（runtime `adb023e3…`），3 个 case、10 条 records、0 differences。覆盖 @Inclusive pattern 起始：every-distinct 10 秒 pattern 的 match 事件按 tag 序路由进新 partition（非聚合 `output last when terminated` 只保留最近行，@10000 输出 {E1,3}；key 过期驱动 @10100 第二 partition {E1,5}）、多事件起始 S0→S1 路由完成内层 pattern 语句、like 起始 + 相关等值 + 双 partition 并发 + 时长终止。公共 API 新增 `NewOverlappingPatternInitiatedTerminatedContextInclusive`/Create 变体与 `NewPatternInitiatedTerminatedContextInclusive`。运行时修复：pattern 起始 match 事件路由（overlap 无条件、非 overlap 仅 @Inclusive；当前触发事件不再二次分析）、pattern-end 终止事件不分析、非聚合 `output last when terminated` 保留最近一行；三个既有单元测试按 Java 探针修正。Manifest v2 更新为 457 case、2,845 条关联、68 个 differential-verified、162 个 differential runtime、70 个 representative-verified、70/70 场景通过；`context.partition` 新增 2 个执行证据。

> 最新补充：Draft 4.135（2026-08-16），新增 `context-init-term-duration` 差分场景，对照 `ContextInitTerm` 的 6 个 execution（runtime `e9c2321d…` ContextStartEndAfterZeroInitiatedNow、`1faa1af2…` ContextStartEndEndSameEventAsAnalyzed、`67e4f12d…` ContextInitTermFilterInitiatedFilterAllTerminated、`bdc0590b…` ContextInitTermFilterAndAfter1Min、`54ff8b2b…` ContextInitTermPatternIntervalZeroInitiatedNow、`45fe11b7…` ContextStartEndStartNowCalMonthScoped），7 个 case、15 条 records、0 differences。覆盖 duration/calendar/same-event 终止语义：`start after 0 sec end after 60 sec` 调度再武装时间周期上下文（E3@60000 精确边界被排除）、同事件终止不进聚合（10/10/10/10）与 insert-into 变体计入聚合（10/11/21/10.5）、filter-all 广播终止、filter 起始 + 1 分钟时长终止（起始事件投影）、pattern `timer:interval(0) or every timer:interval(1 min)` 一次性 OR 起始 + 60 秒时长终止（E3@180000 被排除）、`end after 1 month` 日历月终止（E3@03-01T09:00:00 被排除）。公共 API 新增 `NewScheduledTimePeriodContext`/`CreateScheduledTimePeriodContext`（scheduled-start 时间周期，与 `CreateTimePeriodContext` 的 `@now` 即时边界包含语义分离，引擎级 per-context armed-start 状态）与 `PatternStream.OrExclusive`（terminal 分支完成 quit 整个 OR 并杀死 every-child，对齐 `EvalOrStateNode.isQuitted`；既有 `Or` 保持 every 分支存活语义）。Manifest v2 更新为 456 case、2,843 条关联、456 个 implemented（verification）、67 个 differential-verified、160 个 differential runtime、69 个 representative-verified、69/69 场景通过；`context.partition` 新增 6 个执行证据。

> 最新补充：Draft 4.134（2026-08-16），新增 `context-start-end-correlated` 差分场景，对照 `ContextStartEndPatternWithFilterCorrelatedWithAsName`、`ContextStartEndPatternCorrelated`、`ContextInitTermPatternCorrelated`（runtime `9a99ccda…`、`0d946f8f…`、`1da79402…`），3 个 case、8 条 records、0 differences。覆盖相关 start/end 边界：pattern 起始 + 相关 filter 终止（起始 tag 注入终止谓词）、OR 起始/OR 终止的绑定/未绑定 tag 语义、重叠起始 + 同 bean 相关终止。运行时修复：混合 pattern-start/filter-end 终止谓词求值注入 partition pattern tags；filter-end 谓词须含事件类型守卫（与 Java typed filter 一致）。Manifest v2 更新为 455 case、2,837 条关联、66 个 differential-verified、154 个 differential runtime、68 个 representative-verified、68/68 场景通过。

> 最新补充：Draft 4.133（2026-08-16），新增 `context-init-term-output-clause` 差分场景，对照 `ContextInitTerm` 的 5 个 output-clause execution（runtime `81ca9d4f…`、`2e991ab4…`、`67c0d472…`、`61d4f2f4…`、`d8420feb…`），5 个 case、17 条 records、0 differences。覆盖 `every timer:at` 分钟级起始 + 时长终止下的 output all every N and when terminated（无界分组代表行 + order-by）、output when + when terminated 条件、only-when-terminated、when true/terminated then-set 变量副作用。运行时修复：无界分组 every-N 代表行 flush（窗口化保持 per-event）、终止 flush 无界缓冲行 + 空终止批次（forced + then-set）、partition 起始 when 求值（空 listener + 赋值，先于同刻终止）、setVariablesLocked last-wins。差分协议扩展：read-variable step、variable record（name/value 纳入 DiffTraces）、StepHandler 可返回记录。Manifest v2 更新为 454 case、2,834 条关联、65 个 differential-verified、152 个 differential runtime、67 个 representative-verified、67/67 场景通过。

> 最新补充：Draft 4.132（2026-08-16），新增 `context-init-term-filter-operators` 差分场景，对照 `ContextInitTermFilterAllOperators`、`ContextInitTermFilterBooleanOperator`、`ContextInitTermPatternInitiatedStraightSelect`（runtime `5c073601…`、`f0010ebf…`、`621848b5…`），27 个 case、127 次事件、47 条 records、0 differences。覆盖相关 filter 算子矩阵（=、!=、>、>=、<、<=、in、between、not in、not between、is、is not，双向 + 数值 coerce + null 语义）、算术相关谓词、OR pattern 起始未绑定 tag 投影。运行时修复：重叠 context partition 输出按 partitionID 分配序而非 key 字典序（`sortedPartitionKeys`，5 处迭代点统一）。Manifest v2 更新为 453 case、2,829 条关联、64 个 differential-verified、147 个 differential runtime、66 个 representative-verified、66/66 场景通过；`context.partition` 新增 3 个执行证据。

> 最新补充：Draft 4.131（2026-08-16），新增 `context-init-term-filter-pattern-end` 差分场景，对照 `ContextInitTermFilterAndPattern` 与 `ContextInitTermPatternAndAfter1Min`（runtime `f9e65063…`、`c8a41b14…`），2 个 case、7 条 records、0 differences。覆盖相关 pattern 终止：重叠 filter 起始 + 两步序列 end pattern（`S0(p00=theString) -> S1(p10=theString)`）、`every s0=S0 -> s1=S1(id=s0.id)` 起始 + 时长终止 + 起始 tag 投影。无新增运行时改动与公共 API。Manifest v2 更新为 452 case、2,826 条关联、63 个 differential-verified、144 个 differential runtime、65 个 representative-verified、65/65 场景通过；`context.partition` 新增 2 个执行证据。

> 最新补充：Draft 4.130（2026-08-16），新增 `context-init-term-keyed-aggregation` 差分场景，对照 `ContextInitTermAggregationGrouped`（runtime `e1f8f03e…`），1 个 case、6 条 records、0 differences。覆盖 keyed initiated-terminated 生命周期：partition key = 起始事件 grp、`ContextKeyValue` keyed 终止路由、partition 内 group-by 聚合隔离（CP1/CP2 独立累积）、终止后事件静默、同 key 重新起始从空状态累积。无新增运行时改动与公共 API。Manifest v2 更新为 451 case、2,824 条关联、62 个 differential-verified、142 个 differential runtime、64 个 representative-verified、64/64 场景通过；`context.partition` 新增 1 个执行证据。

> 最新补充：Draft 4.129（2026-08-16），新增 `context-init-term-overlap-duration` 差分场景，对照 `ContextInitTerm` 的 2 个 execution（runtime `b8e68ca2…`、`23719b9e…`），2 个 case、11 条 records、0 differences。覆盖重叠式 duration 终止：每分钟 cron pattern 起始 + 3 分钟终止（多 partition 独立聚合、输出按起始序）、filter 起始 + 1 分钟终止（双 partition 同时到期）。公共 API 新增 `NewOverlappingPatternTerminatedContext`/Create 变体（filter-start/pattern-end 重叠混合形态）。Java oracle 固定 commit `9e1b9f1c…` 可重生成；evidence + mutation（8 类）与 1 个单元回归。Manifest v2 更新为 450 case、2,823 条关联、61 个 differential-verified、141 个 differential runtime、63 个 representative-verified、63/63 场景通过；`context.partition` 新增 2 个执行证据。

> 最新补充：Draft 4.128（2026-08-16），新增 `context-init-term-with-now` 差分场景，对照 `ContextInitTermWithNow` 的 3 个 execution（runtime `8e440e32…`、`20229f5d…`、`8f7f85ae…`），3 个 case、11 条 records、0 differences。覆盖 @now 立即起始：时间周期 `start @now end after 10 sec` + `output last when terminated`（空周期 cnt=0）、`initiated by @Now and pattern [every timer:interval(10)] terminated after 10 sec`（立即 + 每 10s 重复起始 + 时长终止）、`start @now` 无终止累积。运行时修复 4 处（见 CHANGELOG）：deploy 时刻 start 条件处理（@now 物化）、context timer-root 单次触发 + 零进度停止、时间驱动起始的 end 条件按完成时刻武装、`output last when terminated` 输出聚合状态。`ContextInitTermWNowInvalid`（EPL 语法级拒绝）fluent API 无等价形态，保持 inventoried-only 并记录。Manifest v2 更新为 449 case、2,821 条关联、60 个 differential-verified、139 个 differential runtime、62 个 representative-verified、62/62 场景通过；`context.partition` 新增 3 个执行证据。

> 最新补充：Draft 4.127（2026-08-16），新增 `context-init-term-correlated` 差分场景，对照 `ContextInitTerm` 的 4 个 execution（runtime `9a99ccda…`、`4f7224de…`、`d0776e58…`、`c950c1a8…`），3 个 case、4 条 records、0 differences。覆盖 correlated as-name 上下文：pattern/pattern、pattern/filter（新 API `NewPatternInitiatedTerminatedByFilterContext`）、filter/pattern OR timer（`output when terminated` 的 ender tag 投影）。运行时修复：filter-start partition 初始化 end-pattern timer、`Statement.expire` 时间驱动混合 end-pattern 求值、pattern-end 终止输出取消 `definition.end != nil` 门控、start/end pattern tag 重名校验。Java oracle 固定 commit `9e1b9f1c…` 可重生成；evidence + mutation（7 类）与 3 个单元回归测试。Manifest v2 更新为 448 case、2,818 条关联、59 个 differential-verified、136 个 differential runtime、61 个 representative-verified、61/61 场景通过；`context.partition` 新增 4 个执行证据。

> 最新补充：Draft 4.126（2026-08-16），新增 `context-init-term-temporal-fixed` 差分场景，对照 `ContextInitTermTemporalFixed` 的 11 个 execution（runtime `0b7363d4…`、`44efa774…`、`521d7b99…`、`f17d1a63…`、`0462363d…`、`f3b99373…`、`dfb7b1c1…`、`09d7f8c1…`、`c2ee338e…`、`d5217f32…`、`cbfe2ec9…`），11 个 case、65 条 records、0 differences。覆盖 initiated-terminated 上下文全形态：filter/pattern 双端（含相关 end pattern 引用 ContextInitiatingEvent 与终止快照输出）、every-second cron、9-to-5 daily + full-outer join（含反向时钟跳变）、daily 内 `every timer:interval(10 sec)`、turned-off/turned-on 多 statement 中途部署、8–9 与 9–5 分组/非分组聚合。运行时修复 7 处（见 CHANGELOG）：temporal origin 首次 deploy 锚定（修复 epoch origin 导致的 cronWindow 1970→2002 逐秒挂死）、dailyWindow origin 窗口内激活、上下文 pattern 完成后再武装/within 到期终止语义、`patternTimeBatch` root timer 单次触发 + 当前时刻再武装 + 投递时刻求值、tagless pattern `select *` 空行输出、`EqualValues` 指针解引用、兼容协议 `deploy` step（含 mid-case listener attach）。Java oracle 固定 commit `9e1b9f1c…` 可重生成；新增 evidence + mutation 测试（9 类 mutation 全部被差分框架拒绝）与 6 个回归测试；3 个既有测试按 Java probe 修正 one-shot timer 语义。Manifest v2 更新为 447 case、2,814 条关联、58 个 differential-verified、132 个 differential runtime、60 个 representative-verified、60/60 场景通过；`context.partition` 新增 11 个执行证据。

> 最新补充：Draft 4.125（2026-08-15），新增 `subselect-in` 差分场景，对照 `EPLSubselectIn` 的 14 个 execution，14 个 case、52 条 records、0 differences。场景覆盖 IN/NOT IN 子查询在 select 子句（含 OM/Compile 变体）、filter criteria（length(2) 淘汰边界）、where 子句（双侧表达式）的位置形态，nullable 字符串与 boxed 数值 coercion 的三值逻辑（null 候选/外层 → null、空集 `not in` → true 含 null 外层）、null row，以及 keepall 相关 IN 索引形态。全部复用既有 SubqueryIn/In/Not 与 boxed 值语义，无新增运行时改动。新增 evidence/mutation 测试。Manifest v2 更新为 446 case、2,803 条关联、446 个 implemented（verification）、57 个 differential-verified、121 个 differential runtime、59 个 representative-verified、59/59 场景通过；`query.subquery-basic` 新增 14 个执行证据。

> 最新补充：Draft 4.124（2026-08-15），新增 `subselect-aggregated-single-value` 差分场景，对照 `EPLSubselectAggregatedSingleValue` 的 13 个 execution，14 个 case、57 条 records、0 differences。场景覆盖无窗口累积 sum、`select *` + having、select 子句内 OuterField+聚合混合投影、length(3) where 过滤、相关 count 与窗口淘汰、where 子句相关子查询（含 `||` 拼接）、相关 where+having、分组 scalar 子查询（相关 having、having 内相关）、`last(theString)` 相关 having，以及 table + into-table 聚合列 having 子查询（无键与双键分组）。运行时修复：`SubquerySum/SubqueryAvg` 携带包装聚合投影（无窗口事件流源校验通过并统一聚合求值路径）；`SubqueryGroupScalar` 按 distinct group key 判定单组（单组多事件返回首个投影值，对齐 Java 分组折叠）；table 子查询行投影/分组键改用 `Field`（`TableField` 为 trigger/assignment 目标行语义）。新增单元回归与 evidence/mutation 测试。Manifest v2 更新为 445 case、2,789 条关联、445 个 implemented（verification）、56 个 differential-verified、107 个 differential runtime、58 个 representative-verified、58/58 场景通过；`query.subquery-basic` 与 `trigger.table-named-window` 新增执行证据。

> 最新补充：Draft 4.123（2026-08-15），新增 `subselect-aggregated-in-exists-any-all` 差分场景，对照 `EPLSubselectAggregatedInExistsAnyAll` 的 13 个 execution，13 个 case、47 条 records、0 differences。场景用 SupportValueEvent 触发投影 IN/NOT IN、EXISTS/NOT EXISTS 与 ALL/ANY/SOME 量词，对照 SupportBean#keepall 聚合子查询（无分组/按 theString 分组/带 last/first having 过滤），覆盖空集 SQL 语义（无分组聚合空输入产出 null 行、分组空集 IN/ANY/SOME false 且 ALL true、EXISTS false）、having 缺失聚合行 IN 为 null、分组 having 按组求值，以及 named window + FAF delete-all 后 EXISTS 复位。公共 API 新增 `SubqueryGroupKey` option（IN/ANY/ALL/SOME/EXISTS/VALUE 子查询 group by，聚合投影按组候选、量词空集规则按分组区分）；compat 回放协议新增 `faf` step。新增单元回归与 evidence/mutation 测试。Manifest v2 更新为 444 case、2,776 条关联、444 个 implemented（verification）、55 个 differential-verified、94 个 differential runtime、57 个 representative-verified、57/57 场景通过；`query.subquery-basic` 新增 13 个执行证据。

> 最新补充：Draft 4.122（2026-08-15），新增 `resultset-aggregate-count-sum` 差分场景，对照 `ResultSetAggregateCountOneView`、`ResultSetAggregateCountJoin`、`ResultSetAggregateCountSimple` 与 `ResultSetAggregateSumNamedWindowRemoveGroup`（runtime `ef1afacf…`、`5fd3d359…`、`d915c88b…`、`d2a92c1b…`），4 个 case、34 条 records、0 differences。场景用 SupportMarketDataBean#length(3) 分组 irstream `count(*)/count(distinct volume)/count(all volume)` 双跑 DELL 50/null/25/25/25 + IBM 1/null/null/null 序列（view 与 SupportBeanString#length(100) join 两个变体），覆盖 null volume、重复值、窗口淘汰 distinct 重算（50 淘汰 countDistVol 2→1）、跨组淘汰批次（new 为 IBM+DELL、old 为各自 pre-batch 状态）；`count(*)` 在 #time(1) 逐事件 1/2/3；keepall named window + insert + on-delete（按 id 与 delete-all）触发下分组 sum：组删除输出 null 行、iterator 逐组快照。运行时修复：`CountDistinct` 按指向值去重（boxed 指针字段对齐 Java equals 语义）、grouped irstream 行序新事件组在前（对齐 `ResultSetProcessorAggregateGroupedImpl`）。新增单元回归与 evidence/mutation 测试。Manifest v2 更新为 443 case、2,763 条关联、443 个 implemented（verification）、54 个 differential-verified、81 个 differential runtime、56 个 representative-verified、56/56 场景通过；`resultset.aggregate-group-by`、`view.window-core`、`join.basic` 与 `trigger.table-named-window` 新增四个执行证据。

> 最新补充：Draft 4.121（2026-08-15），新增 `resultset-aggregate-limit-snapshot` 差分场景，对照 `ResultSetLimitSnapshot`（`java-runtime-caa47dd0c613a0fc0ff4`）与 `ResultSetLimitSnapshotJoin`（`java-runtime-ba55e3db86bd1b3007eb`），2 个 case、6 条 records、0 differences。覆盖 time(10s) 分组 sum + `output snapshot every 1 seconds`（含 join/order-by 变体）的精确边界语义：snapshot 在同一 tick 包含 deadline == now 的到期事件及其聚合贡献，时钟跳变跨过的 overdue 事件在后续 snapshot 中排除；istream 含边界到期语义保持不变。运行时修复：join 聚合 snapshot 恢复 row-per-event 形状（`aggregateDefinitionSnapshotRowForEvent` 放开 join）；新增 `aggregateSnapshotBoundary` 边界机制，`expireBatch` 到期前捕获 exact-boundary 事件，`snapshotAggregateStateBatch` 求值时重新计入行序、group 聚合与排序。Manifest v2 更新为 442 case、2,759 条关联、53 个 differential-verified、77 个 differential runtime、55 个 representative-verified、55/55 场景通过。

> 最新补充：Draft 4.120（2026-08-15），新增 `resultset-aggregate-group-output` 差分场景，对照 `ResultSetLastNoDataWindow`（`java-runtime-fbb40900955b851741c6`）、`ResultSetWildcardRowPerGroup`（`java-runtime-34a8b14ea96443475b9c`）、`ResultSetUnaggregatedOutputFirst`（`java-runtime-decf2ff614722c1273d1`）与 `ResultSetFirstSimpleHavingAndNoHaving`（`java-runtime-132314df55366e67ec84`），6 个 case、13 条 records、0 differences。运行时修复 `applyLastEveryTimeGrouped`：单 grouping-set 的 row-per-group `output last every time` 只发 new（Java 不发上一周期 old；rollup 多 grouping-set 保留 old 行为）。新增 `RecordStream.Having` 公共 API：无聚合 having 按 Esper 语义降级为行过滤（与 Filter 计划等价）。Manifest 对账修复：`case.output-policy-iterator` 与 `case.filter-window-aggregate-output` 改为 representative-verified 合成场景并移除未实际重放的 Java runtime 声明，四个受影响 runtime 在本轮补齐真实差分证据。Manifest v2 更新为 441 case、2,757 条关联、52 个 differential-verified、75 个 differential runtime、54 个 representative-verified、54/54 场景通过。

> 最新补充：Draft 4.119（2026-08-15），新增 `resultset-aggregate-multikey` 差分场景，对照 `ResultSetOutputLastMultikeyWArray`（`java-runtime-080737b1c181da82710e`）与 `ResultSetOutputAllMultikeyWArray`（`java-runtime-418605c41b57b8a28c09`）。场景用 SupportBean#keepall 按 `(theString, longPrimitive)` 双键分组 + `sum(intPrimitive)` 在 `output last/all every 1 seconds` 下双跑两 case：last 在 t=1s 输出 A/0/12/22、B/1/11/11、C/0/13/13，t=2s 输出 A/0/14/36；all 在 t=1s 输出四个事件累计行、t=2s 输出新事件行 + 未变化 group 代表行重发。Java/Go 各 4 条 records、0 differences，复用双键 group-by 与 output last/all 既有实现，无新增运行时改动。Manifest v2 更新为 440 case、2,757 条关联、440 个 implemented（verification）、53 个 differential-verified、74 个 differential runtime、53 个 representative-verified、53/53 场景通过；`resultset.aggregate-group-by`、`output.core` 与 `view.window-core` 新增两个 execution 证据。

> 最新补充：Draft 4.118（2026-08-15），新增 `resultset-aggregate-join-sort-window` 差分场景，对照 `ResultSetJoinSortWindow`（`java-runtime-412ca9b80556afdf36e8`）。场景用 SupportBean#keepall 单种子 + SupportMarketDataBean#sort(1, volume) 双事件在 irstream `output every 1 seconds` 下双跑 1 条 record，evidence 0 differences。运行时修复：join 侧“进入即被淘汰”同一事件保留 tuple 参与当前 join，同一 tuple 同时进 new/old delta，diff 后从 side 清除；`aggregateBatch` 先 new 后 old，enter-then-leave 净状态正确；聚合移除改为 `sameEventRow`（join tuple 按包裹流事件身份、普通事件按稳定事件身份匹配），同负载替换事件不被误删，scope 移除同步改为先 append new 再 remove old。Manifest v2 更新为 439 case、2,755 条关联、439 个 implemented（verification）、52 个 differential-verified、72 个 differential runtime、52 个 representative-verified、52/52 场景通过；`resultset.aggregate-group-by`、`output.core`、`join.basic` 与 `view.window-core` 新增 `ResultSetJoinSortWindow` 证据。

> 最新补充：Draft 4.117（2026-08-15），`resultset-having-every-events` 场景扩展 `ResultSetHavingJoin`（`java-runtime-1b23cf3fcfceb0d7eb23`）：SupportBean#keepall 单种子 + 同一条 IBM 序列在 irstream `output every 3 events` 下双跑 3 条 records，Java/Go 各 6 条 records、0 differences；复用 join + having + 事件计数 + 淘汰 old 修复，无需新增运行时改动。Manifest v2 更新为 438 case、2,754 条关联、51 个 differential-verified、71 个 differential runtime、51 个 representative-verified、51/51 场景通过；`resultset.aggregate-group-by`、`output.core` 与 `join.basic` 新增 `ResultSetHavingJoin` 证据。

> 最新补充：Draft 4.116（2026-08-15），新增 `resultset-aggregate-join-events` 差分场景，对照 `ResultSetJoinDefault`、`ResultSetJoinAll` 与 `ResultSetJoinLast`（runtime `a67ffb3a47f7812b19d3`、`a54e39addd5960e2ccb4`、`f0c54da1159588825b5d`）。场景用 SupportBeanString#length(100) 种子 + SupportMarketDataBean#length(5) 双跑 output every/all/last every 2 events 三个 case，共 6 条 records、0 differences；复用事件计数与 output-all/join 修复，无需新增运行时改动。Manifest v2 更新为 438 case、2,753 条关联、438 个 implemented（verification）、51 个 differential-verified、70 个 differential runtime、51 个 representative-verified、51/51 场景通过；`resultset.aggregate-group-by`、`output.core`、`join.basic` 与 `view.window-core` 新增三个执行证据。

> 最新补充：Draft 4.115（2026-08-15），新增 `resultset-aggregate-all-having` 差分场景，对照 `ResultSet11AllHavingNoJoin` 与 `ResultSet12AllHavingJoin`（runtime `72011b9d57605c76f1b0`、`598cef9d8eebd4dfd1a2`）。场景用 200–7200ms 序列在 `having sum(price) > 50` + irstream `output all every 1 seconds` 下双跑 view 与 join 两个 case，共 10 条 records、0 differences。运行时修复：having 过滤 old 行的移除批次仍标记 group 已见（`ResultBatch.removedGroupKeys` 记录输入移除 group key），避免边界误重发旧代表行。Manifest v2 更新为 437 case、2,750 条关联、437 个 implemented（verification）、50 个 differential-verified、67 个 differential runtime、50 个 representative-verified、50/50 场景通过；`resultset.aggregate-group-by`、`output.core`、`view.window-core` 与 `join.basic` 新增两个执行证据。

> 最新补充：Draft 4.114（2026-08-15），`resultset-aggregate-join` 场景扩展 `ResultSet10AllNoHavingJoin`（`java-runtime-fff1f571dd69b6dd0579`）：irstream `output all every 1 seconds order by symbol` 双跑 7 次输出，Java/Go 各 51 条 records、0 differences；复用 `OutputAllEveryTime` aggregate-grouped 重构，无需新增运行时改动。Manifest v2 更新为 436 case、2,748 条关联、49 个 differential-verified、65 个 differential runtime、49 个 representative-verified、49/49 场景通过；`resultset.aggregate-group-by`、`output.core` 与 `join.basic` 新增 `ResultSet10AllNoHavingJoin` 证据。

> 最新补充：Draft 4.113（2026-08-15），新增 `resultset-aggregate-all-time-window` 差分场景，对照 `ResultSet9AllNoHavingNoJoin`（`java-runtime-21f1ac2a389796ce96ea`）。场景用 SupportMarketDataBean#time(5.5 sec) + group by symbol + irstream `output all every 1 seconds order by symbol` 双跑 7 次输出，evidence 7 条 records、0 differences。运行时重构 `OutputAllEveryTime` aggregate-grouped 路径：事件行累积 + 被移除 group 代表行补发 + leaving-event old + 无新事件 group 代表行重发，对齐 `ResultSetProcessorAggregateGroupedOutputAllHelper`；rollup/row-per-group 快照 old 语义不变，`OutputAllEveryEvents` 复用同一累积逻辑。Manifest v2 更新为 436 case、2,747 条关联、436 个 implemented（verification）、49 个 differential-verified、64 个 differential runtime、49 个 representative-verified、49/49 场景通过；`resultset.aggregate-group-by`、`output.core` 与 `view.window-core` 新增 `ResultSet9AllNoHavingNoJoin` 证据。

> 最新补充：Draft 4.112（2026-08-15），`resultset-aggregate-join` 场景扩展 `ResultSet14LastNoHavingJoin`、`ResultSet16LastHavingJoin` 与 `ResultSet17FirstNoHavingJoin`（runtime `b3cb0ef477369111027c`、`e0866a0c063a6db46942`、`9a902334e2f0f270f8f5`）：output last（order by symbol，6 条）、output last + having（3 条）与 output first（11 条，含纯淘汰 New），Java/Go 各 44 条 records、0 differences；复用 aggregate-grouped join 与事件锚定修复，无需新增运行时改动。Manifest v2 更新为 435 case、2,746 条关联、48 个 differential-verified、63 个 differential runtime、48 个 representative-verified、48/48 场景通过；`resultset.aggregate-group-by`、`output.core` 与 `join.basic` 新增三个 join 执行证据。

> 最新补充：Draft 4.111（2026-08-15），新增 `resultset-aggregate-join` 差分场景，对照 `ResultSet2NoneNoHavingJoin`、`ResultSet4NoneHavingJoin`、`ResultSet6DefaultNoHavingJoin` 与 `ResultSet8DefaultHavingJoin`（runtime `00013efc7c322d04c3b3`、`ecfb75f6916643de1c5f`、`80532da2ea5ec539be06`、`d2978828701be89efbdb`）。场景用 SupportBean#keepall 种子 + 完整 200–7200ms market 序列双跑四个 case，共 24 条 records、0 differences。运行时修复：`expressionTreeReadsCurrentEvent` 识别 join-field，grouped join 带非 group-key 事件属性时走 AggregateGroupedImpl 语义；时间型 output 调度锚点只在批次携带真实输入时设置（join 种子不锚定）。Manifest v2 更新为 435 case、2,743 条关联、435 个 implemented（verification）、48 个 differential-verified、60 个 differential runtime、48 个 representative-verified、48/48 场景通过；`resultset.aggregate-group-by`、`output.core`、`join.basic` 与 `view.window-core` 新增四个 join 执行证据。
...
本轮新增 `aggregate-firstlastwindow-ext` differential-verified 切片，对照 Java `ResultSetAggregateFirstLastWindow` 的 4 个 execution（`ResultSetAggregateSubquery` `java-runtime-20f27b9b…`、`ResultSetAggregateOutputRateLimiting` `6ae9bc86…`、`ResultSetAggregateLateInitialize` `3d1a2b9d…`、`ResultSetAggregateMixedNamedWindow` `2aed2cac…`）。Go 复用既有 `SubqueryRow`/`MapValue`/`WindowEvents`/`OutputEvery`/`CreateNamedWindow`/`InsertIntoNamedWindow`/`DeleteFromNamedWindow`/`FromNamedWindow`/`GroupBy`/`Sum`/`WindowValues` API，4 个 parity 测试覆盖 subquery（window(sb.*) 子查询在 length(2) 窗口上求值，空窗口 null，淘汰后反映当前窗口）、output-rate-limiting（output every 2 events 输出每事件聚合快照，非 output-last）、late-initialize（named window 先插入再部署 consumer，consumer 看到全部预存行）、mixed-named-window（on-delete 清空组发出 null 聚合行，部分删除后聚合重组）。Java oracle（`ResultSetAggregateFLWExtScenarioOracle`）生成 12 条规范化 trace 记录，4 个 case 全部与 Go 测试断言逐字段一致。Manifest v2 更新为 505 case、2,962 条关联、2,820/4,136 referenced runtime（68.2%）、103 个 differential-verified case、292 个 differential runtime ID；`resultset.aggregate-access` 能力新增 4 个执行证据。

本轮新增 `aggregate-firstlastwindow` differential-verified 切片，对照 Java `ResultSetAggregateFirstLastWindow` 的 5 个 execution（`ResultSetAggregateUnboundedSimple` `java-runtime-5c73ecba…`、`ResultSetAggregateWindowedGrouped` `e4b69ac4…`、`ResultSetAggregateBatchWindow` `eb23cb37…`、`ResultSetAggregateBatchWindowGrouped` `f04c46c8…`、`ResultSetAggregateWindowAndSumWGroup` `b03302bb…`）。Go 复用既有 `First`/`Last`/`FirstEver`/`LastEver`/`WindowValues`/`Sum`/`Multiply` 聚合 API 及 `LengthWindow`/`LengthBatch`/`GroupBy`/`OrderBy`/`WithOldStream` 窗口和输出 API，5 个 parity 测试覆盖 unbounded-simple（无窗口 ever 语义）、windowed-grouped（length(5) 分组 per-group 淘汰 + order-by）、batch-window（length_batch(2) irstream new/old 配对）、batch-window-grouped（length_batch(6) 分组空组 null 行）、window-and-sum-wgroup（length(3) 分组 sum + window(expr) 淘汰驱动空组 null 行）。Java oracle（`ResultSetAggregateFirstLastWindowScenarioOracle`）生成 22 条规范化 trace 记录，5 个 case 全部与 Go 测试断言逐字段一致。Manifest v2 更新为 504 case、2,958 条关联、2,816/4,136 referenced runtime（68.1%）、102 个 differential-verified case、288 个 differential runtime ID；`resultset.aggregate-access` 能力新增 5 个执行证据。

本轮新增 `variables-onset` differential-verified 切片，对照 Java `EPLVariablesOnSet` 的 4 个 execution（`EPLVariableOnSetCompile` `java-runtime-2735f44a…`、`EPLVariableOnSetObjectModel` `60c4c5dc…`、`EPLVariableOnSetSimpleSceneTwo` `91f1de1b…`、`EPLVariableOnSetSubquery` `1588aa4d…`）。Go 复用既有 `RegisterVariable`/`VariableRef`/`OnEvent.SetVariables`/`SetVariableExpr`/`SubqueryRow`/`MapValue`/`Concat`/`LastEvent` API，3 个 parity 测试覆盖：(1) `typed-select`——typed double/long 变量经 on 触发器更新后 dependent select 观测到更新值；(2) `scene-two`——@public 变量跨 statement 共享，on 触发器更新，irstream select 输出当前值；(3) `subquery`——on 触发器从 #lastevent 子查询赋值 string 变量，空子查询赋 null。RUNTIMEOPS 变体和 array-index assignment execution 因 Go API 暂不支持数组索引赋值而不在本切片。Java oracle（`EPLVariablesOnSetScenarioOracle`）生成 20 条规范化 trace 记录，3 个 case 全部与 Go 测试断言逐字段一致。Manifest v2 更新为 503 case、2,953 条关联、2,811/4,136 referenced runtime（68.0%）、101 个 differential-verified case、283 个 differential runtime ID；`epl.variable-onset` 能力新增 4 个执行证据。

本轮新增 `istream-rstream-keywords` differential-verified 切片，对照 Java `EPLOtherIStreamRStreamKeywords` 的 7 个 execution（`EPLOtherRStreamOnly` `java-runtime-d268f6c4…`、`EPLOtherRStreamInsertInto` `0e4714c5…`、`EPLOtherRStreamInsertIntoRStream` `26cbb822…`、`EPLOtherRStreamJoin` `2fd326ba…`、`EPLOtherIStreamOnly` `4903471…`、`EPLOtherIStreamInsertIntoRStream` `bdda0ee0…`、`EPLOtherIStreamJoin` `ccc0bd02…`）。Go 复用既有 `WithRemoveStreamOnly()`/`WithNewStreamOnly()` selector、`InsertInto` 路由与 `Join`/`OnEqual` API，7 个 parity 测试覆盖 rstream-only（length(3) 淘汰触发）、rstream-insert-into（insert into 剥离 select 的 rstream selector，下游收全部 insert）、rstream-insert-into-rstream（只转发淘汰行）、rstream-join（淘汰时输出 join 行）、istream-only（length(1) 只发 insert）、istream-insert-into-rstream（istream 源 + insert rstream into，消费者收淘汰行）、istream-join（匹配时输出一次）。Java 的单一语句中 select-selector 与 insert-into 路由模式相互独立；Go 的 InsertInto 共享同一 selector，故 insert-into 混合场景以同窗口源上的双语句等价建模。OM/Compile 变体（SODA/EPL 文本）为 RStreamOnly 的编译 API 重复，RStreamOutputSnapshot 为无可观测行的 smoke，均不属本切片。Java oracle（`EPLOtherIStreamRStreamScenarioOracle`）生成 15 条规范化 trace 记录，7 个 case 全部与 Go 测试断言逐字段一致。Manifest v2 更新为 502 case、2,949 条关联、2,807/4,136 referenced runtime(67.9%)、100 个 differential-verified case、253 个 differential runtime ID；`eplother.stream-selector` 能力新增 1 个执行证据。

本轮新增 `row-for-all` differential-verified 切片，对照 Java `ResultSetQueryTypeRowForAll` 的 3 个 execution（`ResultSetQueryTypeRowForAllSumMinMax` `java-runtime-6c590766…`、`ResultSetQueryTypeRowForAllSimple` `803e0922…`、`ResultSetQueryTypeRowForAllMinMaxWindowed` `e54e4c73…`）。Go 复用既有 `Sum`/`Min`/`Max`/`Avg`/`Median`/`StdDev`/`Avedev`/`Count`/`CountDistinct` 聚合 API 及 `WithOldStream()` irstream 支持，3 个 parity 测试覆盖：(1) `sum-min-max`——无分组无窗口的运行 sum/min/max，6 个事件递加验证（E1:10/10/10 → E6:350/9/120）；(2) `simple`——无分组无窗口的完整聚合集（avg/sum/min/max/median/stddev/avedev/count/countDistinct）含 irstream old/new 流，3 个事件验证（100→200→150）；(3) `min-max-windowed`——length(2) 窗口的 min/max 含 irstream，滑动窗口淘汰验证（100→200→150 后 min=150）。Java oracle（`ResultSetRowForAllScenarioOracle`）生成 12 条规范化 trace 记录，3 个 case 全部与 Go 测试断言逐字段一致。Manifest v2 更新为 501 case、2,936 条关联、2,800/4,136 referenced runtime(67.7%)、99 个 differential-verified case、246 个 differential runtime ID；`resultset.aggregate-group-by` 能力新增 1 个执行证据。

本轮新增 `variables-use` differential-verified 切片，对照 Java `EPLVariablesUse` 的 3 个 execution（`EPLVariableUseVariableInFilter` `java-runtime-5a38cfa8…`、`EPLVariableUseVariableInFilterBoolean` `849ebec4…`、`EPLVariableUseSimpleSameModule` `5a91cdbc…`）。Go 复用既有 `RegisterVariable`/`VariableRef`/`OnEvent.SetVariables`/`SetVariableExpr` API，3 个 parity 测试覆盖：(1) `variable-in-filter`——on-event 设置 var1IF 变量后，filter(theString=var1IF) 匹配新值（"a"匹配、后续更新为"e"后"e"匹配、"b"/"c"不匹配）；(2) `variable-in-filter-boolean`——两个变量 var1IFB/var2IFB 的 OR filter（"a"匹配var1、"b"匹配var2、更新后"c"匹配var2、"e"匹配var1）；(3) `simple-same-module`——select 子句中引用 boolean 变量返回常量值 true。Java null 初始变量用 sentinel 值模拟。EPRuntime/ConstantVariable/SODA/thread 等 RUNTIMEOPS 变体不属于本切片。Java oracle（`EPLVariablesUseScenarioOracle`）生成 7 条规范化 trace 记录，3 个 case 全部与 Go 测试断言逐字段一致。Manifest v2 更新为 500 case、2,939 条关联、2,797/4,136 referenced runtime（67.6%）、98 个 differential-verified case、243 个 differential runtime ID；`epl.variable-onset` 能力新增 1 个执行证据。

本轮新增 `stream-selector` differential-verified 切片，对照 Java `EPLOtherSelectExprStreamSelector` 的 4 个 execution（`EPLOtherNoJoinWildcardWithAlias` `java-runtime-70753b73…`、`EPLOtherJoinWildcardWithAlias` `bc1c2410…`、`EPLOtherAloneJoinAlias` `4a8e3f2d…`、`EPLOtherAloneJoinNoAlias` `f7193a89…`）。Go 复用既有 `SelectSourceEvent`/`SelectFrom`/`JoinField`/`EventValue` API，4 个 parity 测试覆盖：(1) `nojoin-wildcard-alias`——单流 `select *, win.* as s0`，所有属性 + `s0` 列持有事件本身；(2) `join-wildcard-alias`——join `select *, s1.* as s1stream, s0.* as s0stream`，s0/s1 流列 + s0stream/s1stream 别名流列均持有完整事件；(3) `alone-join-alias`——join `select s1.* as s1`（正向 s1 别名）及 `select s0.* as szero`（反向 s0 别名），结果仅含别名事件列；(4) `alone-join-noalias`——join `select s1.*`（无别名），结果展开为事件的独立属性列（symbol/volume/price）而非包装事件列。Invalid/OM/pattern 变体不属于本切片。Java oracle（`EPLOtherStreamSelectorScenarioOracle`）生成 4 条规范化 trace 记录，4 个 case 全部与 Go 测试断言逐字段一致。Manifest v2 更新为 499 case、2,936 条关联、2,794/4,136 referenced runtime（67.5%）、97 个 differential-verified case、240 个 differential runtime ID；`eplother.stream-selector` 能力新增 1 个执行证据。

本轮新增 `subselect-exists` differential-verified 切片，对照 Java `EPLSubselectExists` 的 5 个 execution（`EPLSubselectExistsInSelect` `java-runtime-06ee960c…`、`EPLSubselectExistsSceneOne` `3ec7a4b1…`、`EPLSubselectExistsFiltered` `3c251773…`、`EPLSubselectTwoExistsFiltered` `98a351db…`、`EPLSubselectNotExists` `41c99f42…`）。Go 复用既有 `SubqueryExists`/`Not`/`And`/`OuterField` API，5 个 parity 测试覆盖：(1) `exists-in-select`——select 子句中的 exists(subquery) 在 S1 窗口为空时返回 false、非空时返回 true；(2) `exists-scene-one`——where 子句中的 exists 过滤 S0 事件（S1 窗口为空时过滤）；(3) `exists-filtered`——correlated exists（s1.id=s0.id）按外部事件 id 过滤；(4) `two-exists-filtered`——两个 exists 条件必须同时满足；(5) `not-exists`——where not exists 在 S1 窗口为空时通过 S0 事件。OM/Compile 变体（SODA/EPL 文本编译）Go 不支持，归入 intentionally-different。Java oracle（`EPLSubselectExistsScenarioOracle`）生成 9 条规范化 trace 记录，5 个 case 全部与 Go 测试断言逐字段一致。Manifest v2 更新为 498 case、2,932 条关联、2,790/4,136 referenced runtime（67.4%）、96 个 differential-verified case、236 个 differential runtime ID；`query.subquery-basic` 能力新增 1 个执行证据。

本轮新增 `insertinto-event-precedence` differential-verified 切片，对照 Java `EPLInsertIntoEventPrecedence` 的 4 个 constant-precedence execution（`EPLInsertIntoEventPrecConstantInsertInto` `java-runtime-b06231c9…`、`EPLInsertIntoEventPrecConstantInfraMergeInsertInto{namedWindow=false}` `a9064a1a…`、`EPLInsertIntoEventPrecConstantInfraMergeInsertInto{namedWindow=true}` `83ed465c…`、`EPLInsertIntoEventPrecConstantOnSplit` `d96c25e6…`）。Go 新增 `SplitIntoWithPrecedence`/`SplitIntoWhenWithPrecedence` 公共 API 及 `SplitStreamBranch.Precedence` 字段，split-stream 分支按 event-precedence 降序路由（高优先级先处理，无优先级最后）；`deliverSplitStreamEvent` 改为 `insertRoutedEventLocked` 以利用 precedence 排序。4 个 parity 测试覆盖：(1) `constant-insertinto`——6 个 insert-into 语句（precedence 4/2/0/5/1/3）路由到 `Out` 事件流，consumer 按 precedence 降序接收（5,4,3,2,1,0）；(2) `constant-merge-table`——merge-into table 的 3 个 insert-into action 按 precedence 降序执行（c,a,b）；(3) `constant-merge-window`——merge-into named window 的 3 个 insert-into action 按 precedence 降序执行（c,a,b）；(4) `constant-on-split`——split-stream 的 3 个分支按 precedence 降序执行（3,2,1）。Java oracle（`EPLInsertIntoEventPrecedenceScenarioOracle`）生成 21 条规范化 trace 记录，4 个 case 全部与 Go 测试断言逐字段一致。Manifest v2 更新为 497 case、2,927 条关联、2,785/4,136 referenced runtime（67.3%）、95 个 differential-verified case、231 个 differential runtime ID；`query.insert-into-route` 能力新增 1 个执行证据。

本轮新增 `insertinto-eventcol-subquery-bean` differential-verified 切片，对照 Java `EPLInsertIntoPopulateEventTypeColumnBean` 的 `EPLInsertIntoColBeanFromSubquerySingle`（4 个 execution：objectarray/map × filter/nofilter）与 `EPLInsertIntoColBeanFromSubqueryMulti`（4 个 execution：objectarray/map × filter/nofilter），共 8 个 runtime（`java-runtime-673bf8a5…`、`c2a177b1…`、`aa746108…`、`09a80627…`、`a8ed4509…`、`8248337b…`、`0e339607…`、`bffdd5c8…`）。Go 新增 `epl_insert_into_eventcol_subquery_parity_test.go`，8 个 parity 测试覆盖两条核心语义契约：(1) scalar subquery into event-typed column（长度 2 的窗口 + SubqueryCardinalityMode(SubqueryNullOnMultiple)——零行匹配/多行匹配返回 null、单行匹配路由 SupportBean_S0 fragment）、inner where（id>=100）过滤；(2) multi-row subquery into array-typed column（keepall 窗口 + SubqueryValues——累积 SupportBean_S0 fragment 数组）、where 1=1 no-op 行为等价无过滤。Java oracle（`EPLInsertIntoEventColSubqueryBeanScenarioOracle`）注册 SupportBean/SupportBean_S0 bean 类与 EventOne objectarray/map schema，internal timer 关闭固定 epoch 0，生成 16 条规范化 trace，8 个 case 全部与 Go 断言逐字段一致。该切片映射到 `query.insert-into-route` 能力；完整的 insert-into event-type column coverage（NonBean 变体、更多 projection 形态、representation 转换和 named window column）仍保持 partial。
