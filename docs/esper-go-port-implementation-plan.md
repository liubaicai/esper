...
本轮继续对照 Java `ExprEnumDocSamples.ExprEnumSubquery`/`ExprEnumNamedWindow` 与 `ExprEnumDataSources.ExprEnumUDFStaticMethod`：Go 增加 `SubqueryValues`/`SubqueryEvents`，将 named-window 多行快照（含 inner predicate）转换成可继续 `EnumSelect` 的 typed slice；同时提供类型安全的 `Func0`/`Func1`/`Func2`，使零参数、单参数和双参数 UDF 返回的集合可以直接进入 `EnumSum` 等枚举链；`Func1`/`Func2` 对 nil 函数或参数返回 Null，不把错误延迟成 panic。`TestEnumerableSubqueryAndZeroArgumentUDFFSources` 固定 E1/E2 named-window 过滤、投影求和及三种 UDF 求和。事件流 subquery、访问聚合/Context/Table/Pattern source、声明表达式、generic component metadata 和完整 invalid 矩阵仍保持开放；该切片继续登记为 `case.expr-enum-subquery-udf`。
...
本轮继续对照 Java `ExprCoreBitWiseOperators` 的 3 个 execution（`java-runtime-b0d354033a204970cb26`、`java-runtime-7819faeecbb3e817d49d`、`java-runtime-e1ecced8a0b539b7ba84`）：新增 `BitwiseAnd`/`BitwiseOr`/`BitwiseXor` 与 `Binary*` aliases，统一支持整数和布尔类型；整数结果保留 Go 声明宽度/符号，布尔值按 Esper 的二元 AND/OR/XOR 求值，Null/Missing 传播为 Null，泛型约束在编译期排除字符串/浮点/对象，nil operand 在 Build 阶段拒绝。`TestBitwiseExpressionsPreserveIntegerWidthAndBooleanSemantics` 与 `TestBitwiseExpressionsPropagateNullAndRejectInvalidBuilders` 已加入 `case.expr-core-bitwise`；Java `TestSuiteExprCore` 27/27 通过。Java EPL/SODA syntax、boxed/primitive 混合提升及完整 ExprCore type/diagnostic 矩阵仍保持 `expr.core` partial。
