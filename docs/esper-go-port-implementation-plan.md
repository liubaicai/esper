...
> 最新补充：Draft 3.45（2026-08-10），`ClientDeployUndeploy` 基础 2/11 已 mapped：unknown-ID rejection、active module-use dependency precondition、deterministic provider/dependent error 与 reverse-order chain removal 已落地。当前 241 mapped / 0 partial / 9 approved-difference。Maven 当前不可用，未执行 Java suite；本轮不需要 MySQL Docker。


## 1. 文档信息

当前切片（Draft 3.45，2026-08-10）开始对照 Java `ClientDeployUndeploy`。Go 在 `Deployment.Dependencies` 的 active module-use ID snapshot 上增加 undeploy precondition；provider 仍被 active dependent 引用时返回结构化 `UndeployPreconditionError`/`ErrorDependency`，不改变运行态。A/B/C/D chain 必须按 D→C→B→A 卸载。资源类别级依赖继续后续切片。

| 项目 | 内容 |
|---|---|
| 文档状态 | Draft 3.45，ClientDeployUndeploy 2/11 已 mapped；unknown ID 与 active deployment dependency precondition、deterministic provider/dependent details、failed-state isolation 和 reverse-order chain removal 已落地；manifest 当前 1,789 条关联、1,763/4,140 唯一 runtime（42.58%）、241 mapped / 0 partial / 9 approved-difference；下一步继续 variable/schema/context/expression/table/window/index/script/class resource dependency preconditions |
