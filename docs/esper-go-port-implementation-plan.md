...
> 最新补充：Draft 3.20（2026-08-09），`ClientInstrumentAuditDocSample` 与 `ClientInstrumentAuditAudit` 2/2 execution 已 mapped。新增 14 类 typed `AuditCategory`、`StatementAudit`/`DataflowBuilder.Audit`、immutable virtual-time `AuditRecord`、Engine-local removable subscription、事务锁外投递和 `%u/%d/%s/%i/%c/%m/%tutc/%tzone` formatter；STREAM callback 严格固定 runtimeTime=1、runtime/deployment/statement/category/message，PROPERTY/EXPRESSION/VIEW/PATTERN/SCHEDULE/INSERT/CONTEXT/DATAFLOW 路径均有 Go fluent 对照。当前 214 mapped / 0 partial / 6 approved-difference。Maven 当前不可用，未执行 Java suite；本轮不需要 MySQL Docker。
...
| 项目 | 内容 |
|---|---|
| 文档状态 | Draft 3.20，ClientInstrumentAudit 2/2 execution 与 MetricsReporting 6/6 execution 已 mapped；typed audit/formatter/lock-safe subscription 和 runtime/statement metrics management surface 已落地，下一步继续 protected deployment-local namespaces 或其他未映射 suite |
