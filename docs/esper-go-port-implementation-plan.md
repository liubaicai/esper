...
| 项目 | 内容 |
|---|---|
| 文档状态 | Draft 2.67，补充 Context 单源 FAF Table/Named Window equality/range candidate 物理执行验证及 `InfraUpdate` 的无 `where` 全量更新、Table 主键 re-key 对照；Go 已将单源 Table/Named Window 的完整 hash equality/IN candidate lookup、普通内连接及两流 outer optional side 的完整 equality/range candidate lookup、Context 单源 candidate lookup 与 B-tree ordered candidate path 接入 FAF，并覆盖 prepared named parameter、插入顺序、范围边界、反向操作数、复合 equality-prefix、source filter、重复 probe key、`JoinWhere`、preserved-side unmatched 行、Context partition selector、FullOuter/链式 outer/unidirectional/OR/UDF/Null/超大 probe 安全回退和变更后索引维护。Java `@Hint`/JVM query-plan hook 不复制为 Go API 外形；Context Join/subquery 物理路径、成本模型和性能阈值仍明确列为后续项 |
...
| 当前修订 | Draft 2.67（2026-08-08）：在 `InfraNWTableFAFResolve` module name-resolution 对照基础上，继续对照 `InfraNWTableFAFIndex` 的物理范围与 Join 执行，并补齐 `InfraUpdate` 的无 `where` 全量更新和 Table 主键 re-key。Go 使用 `IndexHash`/`IndexBTree` 声明索引、`UseIndex`/`UseIndexOn` 结构化 hint、`Plan.IndexPlan` 稳定逻辑摘要和 `NamedWindow.Lookup`；当前已验证单源 Table/Named Window 完整 equality/IN、普通内连接及两流 `LeftOuter`/`RightOuter` optional-side 的完整 equality/range candidate、Context 单源 Table/Named Window candidate、Context inner/两流 outer/相邻 mixed chain candidate、prepared named parameter、B-tree ordered candidate path、inclusive/exclusive 与反向范围、反向比较操作数、复合 equality-prefix + range、source filter、重复 probe key、`JoinWhere`、preserved-side unmatched 行、Context partition selector、FullOuter/链式 outer/unidirectional/OR/UDF/Null/超大 probe 回退和 update/delete/clear 后索引维护。`OnDemand().UpdateAll(...)` 现在覆盖 Named Window/Table 全量更新、ordered assignment、Initial*Field 和 Named Window/Table result contract；Table 更新允许主键变更并原位重建主键/secondary/B-tree index，冲突时保持原子不变。仍不把 FullOuter/right/full-preserving/non-adjacent chain/unidirectional、Context FAF subquery、Java 成本/选择性/performance 宣称为已完成。Java 的 `@Hint` 字符串、`SupportQueryPlanIndexHook` backing class 不作为 Go 契约；Java `TestSuiteInfraNWTable` 26/26 通过，本轮不需要 MySQL Docker。 |
