...
> 最新补充：Draft 2.97（2026-08-09），`EPLOtherSplitStream` 推进至 9/12 execution。分支 predicate/projection 现由既有 subquery registry 提供一致快照：lastevent scalar subquery 覆盖空值、相等/不等门控与 p00/p01 投影；`SubqueryGroupScalar[[]int,int]` 覆盖数组 multikey 的单组 sum 与多组 Null cardinality。新增 2 个 runtime 登记到 partial `case.epl-other-split-stream`；FromClause、SplitPremptiveNamedWindow、SingleInsert 三项仍待关闭。本轮不需要 MySQL Docker。
...
| 项目 | 内容 |
|---|---|
| 文档状态 | Draft 2.97，`EPLOtherSplitStream` 完成 9/12 execution；基础分流之外已覆盖 scalar 与数组 multikey grouped subquery，剩余 contained-from、context single-insert、preemptive named-window 三项继续迁移 |
