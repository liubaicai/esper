...
> 最新补充：Draft 3.46（2026-08-10），`ClientDeployUndeploy` 8/11 已 mapped：在 module-use 边之上补齐资源级依赖前置条件——Named Window/Table/变量/Context/事件类型/声明表达式的跨模块引用在部署时登记，`Engine.Undeploy` 按 Esper `Undeployer` 检查顺序返回携带阻塞 `DeploymentResource` 的结构化错误；`create index` 独立部署无 Go 对应记 approved representation difference。当前 242 mapped / 0 partial / 9 approved-difference。Maven 当前不可用，未执行 Java suite；本轮不需要 MySQL Docker。


## 1. 文档信息

当前切片（Draft 3.46，2026-08-10）继续对照 Java `ClientDeployUndeploy` 的资源级依赖前置条件。Go 在部署时遍历不可变 Plan 收集跨模块 catalog 引用并登记 dependent 边；provider 是其模块最早活跃 deployment 且对象仍被外部引用时，`Engine.Undeploy` 按 named window→table→variable→context→event type→declared expression 顺序返回 `UndeployPreconditionError`/`ErrorDependency`，两侧状态不变；dependent 边随 undeploy/rollout 回滚/Close 清理，Close 改为 force 卸载。script/index/class 资源依赖继续后续切片。

| 项目 | 内容 |
|---|---|
| 文档状态 | Draft 3.46，ClientDeployUndeploy 8/11 已 mapped；unknown ID、active module-use dependency precondition 与 Named Window/Table/变量/Context/事件类型/声明表达式资源级 undeploy 前置条件已落地；manifest 当前 1,795 条关联、1,769/4,140 唯一 runtime（42.73%）、242 mapped / 0 partial / 9 approved-difference；下一步继续 script/index/class resource dependency preconditions 与 `ClientDeployUndeploy` 收尾 |
