...
| 当前修订 | Draft 2.49（2026-08-07）：在 typed JSON 命名 nested fragment catalog 和 Variant 对照基础上，继续对照 Java `ExprEnumDataSources`。Go 新增 typed collection source 轨迹，覆盖 Prev/Sorted View 的访问顺序与淘汰、Named Window/Subquery 空集合语义、部署参数/变量 late binding、nested property schema、method-backed enum object、mapped property、collection cast、sorted access 和 Go-native generic component；并把已有 access aggregate/Table source 证据登记到对应 runtime。新增 1 个 capability case、15 个运行态 Java ID。manifest 进度为 1,241/4,136（约 30.00%），case 数为 183（178 mapped、1 partial、4 approved-difference）；本轮核心验证不需要 MySQL Docker。 |
...
- 当前 capability manifest 已关联 1,241/4,136 个唯一 Java runtime（约 30.00% 的 Java runtime 对账/处置进度）；183 个 case 中 178 个 mapped、1 个 partial、4 个 approved-difference。该比例不是 Java/Go 行为 parity 通过率，也不是全量移植完成度。
