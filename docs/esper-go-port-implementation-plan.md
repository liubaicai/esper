...
| `RowRecogPrev`、`RowRecogRegex`、`RowRecogVariantStream`、`RowRecogDataSet` | 已增加 Go 链式 `Prev`/`Abs`，并为每个 match-recognize 分区保存有限滚动 previous-access 快照；time window 淘汰后仍可按到达顺序取 PREV，已覆盖非分区、分区、双字段分区和 keep-all 场景。tag-aware PRIOR/PREV 变体、正则模式、Variant 输入、数据集/外部 fixture 的共享场景仍未映射 | `partial` |
