...
本轮继续对照 Java `PatternObserverTimerAt` 的 `PatternTimerAtSimple`、`PatternOp` 与 `PatternWMilliseconds`：Go 新增链式 `TimerAtSchedule`，复用类型安全的 `CronSchedule` 解析 wildcard、range、step、秒和毫秒字段，在普通 Pattern 与 Pattern Context 中从部署/分区启动时计算严格下一次日历 occurrence，并在首次触发后清空 observer 状态，保证跨日跳时也不会重复创建。`TestPatternTimerAtScheduleEmitsNextCalendarOccurrenceOnly`、`TestPatternTimerAtScheduleSupportsStepAndMilliseconds` 和 `TestPatternTimerAtScheduleContextCreatesOnePartition` 覆盖 just-before/exact 边界、单次终止、分钟 step、毫秒精度和 context 分区生命周期；对应 runtime ID、Go 测试和差异已补入 `case.pattern-timer-cron`。Java 的微秒/ISO period、timezone/property/SODA、特殊日历运算符、动态重配置及完整 observer/consumption 组合仍未完成，不能将该切片视为 Pattern timer 全量 parity。
