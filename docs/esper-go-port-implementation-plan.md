...
> 最新补充：Draft 3.39（2026-08-09），`ClientCompileLargeWConfig` 最后 3 个 Table aggregation execution 已 mapped，suite 达到 12/12。覆盖 1,000-column fluent aggregate reset、LengthWindow(1) sum enter/leave 与 WindowAccessBy enter/leave；当前 234 mapped / 0 partial / 7 approved-difference。Maven 当前不可用，未执行 Java suite；本轮不需要 MySQL Docker。


## 1. 文档信息

当前切片（Draft 3.39，2026-08-09）对照 Java `ClientCompileLargeTableAggregationReset`、`ClientCompileLargeTableAggregationEnterLeaveMethod` 与 `ClientCompileLargeTableAggregationEnterLeaveAccessAgg`。Go 新增 `ResetTableAggregates(table, columns...)` fluent trigger，分别以完整 1,000-column list 和空 list 表达逐列/整行 reset；reset 后所有 nullable sum 列为 null，下一事件重新起算。另两条在 `LengthWindow(1)` 上固定 1,000 个 sum 与 1,000 个 WindowAccessBy 列的 enter/leave replacement。`ClientCompileLargeWConfig` 现已 12/12 mapped。

| 项目 | 内容 |
|---|---|
| 文档状态 | Draft 3.39，ClientCompileLargeWConfig 12/12 execution 已 mapped；1,000-column Table aggregate reset、sum enter/leave 与 window-access enter/leave 已落地，manifest 当前 1,768 条关联、1,742/4,140 唯一 runtime（42.08%）、234 mapped / 0 partial / 7 approved-difference；下一步继续 Java compiler/runtime inventory 的其他未映射 execution |
