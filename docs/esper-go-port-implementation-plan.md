...
> 最新补充：Draft 3.16（2026-08-09），`ClientMultitenancyInsertInto` 2/2 与 `ClientMultitenancyIndexTable` 1/1 execution 已处置。`DeployPlans`/独立 Deploy 表达单 module 与 compiler-path module，typed public derived stream 保持过滤、晚部署和 lifecycle；Table composite key + secondary equality index Join 固定 A/B→1/2。Java module text/access callback/JVM hook 不复制，continuous Table Join 当前预装载 passive snapshot；protected deployment-local namespaces 留待下一切片。当前 211 mapped / 0 partial / 5 approved-difference。Maven 当前不可用，未执行 Java suite；本轮不需要 MySQL Docker。
...
| 项目 | 内容 |
|---|---|
| 文档状态 | Draft 3.16，client multitenancy insert/index 3/3 execution 已 mapped；typed module routing、late consumer 与 indexed Table Join 已固定，下一步处理 protected deployment-local namespaces或 client instrument 域 |
