...
> 最新补充：Draft 2.99（2026-08-09），`EPLOtherSplitStream` 推进至 11/12 execution。`SplitInto` target 现可直接指向 Named Window，窗口 branch 在同一引擎事务内 preemptive insert，普通 stream branch 延后到 routed queue，因此后声明的 `WinTwo` 对前声明 `OtherStream` 的级联 on-select 已可见；ObjectArray/Map/Avro/JSON/typed JSON 表示矩阵均通过。新增 1 个 runtime 登记到 partial `case.epl-other-split-stream`；仅 SingleInsert 仍待关闭。本轮不需要 MySQL Docker。
...
| 项目 | 内容 |
|---|---|
| 文档状态 | Draft 2.99，`EPLOtherSplitStream` 完成 11/12 execution；基础分流、subquery、contained-event branch source 与 preemptive Named Window 已覆盖，仅剩 context single-insert |
