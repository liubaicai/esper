...
> 最新补充：Draft 3.51（2026-08-10），View domain 扩展：ViewFirstUnique suite 2/2 关闭（firstunique 唯一键首事件+Snapshot 迭代器+order by），ViewKeepAll suite 3/3 关闭（Iterator 有序迭代、WindowStats 分组聚合 IR-pair）；引擎修复 group-by irstream 组创建 null-prior old 行语义（聚合列对空组求值：count(*)→0、sum/avg/min/max→null），对齐 Java ViewKeepAllWindowStats 与 ResultSetAggregateMaxMinGroupBy。当前 270 mapped / 1 partial / 17 approved-difference。本轮不需要 MySQL Docker。
...
| 项目 | 内容 |
|---|---|
| 文档状态 | Draft 3.51，Client domain 100% 关闭（281/281）；View domain 已处置 9 个 runtime（KeepAll 3/3、FirstUnique 2/2、LengthWindow/FirstEvent/LastEvent/FirstLength 各 1），ExprFilter 23、ExprEnum 27、Pattern 3、ResultSet 2、EPLOther 2、EPLInsertInto 1；group-by irstream null-prior 聚合空组求值语义已对齐 Java；manifest 当前 1,922 条关联、1,893/4,140 唯一 runtime（45.72%）、270 mapped / 1 partial / 17 approved-difference；下一步 View 剩余 suite（LengthWindow Iterator/PrevPrior、FirstEvent/LastEvent/FirstLength MarketData、TimeWindow/TimeBatch、Sort）与 ExprFilter 剩余 suite |
