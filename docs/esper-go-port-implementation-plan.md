...
> 最新补充：Draft 3.68（2026-08-11），PatternObserverTimerSchedule suite 7/7 与 PatternObserverTimerScheduleTimeZoneEST 1/1 关闭（7 个新增 runtime ID，DateWithPeriod 此前已由 case.pattern-timer-cron 关联）：timer:schedule 全部 ISO-8601 形式（一次性日期、P 周期、R/Rn 循环、R/date/period 全形式含 include-start）与 named 参数形式（StartAt/calendar+fixed Period/Repetitions -1 无限/IncludeStart）逐场景回放，过去日期锚定跳过已到期次数、三种等价写法一致、followed-by 动态 ISO 逐臂求值、GMT-4 时区计算日期精确触发；typed spec 零 Period+StartAt 走一次性日期路径（对齐 Java date-only），零 Period 无 StartAt 构建期拒绝。当前 287 mapped / 1 partial / 17 approved-difference。本轮不需要 MySQL Docker。
>
...
当前切片（Draft 3.50，2026-08-10）关闭 Java `ExprFilterWhereClauseSimple`（WHERE symbol 过滤等价 LengthWindow+Filter）与 `ExprFilterWhereClauseNumericType`（int/long/float/double 跨类型 EqualOf coercion 与 Cast 算术投影 p1/p2/p3）2 个 execution；同一 Go struct 多类型名注册时改用 `engine.Send(ctx, "SupportBean", event)` 显式路由对齐 Java sendEventBean。同时修复 manifest 完整性：补齐 18 个既有 mapped/partial case 缺失的 capability mapping（共 19 条），`TestCapabilityManifestArtifactValidates` 恢复通过。下一步继续 ExprFilter 剩余 suite（In dynamic/Optimizable/PatternFunc 等）与 View/Pattern 基础 suite。

| 项目 | 内容 |
|---|---|
| 文档状态 | Draft 3.68，Client domain 100% 关闭（281/281）；View domain 22 个唯一 runtime，Pattern domain 75 个（And/Or 7、EveryDistinct 17/17、MatchUntil 9/9、ObserverTimerAt 10/10、ObserverTimerInterval 8/8、ObserverTimerSchedule 7/7+TZ 1/1、ConsumingPattern 10/10、CompositeSelect 2/2、Every 6/6、Not 6/6、FollowedBy 10/10）；ExprFilter 23、ExprEnum 27、ResultSet 2、EPLOther 2、EPLInsertInto 1；manifest 当前 2,011 条关联、1,975/4,140 唯一 runtime（47.71%）、287 mapped / 1 partial / 17 approved-difference；下一步 Pattern 基础 suite（GuardWhile、ComplexPropertyAccess 批量）与 View Expression/Accum 系列、ExprFilter 剩余 |
