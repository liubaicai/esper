...
> 最新补充：Draft 3.21（2026-08-09），`ClientMultitenancyProtected` 6/6 execution 已 mapped。新增 `ProtectedModule()`、`Module.Build`、module-qualified schema/variable/context/expression helper、module-bound Table/Named Window trigger 与 deployment-local runtime materialization/cleanup；A/B 同名 infra、timer variable、initiated context、event type aggregate 和 declared expression 均按 Java 固定轨迹覆盖，同一 protected module 重复活动部署被拒绝，undeploy 后可用 immutable compile artifact 重新部署。当前 215 mapped / 0 partial / 6 approved-difference。Maven 当前不可用，未执行 Java suite；本轮不需要 MySQL Docker。
...
| 项目 | 内容 |
|---|---|
| 文档状态 | Draft 3.21，ClientMultitenancyProtected 6/6 execution 已 mapped；typed protected module identity、deployment-local runtime catalog 与 undeploy cleanup 已落地，下一步继续 public/protected uses/import dependency graph、ambiguity diagnostics 或其他未映射 suite |
