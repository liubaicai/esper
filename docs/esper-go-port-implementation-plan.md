...
> 最新补充：Draft 3.31（2026-08-09），`ClientCompileSyntaxValidate` 2/2 execution 已 mapped。新增 catalog-independent `Environment.ValidateSyntax`，未知 event type 可先通过 typed AST 结构校验，再由普通 Build 处理 semantic resolution；zero/empty model 与可构造 invalid builder state 均有稳定诊断。当前 226 mapped / 0 partial / 7 approved-difference。Maven 当前不可用，未执行 Java suite；本轮不需要 MySQL Docker。


## 1. 文档信息

当前切片（Draft 3.31，2026-08-09）对照 Java `ClientCompileSyntaxValidate`。`Environment.ValidateSyntax(queries ...Query)` 对零个或多个 fluent Query 执行只读 structural pass，不查询 Environment schema/variable/context/module catalog；因此 unknown event type 的语法结构合法，只有普通 `Build` 执行 semantic resolution。验证覆盖 query/source ownership、source-less select、statement/context/module 名称、limit/offset、output policy、statement metadata、stream/filter/window/pattern/contained/derived/join/aggregate source shape、selection alias/nil expression 以及全部 expression constructor configurationError traversal。Java 13 条 lexer/parser message 在 Go 中由 typed builder grammar 消除；raw EPL、line/column、reserved-word/token/quote diagnostics 不复制。

| 项目 | 内容 |
|---|---|
| 文档状态 | Draft 3.31，ClientCompileSyntaxValidate 2/2 execution 已 mapped；catalog-independent typed AST validation、unknown-name deferral、zero/empty model 与 invalid builder diagnostics 已落地，manifest 当前 1,747 条关联、1,721/4,140 唯一 runtime（41.57%）、226 mapped / 0 partial / 7 approved-difference；下一步继续 statement object model、compile exception items 或其他未映射 suite |
