...
> 最新补充：Draft 3.63（2026-08-10），PatternOperatorEveryDistinct suite 17/17 关闭（15 个新增 runtime ID，TimerWithinOverDistinct/EveryDistinctOverTimerWithin 此前已关联）：EveryDistinct 复合根（and/or/sequence/match-until/until/not/within）物化为 everyNode 携带独立 keyset（裸过滤器根保持定义级），every 证伪重启以空 keyset spawn（对齐 EvalEveryDistinctStateNode.evaluateFalse），match-until 仅在子状态永久完成时替换子节点以保留存活 every-distinct 连同 keyset，定义级 distinct key 求值补齐 tag 上下文；新增 EveryDistinctForCalendar 月作用域 calendar 到期与可变参数 EveryDistinct/(expiry, keys...) EveryDistinctFor，非常量 key 构建期拒绝。当前 282 mapped / 1 partial / 17 approved-difference。本轮不需要 MySQL Docker。
>
...
| 项目 | 内容 |
|---|---|
| 文档状态 | Draft 3.63，Client domain 100% 关闭（281/281）；View domain 22 个唯一 runtime，Pattern domain 46 个（And/Or 7、EveryDistinct 17/17、CompositeSelect 2/2、Every 6/6、Not 6/6、FollowedBy 10/10）；ExprFilter 23、ExprEnum 27、ResultSet 2、EPLOther 2、EPLInsertInto 1；manifest 当前 1,982 条关联、1,947/4,140 唯一 runtime（47.03%）、282 mapped / 1 partial / 17 approved-difference；下一步 Pattern 基础 suite（MatchUntil/ConsumingPattern/Observer 批量）与 View Expression/Accum 系列、ExprFilter 剩余 |
