...
> 最新补充：Draft 3.26（2026-08-09），`ClientCompileEnginePath` 4/4 execution 已 mapped。新增 active public deployment 导出的 immutable `Engine.RuntimePath()`，typed catalog resolver 与 runtime Build 保留 sorted module/deployment identity；Table/Named Window index、preconfigured event type aggregate into Table 与 public bus Named Window 使用均已固定 Java 对照结果。当前 221 mapped / 0 partial / 7 approved-difference。Maven 当前不可用，未执行 Java suite；本轮不需要 MySQL Docker。


## 1. 文档信息

当前切片（Draft 3.26，2026-08-09）对照 Java `ClientCompileEnginePath`。Go 用 `Engine.RuntimePath()` 从当前 active deployment 生成 immutable compiler-path snapshot，只导出 public module，保留 preconfigured global catalog，并附带排序后的 module/deployment identity；inactive/private/protected module 不进入路径，undeploy 后 fresh snapshot 移除导出，旧 snapshot 仍保持原 selected module identity。`RuntimePath` 复用 typed `ModulePath` resolver，并提供 Build、variable/expression/script ref helper；Java object-type matrix、Table/Named Window unique index、preconfigured `SupportBean` aggregate into module Table 与 public bus Named Window insert/aggregate 均有运行轨迹。Go 将 index 作为 immutable definition metadata，不部署 textual create-index statement，也不保留 EPCompiled bytecode；path cache、compiler SPI、portable artifact 与 compiler syntax/diagnostic 继续作为后续切片。

| 项目 | 内容 |
|---|---|
| 文档状态 | Draft 3.26，ClientCompileEnginePath 4/4 execution 已 mapped；active public runtime path、immutable snapshot identity、module-qualified infra lazy materialization 与 typed Build 已落地，manifest 当前 1,736 条关联、1,710/4,140 唯一 runtime（41.30%）、221 mapped / 0 partial / 7 approved-difference；下一步继续 compile path cache/SPI/syntax 或其他未映射 suite |
