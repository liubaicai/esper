...
> 最新补充：Draft 3.38（2026-08-09），`ClientCompileLargeWConfig` method/access aggregation 2/12 execution 已 mapped。覆盖 LastEvent 上 5,000 个 Sum/Add projection，以及 KeepAll 上 1,000 个独立 SortedAccessBy 状态、2,000 个 first-event/sorted-collection result columns；suite 累计关闭 9/12。当前 233 mapped / 0 partial / 7 approved-difference。Maven 当前不可用，未执行 Java suite；本轮不需要 MySQL Docker。


## 1. 文档信息

当前切片（Draft 3.38，2026-08-09）对照 Java `ClientCompileLargeWConfig.ClientCompileLargeAggregation` 与 `ClientCompileLargeAggregationAccess` 的 2 个 execution。Go 用 `Sum[int](Add[int](Field, Literal))` 在 `LastEvent` 上构造 5,000 列并固定两轮输入的全部结果；access 路径用 1,000 个 `SortedAccessBy` 在 `KeepAll` 上同时输出 `FirstEvent` 和 `Sorted`，固定 2,000 列 metadata/value。suite 只剩 3 个 Table aggregation execution。

| 项目 | 内容 |
|---|---|
| 文档状态 | Draft 3.38，ClientCompileLargeWConfig 累计 9/12 execution 已 mapped；5,000-column method aggregation 与 2,000-column sorted access aggregation 已落地，manifest 当前 1,765 条关联、1,739/4,140 唯一 runtime（42.00%）、233 mapped / 0 partial / 7 approved-difference；下一步继续剩余 3 个 large Table aggregation execution |
