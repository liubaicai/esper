...
- 当前 capability manifest 已建立 1,373 条 runtime 关联、覆盖 1,360/4,136 个唯一 Java runtime（约 32.88% 的 Java runtime 对账/处置进度）；191 个 case 中 185 个 mapped、2 个 partial、4 个 approved-difference。该比例不是 Java/Go 行为 parity 通过率，也不是全量移植完成度。
...
| `multithread` | 56 / 0 / 56 | 合计 | 4,136 / 1,360 / 2,776 |
