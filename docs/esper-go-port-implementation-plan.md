...
| 项目 | 内容 |
|---|---|
| 文档状态 | Draft 2.68，补充 Context Table 持久 row-to-partition ownership：Table 行拥有稳定内部 identity，Context FAF 首次观察时绑定原始 partition key/properties，后续更新分区字段或主键 re-key 仍按原 partition selector mutation，删除与 DestroyContext 清理归属。此前已完成的单源 Table/Named Window equality/range candidate、普通内连接及两流 outer optional side、Context candidate 与 B-tree ordered candidate，以及 `InfraUpdate` 全量更新/re-key 对照继续有效；普通 FAF mutation 的跨分区整体 rollback、完整 Context Table live-insert ownership 和更广 transaction/concurrency trace 仍列为后续项。Java `@Hint`/JVM query-plan hook 不复制为 Go API 外形；Context Join/subquery 物理路径、成本模型和性能阈值仍明确列为后续项 |
...
| 当前修订 | Draft 2.68（2026-08-08）：继续对照 `InfraNWTableFAF` 的 Context mutation 语义，修复 Context Table 在 FAF 更新分区字段与主键后的持久分区归属。`TableRow` 现在保留稳定内部 identity；Engine 按 table/context/row identity 保存 partition key、代表事件和 selector properties，`Table.Update`/Upsert re-key 保持 identity，Context FAF update/delete/delete-all 复用原归属，真正删除和 `DestroyContext` 清理映射。`TestOnDemandContextPartitionUpdateAndDeleteAllRespectSelector` 覆盖 Table/Named Window、category selector、分区字段变化、主键 re-key、原 partition delete-all 和剩余分区清理。此前的单源 Table/Named Window equality/IN、普通内连接及两流 outer optional side、Context candidate 与 B-tree ordered candidate、`OnDemand().UpdateAll(...)`/Table re-key 对照保持有效；普通 FAF mutation 的跨分区整体 rollback、完整 Context Table live-insert ownership 和更广 transaction/concurrency trace 仍列后续项。Java `@Hint`/`SupportQueryPlanIndexHook` 不作为 Go 契约；Java `TestSuiteInfraNWTable` 26/26 通过，本轮不需要 MySQL Docker。 |
