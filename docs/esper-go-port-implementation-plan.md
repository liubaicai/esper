...
| 项目 | 内容 |
|---|---|
| 文档状态 | Draft 2.51，补充 Java `InfraNWTableFAF.InfraDeleteContextPartitioned` 的 Context selector 与分区内 on-demand mutation 对照；修正 ungrouped row-for-event 聚合语义；Variant 的单列 method-backed member conversion 与 Java application/type-class metadata 仍未关闭；其余历史增量与未完成项见下文 |
...
| 当前修订 | Draft 2.51（2026-08-07）：继续对照 Java `InfraNWTableFAF`，新增 `FromNamedWindow`/`FromTable` 的 Go-native `OnDemand().Insert/UpdateWhere/DeleteWhere/DeleteAll`，覆盖 Named Window/Table 的 wildcard/filter/distinct/count/sum/group/in/join/join-where、Table 参数绑定、Named Window delta dispatch 和 Table/Named Window FAF mutation result contract；同时修正 ungrouped aggregate 的 row-for-event 快照/FAF 结果，使每个源事件都返回共享聚合值。本轮再关闭 `InfraDeleteContextPartitioned{namedWindow=true/false}`：OnDemand 支持 `WithContext`，按 hash/category selector 将 delete/update/delete-all 限定到选中分区，Table 扫描按 context partition 过滤，Named Window 保留 old-row 返回和 consumer delta dispatch。Java `TestSuiteInfraNWTable` 26/26 通过，新增 2 个运行态 Java ID。manifest 进度为 1,271/4,136（约 30.73%），case 数为 184（179 mapped、1 partial、4 approved-difference）；本轮核心验证不需要 MySQL Docker。 |
...
- 当前 capability manifest 已关联 1,269/4,136 个唯一 Java runtime（约 30.69% 的 Java runtime 对账/处置进度）；184 个 case 中 179 个 mapped、1 个 partial、4 个 approved-difference。该比例不是 Java/Go 行为 parity 通过率，也不是全量移植完成度。
...
- 变量的配置、读写、原子更新、上下文变量。
- Fire-and-Forget select/insert/update/delete、准备查询和参数化查询；Go 以 `FromNamedWindow`/`FromTable` 的 `OnDemand()` 链表达目标侧 mutation，返回值必须区分 Named Window old/new delta 与 Table 空结果，并保持同一 engine 事务边界；`WithContext` 与 `ExecuteFireAndForgetWithSelector` 还必须将 update/delete/delete-all 限定到选中的 Context partition。
- 依赖关系、作用域、部署卸载前置条件和资源回收。
- Table 聚合重置、FAF multi-row insert、named/indexed 参数绑定，以及 prepared query 的幂等关闭；还必须覆盖更宽的 Context selector/FAF 生命周期、index/range/in 计划选择、representation conversion、invalid diagnostics、rollback 和并发一致快照。当前 `query.fire-and-forget` 已关闭基础 Named Window/Table snapshot、row-for-event aggregate、join、on-demand 单行 mutation 和 `InfraDeleteContextPartitioned` 的 hash/category partition-local delete 子集；Go-native update/delete-all 已有回归，但 Table row-to-context ownership 仍未持久化。
