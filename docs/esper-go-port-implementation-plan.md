...
| `RowRecogAfter` | 已有三种 skip builder、固定 duration `Interval` 和 `Statement.Snapshot`；interval 后的完整 skip/after 生命周期和 Java iterator 语义仍未对照 | `partial` |
...
| `RowRecogPrev`、`RowRecogRegex`、`RowRecogVariantStream`、`RowRecogDataSet` | 已增加 Go 链式 `Prev`/`Abs`，并为每个 match-recognize 分区保存有限滚动 previous-access 快照；time window 淘汰后仍可按到达顺序取 PREV，已覆盖非分区、分区、双字段分区和 keep-all 场景。新增 `TestRowRecogDataSetFinancialPattern` 对照 Java `A B C* D E* F+`、DEFINE 中的普通 PREV/tag 引用、三段 E7/E8/E9 listener trace 和 `SKIP TO CURRENT ROW` 重叠分支；新增 `TestRowRecogAfterCurrentRowKeepsTheRecognizingBranch` 对照 `RowRecogAfter` 的 A B* continuation；新增 `TestRowRecogRegexMatrix` 对照 Java `RowRecogRegex` 全部 12 组模式，并同时检查 listener 与 `Statement.Snapshot`；新增 `TestRowRecogVariantStreamPreservesMemberType` 对照 PREDEFINED Variant 的 typed member route、concrete type predicate、listener 和 Snapshot，另有非法成员/source/direct-payload 负例。完整 DataSet/外部 fixture trace、Variant/multikey-array 高级矩阵和高级 tag-aware PRIOR/PREV 变体仍未映射 | `partial` |
...
本轮继续对照 Java `RowRecogArrayAccess.RowRecogLambda`、`RowRecogClausePresence`、`RowRecogDataSet` 和 `RowRecogRegex`：Go 在链式 Match Recognize 表达式层新增 `TagSum`、`TagAvg`、`TagMin`、`TagMax`、`TagFirst`、`TagLast`、`TagAny`、`TagAll`、`TagEvents` 和 `TagSize`。这些构造器把 tag 名与元素表达式保留在 AST 中，在 DEFINE 候选分支和 MEASURES 结果阶段按重复捕获顺序求值；可选 tag 的聚合返回 Null、`TagSize` 返回 0、`TagAll` 遵循空集合为 true，未知 tag 在 Build 阶段报 `ErrorUnknownName`。`TestRowRecogTagAggregatesAndEnumeration` 覆盖 A* B 的重复/可选捕获、数值聚合、枚举 any/all、事件数组复制、边界值和未知 tag 负例；`TestRowRecogRegexMatrix` 现在覆盖 RowRecogRegex 全部 12 组组合模式及 listener/Snapshot 结果。Java `TestSuiteRowRecog` 本轮 Maven 运行 23/23 通过。RowRecog 的完整差异仍包括 AFTER MATCH SKIP 全矩阵、time/time-batch row window、engine-wide max-states/prevent-start、variant/multikey array、复杂嵌套 NFA、iterate-only、invalid diagnostics、tag-aware PREV/PRIOR、性能及共享 Java/Go trace；因此清单中的 `case.rowrecog-inventory` 只提升为 partial，不视为完整回归覆盖。
