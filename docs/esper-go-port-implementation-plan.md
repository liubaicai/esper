...
> 最新补充：Draft 3.84（2026-08-12），关闭 ExprFilterExpressions 的 5 个剩余可观测 pattern/enum execution：EnumSyntaxOne/Two、PatternFunc、PatternFunc3Stream、PatternWithExpr。新增 `BetweenRangeOf`/`NotBetweenRangeOf` fluent 开闭区间构造器和 `expr_filter_pattern_enum_parity_test.go`，覆盖 33 个 PatternFunc 关联谓词、三流 null-safe/OR/IN/range 矩阵、typed enum pattern/where 与 every pattern arithmetic correlation。Java outer-class 静态 ID 使用 `javaStaticIds`，Go self-qualified 当前事件字段使用当前 `Field`；其余 EPL/class-literal 语法由 typed API 差异记录。Coverage 2,072/4,140（50.05%），329 case 中 311 mapped / 1 partial / 17 approved-difference。下一步 ExprFilter PromoteIndexToSetNotIn、ShortCircuitEvalAndOverspecified、RewriteWhere、Invalid 及 ViewGroup/ViewIntersect 剩余矩阵。本轮不需要 MySQL Docker。
>
...
当前切片（Draft 3.84，2026-08-12）关闭 ExprFilterExpressions 的 5 个 pattern/enum execution：typed enum pattern/where、PatternFunc 关联谓词、PatternFunc3Stream 三流矩阵与 PatternWithExpr 算术关联。新增 `BetweenRangeOf`/`NotBetweenRangeOf` 表达四种开闭区间，并登记 4 个 mapped case。下一步 ExprFilter 剩余四项与 ViewGroup/ViewIntersect 剩余矩阵。

| 项目 | 内容 |
|---|---|
| 文档状态 | Draft 3.84，Client domain 100% 关闭（281/281）；View domain 88 个唯一 runtime（TimeAccum 10/10、TimeLengthBatch 9/9、FirstTime 3/3、ExternallyTimedWin 5/5、ExternallyTimedBatched 5/5、ExpressionWindow 13/13、ExpressionBatch 13/13、LengthBatch 3/8、Union 1 及既有 KeepAll/Length/First/Last/Time/TimeBatch/Sort/FirstUnique/MarketData 子集），Pattern domain 107 个；ExprFilter 28、ExprEnum 27、ResultSet 2、EPLOther 2、EPLInsertInto 1；manifest 当前 2,072 条关联、2,072/4,140 唯一 runtime（50.05%）、311 mapped / 1 partial / 17 approved-difference；下一步 ExprFilter 剩余四项与 ViewGroup/ViewIntersect 剩余矩阵 |
