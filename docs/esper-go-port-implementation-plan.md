...
> 最新补充：Draft 3.37（2026-08-09），`ClientCompileLargeWConfig` create-schema-and-insert 3/12 execution 已 mapped。覆盖 reversed 5,000-field MAP/ObjectArray schema、CurrentTimestamp 虚拟时间、int→int64 widening、全部值与 ObjectArray 物理顺序；suite 累计关闭 7/12。当前 232 mapped / 0 partial / 7 approved-difference。Maven 当前不可用，未执行 Java suite；本轮不需要 MySQL Docker。


## 1. 文档信息

当前切片（Draft 3.37，2026-08-09）对照 Java `ClientCompileLargeWConfig.ClientCompileLargeCreateSchemaAndInsert` 的 3 个 execution。Go 按 `p4999`…`p0` 注册 reversed 5,000-field MAP/ObjectArray schema，所有字段均为 `int64`；non-widening 用 `AddOf[int64](Literal(index), CurrentTimestamp())` 并把 Engine 虚拟时间推进到 epoch-millis 1,000,000，widening 用 int literal + int64 literal。typed producer route 后逐项固定 `pN=N+1,000,000`，ObjectArray underlying 物理首尾继续反映 reversed schema order。suite 只剩 5 个 large aggregation/table execution。

| 项目 | 内容 |
|---|---|
| 文档状态 | Draft 3.37，ClientCompileLargeWConfig 累计 7/12 execution 已 mapped；reversed 5,000-field schema、virtual timestamp、numeric widening、MAP/ObjectArray insert representation 已落地，manifest 当前 1,763 条关联、1,737/4,140 唯一 runtime（41.96%）、232 mapped / 0 partial / 7 approved-difference；下一步继续剩余 large aggregation/table 5 个 execution |
