...
> 最新补充：Draft 2.96（2026-08-09），启动 `EPLOtherSplitStream` 并完成 7/12 execution。新增 `SplitInto`/`SplitIntoWhen` 与 `OnEvent(...).SplitFirst(...)`/`SplitAll(...)` 类型化链式 API，覆盖 output-first/output-all、首命中终止、全命中投递、无命中原事件 fallback、wildcard/具名 projection、多目标 routed-event 队列、Plan identity 与 invalid Build 门禁；7 个 runtime 已登记到 partial `case.epl-other-split-stream`。FromClause、SplitPremptiveNamedWindow、Subquery、SubqueryMultikeyWArray、SingleInsert 五个 execution 仍为下一切片。本轮不需要 MySQL Docker。
...
| 项目 | 内容 |
|---|---|
| 文档状态 | Draft 2.96，`EPLOtherSplitStream` 完成 7/12 execution；Go 链式 `SplitFirst`/`SplitAll` 覆盖基础分流、fallback、wildcard/投影、Plan identity 与 invalid，剩余 contained-from/subquery/context/named-window 五项继续迁移 |
