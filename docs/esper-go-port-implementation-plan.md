...
| 项目 | 内容 |
|---|---|
| 文档状态 | Draft 2.54，补充 Java `InfraNWTableFAFSubquery` 的 Named Window/Table scalar、join、mutation、correlation、Context、where/group-by 与 invalid 对照；同时修复 FAF 目标行向 `OuterField` 的相关子查询作用域传递。Java 性能/index execution 暂不宣称 timing parity；其余历史增量与未完成项见下文 |
...
| 当前修订 | Draft 2.54（2026-08-07）：在 `InfraNWTableFAF` 的只读/单行/Context mutation/多行 insert 与 `InfraNWTableFAFSubstitutionParams` 的 typed named/positional PreparedQuery 对照基础上，新增 `faf_subquery_parity_test.go` 对照 `InfraNWTableFAFSubquery` 的前 15 个功能 execution。Go 使用 `SubqueryValue`/`SubqueryGroupRows`、`OuterField`、`OnDemand()` 和 `WithContext` 覆盖 scalar、join、insert、未相关/相关 select-update-delete、Context 两窗口/全局窗口、inner where/group-by；FAF 执行时拒绝 event-stream source、source filter、context mismatch。修复 on-demand target row 的 `OuterEvent` 传递和子查询空入站事件回退。4 个 Java performance/index execution 保留为 partial，不把耗时阈值作为行为 parity。Java SODA 路径以 immutable fluent Plan 对应，EPL 保留关键字解析和精确 Java 诊断不作为 Go API 契约；本轮不需要 MySQL Docker。 |
...
- 当前 capability manifest 已关联 1,303/4,136 个唯一 Java runtime（约 31.50% 的 Java runtime 对账/处置进度）；186 个 case 中 180 个 mapped、2 个 partial、4 个 approved-difference。该比例不是 Java/Go 行为 parity 通过率，也不是全量移植完成度。
...
- Named Window/Table 子查询和索引复用；当前已补齐 `InfraNWTableFAFSubquery` 的 scalar/join/mutation/context/where/group-by 功能切片，性能/index execution 与计划复用仍待完成。
