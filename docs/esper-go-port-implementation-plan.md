...
> 最新补充：Draft 3.01（2026-08-09），开始补齐长期遗漏的 client 管理面，`client/basic` 7/7 direct execution 已登记并映射。typed fluent 对照覆盖 CountAll、statement name metadata、字段 Filter、LengthWindow IR 与对象 identity、wildcard/具名 Select，以及虚拟时钟 one-shot timer；Java annotation-array 反射与 EPL 文本由 Go 显式 QueryOption/builder 取代。新增 7 个 runtime 和 `client.basic` capability；当前 196 mapped / 0 partial / 4 approved-difference。本轮不需要 MySQL Docker。
...
| 项目 | 内容 |
|---|---|
| 文档状态 | Draft 3.01，`client/basic` 7/7 direct execution 已映射；此前 `EPLOtherSplitStream` 12/12 保持 mapped，下一步继续 client compile/deploy/runtime 管理面 |
