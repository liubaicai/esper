...
> 最新补充：Draft 2.85（2026-08-08），继续补齐方法源 N-stream 域：对照 Java `EPLFromClauseMethodNStream`，新增 5 个 Go 链式对照测试，覆盖 1 stream + 2 method-historical 的 star-subordinate CartesianLast（`TestFromClauseMethodOneStreamTwoHistStarSubordinateCartesianLastParity`）、forward subordinate（`TestFromClauseMethodOneStreamTwoHistForwardSubordinateParity`），1 stream + 3 method-historical 的 CartesianLast（`TestFromClauseMethodOneStreamThreeHistStarSubordinateCartesianLastParity`）、forward subordinate（`TestFromClauseMethodOneStreamThreeHistForwardSubordinateParity`）与 chain subordinate（`TestFromClauseMethodOneStreamThreeHistChainSubordinateParity`）。Go 以 `JoinMany` + `OnSourcesCompare`/`OnSourcesEqual` + `DependingOn` 表达依赖拓扑，`KeepAll`/`LastEvent` 表达窗口保留；针对 Go join 在 keepall 窗口上每次新事件会输出全量保留状态的实现特点，`fcmMakeSender` 按发送事件 `id` 过滤 listener 行，保留 Java `assertPropsPerRowLastNew` 的每事件断言语义。新增 8 个 Java runtime（`java-runtime-3be00366216aea3c967d`、`java-runtime-f6b78c563ee22b6bc71d`、`java-runtime-b2283f546f03c3d53930`、`java-runtime-40574a2ed45c7fbf33d3`、`java-runtime-c3578b44719efaa0b38b`、`java-runtime-51307ad5fea63359ec49`、`java-runtime-9cc6163d0e611f7d3020`、`java-runtime-93978f529b48869200a6`）已登记到 `case.epl-fromclausemethod-basic` 与 capability `epl.fromclausemethod`。`go vet ./...`、定向 Go 测试、`go test ./compat -count=1` 均通过；Java `TestSuiteEPLFromClauseMethod` 保持 3/3 通过。`EPLFromClauseMethodNStream` 其余 2-stream/3-stream/NW-twice、pure-no-subordinate、outer joins、CacheLRU/CacheExpiry、JoinPerformance、Variable、MultikeyWArray 与完整 Java 方法源诊断仍为后续切片。数据库/方法源 runtime 覆盖审计、MySQL/Maven 固定门禁与当前实测 disposition 仍见第 17.5 节。
