...
本轮验证：Java 使用 JDK 17/Maven 3.9.11 执行 `mvn -pl regression-run '-Dtest=TestSuiteInfraNWTable' '-DfailIfNoTests=false' '-Dgpg.skip=true' test`，26/26 通过；Go 的 `go test ./...`、`go vet ./...`、`go test ./compat` 与 `go test -race -run '^TestTriggerInnerTypeAndVariableBranchesMatchInfraInnerTypeAndVariable$' .` 均通过，manifest 当前为 150 cases/150 mappings 且 runtime/case/test 引用无缺失。本轮核心与对照测试均不需要启动 MySQL。
