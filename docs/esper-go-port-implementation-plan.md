...
> 最新补充：Draft 3.13（2026-08-09），`ClientRuntimeStatementName` 4/4 execution 已处置。新增 `Engine.DeployPlans` 与 deployment-time typed name resolver；未命名 Plan 稳定分配 `stmt-0`/`stmt-1`，同一 deployment 内 duplicate/blank resolver name 在任何 statement 可见前拒绝。Engine active statement catalog 改以 statement ID 为 key，使不同 deployment 可复用同名 statement 且分别 dispatch/undeploy；`Deployment.Statement(name)` 提供无歧义局部查找。新增 4 个 runtime 和 `client.runtime.statement-name` capability；当前 208 mapped / 0 partial / 5 approved-difference。Maven 当前不可用，未执行 Java suite；本轮不需要 MySQL Docker。
...
| 项目 | 内容 |
|---|---|
| 文档状态 | Draft 3.13，`ClientRuntimeStatementName` 4/4 execution 已 mapped；multi-plan deployment、runtime name resolver、deployment-local uniqueness 与 cross-deployment duplicate name 已落地，下一步继续 subscriber 组 |
