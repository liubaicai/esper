...
> 最新补充：Draft 3.12（2026-08-09），`ClientRuntimeStatementAnnotation` 6/6 execution 已处置。新增 typed immutable statement/schema metadata：内置 Description/Tag/Hint/NoLock、带 required/default 属性契约的 custom annotation、primitive/string array、enum、嵌套 annotation、annotation array 与 `reflect.Type` class value；构造期固定 missing/wrong type/duplicate/unknown/nil element/hint parameter invalid，case-insensitive Go enum resolver 替代 Java specific import。Query/Statement/Schema 返回深复制快照，metadata 进入 Plan identity，annotated schema 仍可正常消费事件。新增 6 个 runtime 和 `client.runtime.statement-metadata` capability；当前 207 mapped / 0 partial / 5 approved-difference。Maven 当前不可用，未执行 Java suite；本轮不需要 MySQL Docker。
...
| 项目 | 内容 |
|---|---|
| 文档状态 | Draft 3.12，`ClientRuntimeStatementAnnotation` 6/6 execution 已 mapped；typed statement/schema metadata、custom annotation contract 与 invalid matrix 已落地，下一步继续 statement name/subscriber 组 |
