...
> 最新补充：Draft 3.59（2026-08-10），PatternOperatorEvery suite 6/6 关闭：复合 every（every (a and b)、every (timer:interval and not X)）物化为 AST 单尝试语义，every 重启子节点继承起始标签快照（修复过期捕获遮蔽新捕获）；every a -> b 相关谓词与 timer:within 分支级窗口对照通过。当前 278 mapped / 1 partial / 17 approved-difference。本轮不需要 MySQL Docker。
...
| 项目 | 内容 |
|---|---|
| 文档状态 | Draft 3.59，Client domain 100% 关闭（281/281）；View domain 22 个唯一 runtime（KeepAll/FirstUnique/FirstEvent/LastEvent/FirstLength/LengthWindow/Sort/TimeWindow/TimeBatch 6/10），Pattern domain 11 个（And/Or 3、CompositeSelect 2/2、Every 6/6）；ExprFilter 23、ExprEnum 27、ResultSet 2、EPLOther 2、EPLInsertInto 1；manifest 当前 1,947 条关联、1,912/4,140 唯一 runtime（46.18%）、278 mapped / 1 partial / 17 approved-difference；下一步 Pattern 基础 suite（Not/FollowedBy/EveryDistinct/GuardTimerWithin 批量）、View Expression/Accum 系列与 ExprFilter 剩余 |
