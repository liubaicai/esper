...
本轮继续对照 Java `PatternConsumingFilter` 的 `PatternFollowedBy`/`PatternAnd`/`PatternOr`/`PatternFilterAndSceneTwo`/`PatternInvalid`（`java-runtime-db22761aeeae234aa70f`、`java-runtime-09fdeb1428f1772fd38c`、`java-runtime-fa260698edee4fba58ed`、`java-runtime-6cc7b65fc365ca99568d`、`java-runtime-5742c44f5fc51b6adfa1`）：Go 增加链式 `PatternStream.Consume(level)`，将 consumption level 绑定到最近追加的 Event filter；同一输入 Event 同时命中多个 filter 时，先求最高 level，再只推进该 level 的分支，未被选中的 lower-level 分支保持原 partial state，并覆盖普通/Context 路径、FollowedBy/And/Or、同级扇出、level 0 与负数校验及 Plan hash identity。`TestPatternFilterConsumeFollowedByMatchesEsper`、`TestPatternFilterConsumeAndPreservesSuppressedBranch`、`TestPatternFilterConsumeOrSelectsHighestLevelAndFansOutTies`、`TestPatternFilterConsumeContextKeepsHighestLevelSemantics`、`TestPatternFilterConsumeZeroAndNegativeValidation` 与对应的 plan/runtime 用例固定上述行为。Java 的 filter-scene 全生命周期、完整多分支 cardinality、SODA/表达式文本和 match-until/guard/observer 嵌套仍需继续对照；本轮不宣称 `@consume(N)` 全量 parity。
