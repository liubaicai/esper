...
| regression-lib | 回归场景与断言 | 1,382 | 场景代码位于 main |
...
- 当前 capability manifest 已建立 1,382 条 runtime 关联、覆盖 1,372/4,136 个唯一 Java runtime（约 33.17% 的 Java runtime 对账/处置进度）；191 个 case 中 185 个 mapped、2 个 partial、4 个 approved-difference。该比例不是 Java/Go 行为 parity 通过率，也不是全量移植完成度。
