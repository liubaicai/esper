...
> 最新补充：Draft 3.49（2026-08-10），`ClientDeployListDependencies` 5/5 与 `ClientDeployRedefinition` 4/4 关闭：新增 consumed/provided 部署依赖枚举 API（same-module 跨 deployment 边、nested schema 事件类型依赖、edge 推导 Deployment.Dependencies、undeploy 资源级检查优先），Redefinition 四条按 module 作用域与部署/卸载生命周期映射。当前 246 mapped / 0 partial / 12 approved-difference。本轮不需要 MySQL Docker。


## 1. 文档信息

当前切片（Draft 3.49，2026-08-10）关闭 Java `ClientDeployListDependencies` 与 `ClientDeployRedefinition`：`Engine.DeploymentDependenciesConsumed/Provided` 暴露部署期 catalog 边（7 类对象、script name#paramNum、unknown/blank not-found），same-module 后续 deployment 引用登记 consumer 边并按 `Undeployer.checkDependency` 阻塞 provider 卸载（undeploy 检查顺序调整为资源级优先）；schema 属性事件类型经直接 nested property schema 收集成为依赖；`Deployment.Dependencies` 并集 typed uses 与引用 provider。Redefinition 四条固定 deploy/undeployAll 周期、同 Plan 重部署、on-merge 链、演化定义新身份与同名私有对象 module 作用域共存。下一步 `ClientStage` 系列（需先补 execution inventory 枚举）。

| 项目 | 内容 |
|---|---|
| 文档状态 | Draft 3.49，ClientDeploy 11 个 suite 全部处置（Undeploy 11/11、PreconditionDependency 12/12、PreconditionDuplicate 9/9、ListDependencies 5/5、Redefinition 4/4 及其余已关闭 suite）；部署依赖枚举 API 与 same-module 边/资源级 undeploy 顺序已落地，index/class 为 approved-difference；manifest 当前 1,828 条关联、1,802/4,140 唯一 runtime（43.53%）、246 mapped / 0 partial / 12 approved-difference；下一步 `ClientStage` 系列（需先补 execution inventory 枚举） |
