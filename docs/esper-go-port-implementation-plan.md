...
> 最新补充：Draft 2.94（2026-08-09），关闭 EPLFromClauseMethodWConfig 五个类 17 个 execution：CacheLRU/CacheExpiry（CachedMethodProvider LRUSize=3 / MaxAge=1s 计数命中）、MultikeyWArray（Func2+slices.Equal 数组相等 where；数组/null 数组缓存 key）、Variable（RegisterVariable/RegisterContextVariable 常量/非常量/context 分区服务 + on-set postfix；CreateOverlappingPatternInitiatedTerminatedContext + ContextPatternField 分区隔离）、VariableMapAndOA（变量驱动 map/OA handler iterator-only 快照）、JoinPerformance 四 correctness port（grid/轮次缩减，Go 无 join on-condition index）。为此重构 updateJoin：无依赖方法行改为只 poll 新 trigger 行并按 lineage 保留、随 trigger 级联过期（removeStoredEventsCascade），取代每周期清空+全量重 poll，调用次数/缓存命中贴合 Java 每事件一次 poll；混合语句在无 trigger 接受周期不再 poll 方法侧避免幽灵行；pattern-initiated overlapping 分区 key 改由 initiating 事件身份+语句内序号派生使跨语句 context 变量共享一致。17 个 Java runtime（java-runtime-597fefdbfd7db5d1975e、78d07412e8da6df270e8、9061b2d31f1657c42b04、f5f3c0da0928eeb9a040、96f94a898f2d0bf6aebd、58ee27e23f0c613bec2c、a0e30a24d26193587a3e、8f0721530634e1180670、cd1362f21bdc9b721939、557b2eb22dfaa693a827、844da58ec7403d719727、b6134319ddad78ee7888、7fa38fe9688391d3f9da、1c125ff69ef065d76608、26c863d3cc9d5aca7cc7、2989d0cd737e30ae5633、4448f4d53e5bf0e7688d）已登记。EPLFromClauseMethodVariableInvalid 保持 approved-difference（footprint/null-metadata 反射诊断无链式 API 对应；context 变量作用域校验由 TestFromClauseMethodContextVariableParity + TestContextVariableScopeValidation 覆盖）。Java TestSuiteEPLFromClauseMethodWConfig 5/5、TestSuiteEPLFromClauseMethod 3/3 通过；unidirectional-historical、完整 Java 诊断文本与 join on-condition index 优化仍为后续切片；本轮不需要 MySQL Docker。
...
| 项目 | 内容 |
|---|---|
| 文档状态 | Draft 2.94，关闭 EPLFromClauseMethodWConfig 五个类 17 个 execution（CacheLRU/CacheExpiry/MultikeyWArray/Variable/JoinPerformance），重构 updateJoin 无依赖方法侧 lineage 保留+级联过期语义并修复混合语句幽灵行与跨语句 context 变量共享；EPLFromClauseMethodVariableInvalid 保持 approved-difference；unidirectional-historical、完整 Java 诊断文本与 join on-condition index 优化仍为后续项 |
...
`compat/java-execution-inventory.jsonl` 有 4,136 个 `status=ok` 的唯一可执行 runtime；`compat/capability-manifest.json` 有 191 个 case，其中 185 `mapped`、2 `partial`、4 `approved-difference`。这些状态只代表映射/处置登记，不代表 Java/Go trace 已相等。
