...
| 项目 | 内容 |
|---|---|
| 文档状态 | Draft 2.49，继续补充 `ExprEnumDataSources` 的 typed collection source 传播：Prev/Sorted View、Named Window/Subquery、部署参数/变量、nested property schema、access aggregate 和 Table source；Variant 的单列 method-backed member conversion 与 Java application/type-class metadata 仍未关闭；其余历史增量与未完成项见下文 |
...
| 当前修订 | Draft 2.49（2026-08-07）：在 typed JSON 命名 nested fragment catalog 和 Variant 对照基础上，继续对照 Java `ExprEnumDataSources`。Go 新增 5 条 typed collection source 轨迹，覆盖 Prev/Sorted View 的访问顺序与淘汰、Named Window/Subquery 空集合语义、部署参数/变量 late binding、nested property schema；并把已有 access aggregate/Table source 证据登记到对应 runtime。新增 1 个 capability case、8 个运行态 Java ID。manifest 进度为 1,234/4,136（约 29.84%），case 数为 183（178 mapped、1 partial、4 approved-difference）；本轮核心验证不需要 MySQL Docker。 |
...
- 当前 capability manifest 已关联 1,234/4,136 个唯一 Java runtime（约 29.84% 的 Java runtime 对账/处置进度）；183 个 case 中 178 个 mapped、1 个 partial、4 个 approved-difference。该比例不是 Java/Go 行为 parity 通过率，也不是全量移植完成度。
