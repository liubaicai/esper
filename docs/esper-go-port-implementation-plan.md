...
> 最新补充：Draft 3.02（2026-08-09），`ClientRuntimeEPStatement` 2/2 execution 已映射。新增 `Statement.SubscribeWithReplay`，同步交付 retained Snapshot（含 empty replay），并在回放期间缓冲 live/reentrant event 以保持 replay-first 独立批次；undeploy 后 Snapshot/selector Snapshot/Subscribe/SubscribeWithReplay/Stop/Start 统一返回 `ErrorState`。新增 2 个 runtime 和 `client.runtime.epstatement` capability；当前 197 mapped / 0 partial / 4 approved-difference。本轮不需要 MySQL Docker。
...
| 项目 | 内容 |
|---|---|
| 文档状态 | Draft 3.02，`ClientRuntimeEPStatement` 2/2 execution 已映射；此前 `client/basic` 7/7 与 `EPLOtherSplitStream` 12/12 保持 mapped，下一步继续 client compile/deploy/runtime 管理面 |
