...
> 最新补充：Draft 3.53（2026-08-10），ViewSort suite 6/6 关闭（多键降序/升序、字符串/枚举/数值键、即刻自驱逐、排序访问序 prev 系列）；修复排序窗口相等键排序缺陷——比较器以到达序号降序打破平局（替代 sort+reverse 二次翻转损坏），命名窗口排序 retention 同步修复并新增 entrySeq。当前 275 mapped / 1 partial / 17 approved-difference。本轮不需要 MySQL Docker。
...
| 项目 | 内容 |
|---|---|
| 文档状态 | Draft 3.53，Client domain 100% 关闭（281/281）；View domain 已处置 14 个唯一 runtime（KeepAll 3/3、FirstUnique 2/2、FirstEvent/LastEvent/FirstLength、LengthWindow 3、Sort 6/6 经汇总 case 关联），排序窗口相等键最新优先语义已修复（lineageID 平局打破）；manifest 当前 1,933 条关联、1,898/4,140 唯一 runtime（45.85%）、275 mapped / 1 partial / 17 approved-difference；下一步 ViewTimeWindow/TimeBatch/Expression 系列、ExprFilter 剩余 suite 与 Pattern 批量登记 |
