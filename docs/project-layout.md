...
├── internal/esper/      Esper 私有实现和白盒 parity 测试
├── internal/compat/     Java/Go 对账工具，禁止仓库外导入
├── internal/cmd/        facade/API 校验等仓库生成工具
├── scripts/             构建与结构校验脚本
├── testdata/compat/     版本化兼容性清单和 Java 基线
├── testdata/parity/     跨实现 parity 场景
├── tools/java-oracle/   Java inventory 探针及说明
├── doc.go               公共包文档和生成入口
├── facade_generated.go  公共 `esper` 兼容 facade
├── facade_test.go       仓库外用户视角的 API 烟雾测试
├── go.mod               模块定义
└── Makefile             统一开发入口
```
...
- 根目录保留公共包 `github.com/liubaicai/esper`，但仅承载生成 facade、包文档和
  外部 API 测试。实现与白盒测试位于 `internal/esper`。将公共包搬到 `pkg/esper`
  会破坏现有导入路径；对公共 Go 库而言，根 facade 本身就是常规布局。
...
- 固定测试资产放在 `testdata`；覆盖率、日志、profile、测试二进制和构建产物不得
  提交。
- `facade_generated.go` 由 `go generate .` 生成；结构门禁同时比较生成结果和
  `internal/esper` 的导出对象/方法集，禁止公共 API 漂移。
