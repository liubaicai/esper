...
当前 capability 对账进度为：Java inventory 的 4,140 个可执行 runtime 中，manifest 已建立 1,903 条 runtime 关联、覆盖 1,877 个唯一 runtime，约 45.34%；281 个 capability case 中 263 个标为 mapped、0 个 partial、17 个 approved-difference。45.34% 是“已建立 Java runtime 对账/处置证据”的进度，不是 Java/Go 行为 parity 通过率，也不代表 Esper 全量移植完成。
