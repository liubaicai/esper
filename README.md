...
当前 capability 对账进度为：Java inventory 的 4,140 个可执行 runtime 中，manifest 已建立 2,225 条 runtime 处置记录（distinct runtime-id），约 53.7%；369 个 capability case 中 352 个标为 mapped、0 个 partial、17 个 approved-difference。53.7% 是"已建立 Java runtime 对账/处置证据"的进度，不是 Java/Go 行为 parity 通过率，也不代表 Esper 全量移植完成。
