...
当前 capability 对账进度为：固定 Java commit 的 inventory 中有 4,136 个 `status=ok` runtime，manifest 记录 2,353 条关联，覆盖 2,282 个唯一 runtime（55.1%），尚有 1,854 个 runtime 未建立关联；379 个 case 中 355 个为 `implemented`、7 个为 `differential-verified`、17 个为 `intentionally-different`。另有 7 个代表性场景已通过 Java/Go 差分验证（`context-hash-segmented`、`filter-window-aggregate-output`、`join-length-window`、`output-policy-iterator`、`pattern-timer-interval`、`subquery-length-window`、`named-window-mutation`），NFR 尚未验证。这里的 55.1% 是"已建立 Java runtime 对账/处置证据"的进度，不是 Java/Go 行为 parity 通过率，也不代表 Esper 全量移植完成。
