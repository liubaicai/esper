...
## Current work unit
Active: Draft 4.337 ('filter-rebool-mixed-value-self') — extends the shipped filter-rebool-optimizable chain with ExprFilterOptimizableBooleanLimitedExpr ords 1 and 4.

Frozen contract (pinned by ReboolJavaContract/ReboolGoSurface this session; no re-scout needed — same class, same harness, both executions adjudicated READY zero-engine-work; serial exception recorded here for the delegation gate):
- ord 4 no-value-expr-regexp-self `java-runtime-c21cb50c065d46513b04` static `java-a8b7e007999a8eff5613`: two statements `select * from SupportBean_S0(p00 regexp p01) as a` / `select * from SupportBean_S0(s0.p00 regexp s0.p01) as s0`; S0(1,"abc",".*c") true on both, S0(2,"abc",".*d") false on both. Go: RegexpMatch(Field p00, Field p01) per statement.
- ord 1 mixed-value-regexp-rhs `java-runtime-f6199a434eee24910555` static `java-04dfad86bf629b059d7c`: 6-statement block `@name('var') create constant variable string MYVAR = '.*abc.*';` / s0 `theString regexp MYVAR` / `@name('ctx') create context MyContext start SupportBean_S0 as s0;` / s1 context MyContext `theString regexp context.s0.p00` / s2 `pattern[s0=SupportBean_S0 -> every SupportBean(theString regexp s0.p00)]` / s3 `theString regexp '.*' || 'abc' || '.*'`. Timeline: S0(1,".*abc.*") starts context + arms pattern (no output), SB("xabsx",0) all-false on s0-s3, SB("xabcx",0) all-true on s0-s3. Java plan assertions (shared REBOOL index/value across the four filters) are engine-internal and out of scope per class convention.
- Go shapes: s0 RegisterVariable("MYVAR", ".*abc.*", ConstantVariable()) + RegexpMatch(Field theString, VariableRef[string]("MYVAR")) (precedent expr_filter_variable_reference_test.go); s1 pattern-start adaptation as in 4.336 + ContextPatternField; s2 PatternFrom(From[S0],"s0",true).Then(PatternFrom(From[bean],"sb", RegexpMatch(Field theString, TagField("s0","p00"))).Every()); s3 RegexpMatch(Field theString, Concat(Literal(".*"),Literal("abc"),Literal(".*"))).

- [x] Freeze contract (this session's scouts; no independent read-only task remains — recorded as the delegation-gate rationale).
- [x] Asset writer: 2 scenario cases appended (agent_a3ddebe1; mixed-value 6-statement epl with 4-element arrays, self case with ;\n separator since the oracle compiles one deployment; trace 34 records, double-run byte-identical, prior 29 byte-identical).
- [x] Primary: 2 new parity tests (12/12 rebool tests PASS; one build fix — build closure returns Plan not Query) + evidence extension.
- [x] Manifest (+2 runtime IDs/names/goTests, notes — class 12/14 DV), summary 861→863 DV runtime IDs / associations 3484→3486 / referenced 3243→3245 / unreferenced 893→891.
- [x] Roadmap supplement + expr 97→95 (5.2/6.1; the two runtimes are suite/expr/filter); CHANGELOG 4.337 entry.
- [x] Gates + independent parity review (agent_8098104d-e3a5-46ba-bd71-521e1276b128): OVERALL PASS on all five dimensions with an independent oracle rerun byte-identical to the checked-in trace and all nine roadmap domain rows recomputed-exact; two PLANS bookkeeping P3s (missing 4.336 Shipped line, stale 4.336 block) fixed in the same work unit and re-confirmed by the same reviewer.
- [ ] Commit and push (Git owns identity; no hash recorded here).

4.336 closing summary: ords 6/7/8/12/13 differential-verified (29-record trace, 10/10 rebool tests); roadmap domain tables were initially decremented on the wrong row and were recomputed to ground truth (epl 286/infra 156/event 151/expr 97/resultset 41/context 45/multithread 56/rowrecog 34/view 27, sum 893) per review F1, re-confirmed OVERALL PASS by agent_c5554d83.
