...
- Current target: Draft 4.380 'timebatch-historical-release' - ENGINE investigation + fix: time_batch over a historical driving stream buffers without releasing joined rows at the time boundary (probe repro: FromHistorical + TimeBatch(10s) + AdvanceTime -> flushRows empty); fix the window-release/historical-poll interaction, pin with engine tests, then differential-cover EPLDatabaseTimeBatch (java-runtime-fb0cea6fe1e469ee8237). Follow-up: TimeBatchOM/Compile (SODA surface) deferred; epstatement-pattern/typed-ports dispositions; database slice 3; DV-list consolidation; perf 4.9.
...
## Current work unit
Active: Draft 4.380 ('timebatch-historical-release').

- ENGINE unit (shared-core single writer delegated with full charter). Repro: FromHistorical + From[bean].Window(TimeBatch(10s)) + JoinMany + AdvanceTime(10s) delivers nothing (flushRows empty) while Java delivers the accumulated joined rows [100,50,20] as one batch at the boundary (iterator shows rows pre-flush).

- [x] TimeBatch probe executed (throwaway test, removed after the finding): time_batch over a historical driving stream buffers without releasing joined rows at the AdvanceTime boundary (flushRows empty) — the window-release/poll interaction needs engine-level investigation; TimeBatch triple deferred with the probe finding documented in the manifest difference. OM/Compile additionally lack the SODA/eplToModel compile surface.
- [x] Dispositions enumerated for the eleven remaining executions (reviewer agent_59146f9b: names/IDs verified 1:1) (manifest difference; no status/count change): TimeBatch triple deferred (probe finding + OM/Compile compile-path); MySQLDatabaseConnection raw-JDBC smoke; InvalidSQL intentionally-different diagnostic (version-specific 1064 text); InvalidBothHistorical/InvalidPropertyEvent/InvalidPropertyHistorical/Invalid1Stream/InvalidSubviews compile-path diagnostic asserts (Go typed builder structurally prevents circular substitution, self-reference resolves Missing zero-rows engine-pinned, views-on-historical accepted divergence); RestartStatement deferred pending real-MySQL connection accounting.
- [x] Manifest difference + facts (CHANGELOG/roadmap) + gates + review + commit. Reviewer agent_59146f9b OVERALL PASS: eleven IDs match the inventory exactly, manifest diff is strictly difference-only (one appended sentence, summary byte-identical), the TimeBatch deferral rationale is consistent with the engine's poll-at-arrival/batch-defer architecture (runtime.go poll-at-arrival vs Java window-release re-drive), and the serial-scope delegation note satisfies the AGENTS.md gate. P3 noted for a follow-up unit: the stale docstring in database_join_class_parity_test.go:714-716 (claims views-on-historical "rejected at Build") should be corrected to match the documented acceptance divergence.
