...
- Updated: 2026-09-01; Draft 4.297 implementation and focused replay integration are active from the clean c257bce20 baseline. Grouped sorted runner, tests, scenario, Java oracle, generated traces/evidence, asymmetric Go-only normalization, manifest case/mapping, repaired central summary, roadmap, and CHANGELOG are present. Pinned Java and Go trace regeneration is byte-identical; grouped focused tests and manifest validation pass; independent final review, full gates, and commit remain.
- Baseline: `HEAD` == `origin/master` at `c257bce20026c8cf45d0888dfba05fce9add7818` (`feat(resultset): port sorted table access 3 runtimes`); the grouped sorted slice is integrated locally but not yet committed.
- Closed predecessor (Draft 4.296): `ResultSetAggregationMethodSorted.java` ordinals 7-9 were differentially verified through `resultset-aggregate-sorted-table-access`; independent review PASS found no behavioral, provenance, evidence, or documentation defects. The grouped review finding was addressed by separating Java and Go normalizers so the mutation test rejects corrupted Java `sortcol`.

## Current work unit
Draft 4.297 is frozen as ordinal 11 `ResultSetAggregateSortedGrouped` in fixed `ResultSetAggregationMethodSorted.java`, source lines 112-148. The Java source commit `9e1b9f1cc9117fea4bf33ab043762c045d73839c` is read-only. Runtime ID is `java-runtime-3ffe177eb6a90acb699a`; static candidate is `java-5a1abe9fb7420df7353d`; shared inventory ID is `java-0ded2b32c81677a9d36f`; flags are empty. The keyed table `MyTable(k0 string primary key, sortcol sorted(intPrimitive) @type('SupportBean'))` receives unwindowed `SupportBean` rows grouped by `theString`; `SupportBean_S0` p00 triggers select `sorted()`, `firstKey()`, and `lastKey()`. The exact replay has eleven steps and five scalar oracle rows: missing A `[null,null]`, A10/A20 `[10,20]`, duplicate A10/A21 `[10,21]`, later A `[10,21]`, and B `[100,100]`.
Fresh pinned Java replay confirms `sortcol` is `{state:"null"}` for every row, including populated groups, while `firstkey`/`lastkey` carry the asserted values. This is the fixed Java scalar projection of `sorted()`; its collection accessor is not the event-property value serialized by the oracle. Go's generic reflected `Method[[]esper.Event]` intentionally exposes the typed collection, so the bounded parity runner must normalize only this Java-observable `sortcol` field to Null. No shared `internal/esper` change is authorized.

## Delegation checkpoint
`NextSortedJavaContract` (`java-oracle-scout`) and `NextSortedGoSurface` (`scout`) completed concurrently before this freeze, confirming the exact source/runtime/static/inventory IDs, trigger sequence, null/duplicate semantics, and sufficient existing typed API. `GroupedSortedRootCause` and `GroupedSortedRootCause-2` independently confirmed that Method reflection, table coercion, and generic event-array normalization are not defects; the fresh oracle replay established the scalar-vs-enumeration boundary. The asset worker delivered the Java oracle, launcher, and scenario. Primary owns the bounded Go runner, runner dispatch integration, focused tests, generated traces/evidence, manifest, roadmap, CHANGELOG, validation, review, commit and push. No `internal/esper` writer is authorized.

- Finish verification for the integrated grouped sorted slice: regenerate pinned Java and Go traces, compare zero differences, exercise mutations, update central facts/docs, obtain independent parity review, run complete local gates, and create one semantic commit pushed to `master`. Do not write a post-push checkpoint-only commit.
- Review follow-up from `GroupedSortedReview-2`: add checked-in grouped evidence scenario/Java metadata equality and require the Java oracle's scalar `sortcol` projection to be exactly `{state:"null"}`; re-run focused tests and oracle/evidence generation before independent final review.
...
- The pinned Java oracle launcher regenerated `testdata/parity/resultset-aggregate-sorted-grouped.trace.json` from `/home/baicai/app/esper` at `9e1b9f1cc9117fea4bf33ab043762c045d73839c`; Java 17 emitted five listener records. The regenerated Java trace and Go replay are byte-identical to their checked-in traces (SHA-256 `997571f94016074530c956943a1b35ce357a4f946ed8a330d64ba41621c0ef8d` and `d943bf647020b4cef0cef67e7a91a1cf1075df9af0e784bb717a66f3f5664da0`).
- Differential evidence is passing with five Java records, five Go records, and zero differences; final regenerated evidence is byte-identical to checked-in evidence (SHA-256 `53a5739ecf68ed44665e4717a7d7fb1f97a6ca2b037bfe4a160ab181616778da`). Focused grouped replay, checked-in evidence, trace-mutation, malformed-scenario, and runtime-mapping tests passed; `TestCapabilityManifestArtifactValidates` passed; shell/JSON/layout checks and `git diff --check` passed.
- Independent reviewers `GroupedSortedReview-2` and `GroupedSortedReviewFinal` returned PASS with no P0-P3 findings after re-reviewing the asymmetric normalizer, scenario/metadata equality, Java scalar `sortcol` null assertion, generated traces/evidence, and manifest/docs. `make check` passed, and full `go test -race ./... -count=1 -timeout 600s` passed across every package, including `internal/app/parity` and `internal/esper`.
- Persisted evidence and manifest checks are complete. Manifest totals are 588 cases, 201 differential-verified cases, 736 differential-verified runtime IDs, and 3,357 runtime associations (3,133 referenced runtimes); the new grouped case is registered as `differential-verified`.

## Delivery

- Draft 4.297 is ready for one semantic commit/push. Git will remain authoritative for the resulting commit identity.
