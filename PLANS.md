...
- Current target: Draft 4.330 candidate 'infra-named-window-insert-from' — InfraNamedWindowInsertFrom.java prefetch scouts dispatched concurrently with the 4.329 review.

## Current work unit
Active: Draft 4.329 ('infra-named-window-on-update-misc') — the remaining 4 executions of InfraNamedWindowOnUpdate.java as ONE differential case (same file/harness/mode family as shipped 4.328; separate loader/runner mode). Contract frozen from 'OnUpdateJavaContract' (agent_480e7832, all 8 executions) + 'OnUpdateVariantsGoSurface' (agent_8dd095cb, ALL READY zero engine work).

Frozen contract (ordinals are executions() ordinals; javaFlags [] per shipped-sibling convention — static-manifest's file-scoped SERDEREQUIRED is an over-approximation, only ord 4 declares it in Java, no Go-harness effect):
- ord 0 non-property-set: InfraUpdateNonPropertySet, java-runtime-5d041a3958410a90fa9a, static java-26306de901985e70904f. EPL: create `@public create window MyWindowUNP#keepall as SupportBean` (unnamed in Java; @public is the oracle separate-module convention), insert `insert into MyWindowUNP select * from SupportBean`, update `@name('update') on SupportBean_S0 as sb update MyWindowUNP as mywin set mywin.setIntPrimitive(10),     setBeanLongPrimitive999(mywin)` (5 spaces before setBean; verbatim incl. method-call set clauses — the ORACLE compiles it with a plugin single-row function hosted on the oracle class mirroring the suite config; the GO runner ports it as the approved-difference SetColumn("intPrimitive", Literal(10)) + SetColumn("longPrimitive", Literal[int64](999)), precedent epl_other_update_istream_parity_test.go:1599-1602). Listener on the UPDATE statement (assertPropsPerRowLastNew). Sends SupportBean E1/1 then SupportBean_S0 id=1. Trace: 1 listener record projected to [intPrimitive,longPrimitive]: new [{10,999}], old [{1,0}] (pre-image; longPrimitive default 0; Java delivers before-image on update-istream per EPLOtherUpdateIStream.java:812; Go processUpdateStream builds New+Old at runtime.go:8638). Steps: 7 (no snapshot, iteratorSnapshots 0).
- ord 3 subclass: InfraSubclass, java-runtime-9ea51cb84d163be79693, static java-d920e63f2081f1d72d2f. EPL: create `@name('create') @public create window MyWindowSC#keepall as select * from SupportBeanAbstractSub`, insert `insert into MyWindowSC select * from SupportBeanAbstractSub`, update `on SupportBean update MyWindowSC set v1=theString, v2=theString`. Java hierarchy (base v1 + sub v2; constructor SupportBeanAbstractSub("value2") sets v2, v1 stays null) collapses to one Go struct {V1 *string, V2 *string} registered as SupportBeanAbstractSub (pointer fields for null-exact rendering; Go struct inheritance is a documented approved difference). Listener on create; the oracle emits ALL invocations (listenerReset in the Java test affects assertions only, not the trace): seq1 insert delivery new [{v1:null,v2:"value2"}], seq2 update delivery new [{v1:"E1",v2:"E1"}] plus whatever old image both engines deliver (runner records batch.Old; pin finalized from the generated traces). Sends SupportBeanAbstractSub {"v1":null,"v2":"value2"} then SupportBean E1/1. Update predicate Literal(true) (nil rejected, trigger_test.go:813). Steps: 7, iteratorSnapshots 0.
- ord 4 copy-method: InfraUpdateCopyMethodBean, java-runtime-43feff597147c7867ca7, static java-6eb1ea7dc80c3078f47f. EPL: create `@name('window') @public create window MyWindowBeanCopyMethod#keepall as SupportBeanCopyMethod`, insert `insert into MyWindowBeanCopyMethod select * from SupportBeanCopyMethod`, update `on SupportBean update MyWindowBeanCopyMethod set valOne = 'x'`. Plain struct {ValOne,ValTwo string}; field-wise copy approved difference (no copy hook; Serializable oracle mirror suffices, no setCopyMethod config needed). Sends SupportBeanCopyMethod {"valOne":"a","valTwo":"b"} then zero-value SupportBean (trigger values unobservable). Snapshot (statement `window`) projects [valOne] → [{valOne:"x"}]. Steps: 8, iteratorSnapshots 1.
- ord 5 wrapper: InfraUpdateWrapper, java-runtime-09f56f49bab53388bb2d, static java-6b566143dbbb00ea202b. EPL: create `@name('window') @public create window MyWindow#keepall as select *, 1 as p0 from SupportBean`, insert `insert into MyWindow select *, 2 as p0 from SupportBean`, update `on SupportBean_S0 update MyWindow set theString = 'x', p0 = 2`. Go window struct {theString,intPrimitive,p0} (schema must declare p0); insert = CopyMatchingFields() + SetColumn("p0", Literal(2)) — mixed wildcard+explicit needs a Build-time check, fallback 3× explicit SetColumn; update = UpdateNamedWindow(Literal(true), SetColumn("theString", Literal("x")), SetColumn("p0", Literal(2))). Sends SupportBean E1/100 then SupportBean_S0 id=-1. Snapshot (statement `window`) projects [theString,p0] → [{theString:"x",p0:2}]. Steps: 7 (merged create+insert), iteratorSnapshots 1.
Wrapper deviation (asset-writer, probe-proven on the fixed tree): create+insert merge into ONE deploy step — a create-window-as-select with wildcard + extra property never registers its type among public types, so a separate insert module cannot resolve it; this mirrors how the Java suite itself compiles them, and the Go runner deploys the insert from the merged create step. Also verified differentially: the subclass update delivery carries the old pre-image [{v1:null,v2:"value2"}] on the create consumer (Java default irstream delivery). Totals: 7+7+8+7 = 29 steps, 5 records (1+2 listener + 2 snapshots). Mode: 'infra-named-window-on-update-misc'/-diff; new file internal/app/parity/infra_named_window_on_update_misc.go; run.go two dispatch arms; 6-test family in run_test.go.

- [x] Freeze on-update variants contract (Java contract from completed prefetch + variants Go-surface scout).
- [x] Author oracle, script, and scenario via asset writer (agent_03d45f5c; end-to-end oracle run; writer probe-proved the wrapper create+insert merge, 29 steps not 30).
- [x] Implement infra-named-window-on-update-misc runner, modes, and 6-test family.
- [x] Generate traces and zero-difference evidence (5/5, evidence passing).
- [x] Update manifest (620/233/831/3454), roadmap, and CHANGELOG.
- [x] Run gates and independent parity review: make check PASS, git diff --check clean; OnUpdateMiscParityReview OVERALL PASS (one P2 stale wrapper step count and three P3 prose notes fixed, re-confirmed by the same reviewer).
- [x] Commit and push the unit (Git owns identity; no hash recorded here).

Integration notes: primary synced the runner to the writer's probe-proven wrapper deviation (merged create+insert deploy; the Go runner deploys the insert from the merged create step) and fixed two primary bugs the first replay exposed (doubled @name in the observed-vs-public EPL constants; missing WithOldStream on the subclass create consumer — Java statements default to irstream, so the update's old pre-image is part of the pinned trace). The misc validator pins per-case payload values, closing the payload-value-drift mutation the first test run exposed as previously unobserved.
