...
- Updated: 2026-08-28
- Baseline: `HEAD` == `origin/master` at Draft 4.273 (`6e721eac6`), pushed. Draft 4.274 is the active sorted table multi-criteria follow-up.
- Closed unit (Draft 4.273, commit `6e721eac6`): `ResultSetAggregateSortedMinMaxBy.java` execution `ResultSetAggregateMultipleCriteriaSimple` was differential-verified with 4 Java/Go records and zero differences; strict scenario payload and trace mutation validation passed independent parity review.
...
## Current work unit
Draft 4.274: `ResultSetAggregationMethodSorted.java`, execution `ResultSetAggregateSortedMultiCriteria` (ordinal 10, inventory `java-0ded2b32c81677a9d36f`, static candidate `java-427c23ee540c6e2e7cf9`). The fixed Java oracle is `/root/app/esper` at commit `9e1b9f1cc9117fea4bf33ab043762c045d73839c`. Runtime ID is `java-runtime-dd79ba5aba4eb4ec0a1a`. The contract is one EPL-only table-backed lifecycle: `MyTable.sortcol` stores `sorted(*)` keyed lexicographically by `(theString, intPrimitive)`; seven `SupportBean` sends (`E1a/1`, `E1b/1`, `E4b/4`, `E6a/6`, `E6b/6`, `E8/8`, `E9/9`) precede one `SupportBean_S0(-1)` trigger. The listener emits composite `HashableMultiKey` values `firstkey=(E1a,1)`, `lastkey=(E9,9)`, `lowerkey=(E1b,1)` for query `(E4,1)`, and `higherkey=(E4b,4)` for query `(E4b,-1)`.
Delegation checkpoint: `NextSortedJava4` and `NextSortedGo4` completed the required read-only prefetch. Java froze source lines 151-176 and 606-614, inventory/runtime/static IDs, and the exact key contract; Go confirmed `SortedAccessByMulti`, `SortedMultiKey`, `CreateTable`, `IntoTable`, `TableField`, `Method`, and `OnEvent.SelectFromTableWhere` are reusable with no expected `internal/esper` semantic change. N+1 writes had not started before this unit became active.
## Progress
Contract is frozen. Runner/assets, dispatch, focused mutation tests, Java/Go traces, and zero-difference evidence are integrated. Manifest includes the target case/mapping and summary `567/565/179/694/3317/3104`; roadmap, CHANGELOG, and README are synchronized. Independent parity review PASS; focused tests, direct Java/Go replay, evidence comparison, `go vet ./...`, `go test ./... -count=1 -timeout 240s`, `go test ./internal/compat/... -count=1`, `make check`, layout, and diff checks pass.
## Delivery
Required gates pass: focused parity tests and direct replay, regenerated fixed-Java trace, zero-difference evidence, independent parity review, `go vet ./...`, `go test ./... -count=1 -timeout 240s`, `go test ./internal/compat/... -count=1`, `make check`, layout/diff checks. Create one semantic commit and push; do not add a checkpoint-only hash update.
## Handoff
