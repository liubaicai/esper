...
- Updated: 2026-09-03; Draft 4.308 (`resultset-output-limit-row-limit-negative-rowcount`) is the current semantic work unit, targeting fixed Java `ResultSetOutputLimitRowLimit.java` ordinal 7 (`ResultSetGroupedSnapshotNegativeRowcount`); implementation, replay, differential evidence, and central facts are complete, with final gates and delivery pending.
- Baseline: Draft 4.307 (`resultset-output-limit-row-limit-context-grouped`) was committed and pushed at `f7c32288e`; its two runtimes have zero-difference evidence and passed `make check` plus `make test-race`.
## Current work unit
Ordinal 7 uses `@name('s0') select theString, sum(intPrimitive) as mysum from SupportBean#length(5) group by theString output snapshot every 10 seconds order by sum(intPrimitive) desc limit -1 offset 1`. Java advances to 1000 ms before deployment, snapshots an empty iterator, sends E1/10, E2/5, E3/20, E1/30, advances to 11000 ms, and emits the current grouped rows after descending sort with offset 1 and negative limit semantics: E3/20 then E2/5. The execution attaches listener `s0`, asserts iterator before input and listener output at 11 seconds, then undeploys.
The typed Go implementation uses `LengthWindow(5)`, `GroupBy`, `Sum`, `OutputSnapshotEvery(10*time.Second)`, descending `OrderBy(ResultField("mysum"))`, `Limit(-1)`, and `Offset(1)`. The runtime already treats negative limits as unlimited; the semantic gap was query validation, which now accepts negative rowcount while continuing to reject negative offsets. Keep ordinals 8/9 separate. Primary owns shared semantic integration, runner/tests, generated traces/evidence, manifest, roadmap, CHANGELOG, validation, review, commit, and push.
## Delegation checkpoint
The ordinal-7 read-only scouts `Ordinal7JavaScout` (`java-oracle-scout`) and `Ordinal7GoScout` (`scout`) completed concurrently. Java contract: fixed commit `9e1b9f1cc9117fea4bf33ab043762c045d73839c`, source `regression-lib/src/main/java/com/espertech/esper/regressionlib/suite/resultset/outputlimit/ResultSetOutputLimitRowLimit.java:259-280`, shared static/inventory ID `java-0109d52ee4e8b36575b9`, runtime `java-runtime-6bf4cf3ded03c9ff57fa`, ordinal 7, execution `ResultSetGroupedSnapshotNegativeRowcount`, flags empty. Go scout confirms `Limit`/`Offset` in `stream.go`, `applyResultWindow` already supports negative limit as unlimited, and `plan.go:1991-1994` plus `syntax_validation.go:50-52` reject all negative values. Contract is frozen. `Ordinal7Assets` (`parity-asset-worker`) owns only the Java oracle wrapper, shell script, and scenario JSON; `Ordinal7Core` (`go-slice-worker`) owns only query validation and the focused Go parity test. Primary owns the new parity runner, dispatch and mutation tests, generated traces/evidence, central facts, validation, review, commit, and push.
- Independent parity review `Ordinal7Review-3`: PASS for ordinal-7 semantics, parity traces, evidence, and regression safety; no P0/P1/P2 semantic findings. Central fact omission was resolved by registering the differential case and mapping in `testdata/compat/capability-manifest.json`; static/source manifests already contain the underlying Java candidate records and remain generated inventory inputs.
- Targeted validation before full gates: Java/Go output-limit runner tests, affected Esper semantic tests, compatibility manifest validation, and `git diff --check` all pass; checked-in Java and Go traces compare with zero differences and mutation tests reject value/order/time/count changes. Full `make check`, race gate, commit, and push remain.
## Next work unit
After this semantic commit, continue `ResultSetOutputLimitRowLimit.java` ordinal 8 invalid compilation diagnostics; keep ordinal 9 variable-backed limit/offset semantics separate.
## Prior outcomes
