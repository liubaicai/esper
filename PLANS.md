...
activity or a single coverage percentage.
 ...
 - Updated: 2026-08-28
 - Baseline: `HEAD` == `origin/master` at Draft 4.277 (`325073e046fe1fb66ba6471da17d30c6c0ec240c`), pushed; worktree was clean before this selection.
 - Closed unit (Draft 4.277, commit `325073e04`): `ResultSetAggregateMaxMinGroupBy.java` ordinals 1 and 2 were differential-verified with 24 Java/Go records and zero differences; reviewer `ReviewDraft4277` returned PASS.
...
## Current work unit
Draft 4.278: extend `ResultSetAggregateMaxMinGroupBy.java` with adjacent executions `ResultSetAggregateMinMaxJoin` and `ResultSetAggregateMinNoGroupSelectHaving` (suite ordinals 3 and 5; static candidates `java-6b177406a733a140a453` and `java-89cead06d92ef2fbf53c`; runtime IDs `java-runtime-802aec9425dc772e3aa5` and `java-runtime-060f5af73edf420ef45a`; outer static candidate `java-0ca3164935a71ca95033`). The fixed Java oracle is `/root/app/esper` at commit `9e1b9f1cc9117fea4bf33ab043762c045d73839c`.
Candidate selection is evidence-backed by `resultset.aggregate-group-by.remaining`, and both read-only scouts confirm one natural source-local aggregate boundary with two independent cases. Java contract: ordinal 3 deploys `@name('s0') select irstream symbol, min(volume) as minVol, max(volume) as maxVol, min(distinct volume) as minDistVol, max(distinct volume) as maxDistVol from SupportBeanString#length(100) as one, SupportMarketDataBean#length(3) as two where (symbol='DELL' or symbol='IBM' or symbol='GE') and one.theString = two.symbol group by symbol`; seed DELL and IBM before the shared 12-event sequence, yielding 12 listener callbacks with old/new row counts `[1,1,1,1,1,2,2,2,1,1,1,1]`, IBM before DELL in two-row callbacks, Long extrema and null-skipping.
Go contract: use typed `RegisterStruct` market `{Symbol string, Price float64, Volume *int64}` and string `{TheString string}`; join `From` windows, `Join(... OnEqual)`, `GroupBy(JoinField[string](1, "symbol"))`, cast boxed volume to int64 for extrema/distinct; select/having uses length(5), `Aggregate(Alias("symbol", symbol), Alias("mymin", Min[int64](volumeLong)))`, and raw boxed-volume comparison against float min*1.3. No shared-core production gap is known; parity runner is the normal implementation boundary. New independent scenario ID: `resultset-aggregate-minmax-groupby-join-select-having`, two cases in execution order (`minmax-join`, `min-no-group-select-having`), 12+2=14 records. Allowed writes: parity runner/mode/tests and Java oracle/scenario script; primary owns generated traces/evidence and central facts.
Delegation checkpoint: Java scout `NextMinMaxJavaContract` and Go scout `NextMinMaxGoSurface` completed read-only. Their reports freeze source/runtime/static identity, listener/window/join/HAVING semantics, typed API coverage, and the above scenario/mutation boundary. Independent reviewer `ReviewDraft4278` returned PASS with no P0-P2 findings after inspecting the full implementation, assets, traces, evidence, and tests.

## Delivery
Focused aggregate mode tests, direct fixed-Java/Go replay, zero-difference evidence, four trace mutations, checked-in evidence replay, malformed scenario checks, `go vet ./...`, `go test ./... -count=1 -timeout 240s`, `go test ./internal/compat/... -count=1`, `make check`, layout, and diff checks pass. Central manifest and documents are updated for Draft 4.278; final independent parity review remains required before commit.
## Handoff
After Draft 4.278 delivery, verify the pushed ref read-only and select the next roadmap/manifest work unit; never add a checkpoint-only hash update.
Prefetched N+1 (read-only): `ResultSetAggregateMedianAndDeviation.java` ordinals 0-2, three runtimes `java-runtime-101db664ea346721e986`, `java-runtime-2f533a8a1bc0d93ae649`, and `java-runtime-2667a7eadeb7a458d7ce`; static candidates `java-b11b233b0ea7da05217f`, `java-66a516ce6e14456627b2`, and `java-a5c65b307ecdb2d0355b`. The source-local contract is grouped filtered length(5) median/all, median/distinct, stddev and avedev with irstream, plus two grouped join variants seeded by DELL/IBM/AAA; typed Go Median/StdDev/Avedev/DistinctAggregate and Join APIs exist, with no shared-core gap indicated. The Java scout failed once on an upstream HTTP/2 stream but the same contract was independently confirmed by the Go scout and direct inventory/source reads; no N+1 writes have started.
