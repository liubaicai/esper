...
- Current target: Draft 4.340 'insert-into-until-pattern-array' — EPLInsertIntoPopulateUnderlying ords 9+10; contracts already pinned by PopulateUnderlyingJavaContract (agent_fae76b58) + PopulateUnderlyingGoSurface (agent_8cd5b2e7: valid halves READY, invalid halves implemented-only).
...
## Current work unit
Active: Draft 4.340 ('insert-into-until-pattern-array') — extends the epl-insert-into-populate-underlying chain with ords 9 and 10.

Frozen contract (pinned by the 4.338 scouts covering all 9 open executions; no re-scout — serial exception recorded in the delegation checkpoint):
- ord 9 EPLInsertIntoArrayPOJOInsert `java-runtime-56175b64c415bf33ddf1` static `java-bfd55528c9aca96a5ffe`: path-deploys 3 `@public create schema X as <class>` for the suite's public static local classes (FinalEventValid{SupportBean_S0 startEvent; SupportBean[] endEvent}, FinalEventInvalidNonArray, FinalEventInvalidArray). Valid: `INSERT INTO FinalEventValid SELECT s as startEvent, e as endEvent FROM PATTERN [every s=SupportBean_S0 -> e=SupportBean(theString=s.p00) until timer:interval(10 sec)]`; advanceTime(0); sends S0(1,"G1"), SB("G1",2), SB("G1",3); advanceTime(10000) → one routed row startEvent.id=1/p00=G1, endEvent len 2 intPrimitive 2,3. Invalid halves: array column into non-array property rejects at Go Build; single event into array property is accepted at Build/deploy but rejected at route-execution time (empirical; Java reports the same TypeMismatch at compile) — implemented-only.
- ord 10 EPLInsertIntoArrayMapInsert `java-runtime-facd4a0c64ae48571ee6` static `java-3980f002801d7b6b6d3b`: same until-pattern shape over map schemas EventOne(id string)/EventTwo(id,val int)/FinalEventValid(startEvent EventOne, endEvent EventTwo[]) (+ invalid variants); Java loops 6 representations with representation-specific assertion surfaces. Go models the map/objectarray representations (Go JSON registrations a superset, not pinned; Avro not modeled by the map-based oracle); invalid halves implemented-only.
- Go shapes: `PatternFrom(one, "s", true).Then(PatternFrom(two, "e", Equal(theString, TagField(s,p00))).MatchUntilExpr(nil,nil).Until(TimerInterval(two, 10*time.Second))).Select(Alias("startEvent", PatternEvent("s")), Alias("endEvent", TagEvents("e"))).InsertInto(...)` (precedent json_provided_underlying_test.go:317-360); Engine.AdvanceTime runtime.go:4232.
- Deferrals after this unit: ord 12 (invalidity implemented-only Build-error unit restricted to reproducible sub-cases).

- [x] Freeze contract (4.338 scouts; no independent read-only task remains — delegation-gate rationale recorded).
- [x] Asset writer: 4 cases appended (agent_aedf9f87; array-pojo-until + array-map-until objectarray/map/default; oracle gained a no-record advanceTime op and EventBean-wrapper normalization; trace 30 records = 26 prior byte-identical + 4 new, triple-run byte-identical).
- [x] Primary: 3 parity tests (POJO until, map until ×3 reps, invalid ×2 subtests; all PASS — fixes: time import, objectarray sends take positional []any, and the single-into-array half empirically rejects at route-execution time (Build/deploy accept) so the premature approved-difference claim was corrected to the phase-accurate disposition) + evidence extension.
- [x] Manifest (+2 runtime IDs/names/goTests, notes — class 12/13 DV), summary 869→871 DV runtime IDs / associations 3492→3494 / referenced 3251→3253 / unreferenced 885→883.
- [x] Roadmap supplement + epl 280→278 (5.2/6.1 and section-4 prose); CHANGELOG 4.340 entry.
- [x] Gates + independent parity review (agent_2453e2d1-d517-4dd3-b627-55b75766e0e6): OVERALL PASS with an independent oracle recompile+rerun byte-identical to the checked-in trace and manifest/domain arithmetic recomputed-exact; one P2 (caseMatchSummary missing the 4 new cases) and three P3s fixed in the same work unit — including an empirically-driven correction: single-into-array is ACCEPTED at Build/deploy and rejected at route-execution time (AdvanceTime returns the startEvent TypeMismatch), so the premature "accepted and wrapped approved difference" claim was corrected to the phase-accurate disposition in every central file; the reviewer independently reproduced the disposition before re-confirming OVERALL PASS. Residual P3s (stale oracle header phrase, error-content assertion, dead subscription) also fixed pre-commit.
