...
## Current work unit
Active: Draft 4.325 ('infra-nwtable-subq-correl-join') — correlated subquery inside a 2-stream last-event join over NW/table; contract frozen from both prefetch scouts; asset writer dispatched, primary implementing the runner.

Frozen contract (scouts 'JavaContract4325' + 'GoSurface4325'): 3 executions of InfraNWTableSubqCorrelJoinAssertion (java-runtime-a1f99ce62c1931e994d8 (T,F) nw-no-share; java-runtime-00bf2cd9669115eb2950 (T,T) nw-index-share; java-runtime-ffb6fe55a1ecde12a134 (F,F) table; shared static java-0ce6cd8b5317076e9a5a; flags []). Identical timelines: create `@public create window MyInfra#unique(theString) as select * from SupportBean` (table: `@public create table MyInfra(theString string primary key, intPrimitive int primary key)`; index-share case prefixed `@Hint('enable_window_subquery_indexshare') `) → unnamed `insert into MyInfra select theString, intPrimitive from SupportBean` → s0 `@name('s0') select (select intPrimitive from MyInfra where theString = s1.p10) as val from SupportBean_S0#lastevent as s0, SupportBean_S1#lastevent as s1` (listener s0 only; create/insert unnamed, auto-generated names not pinned). Sends: SupportBean E1/10, E2/20, E3/30 (feed MyInfra) → S0{id:1,p00:E1} SILENT (S1 lastevent empty, join incomplete) → S1{id:1,p10:E2} val=20 → S0{p00:E3} val=20 (subquery reads s1.p10 still E2) → S1{p10:E1} val=10 → S1{p10:E3} val=30. 4 records per case (new-only, val single field), 12 records total; 39 steps (3 markers + 3×(3 deploys + 5 sends + undeploy-all)).

Go surface (scout adjudication): ALL READY, zero engine work. JoinField[string](1, "p10") resolves join-outer fields inside subquery predicates (expr.go:486-518, JoinEvents populated for subqueries; precedents subselect_filtered.go:412-456 joined-2/3-streams with JoinField[any](0/2,...); engine test subquery_join_test.go). LastEvent cross join: JoinMany(JoinSource(From[S0].Window(LastEvent())), JoinSource(From[S1].Window(LastEvent()))) — zero-condition cross join precedent resultset_querytype_aggregate_grouped_having.go:558-570; incomplete-join silence is ordinary inner-join semantics. NW unique retention via env.Schema("SupportBean") + CreateNamedWindow(env, "MyInfra", schema, NamedWindowRetention(Unique(Field("theString")))) created before engine (epl_subselect_within_filter_having.go:374-395 — exact  shape). Table composite PK: PrimaryKeyColumn[string]("theString") + PrimaryKeyColumn[int]("intPrimitive") (infra_table_insert_into.go:195-199). Composite-PK prefix index selection is a verify-point (snapshot-scan fallback is parity-safe either way). Index-share hints as WithStatementHints metadata; behavior-neutral. Mode 'infra-nwtable-subq-correl-join'/-diff; file internal/app/parity/infra_nwtable_subq_correl_join.go; no compat op additions.

- [x] Freeze subq-correl-join contract (parallel prefetch scouts).
- [x] Author oracle, script, and scenario via asset writer.
- [x] Implement infra-nwtable-subq-correl-join runner, modes, and tests.
- [x] Generate traces and zero-difference evidence.
- [x] Update manifest, roadmap, and CHANGELOG.
- [x] Run gates and independent parity review: SubqCorrelJoinParityReview PASS (agent_c7493a47).
Subq-correl-join review: parity review `SubqCorrelJoinParityReview` (independent read-only reviewer, agent `agent_c7493a47-ea9f-4ca7-ab83-97ac1d6e7494`) returned PASS across all areas with no P1/P2 findings. Full 14-package suite green; oracle replay byte-identical (cmp=0); diff evidence 12/12 zero-diff. Verified: JoinField join-outer reference in subquery predicate, unique-NW retention, composite-PK table lookups (prefix index selection parity-safe), join-incomplete silence on the first S0 send, and index-share hint plan-neutrality.
- [x] Commit and push the unit (Git owns identity; no hash recorded here).

## Delegation checkpoint

Draft 4.325 unit:
- Prefetch scouts (read-only, concurrent, before implementation): Java contract scout 'JavaContract4325' ('agent_506a001f-db36-4216-a20c-d267bc33c4c4') pinned all 3 execution timelines (12 records), the join-incomplete silence semantics, ordinal caution (inventory ordinals 0/1/2; flags the Uncorrel sibling's 0/1/0/1 deviation), and payload pins; Go surface scout 'GoSurface4325' ('agent_560bce0d-4fa9-4383-ad7c-afa7a18c7e46') adjudicated all items READY with the JoinField outer-reference precedents and one verify-point (composite-PK prefix index selection; snapshot-scan fallback parity-safe).
- Asset writer (isolated parity-asset writer, concurrent with primary runner work): owns 'tools/java-oracle/InfraNWTableSubqCorrelJoinScenarioOracle.java', 'tools/java-oracle/run-infra-nwtable-subq-correl-join.sh', 'testdata/parity/infra-nwtable-subq-correl-join.json'. 12 records (4 per case); ordinals pinned 0/1/2 per inventory.
- Primary owns the Go runner ('internal/app/parity/infra_nwtable_subq_correl_join.go'), run.go wiring, run_test.go family, traces/evidence, manifest, roadmap, CHANGELOG, PLANS.md, gates, review, commit, and push.
