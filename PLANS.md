...
- Current target: next-unit selection. Candidates: (a) DV-list consolidation unit (backfill per-case differentialVerifiedRuntimeIds for old chain entries, or codify case-level DV in quality-strategy line 40); (b) invalidity-policy dispositions (EPLSubselectAggregatedInvalid, EPLSubselectInvalid, exists-suite SODA/OM surface); (c) new domain slices: database join chains, dataflow DV upgrades, method sources, infra tables/named-window mutation/transaction; (d) subselect policy-gated performance suites (policy decision required). adding a Go preeval-off control for self-subselects (Java semantics: ConfigurationRuntimeExpression.selfSubselectPreeval=false selects MultiMatchHandlerSubqueryPostevalNoDedup — the outer statement's filter/where evaluates BEFORE the subselect window accepts the triggering event; Go ordering point runtime.go:6878 subqueryRegistry.accept) and unlocking EPLSubselectOrderOfEvalNoPreeval (java-runtime-ac2d59129befe9a3b088, static java-04d47cea26b120f5f805) into case.epl-subselect-order-of-eval-index. Known from the 4.353 scouts: the Java oracle needs a SECOND runtime from a second Configuration with setSelfSubselectPreeval(false), distinct URI, advanceTime(0), parameterised runCase, EXPECTED_RECORDS 45→47, EXPECTED_STEPS 170→177, script gates (cases 4→5, ordinals [0,1,0,1,0], SupportBean sends 17→19, deploy/undeploy 26→28); the two new records render full 20-property SupportBean select-* rows (charPrimitive as \u0000, primitive zero columns as 0). Follow-up queue: DV-list consolidation, invalidity dispositions, database/dataflow/method-source slices. The Go surface scout adjudicated the wildcard-in GREEN (SubqueryIn[any] over EventValue[any] whole-event rows; S1/S2 as distinct Go struct types so structural DeepEqual reproduces the reference-identity booleans for the pinned sends; anyObject as an `any` interface field; replay decode must type-discriminate the nested anyObject by field shape) and EPLSubselectOrderOfEvalNoPreeval RED-for-runner-only: the Java execution is a runtime-configuration test (TestSuiteEPLSubselectWConfig sets selfSubselectPreeval=false) while the Go engine hardwires subquery acceptance before where-evaluation (runtime.go:6878) with no equivalent option — the identical builders produce the preeval-on (silent) trace. Merging it would require an internal/esper engine option (defer self-subselect acceptance until after the statement's own filter/where evaluation) — deferred to a dedicated engine unit 4.354+ with the order-of-eval-index chain (preeval-on pins) as the blast-radius reference. Remaining subselect domain after 4.353: the NoPreeval engine unit, invalidity/intentionally-different manifest dispositions, and the four policy-gated performance suites.
...
## Current work unit
Active: Draft 4.354 ('subselect-preeval-off').

- [x] Parallel read-only scouts dispatched. Java wiring scout agent_87f52729 COMPLETE: configuration API pinned (ConfigurationRuntimeExpression.selfSubselectPreeval default true; must be set BEFORE EPRuntimeProvider.getRuntime — services init picks MultiMatchHandler once and known URIs return the existing live runtime, so the second runtime needs a distinct URI like ID+"-oracle-no-preeval"); execution byte-exact (EPL1 doubled @name kept verbatim per sibling convention; both statements' EPLs identical to the oracle's EPL_PREEVAL_ONE/TWO constants); oracle registers the real SupportBean bean class so the full 20-property select-* row materializes (expected record JSONs pinned incl. charPrimitive \u0000 and primitive zero columns as 0); complete change list for oracle (second Configuration + runCase call-site + validator clone + EXPECTED_RECORDS 47 / EXPECTED_STEPS 177 + DESCRIPTION naming the third source), scenario (three list appends + 5th case entry + 7 steps), script (12 scenario-gate constants + records 45→47 + [45:47] slice checks). Go engine design scout agent_9e1314e4 in flight.
- [x] Freeze contract; engine option (primary) + assets (writer) + runner case. (Writer agent_680ff3fa: chain-B oracle second preeval-off Configuration/URI, 5th case + 7 steps, script gates, Java trace regenerated 45→47 with prefix byte-identical and both new records exact. Primary: WithSelfSubselectPreeval option (querySpec/Query selfSubselectPosteval field, Statement.process deferral under held mu across all non-context exit points, description token, facade regenerated) + validation tests (default silent + off emits-first with deferred acceptance) + blast-radius suite green; runner no-preeval case over the full 20-column SupportBean bean (char NUL decode mirror, case-aware decode), metadata/expectations/description const extended.)
- [x] Manifest, evidence, roadmap, CHANGELOG. (Evidence passing 0 differences, 5 IDs/5 executions, 47 records per side; manifest case +1 ID/name/static/DV/source, notes rewritten; summary dvIds 899 (+1), assoc 3529, referenced 3276, unreferenced 860; roadmap epl 261→260 + supplement; CHANGELOG entry.)
- [x] Gates: affected packages + make check + git diff --check (green; recorded before commit).
- [x] Independent parity review. Reviewer agent_2a37d0ac-f6c8-4961-b20a-a4c38da278ff OVERALL PASS: engine correctness verified line-by-line (default path identical, defer ordering under held mu, all non-context exit points covered, context untouched, shadowed-err audit, snapshots untouched); Java contract, traces/evidence (canonical-bytes prefixes), row-shape fidelity, manifest recomputation (899/3529/3276/860), test adequacy, prose, and ownership all verified; three probes re-derived from Java sources (preeval handler classes + ReturnIfNoMatch empty-set rule; URI isolation; failing-filter ingestion path). Dispositioned findings: P2 selfSubselectPosteval propagation added to PatternQuery.Query (parity with iterableUnbound copy); P3 WithIterableUnbound doc comment restored to its function + facade regenerated; P3 cutOffPointZero prose replaced with property-accurate wording in CHANGELOG/roadmap/PLANS; P3 informational (deferred accept receives the attached variables map — clone, no observable effect) recorded for the ledger; P3 informational gap closed with a new discriminating engine test TestSelfSubselectPreevalOffIngestsFilterFailingEvent (a where-rejected event still enters the unique window posteval). Gates re-run green after the fixes.
- [x] Commit and push (single semantic commit with all checkpoint edits folded in).
