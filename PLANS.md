...
- Updated: 2026-09-01; Draft 4.293 is the active work unit on the pushed Draft 4.292 baseline.
- Baseline: `HEAD` == `origin/master` at `e2f57e69a7dc41495d2d305609cd6a060ebee165` (`feat(resultset): port rollup having iterator`); the worktree is clean before Draft 4.293 implementation.
- Closed predecessor (Draft 4.292): `ResultSetQueryTypeRollupHavingAndOrderBy.java` ordinals 2-3 were differentially verified through `resultset-querytype-rollup-having-iterator` with 8 Java/Go iterator snapshot records per side and zero differences.

## Current work unit
Draft 4.293 is frozen as fixed `ResultSetQueryTypeRollupHavingAndOrderBy.java` ordinals 4-7: `ResultSetQueryTypeOrderByTwoCriteriaAsc{join=false}`, `{join=true}`, `ResultSetQueryTypeUnidirectional`, and `ResultSetQueryTypeOrderByOneCriteriaDesc`. Runtime IDs are `java-runtime-b09e67f84d7426e94834`, `java-runtime-7859d904aebfc874ac11`, `java-runtime-7f3911bcedf70526d5df`, and `java-runtime-2ff9039bfd91c9054507`; shared inventory/static ID is `java-24e47ca92d533e352474`, nested static candidates are `java-715eae421ccf37a5062a` and `java-ba719758f9ea057c8d79`, all flags are empty, and Java source commit `9e1b9f1cc9117fea4bf33ab043762c045d73839c` is read-only. The exact contracts are: ordinals 4/5 use `@Name('s0') select irstream theString as c0, intPrimitive as c1, sum(longPrimitive) as c2 from SupportBean#time_batch(1 sec)` with optional `, SupportBean_S0#lastevent`, `group by rollup(theString, intPrimitive) order by theString, intPrimitive`; ordinal 6 uses `select theString as c0, intPrimitive as c1, sum(longPrimitive) as c2 from SupportBean_S0 unidirectional, SupportBean#keepall group by cube(theString, intPrimitive)`; ordinal 7 uses the time-batch rollup with `order by theString desc`. The scenario pins exact event/time boundaries, silent between-batch behavior, nullable subtotal dimensions and sums after expiry, ascending null-first two-key order, descending one-key order, unidirectional passive retention/transient driver behavior, and irstream new/old row pairing.
Ownership is frozen: primary owns the new parity runner/scenario/tests, `internal/app/parity/run.go`, generated Java/Go traces/evidence, manifest, roadmap, CHANGELOG, and `PLANS.md`; one isolated `parity-asset-worker` owns only the new Java oracle class and launcher under `tools/java-oracle/`. No `internal/esper` semantic change is planned unless differential evidence demonstrates a core defect; shipped Draft 4.292 files remain untouched. Scenario validators must reject duplicate/extra/missing metadata, payload/type errors, wrong case order, wrong virtual-time/event sequencing, and trace row/value/order/time/count mutations. The candidate is intentionally one scenario with four cases because all four executions share the fixed source, typed SupportBean payload, rollup/time-batch observation, and exact order/null contracts while retaining separate runtime IDs.

## Delegation checkpoint
`NextJavaCandidate-2` (`java-oracle-scout`) and `NextGoSurface-3` (`scout`) completed concurrently before implementation. They confirmed no existing manifest mapping for the four runtime IDs, exact Java source behavior, and reusable typed APIs (`TimeBatch(1s)`, `GroupByRollup`, `GroupByCube`, `Sum[int64]`, `WithOldStream`, `OrderBy`, `Ascending`/`Descending`, `JoinMany`/`Unidirectional`, `KeepAll`). Java/Go scout reports are retained at `agent://NextJavaCandidate-2` and `agent://NextGoSurface-3`. The isolated oracle asset writer must not modify shared semantics, manifest/docs, generated traces/evidence, or commit/push; all validation, evidence generation, manifest facts, independent review, commit, and push remain primary-owned.

- Draft 4.293 selection and contract freeze complete; `NextJavaCandidate-2` (`java-oracle-scout`) and `NextGoSurface-3` (`scout`) completed concurrently before implementation. `Draft4293OracleAssets` (`parity-asset-worker`, isolated) is authoring only the new Java oracle class and launcher under `tools/java-oracle/`; primary owns the Go runner/scenario/tests, generated artifacts, manifest/docs, validation, review, commit and push.
- Primary implementation uses existing typed `TimeBatch`, `GroupByRollup`, `GroupByCube`, `Sum`, `WithOldStream`, `OrderBy`, and `JoinMany`/`Unidirectional` surfaces; no `internal/esper` semantic change is planned absent a differential failure.
- Draft 4.293 focused implementation is present: strict scenario/runner validation, four-case typed replay, isolated Java oracle assets, generated Java/Go traces, and differential evidence are tracked. The runner construction defect was fixed by passing both ascending order keys in one variadic `OrderBy(c0, c1)` call; no shared engine change was required.
- Draft 4.293 manifest/doc facts are now updated and `jq empty testdata/compat/capability-manifest.json`, `go test ./internal/compat -run TestCapabilityManifestArtifactValidates -count=1 -timeout 600s`, and `git diff --check` pass. Manifest summary is 584 cases, 582 implemented, 197 differential-verified cases, 728 runtime IDs, 3349 associations, and 3132 referenced runtime IDs.
- Remaining verification: run the complete Draft 4.293 replay/mutation/evidence checks, full local gates, and independent parity review; then commit and push only after all pass.
- Independent parity review `OrderParityReview` completed: PASS, no P0-P2 findings; the only P3 was duplicate `## Delivery` headings, now removed. Review confirmed dispatch, strict scenario/oracle validation, launcher pinning, runtime/static IDs, and focused test coverage.

## Delivery
- Do not commit or push until Draft 4.293 Java and Go traces, zero-difference evidence, mutation rejection, manifest consistency, full local gates, and independent parity review pass. After the semantic commit, verify the pushed ref read-only and do not edit `PLANS.md` only to add its hash.
...
- The pinned Java oracle launcher regenerated `testdata/parity/rollup-grouping-funcs-faf-dedicated.trace.json` from `/root/app/esper` at `9e1b9f1cc9117fea4bf33ab043762c045d73839c`; Java 17.0.20 emitted one FAF record with 12 rows. The regenerated trace is byte-identical to the checked-in trace (SHA-256 `a7a59711b8edb27b1d61c47a0c237e76d72302998f055f53eb4667481c202d14`), and Go replay is byte-identical to the checked-in Go trace.
- Differential evidence is passing with one Java record, one Go record, 12 rows per side, and zero differences. Focused FAF replay/evidence/mutation/malformed-scenario tests and the `TestCapabilityManifestArtifactValidates` manifest test passed; regenerated evidence is byte-identical to checked-in evidence (`83208319e1da0d22ec0f62e6b773702bd82f326b8eba7fe0c22a8960c36e1342`).
- Independent reviewers `ReviewDraft4291` and `ReviewDraft4291-2` returned PASS with no actionable P0-P2/P3 findings. `make check`, full `go test -race ./... -count=1 -timeout 600s`, targeted outer-chain race coverage, shell/JSON/layout checks, and `git diff --check` passed.
- Persisted evidence and manifest checks are complete. Manifest totals are 582 cases, 195 differential-verified cases, 722 differential runtime IDs, and 3,343 runtime associations (3,126 referenced runtimes); the new case is registered as `differential-verified`.

## Delivery

- Draft 4.291 is ready for one semantic commit/push. Git will remain authoritative for the resulting commit identity.
