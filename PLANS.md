...
- Updated: 2026-09-01; Draft 4.295 is fully implemented, independently reviewed PASS, and ready for final local gates and semantic commit.
- Baseline: `HEAD` == `origin/master` at `d6d402287` (`feat(resultset): port sorted no-data-window aggregates`); Draft 4.295 changes are uncommitted and confined to the aggregate window parity surface plus central facts.
- Closed predecessor (Draft 4.294): `ResultSetAggregateSortedMinMaxBy.java` ordinal 6 was differentially verified through `resultset-aggregate-sorted-no-data-window`; review PASS found no behavioral, provenance, evidence, or documentation defects.

## Current work unit
Draft 4.295 is frozen as the three valid table/window-access executions in fixed `ResultSetAggregationMethodWindow.java`, ordinals 1-3: `ResultSetAggregateWindowTableAccess`, `ResultSetAggregateWindowTableIdentWCount`, and `ResultSetAggregateWindowListReference`. The Java source commit `9e1b9f1cc9117fea4bf33ab043762c045d73839c` is read-only. Runtime IDs, in source order, are `java-runtime-785f2999e48fbaa6eb77`, `java-runtime-9ef8f9a367e788b5afec`, and `java-runtime-6652f083e2b0f3dffc04`; static candidates are `java-f881d0116e155d3e9063`, `java-804f7ea3bb20de0d35e8`, and `java-f1014305fc8b701e8ea0`; all flags are empty; shared collection inventory ID is `java-313287657d14b5c686f8`. The scenario ID is `resultset-aggregate-window`, with table-access, table-ident-count, and table-list-reference cases; each fresh Go runtime reproduces the Java listener rows, including Null first/last on the empty table, length(2) FIFO eviction, scalar property/count access, and ordered list reference.

## Delegation checkpoint
`ReviewDraft4294-3`, `ScoutNextJava4295`, and `NextGoSurface-4` completed concurrently before this freeze. `WindowRunnerReview` independently reviewed the typed runner; primary fixed its trigger payload binding defect. `ReviewDraft4295` first found missing checked-in assets, which primary added. `ReviewDraft4295Final` then returned PASS with no remaining P0/P1/P2 correctness or evidence-integrity findings; its earlier launcher finding was withdrawn after confirming the explicit `--esper-root` contract. Primary owns generated trace/evidence, central facts, validation, commit and push.
Implementation is complete in `internal/app/parity/resultset_aggregate_window.go` and `internal/app/parity/run.go`. The checked-in Java and Go traces each contain six listener records, the evidence is passing with zero differences, and the checked-in Go trace is structurally asserted and compared to both evidence and current replay. Manifest, roadmap, and CHANGELOG register the three runtimes and totals 586 cases, 584 implemented, 199 differential-verified, 732 differential runtime IDs, and 3353 runtime associations (3133 referenced). Focused replay, mutation, malformed-scenario, checked-in-evidence, manifest, compatibility, help, trace identity, and `git diff --check` checks pass.

## Delivery
- Run the complete local gates and read-only post-generation checks; then create one semantic commit and push to `master`. Do not write the new commit hash into tracked files afterward.
