...
- Current target: Draft 4.339 'insert-into-charsequence-factory' — EPLInsertIntoPopulateUnderlying ords 7+8; contracts already pinned by PopulateUnderlyingJavaContract (agent_fae76b58) + PopulateUnderlyingGoSurface (agent_8cd5b2e7, both READY).
...
## Current work unit
Active: Draft 4.339 ('insert-into-charsequence-factory') — extends the epl-insert-into-populate-underlying chain with ords 7 and 8.
...
- ord 7 EPLInsertIntoCharSequenceCompat `java-runtime-f04a53b7cbe0320f681f` static `java-2c966eacbdd140219337`: zero-event deploy smoke — for each representation (Java: OBJECTARRAY/MAP/AVRO/DEFAULT, JSON skipped) `<rep>create schema ConcreteType as (value CharSequence)` + `insert into ConcreteType select "Test" as value from SupportBean` must compile AND deploy. Go: string-typed field receives the literal in every representation the Go engine registers (map/objectarray/json = superset of the Java reps Go can model; Go has no Avro). Oracle: cases with no sends produce no records — the validation is compile+deploy success (oracle exit 0).
- ord 8 EPLInsertIntoBeanFactoryMethod `java-runtime-e79b48b60120e27bafbc` static `java-7444c102966e032f2884`: (1) `insert into SupportBeanString select 'abc' as theString from MyMap` + listener AND subscriber → send empty MyMap → theString=abc (Go: one Subscribe observes the same routed row — observable equivalent of the listener+subscriber pair); (2) `insert into SupportSensorEvent(id, type, device, measurement, confidence) select 2, 'A01', 'DHC1000', 100, 5 from MyMap` → id=2, type=A01, device=DHC1000, measurement=100.0, confidence=5.0 (int literal widens to double). Java's two reflection sanity checks (factory-configured classes must have no usable default ctor) are Java-reflection introspection of support classes, not engine behavior — deferred as non-engine (approved difference; the Go factory origin is plain struct registration, observably identical population).
- Deferrals: ords 9/10 (until-pattern per-representation array population; invalid halves implemented-only) → next unit; ord 12 (invalidity implemented-only Build-error unit restricted to reproducible sub-cases).

- [x] Freeze contract (4.338 scouts; no independent read-only task remains — delegation-gate rationale recorded).
- [x] Asset writer: 5 cases appended (agent_3122d135; charsequence ×3 representations with zero sends, factory string+sensor; trace 26 records = 24 prior byte-identical + 2 new, double-run byte-identical; FQN correction — the factory classes live in regressionlib.support.bean).
- [x] Primary: 2 parity tests (5 subtests; all PASS — two build fixes: RegisterMap/RegisterObjectArray are package-level, FromAny uses the .Select method form) + evidence extension.
- [x] Manifest (+2 runtime IDs/names/goTests, notes — class 10/13 DV), summary 867→869 DV runtime IDs / associations 3490→3492 / referenced 3249→3251 / unreferenced 887→885.
- [x] Roadmap supplement + epl 282→280 (5.2/6.1 and section-4 prose); CHANGELOG 4.339 entry.
- [x] Gates + independent parity review (agent_de9cf9c7-5ea1-4a6f-b66a-eac650ad22d8): OVERALL PASS on all five dimensions with an independent oracle rerun byte-identical to the checked-in trace and manifest/domain arithmetic recomputed-exact; one P2 (caseMatchSummary missing the 5 new cases) and three P3s (imprecise test doc-comments, PLANS stale 4.338 tail on the Current-target line, lagging checkbox) all fixed in the same work unit and re-confirmed by the same reviewer.
