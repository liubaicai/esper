...
activity or a single coverage percentage.
 ...
 - Updated: 2026-08-29
 - Baseline: `HEAD` == `origin/master` at Draft 4.278 (`07b0deb6e6f8be4b10ae6426867ba9cc3d1692d1`), pushed; worktree was clean before Draft 4.279.
 - Closed unit (Draft 4.279, pending commit): `ResultSetAggregateMedianAndDeviation.java` ordinals 0-2 were differentially replayed with 21 Java/Go records and zero differences; persistent StdDev enter/leave/clone repair, NaN poisoning coverage, strict root/case metadata loading, duplicate-key rejection, and raw scenario mutation tests all pass. Independent reviewer `ReviewDraft4279-2` returned PASS with no P0-P2 findings.
 - Closed unit (Draft 4.278, commit `07b0deb6e`): `ResultSetAggregateMaxMinGroupBy.java` ordinals 3 and 5 were differential-verified with 14 Java/Go records and zero differences; reviewer `ReviewDraft4278` returned PASS.
 - Closed unit (Draft 4.277, commit `325073e04`): the same suite's ordinals 1 and 2 were differential-verified with 24 Java/Go records and zero differences; reviewer `ReviewDraft4277` returned PASS.
 - Closed unit (Draft 4.276, commit `ef04f09cb`): the same suite's ordinals 0 and 4 were differential-verified with 14 Java/Go records and zero differences; reviewer `ReviewMinMaxParity` returned PASS.

## Current work unit
Draft 4.279: extend `ResultSetAggregateMedianAndDeviation.java` with ordinals 0-2: `ResultSetAggregateStmt`, `ResultSetAggregateStmtJoinOM`, and `ResultSetAggregateStmtJoin` (static candidates `java-b11b233b0ea7da05217f`, `java-66a516ce6e14456627b2`, `java-a5c65b307ecdb2d0355b`; runtime IDs `java-runtime-101db664ea346721e986`, `java-runtime-2f533a8a1bc0d93ae649`, and `java-runtime-2667a7eadeb7a458d7ce`). The fixed Java oracle is `/root/app/esper` at commit `9e1b9f1cc9117fea4bf33ab043762c045d73839c`.
Candidate selection is evidence-backed by `resultset.aggregate-group-by.remaining` and the completed Java/Go scout reports. Java contract: ordinal 0 uses filtered `SupportMarketDataBean#length(5)` grouped `irstream` `median(all price)`, `median(distinct price)`, `stddev(all price)`, and `avedev(all price)` over DELL/IBM/GE; it then redeploys a separate ungrouped `stddev(price)` `length(3)` statement and records six new-only callbacks for NaN sensitivity. Ordinals 1-2 use SupportBeanString DELL/IBM/AAA seeds and grouped `length(5)` join variants with the same four aggregates; OM additionally copies and serializes the SODA model and asserts exact EPL. The shared DELL sequence is 10, 20, 20, 90, 5, 90, 30, with seven old/new callback pairs and exact aggregate values. Go contract: use typed market `{Symbol string, Price float64, Volume int64}` and string `{TheString string}`; build grouped `Median`, `DistinctAggregate(Median)`, `StdDev`, and `Avedev` selections, filter symbols, and model both join executions with typed `Join`/`JoinField` and length windows. The NaN phase uses a separate typed statement and must preserve NaN output after three NaN events plus 1, 2, 3. The shared trace protocol cannot JSON-encode NaN, so the differential scenario covers the finite aggregate/join cases; the NaN behavior remains a focused Go semantic assertion in the runner tests, not fabricated JSON evidence. The shared `StdDev` implementation now uses the Java `AggregatorStddev` Welford update order (`mean += (p-oldmean)/cnt`, then `qn += (p-oldmean)*(p-mean)`), preserving Java's one-ulp result for the finite sequence. The median runner also pins every scenario root/case/step field, rejects duplicate keys and trailing JSON, and compares the same strict metadata as the Java oracle.
Delegation checkpoint: Java and Go read-only scouts froze runtime IDs, source contracts, callback counts, NaN/null semantics, and file ownership. Writes were disjoint: the Go worker owned runner/dispatch/tests and the parity-asset worker owned the Java oracle launcher and scenario JSON. Neither wrote generated traces/evidence or central manifest/docs.

Required gates for Draft 4.279 passed: focused typed aggregate tests, fixed-Java oracle replay, Go replay, zero-difference evidence, four trace mutations, malformed and raw scenario metadata mutations, checked-in evidence replay, manifest/evidence/document consistency, `go vet ./...`, `go test ./... -count=1 -timeout 240s`, `make check`, layout, diff checks, independent parity review, and strict loader follow-up tests.
## Handoff
After Draft 4.279 delivery, verify the pushed ref read-only and select the next roadmap/manifest work unit; never add a checkpoint-only hash update.
