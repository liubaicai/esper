...
- Current target: Draft 4.371 'dataflow-captive-lifecycle' — upgrade case.dataflow-captive-emitter-ports (implemented-not-DV, 2 rts) to DV via the captive lifecycle chain: EPLDataflowAPIStartCaptive — 1 execution (java-runtime-407e42478546e709d774, the whole class is one direct execution); FanInOut (java-runtime-6309a6b7e0f0ba981a97) stays implemented-not-differential. Follow-up queue: 4.372 FanInOut + Factorial (ports/feedback); dataflow-lifecycle (concurrency-heavy, 18 rts); DV-list consolidation; database slices; perf doc §4.9 queue (incremental join first).
