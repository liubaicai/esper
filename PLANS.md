...
- Shipped: Draft 4.327 ('infra-nwtable-subq-filtered-correl') committed; Git owns identity.
- Current target: Draft 4.328 candidate 'infra-nwtable-on-update' multikey-with-array pair — contract freeze in progress from prefetch scouts (Java scout complete, Go surface scout adjudication pending).

## Current work unit
Active: Draft 4.328 candidate ('infra-nwtable-on-update') — on-update multikey-with-array pair of InfraNamedWindowOnUpdate.java. Prefetch status: OnUpdateJavaContract (agent_480e7832-b141-4b55-a5d2-f16eebcc1493) complete — recommends ordinals 6+7 as ONE closed loop: InfraUpdateMultikeyWArrayPrimitiveArray (java-runtime-9870ee394d3099ef85e8, static java-0b7af7284d24833e1753) and InfraUpdateMultikeyWArrayTwoFields (java-runtime-a25a18f2754aeae08015, static java-cdea340e2ff2cabfd0f3), both flags []; shared runtime surface (SupportEventWithManyArray{int[] intOne} window + SupportEventWithIntArray trigger, keepall create + insert + on-update with array-equality where), iterator-only assertions (assertPropsPerRowIteratorAnyOrder on create, no listeners), int[] deep-equality matching including empty-array match, no-match rows untouched; expected states after triggers: ord6 [[E1,13],[E2,10],[E3,11],[E4,12]], ord7 [[ID1,12],[ID2,10],[ID3,11]] with two non-matching triggers. milestone(0) mid-timeline is a harness no-op for event flow. Second queue: ordinals 1+2 (intersect/union pair, pins UPDATE old/new listener streams). Deferred: ord 0 (plugin SRO + method-call set clauses), ord 3 (abstract subclass typing), ord 4 (copy-method config + SERDEREQUIRED), ord 5 (wrapper select + extra property). OnUpdateGoSurface (agent_79ed2dad-cd95-4dd8-b0b6-c1a7ecc4d486) adjudication pending — no 4.328 writes before its verdict and before 4.327 is committed.

- [ ] Freeze on-update multikey contract (Go surface adjudication: UpdateNamedWindow builder, set-clause assignment surface, array-equality where, iterator/snapshot op needs).
- [ ] Author oracle, script, and scenario via asset writer.
- [ ] Implement runner, modes, and 6-test family.
- [ ] Generate traces and zero-difference evidence.
- [ ] Update manifest, roadmap, and CHANGELOG.
- [ ] Run gates and independent parity review.
- [ ] Commit and push the unit (Git owns identity; no hash recorded here).
...
Draft 4.328 prefetch (in progress, read-only): scouts 'OnUpdateJavaContract' (agent_480e7832-b141-4b55-a5d2-f16eebcc1493, complete) and 'OnUpdateGoSurface' (agent_79ed2dad-cd95-4dd8-b0b6-c1a7ecc4d486, pending) launched concurrently with the 4.327 review. No 4.328 writes before 4.327 is committed and the Go-surface adjudication freezes the contract.

Prior 4.327 unit: prefetch scouts 'FilteredCorrelJavaContract' (`agent_9cd7d0cb-d1c5-4f9f-8110-d3df3f13b291`) + 'FilteredCorrelGoSurface' (`agent_57aa3160-d458-440e-ad67-1e225252dd04`) ran concurrently before implementation; asset writer `agent_3667f273-434c-4eaf-b4aa-db55769db5a1` authored the oracle/script/scenario trio on disjoint files; primary owned the runner, run.go wiring, run_test.go family, traces/evidence, manifest, roadmap, CHANGELOG, PLANS.md, gates, review, commit, and push, and fixed the oracle S0 send-type mapping during bring-up. Independent parity review `FilteredCorrelReview` (`agent_6761c0e0-3c0e-4c49-9eab-69cd92229352`) returned OVERALL PASS on all five dimensions; the single P2 (stale "80 steps") was fixed and re-confirmed by the same reviewer.
