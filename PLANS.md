...
- Updated: 2026-08-29
- Baseline: `HEAD` == `origin/master` at Draft 4.280 (`019eb73fdc5e10eb660ce0991364d3a64702e257`), pushed; worktree clean before Draft 4.281.
- Closed unit (Draft 4.280; validation and independent review complete): `ResultSetAggregateMinMax.java` ordinal 0, `ResultSetAggregateMinMaxNoDataWindowSubquery` was differentially verified with 4 Java/Go records and zero differences. The typed unbounded `Max`/`Min` plus `SubqueryValue` over `LastEvent` contract covers silent S0-only updates, empty scalar Null state, exact event order, fixed metadata, strict malformed/duplicate/payload validation, and value/order/time/count mutations; independent parity reviewer `ReviewDraft4280` returned PASS.
...
## Current work unit
Draft 4.281: port `ResultSetAggregateMinMax.java` ordinal 2 and ordinal 3, `ResultSetAggregateMinMaxNamedWindowWEver{soda=false}` and `{soda=true}`. Shared static candidate is `java-09115f6e876ce88a42f9`; execution inventory ID is `java-09115f6e876ce88a42f9`; runtimes in source order are `java-runtime-7d0a94525b0038b397fd` and `java-runtime-840f1ca5610dd00d3ec0`; static flags are `EXCLUDEWHENINSTRUMENTED` while runtime rows carry no flags. The fixed Java oracle is `/root/app/esper` at commit `9e1b9f1cc9117fea4bf33ab043762c045d73839c`.
Java contract is setup `@public create window NamedWindow5m#length(2) as select * from SupportBean; insert into NamedWindow5m select * from SupportBean;`, followed by `@name('s0') select min(intPrimitive) as lower, max(intPrimitive) as upper, minever(intPrimitive) as lowerever, maxever(intPrimitive) as upperever from NamedWindow5m`. Each fresh case sends `SupportBean(null,1)`, `5`, `3`, `6` and yields four new-only rows `[1,1,1,1]`, `[1,5,1,5]`, `[3,5,1,5]`, `[3,6,1,6]`; length eviction changes current extrema while ever extrema retain 1. SODA true and EPL false differ only in Java representation path.
Go contract uses a typed `SupportBean`, registered named-window schema with `NamedWindowRetention(LengthWindow(2))`, `InsertIntoNamedWindow`/`CopyMatchingFields`, `FromNamedWindowAs[SupportBean]`, and `Min`/`Max`/`MinEver`/`MaxEver` in a listener query. Both cases use fresh environments and their pinned runtime IDs; strict scenario, payload, metadata, static flags, trace mutation, and checked-in evidence validation is required.
Delegation checkpoint: read-only Java scout `MinMaxNamedWindowJava` and Go scout `MinMaxNamedWindowGo` matched source/runtime/static identity and observable behavior; no shared `internal/esper` semantic gap. Disjoint writes were assigned to `MinMaxNamedWindowGo-2` (`internal/app/parity` runner/dispatch/tests) and `MinMaxNamedWindowAssets` (Java oracle/launcher/scenario); primary owns generated traces/evidence, manifest, roadmap, CHANGELOG, README, validation, review, commit, and push. Independent reviewer `ReviewDraft4281-2` found and the primary repaired a static-candidate flag provenance defect before final verification.

## Handoff
After Draft 4.281 delivery, verify the pushed ref read-only and select the next manifest/roadmap work unit; never add a checkpoint-only hash update.
