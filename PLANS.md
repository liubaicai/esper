...
- Current target: Draft 4.372 'dataflow-ports-feedback' — complete BOTH case.dataflow-captive-emitter-ports (add FanInOut java-runtime-6309a6b7e0f0ba981a97 to its dv list) and case.dataflow-feedback (Factorial java-runtime-98c5bdc6afa84705b9db + LargeNumOps java-runtime-bddc8c72bd9d2a2c8869) to DV via the ports/feedback chain over all three executions of EPLDataflowInputOutputVariations.java. Follow-up queue: dataflow-lifecycle (concurrency-heavy, 18 rts); dataflow-exceptions/invalid-graph/typed-ports/custom-properties dispositions; DV-list consolidation; database slices; perf doc §4.9 queue (incremental join first).
