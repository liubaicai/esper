...
- Current target: Draft 4.342 'event-precedence-contained-outputrate' — EPLInsertIntoEventPrecedence ords 4+9; contract frozen from EventPrecedenceJavaContract (agent_53742bea) + EventPrecedenceGoSurface (agent_5ec65a8f: both READY zero engine work).
...
## Current work unit
Active: Draft 4.342 ('event-precedence-contained-outputrate') — extends the shipped insertinto-event-precedence chain (case.insertinto-event-precedence, ords 0-3 DV; harness: tools/java-oracle/EPLInsertIntoEventPrecedenceScenarioOracle.java + testdata/parity/insertinto-event-precedence.{json,trace.json,evidence.json} + internal/esper/epl_insert_into_event_precedence_parity_test.go) with ords 4 and 9.

Frozen contract (from the Java source plus both scouts; zero engine work):
- ord 4 NonConstInsertIntoContainedEvent `java-runtime-e99c72ba6838bd3b23f2` static `java-54f031b12e38a8768102`: `@public @buseventtype create schema LvlA as <class>; insert into LvlB event-precedence(precedence) select * from LvlA[b]; insert into LvlC event-precedence(precedence) select * from LvlB[c]; @name('s0') select id from LvlC;` — 5 LvlA sends with flattened LvlC id orders pinned by the Java assertions (writer transcribes the exact constructions from the source). Go: Unnest(From[LvlA], Property[[]LvlB](EventValue, "b")) + zero-selection Select routes the child event; EventPrecedence(Field precedence) evaluated against the routed target event; chained routes interleave in the shared precedence queue (runtime.go:4735-4830, 4887-4915).
- ord 9 ConstantInsertIntoOutputRate `java-runtime-aa3e3052e879188342c6` static `java-facdb3c80027dcca2998`: three `insert into Out event-precedence(...) select 1 + intPrimitive * 10 as id from SupportBean output every 2 events` with precedences 1, 2, and computeEventPrecedence(3, *) (static fn over the routed output event; Go collapses to Literal(3) — behaviorally identical, approved adaptation). Sends E1(intPrimitive=1), E2(2) → one batch of 6 rows {13},{23},{12},{22},{11},{21} (precedence 3 first, then 2, then 1; FIFO within). Go: OutputEvery(2) on the three routes (OutputEveryPolicy runtime.go:9297-9349, routes queued from the post-policy batch).
- ord 10 sub-case 5 rider (implemented-only): FAF + event-precedence rejected — Go faf.go already emits "fire-and-forget routes do not allow event-precedence" (route_test.go:1276 precedent); Java message differs and message parity is not required.
- Deferrals: ords 5-8 (subquery-driven precedence) → future ENGINE unit with three concrete gaps: visitQueryExpressions never registers eventPrecedence/split-branch/merge-action precedence subqueries (plan.go:3464-3712); evaluatePrecedenceExpr lacks variables so subquery lookups silently degrade to precedence 0 (runtime.go:4797, split_stream.go:357-358 drops variables; merge path correct); TriggerQuery.Query drops spec.eventPrecedence (trigger.go:676-711). ord 10 remainder: sub-cases 1-4 (no Build-time precedence-expression validation) ride with that engine unit; sub-cases 6-7 (table-target precedence; reserved-keyword parse error) are no-Go-rejection-surface dispositions.

- [x] Freeze contract (parallel prefetch scouts complete).
- [x] Asset writer: 2 cases appended (agent_0bcce3d2; trace 57 records = 21 prior byte-identical + 36 new, double-run byte-identical; oracle registers mirror LvlA/LvlB/LvlC classes and local computeEventPrecedence because the script classpath excludes regression-lib — shapes/semantics replicate the suite).
- [x] Primary: 2 parity tests (contained chain + output rate; all 6 precedence tests PASS — fixes: explicit Unnest type args, per-statement id offsets 1/2/3 which the first draft unified making the assertion unobservable) + evidence extension.
- [x] Manifest (+2 runtime IDs/names/goTests, notes — class 6/11 DV), summary 871→873 DV runtime IDs / associations 3495→3497 / referenced 3254→3256 / unreferenced 882→880.
- [ ] Roadmap supplement + epl 277→275 (5.2/6.1/section-4); CHANGELOG 4.342 entry.
- [x] Gates + independent parity review (agent_9c249370-a057-4c74-99dc-364b1ab4fc6c): OVERALL PASS on all five dimensions with an independent oracle rerun byte-identical to the checked-in trace and manifest/domain arithmetic recomputed-exact (epl 275/infra 156/event 151/expr 95 matching the tables); the only finding was a P3 (evidence file re-indented 1-space vs the 2-space sibling convention) fixed pre-commit by re-normalizing to 2-space.
