...
- Updated: 2026-08-28
- Baseline: `HEAD` == `origin/master` at Draft 4.271 (`a3aba2c85`), pushed. The grouped output-last Nth unit is committed; the sorted min/max-by unit is now active.
...
## Current work unit
Draft 4.272: `ResultSetAggregateSortedMinMaxBy.java`, executions `ResultSetAggregateGroupedSortedMinMax`, `ResultSetAggregateMultipleOverlappingCategories`, and `ResultSetAggregateMinByMaxByOverWindow` (ordinals 0-2, source inventory `java-553516b9d01c12a13172`). The fixed Java oracle is `/root/app/esper` at commit `9e1b9f1cc9117fea4bf33ab043762c045d73839c`. Runtime IDs are `java-runtime-f1f2e402fb4c6783cf33`, `java-runtime-7387fafedcfa741088b9`, and `java-runtime-5609d311148a91227b28`, in execution order. The contract covers grouped `#groupwin(longPrimitive)#length(3)` current window/sorted/minby/maxby plus ever extrema, overlapping current/ever categories with equal-key identity, and `#length(5)` current-vs-ever extrema with projected event fields through FIFO eviction. No timer, null, old-stream, or invalid path is in scope.

Delegation checkpoint: `NextAggregateJava-3` and `NextAggregateGo-3` completed read-only scouts before writes. They froze the source lines, runtime IDs, deterministic event sequences, typed API paths (`GroupWindow`, `SortedEvents`, `MinBy`/`MaxBy`, `MinByEver`/`MaxByEver`, `EventValue`, `Property`), and no required shared-core change. The singleton `SortedMinMaxAssets` parity-asset worker was hard-aborted before producing files; no safe file-disjoint sibling remained, so the primary owned the bounded asset files. Independent reviewer `ReviewSortedMinMax` first found P2s in shell numeric validation and duplicate JSON object-key handling. The primary repaired both validators: Python keeps lexical integers, rejects float/constants/out-of-range values and duplicate keys; Go rejects duplicate fields and trailing JSON with a token decoder. The final re-review returned PASS with no remaining P0-P2 findings.
Replay, facts, and review are complete: the pinned Java oracle regenerated `testdata/parity/resultset-aggregate-sorted-minmax-by.trace.json` with 25 records; a fresh `--skip-build` oracle replay was byte-identical; Go differential replay wrote matching evidence with 25/25 records and zero differences; focused `go test ./internal/app/parity ./internal/compat -count=1` passed after the duplicate-key repair; and manifest registration records the case, three runtime IDs, evidence paths, and derived counts. Full `go vet ./...`, `go test ./... -count=1 -timeout 240s`, `go test ./internal/compat/... -count=1`, layout/diff checks, runner syntax/manifest checks, and the pinned-oracle smoke replay all passed. Independent `ReviewSortedMinMax` final re-review returned PASS with no P0-P2 findings. The unit is ready for its semantic commit and push.
Delivery: complete. Commit and direct push to `master` are the remaining actions; do not edit tracked files afterward except for read-only ref verification.
## Handoff
