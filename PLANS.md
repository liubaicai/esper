...
- Current target: Draft 4.370 'dataflow-eventbus-sink' — complete case.dataflow-eventbus-collector (implemented-not-DV, 5 rts) to DV via the eventbus sink family: EPLDataflowOpEventBusSink.java {AllTypes, Beacon, SendEventDynamicType} — 3 executions; the case's 2 EventBusSource IDs are DV via 4.369's chain and Beacon via case.dataflow-connector-output, so the per-case dv list carries all 5 IDs with the cross-chain split documented. Follow-up queue: dataflow-lifecycle (concurrency-heavy, 18 rts); DV-list consolidation; database slices; perf doc §4.9 queue (incremental join first).
