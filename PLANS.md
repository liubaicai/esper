...
- Updated: 2026-08-28
- Baseline: `HEAD` == `origin/master` at Draft 4.270 (`b6d4c7eda`), pushed. The indexed first/last unit is committed; the grouped output-last Nth unit is now being closed.
...
- Closed unit (Draft 4.270, commit `b6d4c7eda`): `ResultSetAggregateFirstLastWindow.java` indexed `ResultSetAggregateFirstLastIndexed` was differential-verified with 4 Java/Go records and zero differences; indexed First/Last null, bounds, and FIFO-eviction mutations were rejected.
## Current work unit
Draft 4.271: `ResultSetAggregateNTh.java`, direct execution `ResultSetAggregateNTh` (ordinal 0, source inventory `java-a8b86711394ba51ed0a9`, runtime `java-runtime-1a257602734874ff3fc4`). The fixed Java oracle is `/root/app/esper` at commit `9e1b9f1cc9117fea4bf33ab043762c045d73839c`. Contract is `@name('s0') select theString, nth(intPrimitive,0) as int1, nth(intPrimitive,1) as int2 from SupportBean#keepall group by theString output last every 3 events order by theString`; each lifecycle phase sends G1/10,G2/11,G1/12; G2/30,G2/20,G2/25; G1/-1,G1/-2,G2/8, yielding three new-only ordered batches. Java repeats the valid statement through compileDeploy and eplToModelCompileDeploy after undeploy, so the pinned direct trace contains six listener records total. The invalid `nth()` diagnostic remains outside typed parity.
Delegation checkpoint: `NextNthJavaContract` and `NextNthGoSurface` completed read-only scouts before writes; `NthParityAssets` authored disjoint oracle/scenario/Go-unit assets; `NthParityReview` independently reviewed the integrated tree and returned strict PASS after the exact scenario-shape validator and malformed-shape tests were added. `NextAggregateJava-3` and `NextAggregateGo-3` completed the one allowed read-only prefetch for `ResultSetAggregateSortedMinMaxBy` ordinals 0-2; no future writes started. No shared semantic production writer was needed.
## Progress
Contract, implementation, pinned Java trace, Go replay, and focused validation are complete. The runner now rejects any Nth scenario that does not exactly match the ten-step SupportBean shape accepted by the Java shell/oracle validator; malformed missing-field, extra-field, non-integer, and extra-case tests pass.
- Validation completed: pinned Java regeneration is byte-identical to the checked-in trace; Nth differential replay writes passing evidence with six Java/six Go records and zero differences; focused internal/esper and parity tests, compatibility manifest tests, `go vet ./...`, and the full `make check` gate pass.
## Delivery
Required remaining before commit: final diff review, then one semantic commit and push. The manifest, README, roadmap, CHANGELOG, and Java-oracle runner documentation are updated to the validated Nth facts before delivery.
- Prefetch queue (read-only): fixed Java `ResultSetAggregateSortedMinMaxBy.java` ordinals 0-2 (`ResultSetAggregateGroupedSortedMinMax`, `ResultSetAggregateMultipleOverlappingCategories`, `ResultSetAggregateMinByMaxByOverWindow`) with runtime IDs `java-runtime-f1f2e402fb4c6783cf33`, `java-runtime-7387fafedcfa741088b9`, and `java-runtime-5609d311148a91227b28`; `NextAggregateJava-3` and `NextAggregateGo-3` froze the contract and typed Go surface. Do not start writes until this Nth unit is committed.
