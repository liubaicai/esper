...
- Updated: 2026-09-03; Draft 4.307 (`resultset-output-limit-row-limit-context-grouped`) is the active work unit. It targets fixed Java `ResultSetOutputLimitRowLimit.java` ordinals 0 and 4: order-optimized limit-one batch/context termination and fully grouped ordered aggregate iterator behavior.
- Baseline: Draft 4.306 (`resultset-output-limit-row-limit`) was committed and pushed at `ea50747a2`; the worktree was clean after the verified two-runtime row-limit slice. Its runtimes `java-runtime-840e283d8ac639591083` and `java-runtime-20d0d1451ce549bf53b5` remain recorded with zero-difference evidence.
- Draft 4.307 Java contract: ordinal 0 runtime `java-runtime-0a4f187046b734b5dc9a`, static ID `java-3f5845fdac39997e9b3d`, execution `ResultSetLimitOneWithOrderOptimization`; ordinal 4 runtime `java-runtime-19d52dc587ea246a3e07`, static ID `java-bf49a03f52cc732a2cf6`, execution `ResultSetFullyGroupedOrdered`; both use fixed Java commit `9e1b9f1cc9117fea4bf33ab043762c045d73839c`, source `regression-lib/src/main/java/com/espertech/esper/regressionlib/suite/resultset/outputlimit/ResultSetOutputLimitRowLimit.java`, flags empty, and shared inventory ID `java-0109d52ee4e8b36575b9`.
## Current work unit
Ordinal 0 covers both non-context `length_batch(10)`/`length_batch(5)` order-and-limit checks and `StartS0EndS1` keepall statements using `output snapshot when terminated`; no listener output occurs before S1, each context termination emits the sorted limited row, and a subsequent S0/S1 cycle proves state reset. Single-key output is A, B, C; multi-key output is F/10, G/12, G/11. Ordinal 4 covers `length(5)` grouped `sum(intPrimitive)` ordered by the aggregate with limit 2, asserting iterator snapshots [E1/90], [E2/5,E1/90], [E2/5,E3/60], [E2/5,E1/90], [E1/90,E3/100].
The typed Go implementation reuses `CreateInitiatedTerminatedContext`, `WithContext`, `KeepAll`, `OutputSnapshotWhenTerminated`, `LengthBatch`, `LengthWindow`, `GroupBy`, `Sum`, `OrderBy`, `Limit`, listener replay, and iterator snapshots. The scenario/oracle validators pin Java metadata, lifecycle boundaries, payload order, listener and iterator records, aggregate replacement/eviction, and value/order/state/time/record-count mutations.
## Delegation checkpoint
Draft 4.307 contract was frozen by completed read-only scouts `NextOutputJavaScout` (`java-oracle-scout`) and `NextOutputGoScout` (`scout`). `RowLimitNextAssets` (`parity-asset-worker`) owns only the new Java oracle, shell wrapper, and scenario JSON; `RowLimitNextCore` (`go-slice-worker`) owns only the new disjoint `internal/esper/resultset_output_limit_row_limit_context_grouped_parity_test.go`. Primary owns the existing parity runner extension and integration tests, generated traces/evidence, manifest, roadmap, CHANGELOG, PLANS, validation, review, commit, and push. Ordinals 7, 8, and 9 remain separate candidates because they require negative-limit timer behavior, compile-invalid diagnostics, and variable-backed limit/offset semantics.
- Targeted validation and differential evidence for Draft 4.307: Java oracle generation via `tools/java-oracle/run-resultset-output-limit-row-limit-context-grouped.sh --esper-root /root/app/esper --scenario testdata/parity/resultset-output-limit-row-limit-context-grouped.json --output testdata/parity/resultset-output-limit-row-limit-context-grouped.trace.json --skip-build`; Go replay and diff both pass with 30 records per side and zero differences; affected Go and runner tests pass, including 5 trace mutations and malformed-scenario coverage. Manifest summary and roadmap/CHANGELOG facts now record 598 cases, 211 differential-verified cases, 765 differential runtime IDs, and 3386 associations.
- Independent parity review `GroupedParityReview`: PASS, no P0/P1/P2 defects. Optional P3 hardening was applied: iterator-only ordinal 4 now rejects unexpected listener records rather than filtering them silently. Per-phase ordinal-0 runtime URIs remain suffixed for deployment isolation; normalized traces do not expose them and metadata remains pinned to Java IDs.
- Full local gates: `make check` PASS (`check-layout`, `go vet ./...`, `go test ./... -count=1 -timeout 240s`); `make test-race` PASS (`go test -race ./... -count=1 -timeout 600s`, including internal/app/parity and internal/esper). The independent review is PASS; Draft 4.307 is ready for one semantic commit and push.
## Next work unit
After this semantic commit, continue `ResultSetOutputLimitRowLimit.java` ordinal 7 negative rowcount; keep ordinals 8/9 invalid and variable semantics separate.
## Prior outcomes
