...
- Updated: 2026-09-01; Draft 4.296 is fully implemented, differentially verified, independently reviewed PASS, and all required local gates pass; ready for one semantic commit/push.
- Baseline: `HEAD` == `origin/master` at `cb4fce9034eae9fce7f947f1bb98d48a9df318e` (`feat(resultset): port aggregate window access`); Draft 4.296 is the scoped uncommitted semantic delta.
- Closed predecessor (Draft 4.295): `ResultSetAggregationMethodWindow.java` ordinals 1-3 were differentially verified through `resultset-aggregate-window`; independent review PASS found no behavioral or evidence-integrity defects.

## Current work unit
Draft 4.296 is frozen as the three valid table-backed sorted-access executions in fixed `ResultSetAggregationMethodSorted.java`, ordinals 7-9: `ResultSetAggregateSortedGetContainsCounts`, `ResultSetAggregateSortedSubmapEventsBetween`, and `ResultSetAggregateSortedNavigableMapReference`. The Java source commit `9e1b9f1cc9117fea4bf33ab043762c045d73839c` is read-only. Runtime IDs, in source order, are `java-runtime-d7d60fec056d6eb16659`, `java-runtime-0c0e45c751e3cafeb454`, and `java-runtime-2117ba8232651a61a058`; static candidates are `java-ad219f9a27aebc7885fc`, `java-4b989f8ba297c8e8d426`, and `java-db7b9e752ef9e4000cc5`; all flags are empty; shared collection inventory ID is `java-0ded2b32c81677a9d36f`. The scenario ID is `resultset-aggregate-sorted-table-access`; its three case markers replay 7 seed events plus 12 get probes, 7 seed events plus 312 inclusive/exclusive submap probes, and 7 seed events plus one navigable-map probe. The typed Go surface uses `SortedAccessValue[int, esper.Event]` in a public table, `Method` calls for table-backed access, and `EnumFirstOf`/`EnumLastOf` for chained collection access.

## Delegation checkpoint
The archived pre-freeze batch ran `NextJavaResultset` (`java-oracle-scout`) and `NextGoResultset` (`scout`) concurrently; both froze the exact ordinals, runtime/static IDs, shared inventory ID, seed vector, and observable contracts, and both found the existing `SortedAccessValue`/`SortedAccessExpression` API sufficient, so no `internal/esper` writer was authorized. The independent review ran as `SortedAccessReview-2` (`parity-reviewer`) and returned PASS with no P0-P2 findings or evidence-integrity defects. The one-unit N+1 prefetch ran concurrently as `NextSortedJava-3` (`java-oracle-scout`) and `NextSortedGo-4` (`scout`), freezing ordinal 11 `ResultSetAggregateSortedGrouped` (`java-runtime-3ffe177eb6a90acb699a`, static `java-5a1abe9fb7420df7353d`, shared inventory `java-0ded2b32c81677a9d36f`); no N+1 writes started. Primary owns the parity runner, scenario, generated Java/Go traces and evidence, tests, manifest, roadmap, CHANGELOG, validation, commit and push. Recovery asset worker `RecoverSortedAssets` owned only the scenario/oracle/launcher files; generated traces/evidence and all central facts remained primary-owned. File ownership is disjoint.

Validation milestone: focused replay/mutation tests pass after the strict null-boolean fix; pinned Java and Go traces are byte-identical to checked-in artifacts (325 records per side), direct differential comparison passes with zero differences, and manifest/inventory validation passes. `make check` passes (`check-layout`, `go vet ./...`, and `go test ./... -count=1 -timeout 240s`).

## Delivery
- Keep the worktree scoped to Draft 4.296; record validation and review before the semantic commit. Do not write the new commit hash into tracked files afterward.
