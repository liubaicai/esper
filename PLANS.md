...
- Current target: Draft 4.373 'dataflow-exceptions' — upgrade case.dataflow-exceptions (1 rt: java-runtime-c1d106a7e1cc70655006) to DV via the exception-propagation chain; invalid-graph and custom-properties confirmed implemented-not-differential (no manifest change). Follow-up queue: dataflow-lifecycle (concurrency-heavy, 18 rts); remaining dataflow dispositions (epstatement-pattern, typed-ports, feedback-completion follow-ups); DV-list consolidation; database slices; perf doc §4.9 queue (incremental join first).
