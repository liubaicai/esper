...
- Updated: 2026-09-03; Draft 4.306 (`resultset-output-limit-row-limit`) is complete and ready for the semantic commit. It targets fixed Java `ResultSetOutputLimitRowLimit.java` ordinals 1 and 3: wildcard `irstream` `length_batch(3)` with `limit 1`, including the SODA model round-trip variant.
- Baseline: Draft 4.305 (`resultset-querytype-local-group-by`) was committed and pushed at `2bc944616`; current worktree contains only Draft 4.306 changes and generated parity assets. Its local-group runtime `java-runtime-87b3bcfdf164bc939fe5` is recorded with zero-difference evidence and all required gates.
- Draft 4.306 Java contract: ordinal 1 runtime `java-runtime-840e283d8ac639591083`, static ID `java-2d2f2a8e404c07c322c6`, execution `ResultSetBatchNoOffsetNoOrder`; ordinal 3 runtime `java-runtime-20d0d1451ce549bf53b5`, static ID `java-789d7de75e91392eb5e1`, execution `ResultSetBatchOffsetNoOrderOM`; both use fixed Java commit `9e1b9f1cc9117fea4bf33ab043762c045d73839c`, source `regression-lib/src/main/java/com/espertech/esper/regressionlib/suite/resultset/outputlimit/ResultSetOutputLimitRowLimit.java`, flags empty, and shared inventory ID `java-0109d52ee4e8b36575b9`.
## Current work unit
The paired scenario replays the same six `SupportBean` events for both cases: E1, E2, E3 form the first batch and emit new E1; E4, E5, E6 form the second batch and emit new E4 plus old E1. Iterator snapshots before/after the batches assert the limit is applied at batch flush and that old/new order is preserved. Ordinal 3 pins `EPStatementObjectModel.toEPL()` to `select irstream * from SupportBean#length_batch(3) limit 1` and replays EPL-to-model compilation.
The typed Go implementation uses existing `LengthBatch(3)`, `WithOldStream`, `Limit(1)`, listener/replay, and iterator surfaces; no EPL is used as the public Go construction path. The scenario loader rejects duplicate JSON keys, unknown fields, null/non-string payload values, non-integer values, and trailing JSON. Integration tests cover direct replay, passing differential evidence, value/old-new/time/record-count mutations, checked-in evidence replay, malformed scenarios, and runtime mapping.
## Delegation checkpoint
Draft 4.306 prefetch scouts `OutputLimitJavaPrefetch-2` (`java-oracle-scout`) and `OutputLimitGoPrefetch-2` (`scout`) completed concurrently, read-only; they froze ordinals 1/3, IDs, batch old/new contract, and existing typed surfaces. Contract is frozen. `RowLimitAssets` (`parity-asset-worker`) owns only the new Java oracle, shell wrapper, scenario, and dedicated parity runner source; `RowLimitCore` (`go-slice-worker`) owned only the disjoint `internal/esper/resultset_output_limit_row_limit_parity_test.go`, now applied. Primary owns `internal/app/parity/run.go` dispatch, integration tests, generated traces/evidence, manifest, roadmap, CHANGELOG, PLANS, validation, review, commit, and push. `RowLimitParityReview` (`parity-reviewer`) independently returned PASS with no P0/P1/P2/P3 findings. N+1 scouts `NextOutputJavaScout` and `NextOutputGoScout` completed concurrently; the next candidate is ordinal 0 plus ordinal 4, with writes deferred until this semantic commit.
- Targeted validation passed: `go test ./internal/esper -run '^TestResultSetOutputLimitRowLimitLengthBatchParity$' -count=1`; `go test ./internal/app/parity -run '^TestRunResultsetOutputLimitRowLimit' -count=1`; pinned Java oracle generated `testdata/parity/resultset-output-limit-row-limit.trace.json`; Go replay generated `.go.trace.json`; differential runner generated `.evidence.json` with `status=passing`, 16 records per side, and zero differences; `go test ./internal/compat -run '^TestCapabilityManifestArtifactValidates$' -count=1` passed after manifest reconciliation; `git diff --check` passed.
- Full local gates passed: `make check` (layout, `go vet ./...`, and `go test ./... -count=1`); `go test -race ./... -count=1 -timeout 600s` passed. Central manifest/evidence/document consistency is current: 597 cases, 210 differential-verified cases, 763 differential runtime IDs, 3384 associations, and the registered row-limit case maps to `output.when-basic`.
- Acceptance is complete: the independent parity review passed with no findings, all required local gates passed, and the central manifest/roadmap/CHANGELOG facts record both runtime IDs and evidence. The semantic commit remains.
## Next work unit
After this semantic commit, begin the prefetched `ResultSetOutputLimitRowLimit.java` ordinal 0 context-termination and ordinal 4 grouped-iterator slice; keep ordinal 7 negative rowcount and ordinals 8/9 invalid/variable semantics as separate candidates.
## Prior outcomes
