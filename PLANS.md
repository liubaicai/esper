...
- Current target: Draft 4.369 'dataflow-eventbus-source' — resume the parity roadmap (perf repair rounds 4.366-4.368 shipped through e13fe3816). NEW differential chain upgrading case.dataflow-eventbus (implemented-not-DV, 3 rts) to DV via the eventbus ingress family: EPLDataflowOpEventBusSource.java {EPLDataflowAllTypes, EPLDataflowSchemaObjectArray} + EPLDataflowOpFilter.java {EPLDataflowAllTypes, EPLDataflowInvalid} — 4 executions, one ingress family; Filter Invalid is invalidity-policy. Follow-up queue: 4.370 eventbus sink family (EventBusSink{AllTypes,Beacon,SendEventDynamicType} completing case.dataflow-eventbus-collector); dataflow-lifecycle (concurrency-heavy, 18 rts); DV-list consolidation; database slices; perf doc §4.9 queue (incremental join first).
