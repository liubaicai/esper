...
- Current target: Draft 4.361 'dataflow-select-representation' — NEW differential chain upgrading case.dataflow-select-representation (implemented-not-DV) to DV via EPLDataflowOpSelect's wrapper-representation executions: EPLDataflowOpSelectWrapper{wrapperWithAdditionalProps=false} (java-runtime-9550df89e223e839e414, ord 9) and {wrapperWithAdditionalProps=true} (java-runtime-13cbd82acb5791b30d07, ord 10). Follow-up queue: EPStatementSource chain, DV-list consolidation, invalidity dispositions, database slices.
...
## Current work unit
Active: Draft 4.361 ('dataflow-select-representation').

- [x] Parallel read-only scouts dispatched and complete (Java contract scout agent_cab2804c: wrapper graphs byte-exact, Pair-split semantics with underlying={value} / additional={hello} namespaces, positional-assertion quirk documented; Go surface scout agent_8f4287c7: no Go wrapper surface — SelectEvent overlay merges to a flat union, PreserveInput+selections without OutputEventType is a Build error, so the honest shape is the flat union with the split documented as representation-only).
- [ ] Freeze contract; oracle/scenario/trace assets (writer) + Go runner + mode + tests (primary).
- [x] Manifest (case.dataflow-select-representation DV upgrade), evidence, roadmap, CHANGELOG. (Writer agent_0d575a9e delivered oracle (real op classes, internal Pair-split assertions, byte-exact EPL incl. the flag-dependent insert-into line), scenario, script (exit 0, deterministic), Java trace 2 records matching the frozen contract and the Go probe exactly. Evidence passing 0 differences, 2 IDs/2 executions; BOTH trace artifacts committed; summary dvIds 918 (+2 wrapper IDs), assoc 3544 / ref 3283 / unref 853 unchanged, dvCases 249; roadmap count stays 253 + supplement; CHANGELOG entry. EPLDataflowAllTypes remains umbrella-implemented, not claimed DV. Frozen disposition: the trace pins the FLAT UNION surface; the Java Pair split (underlying={value} vs additional={hello} namespaces) is representation-only metadata documented in the case difference — a future wrapper-representation engine surface could pin the split.)
- [x] Gates: affected packages + make check + git diff --check (green; recorded before commit).
- [x] Independent parity review. Reviewer agent_ddbb48fe-db06-469b-af5e-0f2f27d287c6 OVERALL PASS: Java contract (byte-exact EPL incl. the flag-dependent insert-into line, Pair-split internal assertions), builder fidelity (flat rendering exact, zero internal/esper changes), trace/evidence integrity (programmatic embed-vs-file equality), manifest recomputation (dvIds 918 = 916 + 2 wrapper IDs; AllTypes correctly carved out of the DV list), test adequacy (4 mutations), prose (roadmap count stays 253). P2 fixed in-unit: dataflow.graph capability DV list extended +2 (13→15), matching the invariant that held at HEAD. P3s fixed in-unit: dead duplicate error check removed; checkpoint hygiene (scouts ticked with agent IDs, flat-union disposition recorded).
- [x] Commit and push (single semantic commit with all checkpoint edits folded in).
