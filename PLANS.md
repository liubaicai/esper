...
- Updated: 2026-08-28
- Baseline: `HEAD` == `origin/master` at Draft 4.272 (`7ecf9fd5b`), pushed. Draft 4.273 is the active sorted multi-criteria follow-up.
- Closed unit (Draft 4.267, commit `8de7ede9e`): `ResultSetAggregateFiltered.java` filtered-all execution was differential-verified with 29 Java/Go records and zero differences.
- Closed unit (Draft 4.268, commit `ebc116623`): `ResultSetAggregateFirstLastWindow.java` current-window executions were differential-verified with 11 Java/Go records and zero differences.
- Closed unit (Draft 4.270, commit `b6d4c7eda`): `ResultSetAggregateFirstLastWindow.java` indexed execution was differential-verified with 4 Java/Go records and zero differences.
- Closed unit (Draft 4.271, commit `a3aba2c85`): `ResultSetAggregateNTh.java` direct execution was differential-verified with 6 Java/Go records and zero differences; malformed scenario-shape and trace mutations were rejected.
- Closed unit (Draft 4.272, commit `7ecf9fd5b`): `ResultSetAggregateSortedMinMaxBy.java` executions `ResultSetAggregateGroupedSortedMinMax`, `ResultSetAggregateMultipleOverlappingCategories`, and `ResultSetAggregateMinByMaxByOverWindow` were differential-verified with 25 Java/Go records and zero differences; strict lexical/range/duplicate-key validation and malformed/trace mutations were reviewed PASS.

## Current work unit
Draft 4.273: `ResultSetAggregateSortedMinMaxBy.java`, execution `ResultSetAggregateMultipleCriteriaSimple` (ordinal 4, source inventory `java-553516b9d01c12a13172`). The fixed Java oracle is `/root/app/esper` at commit `9e1b9f1cc9117fea4bf33ab043762c045d73839c`. Runtime ID is `java-runtime-9f50a7b345919de5a8cf`. The contract is one fresh `SupportBean#keepall` lifecycle with `sorted(theString desc, intPrimitive desc) as c0`, preserving full `SupportBean` identity and emitting the exact ordered arrays after `C/10`, `D/20`, `C/15`, and `D/19`: `[C/10]`, `[D/20,C/10]`, `[D/20,C/15,C/10]`, `[D/20,D/19,C/15,C/10]`. No eviction, ever aggregate, null/missing input, old-stream, timer, or invalid path is in scope.
Delegation checkpoint: `NextSortedJava-3` and `NextSortedGo-3` completed read-only scouts before writes; `ReviewDraft4273` independently reviewed the integrated diff and returned PASS. The review confirmed source/runtime identity, four ordered full-event arrays, exact scenario/oracle validation, int32 bounds, and mutation rejection. Read-only prefetch `NextSortedJava4` and `NextSortedGo4` agree that Draft 4.274 should target `ResultSetAggregationMethodSorted.java` execution `ResultSetAggregateSortedMultiCriteria` (ordinal 10, runtime `java-runtime-dd79ba5aba4eb4ec0a1a`, static `java-427c23ee540c6e2e7cf9`) after this unit is committed; no N+1 writes have started.
## Progress
Assets and typed runner are integrated. The pinned Java oracle generated 4 listener records; Go replay produced the same four ordered full-event arrays, and persisted differential evidence reports passing with zero differences for runtime `java-runtime-9f50a7b345919de5a8cf`. Focused parity tests, direct mode smoke replay, `go vet ./...`, `go test ./... -count=1 -timeout 180s`, `go test ./internal/compat/... -count=1`, `make check`, layout/diff checks, manifest checks, and the independent parity review all pass. The supplementary `go test -race ./... -count=1 -timeout 180s` passed every package through `internal/app/sourcemanifest` and the affected `internal/app/parity` tests, then timed out in the unrelated existing `internal/esper.TestDatabaseJoinPerfWithCacheInKeywordMultiIndexMatchesJava`; race is not a required per-execution gate. Manifest, roadmap, CHANGELOG, and README now record 566 cases, 178 differential-verified cases, 693 differential runtime IDs, 3316 associations, and 3104 referenced runtimes. Pending: final diff inspection and commit/push.
## Delivery
Required per-execution gates are complete: `go vet ./...`, `go test ./... -count=1 -timeout 180s`, `go test ./internal/compat/... -count=1`, `make check`, layout/diff checks, persisted evidence verification, and independent parity review. The supplementary full race command timed out only at the unrelated database performance test after the changed parity surface passed.
## Handoff
After delivery, verify the pushed ref read-only and select the next roadmap/manifest work unit; do not edit this file only to add the new commit hash.
