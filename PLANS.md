...
- Updated: 2026-08-28
- Baseline: `HEAD` == `origin/master` at Draft 4.267 (`8de7ede9e`), pushed. Draft 4.268 current-window aggregate access assets and metadata are the only current worktree changes.
...
## Current work unit
Draft 4.268: `ResultSetAggregateFirstLastWindow.java` current-window access slice, source executions `ResultSetAggregateFirstLastWindowNoGroup` (ordinal 15, runtime `java-runtime-7e5e6b41dd7cb8e65879`) and `ResultSetAggregateFirstLastWindowGroup` (ordinal 16, runtime `java-runtime-e48086834b488cb35e3c`). The fixed Java oracle is `/root/app/esper` at commit `9e1b9f1cc9117fea4bf33ab043762c045d73839c`. Contract is limited to typed `first`, `last`, and `window` over `SupportBean#length(5)` grouped and `SupportBean.win:length(2)` ungrouped, deterministic FIFO eviction, new-only listener rows, and source-defined row ordering. It excludes indexed/prev/nth, wildcard event access, batch/output-rate, joins, subqueries, invalid diagnostics, old-stream, iterator, FAF, and lifecycle variants.
Delegation checkpoint: `AggregateAccessJavaContract` and `AggregateAccessGoSurface` completed read-only scouts. Existing typed Go APIs and tests cover the contract; no shared `internal/esper` writer is needed. Future files are limited to disjoint parity assets: runner/oracle/scenario/trace/evidence and parity tests, plus central mode/manifest/docs owned by primary.
Primary owns generated traces/evidence, manifest, roadmap, CHANGELOG, validation, review, commit, and push.
## Progress
- Pinned Java replay and Go differential replay pass: 11 Java/Go records, 0 differences, exact NoGroup→Group runtime order, and mutation checks rejecting altered values/row order. The dedicated case and capability mapping are registered; manifest facts are 561 cases, 173 differential-verified cases, 686 differential runtime IDs, and 3309 runtime associations. Focused tests, `go vet ./...`, `go test ./... -count=1 -timeout 240s`, `go test -race ./... -count=1 -timeout 600s`, `make check`, asset syntax/schema checks, and `git diff --check` pass. Independent `AggregateAccessReview` strict review is PASS.
## Delivery
- All precommit requirements are satisfied: pinned Java replay, Go replay, zero-difference evidence, focused tests, full local gates, independent parity review, and manifest/docs consistency. Create one semantic commit and push; then verify the remote ref read-only and select the next roadmap work unit.
