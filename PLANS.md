...
- Updated: 2026-08-28
- Baseline: `HEAD` == `origin/master` at Draft 4.268 (`ebc116623`), pushed. The current-window aggregate access unit is committed; the next Prev/Nth indexed-access unit is now being prepared.
...
- Closed unit (Draft 4.265, commit `a439cff87`): `ResultSetAggregateCountSum.java` ordinals 1–5 were differential-verified with 55 Java/Go records and zero differences; the shared `Avg` repair covers nested aggregate historical prefixes.
...
- Closed unit (Draft 4.267, commit `8de7ede9e`): `ResultSetAggregateFiltered.java` filtered-all execution was differential-verified with 29 Java/Go records and zero differences; the dedicated scenario preserves the broad aggregate-filtered inventory umbrella.
- Closed unit (Draft 4.268, commit `ebc116623`): `ResultSetAggregateFirstLastWindow.java` current-window executions `ResultSetAggregateFirstLastWindowNoGroup` and `ResultSetAggregateFirstLastWindowGroup` were differential-verified with 11 Java/Go records and zero differences; mutation checks reject altered values and grouped row order.
## Current work unit
Draft 4.269: `ResultSetAggregateFirstLastWindow.java` indexed previous-access slice, source execution `ResultSetAggregatePrevNthIndexedFirstLast` (ordinal 6, runtime `java-runtime-3733f40a5c6d7be2c175`). The fixed Java oracle is `/root/app/esper` at commit `9e1b9f1cc9117fea4bf33ab043762c045d73839c`. Contract is one fresh `SupportBean#length(3)` statement using typed `prev(intPrimitive, 0..2)`, `nth(intPrimitive, 0..2)`, and `last(intPrimitive, 0..2)`; send E1/10 through E4/13; assert new-only rows, out-of-range nulls, and FIFO post-eviction shifts. It excludes ordinal 5's join/variable/dynamic-index branches, wildcard/star, invalid diagnostics, old-stream, iterator, FAF, output-rate, and lifecycle variants.
Delegation checkpoint: `NextAggregateAccessJava` and `NextAggregateAccessGo` completed read-only scouts; `NextAggregateAccessJava` selected ordinal 6 as the safest isolated candidate and `NextAggregateAccessGo` confirmed existing typed `Prev`, `Nth`, and indexed `Last` APIs. No shared `internal/esper` writer is authorized. After contract freeze, `PrevNthAssets` owns only the Java oracle, shell runner, and scenario input; primary owns the Go parity runner, central mode/tests, generated trace/evidence, manifest, roadmap, CHANGELOG, validation, review, commit, and push.
## Progress
Candidate contract is frozen from fixed Java source lines 207-243 and inventory line 3324; existing typed APIs and tests cover the required aggregate operators. Implementation and parity assets remain.
Implementation complete: indexed `Prev`, `Nth`, and indexed `Last` are wired through a fresh typed Go aggregate query; the generated pinned Java trace and Go replay each contain 4 listener records and the differential evidence is passing with zero differences. Focused aggregate, runner, compat, manifest, vet, shell syntax, and diff checks pass. `facade_generated.go` was regenerated and `check-layout.sh` now passes.
Manifest metadata is updated for one new differential-verified case and runtime, including aggregate-access references and summary counts (562 cases, 174 differential-verified cases, 687 differential runtime IDs, 3310 associations, 101/101 representative scenarios); JSON and manifest tests pass.
Roadmap, README, and CHANGELOG now record Draft 4.269 and the same manifest-derived counts. `IndexedAggregateReview` strict re-review PASS: no concrete P0-P2 correctness or evidence-integrity findings remain after the hardened five-step scenario guard.
Full `make check` passed: layout, vet, and `go test ./... -count=1 -timeout 240s`; all focused aggregate/runner/compat/manifest tests, pinned Java replay, zero-difference evidence, JSON/schema, shell syntax, and diff checks passed.
Prefetched N+1 candidate: Java `ResultSetAggregateFirstLastIndexed` (ordinal 5, runtime `java-runtime-30f62dc2e86a7e1a1e80a0`) is isolated first/last indexed access over length(3); join and dynamic-variable phases remain separate follow-up boundaries. `NextIndexedFirstLastJava` and `NextIndexedFirstLastGo` completed read-only scouts; no N+1 writes begin before this unit is reviewed and committed.
## Delivery
Required before commit: generated pinned Java trace, Go differential replay with zero-difference evidence, focused tests, full local gates, independent parity review, manifest/docs consistency, then one semantic commit and push.
