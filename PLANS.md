...
- Updated: 2026-08-31
- Baseline: `HEAD` == `origin/master` at Draft 4.286 (`372aca55d2db319130bb441cfacf969c456b8c81`), pushed; Draft 4.287 is the current implementation unit.
- Closed unit (Draft 4.286; validation and independent review complete): `ResultSetQueryTypeRowForAll.java` ordinal 10 `ResultSetQueryTypeRowForAllSelectAvgStdGroupByUni` was differentially verified with 4 Java/Go records and zero differences; recursive `univariate-statistics*` aggregate dependency detection preserved ordinal-nine row-for-all semantics and aligned `#uni` grouped averages; strict replay/mutation validation, manifest/evidence consistency, README/roadmap/CHANGELOG alignment, layout, vet, full Go tests, and `make check` passed; reviewer `ReviewDraft4286` returned PASS.
- Closed unit (Draft 4.285; validation and independent review complete): `ResultSetQueryTypeRowForAll.java` ordinal 9 `ResultSetQueryTypeRowForAllSelectAvgExprStdGroupBy` was differentially verified with 2 Java/Go records and zero differences; the bounded aggregate fix preserves pure aggregate row-for-all semantics while materializing implicit grouping only for non-key projections or into-table targets; strict replay/mutation validation, manifest/evidence consistency, README/roadmap/CHANGELOG alignment, layout, vet, full Go tests, and `make check` passed; reviewer `ReviewDraft4285` returned PASS.
- Closed unit (Draft 4.284; validation and independent review complete): `ResultSetQueryTypeRowForAllHaving.java` ordinal 2 `ResultSetQueryTypeAvgRowForAllWHavingGroupWindow` was differentially verified with 3 Java/Go records and zero differences; strict replay/mutation validation, manifest/evidence consistency, README/roadmap/CHANGELOG alignment, layout, vet, full Go tests and `make check` passed; reviewer `ReviewDraft4284-2` returned PASS.
- Closed unit (Draft 4.283; validation and independent review complete): `ResultSetQueryTypeRowForAllHaving.java` ordinal 1 `ResultSetQueryTypeRowForAllWHavingSumJoin` was differentially verified with 3 Java/Go records and zero differences; strict replay/mutation validation, manifest/evidence consistency, README/roadmap/CHANGELOG alignment, layout, vet, full Go tests and `make check` passed; reviewer `ReviewDraft4283` returned PASS.
...
## Current work unit
 Draft 4.287 ports the smallest safe next slice: fixed Java `ResultSetQueryTypeRowForAll.java` ordinal 12 `ResultSetQueryTypeRowForAllStaticMethodDoubleNested` (shared inventory `java-00cfc4061ba3a5f163ff`, runtime `java-runtime-f8487c524573a3a2f08c`, static candidate `java-421838b50fc1c8eff9f6`, no flags). The exact Java EPL imports `MyHelper` and selects `MyHelper.doOuter(MyHelper.doInner(last(theString)))` from `SupportBean`; one `SupportBean("E1", 1)` yields one new-only listener row `c0="oiE1io"`, with no iterator, old stream, timer, deletion, null/type, or multi-event assertions. Typed Go composes `Last[string]` with `Func1` inner/outer expressions over a registered `SupportBean` and subscribes before the send.

 The ordinal-twelve contract is frozen by concurrent read-only scouts `Ordinal12JavaScout` (`java-oracle-scout`) and `Ordinal12GoScout` (`scout`), both completed. Java facts: one listener record, exact helper composition, runtime `java-runtime-f8487c524573a3a2f08c`, shared inventory `java-00cfc4061ba3a5f163ff`, static candidate `java-421838b50fc1c8eff9f6`, source `regression-lib/src/main/java/com/espertech/esper/regressionlib/suite/resultset/querytype/ResultSetQueryTypeRowForAll.java`, flags empty. Go facts: existing typed `Last` plus nested `Func1` and aggregate planner traversal are sufficient; no shared `internal/esper` change is expected. Asset writer `Ordinal12Assets` owns only the dedicated Java oracle, launcher, and scenario JSON; the primary owns the runner, Go replay, tests, traces, evidence, manifest, and migration facts. File ownership is disjoint.
 Fresh Java oracle replay (Java 17, pinned commit) byte-compares with the checked-in trace; focused direct-replay, differential-evidence, trace-mutation, checked-in-evidence, and malformed-scenario tests all pass. Go replay and compatibility/manifest checks pass. The checked-in differential evidence and Go trace are validated; independent parity review returned PASS after the runner help-text regression was repaired.

## Recent outcome


Draft 4.287 validation and independent review are complete and ready for one semantic commit/push.
- Pinned Java oracle launcher regenerated `testdata/parity/resultset-querytype-row-for-all-static-method-double-nested.trace.json` from `/root/app/esper` at `9e1b9f1cc9117fea4bf33ab043762c045d73839c`; Java 17.0.20 emitted 1 record, and `cmp` matched the checked-in trace. Go replay emitted the same 1 normalized record and `cmp` matched the checked-in Go trace.
- Differential replay wrote passing evidence with 1 Java record, 1 Go record, and 0 differences. Focused direct-replay, differential-evidence, trace-mutation, checked-in-evidence, malformed-scenario, help-mode, and compatibility/manifest tests passed. Full local gates passed: `go vet ./...`, `go test ./... -count=1 -timeout 240s`, layout, and `git diff --check`.
- Independent parity review `ReviewDraft4287` returned PASS with no actionable findings; follow-up review `ReviewDraft4287-2` found and the primary restored the retained Draft 4.282 history entry. Both review scopes are now closed.

## Delivery
- Draft 4.287 is ready for one semantic commit/push. Git will remain authoritative for the resulting commit identity.
