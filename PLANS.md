...
- Updated: 2026-09-01; Draft 4.291 is validated and ready for semantic commit/push; Draft 4.290 is shipped at `e2329cf34b691f4d29129c57a9b5167a02c7d8eb`.
- Baseline: `HEAD` == `origin/master` at Draft 4.290 (`e2329cf34b691f4d29129c57a9b5167a02c7d8eb`), pushed; the current worktree contains only Draft 4.291 changes pending its semantic commit/push.
- Closed unit (Draft 4.287; validation and independent review complete): `ResultSetQueryTypeRowForAll.java` ordinal 12 `ResultSetQueryTypeRowForAllStaticMethodDoubleNested` was differentially verified with 1 Java/Go record and zero differences; typed `Last[string]` plus nested `Func1` matched `MyHelper.doOuter(MyHelper.doInner(last(theString)))`; strict replay/mutation/malformed validation, manifest/evidence consistency, README/roadmap/CHANGELOG alignment, layout, vet, full Go tests, and `git diff --check` passed; reviewers `ReviewDraft4287` and `ReviewDraft4287-2` returned PASS.
- Closed investigation (Draft 4.288; no semantic change): fixed Java `ResultSetQueryTypeRowForAll.java` ordinal 11 `ResultSetQueryTypeRowForAllNamedWindowWindow` is already covered by `case.row-for-all`, `testdata/parity/resultset-row-for-all.json`, its six-record Java trace/evidence, and `TestResultSetQueryTypeRowForAllNamedWindowWindowParity`. Read-only scouts `Ordinal11JavaScout` and `Ordinal11GoScout`, with prefetch confirmation from `NextOrdinal11Java`, verified the exact runtime/static/source IDs, named-window FIFO/post-removal old-stream contract, and existing typed Go lifecycle; duplicating a dedicated case would inflate evidence without adding coverage.

## Current work unit
Draft 4.291 is frozen as fixed `ResultSetQueryTypeRollupGroupingFuncs.java` ordinal 2 `ResultSetQueryTypeFAFCarEventAndGroupingFunc`, runtime `java-runtime-e8f49362431d4c33baa8`, shared/static ID `java-1ae66c9985dc53282af0`, inventory flag `FIREANDFORGET`; `static-manifest.json` also carries `INVALIDITY` on the shared candidate and that metadata discrepancy is preserved, not treated as this runtime's behavior. Java creates public `CarWindow#keepall` over `SupportCarEvent`, inserts all source events, sends six events in fixed order `(skoda,france,10000)`, `(skoda,germany,5000)`, `(bmw,france,100)`, `(bmw,germany,1000)`, `(opel,france,7000)`, `(opel,germany,7000)`, then executes one FAF query with `grouping sets((name,place),name,place,())`. The observable result is one 12-row FAF record: six detail rows in send order, name subtotals `skoda=15000`, `bmw=1100`, `opel=14000`, place subtotals `france=17100`, `germany=13000`, and overall `30100`; grouping bits/gid are `(0,0,0)`, `(0,1,1)`, `(1,0,2)`, `(1,1,3)`, with Java grouping metadata `Integer` and Go normalized JSON integer values.
Ownership is frozen: primary owns `internal/app/parity/rollup_grouping_faf.go`, `internal/app/parity/run.go`, `internal/app/parity/run_test.go`, scenario/traces/evidence, manifest and documents; one isolated `parity-asset-worker` owns only `tools/java-oracle/ResultSetQueryTypeRollupGroupingFAFScenarioOracle.java` and `tools/java-oracle/run-resultset-querytype-rollup-grouping-faf.sh`. Fixed Java remains read-only. Scenario shape is one `faf-grouping-snapshot` case with strict metadata, six `send` steps, and one `faf` step for `s0`; no listener, old stream, virtual time, iterator, or invalid execution is in scope.
Delegation checkpoint: read-only scouts `NextGroupingJava` (`java-oracle-scout`) and `NextGroupingGo` (`scout`) completed concurrently after Draft 4.290. They confirmed the exact IDs/source contract, the static `INVALIDITY` metadata discrepancy, reusable named-window insertion and `ExecuteFireAndForget` APIs, exact 12-row order, and no shared `internal/esper` change. Isolated asset writer `FAFGroupingOracleAsset` (`parity-asset-worker`) completed only `tools/java-oracle/ResultSetQueryTypeRollupGroupingFAFScenarioOracle.java` and `tools/java-oracle/run-resultset-querytype-rollup-grouping-faf.sh`; primary will validate the assets and generate all traces/evidence. `ReviewDraft4290` returned PASS for the shipped predecessor.

- Closed unit (Draft 4.289; validation and independent review complete): `ResultSetQueryTypeRollupDimensionality.java` ordinals 10, 11, and 17 were differentially verified through five dedicated cases with 21 Java/Go records and zero differences; typed grouping sets/cube/context rollup replay, strict metadata/payload/marker validators, trace mutations, checked-in evidence, manifest consistency, README/roadmap/CHANGELOG alignment, layout, vet, full Go tests, race, and `git diff --check` passed. Independent reviewer `ReviewDraft4289` initially audited the worktree, identified missing generated assets and documentation, and the primary regenerated/corrected all findings; final reviewer `ReviewDraft4289Final` returned PASS with no actionable P0-P2 findings.
## Recent outcome
- Draft 4.291 added `resultset.aggregate-dimensional` case `rollup-grouping-funcs-faf-dedicated` for `ResultSetQueryTypeRollupGroupingFuncs.java` ordinal 2. One fixed Java/Go FAF record with 12 rows matched with zero differences; checked-in evidence, manifest totals, documentation, full local gates, and both independent parity reviews are recorded below.

## Delivery
- Draft 4.291 is complete: implementation, scenario, Java/Go traces, evidence, manifest, and documentation are updated; focused replay/mutation, manifest, layout, vet, full tests, and race gates pass; `ReviewDraft4291` and `ReviewDraft4291-2` returned PASS. The next action is one semantic commit and push to `master`; after commit only read-only remote verification remains.
