...
- Updated: 2026-09-02; Draft 4.304 (`resultset-aggregate-filtered-w-math-context`) is the active work unit. It targets fixed Java `ResultSetAggregateFilteredWMathContext.java` ordinal 0 and its configured BigDecimal average semantics; implementation, parity assets, generated traces/evidence, and manifest facts are integrated. Independent parity review and all local gates pass; semantic commit and push remain.
- Baseline: `HEAD` == `origin/master` at `dbb434c5e`; the worktree was clean before this checkpoint update. Draft 4.303 (`resultset-aggregate-filter-named-parameter`) is committed and pushed at this baseline.
- Closed predecessor (Draft 4.303): `ResultSetAggregateFilterNamedParameter.java` ordinals 4-7 were differentially verified with zero differences and full local gates.
## Current work unit
Draft 4.304 covers ordinal 0 of `ResultSetAggregateFilteredWMathContext.java`: `ResultSetAggregateFilteredWMathContext`; Java runtime `java-runtime-fa8b6d5d6fb58a905f23`; static ID `java-aba2cfbf41a3be9809f1`; Java commit `9e1b9f1cc9117fea4bf33ab043762c045d73839c`; flags empty.
The fixed Java source is `/root/app/esper/regression-lib/src/main/java/com/espertech/esper/regressionlib/suite/resultset/aggregate/ResultSetAggregateFilteredWMathContext.java`; suite configuration is `/root/app/esper/regression-run/src/test/java/com/espertech/esper/regressionrun/suite/resultset/TestSuiteResultSetAggregateWConfig.java`. The Go API must expose an environment/compiler-level decimal math-context option, not an EPL string or an expression-only workaround.
The observable contract is one fresh unbounded `avg(bigdec)` listener lifecycle: compiler MathContext precision 2 with HALF_UP; source helper discards `setScale`, so inputs are BigDecimal `0` scale 0, `0` scale 0, `1` scale 0; one new row follows each send with `c0=0`, `c0=0`, then `c0=0.33` (scale 2 in Java). No filter/window, old stream, iterator, FAF, virtual time, explicit error, or cleanup failure is exercised; final Java assertion observes only the third row but the replay trace records all three.
## Delegation checkpoint
Draft 4.304 scouts `MathContextJava-2` (`java-oracle-scout`) and `MathContextGo-2` (`scout`) completed concurrently. Java confirmed the fixed runtime/configuration and 0,0,0.33 contract; Go confirmed `AvgExact` is unlimited and no environment/compiler math-context surface exists. Contract is frozen. Shared-core writer owns the environment/compiler option, plan/runtime propagation, and aggregate behavior in assigned `internal/esper` files. File-disjoint asset writer owns the new scenario/oracle/runner/test source files only. Primary owns generated traces/evidence, manifest, roadmap, CHANGELOG, validation, review, commit, and push.
- Core acceptance: configured typed environment builds and deploys an `AvgExact` BigDecimal projection whose third result is the rounded rational `33/100`, while an unconfigured environment preserves unlimited exact `1/3`; plan identity includes the compile-time context and invalid context options fail at build/configuration.
- Asset acceptance: replayable deterministic scenario sends the three scale-discarding payloads, Java oracle runs the pinned source/config, Go runner emits the same three listener records, and no worker hand-authors trace/evidence or central facts.
- Targeted validation: decimal-context, parity-family, and manifest tests pass; pinned Java and Go replay outputs match their checked-in traces; differential mode passes with zero differences and generated evidence matches the checked-in evidence; mutation and malformed-scenario checks pass.
- Final validation: `make check` passes (`check-layout`, `go vet ./...`, `go test ./... -count=1`); `make test-race` passes (`go test -race ./... -count=1 -timeout 600s`). `git diff --check`, JSON validation, and oracle shell syntax checks pass. Independent reviewer `MathContextReview-3` verdict PASS with zero P0/P1/P2/P3 findings and no follow-up required.
## Prior outcomes
