...
- Updated: 2026-09-01; Draft 4.294 integration, independent review, and all required verification are complete on the pushed Draft 4.293 baseline; tracked facts are ready for the semantic commit.
- Baseline: `HEAD` == `origin/master` at `03061461f` (`feat(resultset): port rollup orderby unidirectional`); the worktree was clean before Draft 4.294 selection.
- Closed predecessor (Draft 4.293): `ResultSetQueryTypeRollupHavingAndOrderBy.java` ordinals 4-7 were differentially verified through `resultset-querytype-rollup-orderby-unidirectional`; review PASS found no behavioral or evidence defects. The stale checkpoint was corrected here as part of this semantic unit, not as a checkpoint-only change.

## Current work unit
Draft 4.294 is frozen as fixed `ResultSetAggregateSortedMinMaxBy.java` ordinal 6, `ResultSetAggregateNoDataWindow`. Runtime ID is `java-runtime-af551963966a83d26468`; static candidate is `java-f645b6fb41fd8c63d7f0`; the collection execution inventory's shared outer-class ID is `java-553516b9d01c12a13172`; flags are empty; and Java source commit `9e1b9f1cc9117fea4bf33ab043762c045d73839c` is read-only. The observable contract is unwindowed `SupportBean` `maxbyever(intPrimitive).theString`, `minbyever(intPrimitive).theString`, `maxby(intPrimitive).theString`, and `minby(intPrimitive).theString`, with four new-only listener rows: E1/1 => E1,E1,E1,E1; E2/2 => E2,E1,E2,E1; E3/0 => E2,E3,E2,E3; E4/3 => E4,E3,E4,E3. No time, eviction, old stream, iterator, join, or null payload is in scope.

## Delegation checkpoint
`ReviewDraft4293`, `ScoutNextJava`, and `ScoutNextGo` completed concurrently before this freeze. Review found Draft 4.293 behavior/evidence sound and only the stale plan checkpoint; Java and Go scouts independently confirmed this one-runtime sorted no-window candidate and existing typed `MinBy`/`MaxBy`/`MinByEver`/`MaxByEver` surfaces. Their reports are retained at `agent://ReviewDraft4293`, `agent://ScoutNextJava`, and `agent://ScoutNextGo`. The isolated oracle asset writer must not modify shared semantics, generated traces/evidence, manifest/docs, or commit/push; primary performs all validation, evidence generation, review, commit, and push.
-Draft 4.294 selection and contract freeze are complete. Implementation extends the sorted aggregate parity surface without changing `internal/esper`; the candidate is intentionally one execution because its unwindowed current/ever semantics are a self-contained differential unit.
- Replacement reviewer `ReviewDraft4294Final` completed the post-fix audit and returned PASS after static provenance was corrected to candidate `java-f645b6fb41fd8c63d7f0`, the quoted-integer regression was registered, and roadmap/CHANGELOG entries were added. No follow-up findings remain.
- Draft 4.294 validation passed: pinned Java launcher regenerated the four-record trace from `/home/baicai/app/esper`; Go replay matched its checked-in trace; focused replay, malformed-scenario, quoted-integer, differential-evidence, checked-in-evidence, trace-mutation, manifest/inventory, JSON, and `git diff --check` checks passed. `go test ./internal/compat ./internal/app/manifest -count=1` passed.

## Delivery
- All required Draft 4.294 gates passed; create one semantic commit and push to `master`, then verify the pushed ref read-only without a checkpoint-only follow-up edit.
