...
- Current target: Draft 4.314 implemented — `case.resultset-orderby-multi-delivery` differential-verified (3 records per side, 0 differences) with the parity-driven patternBatch engine fix; gates and independent review PASS; commit and push remain.
## Current work unit
Draft 4.314 shipped the following (all checked):
- Engine fix (single writer, primary): `internal/esper/runtime.go` patternBatch now recounts `outputInserted/outputRemoved` from the collected match rows after the NFA loop (Java counts rows posted to the output process view, not input events) and sorts the delivered batch when `plan.query.orderBy` is set (ESPER-409 batch-level order-by, effective even without an output policy); `internal/esper/plan.go` replaces the blanket pattern order-by rejection with a match-recognize-style ResultField validation against patternSelections. Regression tests `TestPatternOutputLimitCountsMatchRows` and `TestPatternImmediateDeliverySortsBatch` pin both behaviors and fail under the pre-fix engine. Full `internal/esper`, `internal/app/parity`, and `internal/compat` suites pass.
Prefetch (Draft 4.314, contract organized, no writes yet): both read-only scouts completed. `MultiDeliveryJavaContract` (`agent_ec8cd388-c0ad-4830-86b1-8798f5eb96a0`) adjudicated the three sequential statement parts (Java lines 59-124, each its own deploy/undeploy cycle; time persists across parts: 0 -> 1000 -> 11000 absolute; milestones are no-ops):
- `multi-delivery-pattern`: `select a.theString from pattern [every a=SupportBean(theString like 'A%') -> b=SupportBean(theString like 'B%')] order by a.theString desc`; events A1/1, A2/2, B/3 -> exactly ONE callback of 2 rows [A2, A1], result field literally `a.theString`, time epoch zero.
- `multi-delivery-pattern-output-limit`: same pattern + `output every 3 events`; events A1, A2, A3, B -> exactly ONE callback of 3 rows [A3, A2, A1]; Java counts pattern-match ROWS into the output process view (OutputConditionCount driven from OutputProcessViewConditionDefault), so the limit fires on B.
- `multi-delivery-groupwin-time`: `select rstream theString from SupportBean#groupwin(theString)#time(10) order by theString desc`; advance-time 1970-01-01T00:00:01Z, A1, A2, advance-time 1970-01-01T00:00:11Z -> exactly ONE callback of 2 new rows [A2, A1] (rstream swaps removal rows into new data; both groups expire in one timer cycle; order-by-sorted removal batch is the ESPER-409 contract; the pinned oracle run must empirically confirm [A2, A1] and the 00:00:11Z timestamp).
`MultiDeliveryGoSurface` (`agent_2631b467-3ab3-418e-b26e-8a17a71a2e6b`) mapped the Go surface and found TWO engine-level facts: (1) `every a -> b` maps to `PatternFrom(...).Every().FollowedBy(...)` (every on the left leg; `.Then(...).Every()` is the wrong grouped form), LIKE via `LikeOf(theString, Literal("A%"))`; (2) **order-by keys on pattern statements must use `esper.ResultField[string]("theString")`, not `TagField`** — TagField has no resultRow fallback and returns Null in the batch sorter (expr.go:1325-1342), so the projection alias should be `theString`/`a.theString` accordingly. (3) **Predicted hard divergence on part 2**: Go's `patternBatch` sets `outputInserted` from the source-chain insert delta (one per input event) and never recomputes it from match rows (runtime.go:17309-17312, no reassignment through 17468), so `output every 3 events` fires/resets at A3 and suppresses the B batch of 3 rows. Parity-driven engine fix: recompute `outputInserted/outputRemoved` from the collected match rows after the NFA loop (~runtime.go:17457), plus focused engine regression tests. Engine precedent: single writer (primary) for the shared runtime fix.
Plan: primary owns the engine fix + Go runner (`internal/app/parity/resultset_orderby_multi_delivery.go`, modes `resultset-orderby-multi-delivery`/-diff) + tests; asset writer owns the new Java oracle/script/scenario on the frozen contract; trace generation must confirm 3 records, part-3 [A2, A1] at 00:00:11Z, and no extra records from the no-op advanceTime(0).
Delegation: asset writer `agent_df37b522-41e8-4db1-952f-22f8bd93811d` (isolated) authored the oracle/script/scenario; primary wrote the engine fix, runner, run.go modes, and the six-test family on disjoint files. Integration repairs owned by the primary: (1) the frozen contract wrongly pinned part-3 A2 as intPrimitive 2 — the Java source (line 111) sends 1; corrected in scenario, oracle GROUPWIN_SEND_INTS, and the Go runner's GroupTime array; (2) an over-broad sed during that correction also changed the pattern parts' A2/2 pins — restored. Join runner precedent applied for run.go modes and tests.
- [x] Freeze multi-delivery contract (prefetch).
- [x] Implement patternBatch recount + pattern order-by engine support with regression tests.
- [x] Implement resultset-orderby-multi-delivery runner, modes, and tests.
- [x] Author multi-delivery oracle, script, and scenario via asset writer.
...
- [ ] Commit and push resultset-orderby-multi-delivery unit.
Multi-delivery verification: `go test ./internal/app/parity -run 'TestRunResultsetOrderbyMultiDelivery' -count=1` passed (direct replay, passing evidence, 9 mutations reject, checked-in evidence matches both traces and live replay, 9 malformed-scenario mutations reject, runtime-ID mapping). The pinned Java 9.0.0 oracle compiled, emitted three records, and regenerated the checked-in Java trace byte-identically (`cmp=0`); evidence is passing with 3 Java records, 3 Go records, 0 differences, and one frozen runtime ID (Java trace SHA-256 `57d3b346b0f40b3264da4230189c75bdc1bdb631ec3979cc0ef5f071f3d67726`, Go trace SHA-256 `05b6a8edd7b71cd9377ba77bbf2fb5e4033bf2b9f6a4d8e1aa45c9f997f4fe7f`). The runtime was already referenced by the umbrella `case.resultset-orderby-simple` inventory, so referenced/unreferenced stay 3166/970 while associations grow 3401 -> 3402. Full gates: `gofmt` clean, `go vet ./...`, complete `go test ./... -count=1`, layout equivalents (tracked-generated, batched `gofmt -l`, facade `cmp`, apidump `cmp`), `git diff --check`, JSON parses, and `bash -n` on the new runner script all pass; the recorded race-gate host limitation still applies. Final parity review `MultiDeliveryReview` (independent read-only reviewer, agent `agent_8507c95d-32f8-4934-b27f-1a9d929b58e3`) returned PASS with zero P1/P2 findings and three accepted P3 notes: (1) this stale "Current target" line — fixed before commit; (2) the initial per-delta count assignment in patternBatch is now dead work (kept intentionally: outputCountsSet=true set there remains load-bearing for the limiter gates, and collapsing it pre-commit adds risk for cosmetics); (3) `resultsetOrderbyMultiDeliveryRuntimeID` ignores caseName — intentional single-runtime mapping consistent with the scenario.
