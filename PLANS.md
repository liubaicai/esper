...
activity or a single coverage percentage.
 ...
 - Updated: 2026-08-28
 - Baseline: `HEAD` == `origin/master` at Draft 4.276 (`ef04f09cb1ccba607125cb17c9348ec45c5c6870`), pushed. Draft 4.276 MinMax/HAVING passed focused/full gates and independent parity review.
 - Closed unit (Draft 4.276, commit `ef04f09cb`): `ResultSetAggregateMaxMinGroupBy.java` ordinals 0 and 4 were differential-verified with 14 Java/Go records and zero differences; reviewer `ReviewMinMaxParity` returned PASS.
 - Closed unit (Draft 4.275, commit `77b01e8d1`): `ResultSetQueryTypeRowForAll.java` ordinals 4-8 were differential-verified with 33 Java/Go records and zero differences.

## Current work unit
Draft 4.277: extend `ResultSetAggregateMaxMinGroupBy.java` with adjacent executions `ResultSetAggregateMinMaxOM` and `ResultSetAggregateMinMaxViewCompile` (suite ordinals 1 and 2; static candidate `java-0ca3164935a71ca95033`; runtime IDs `java-runtime-2408009c4113ea2214c4` and `java-runtime-30dbed587648347e610a`). The fixed Java oracle is `/root/app/esper` at commit `9e1b9f1cc9117fea4bf33ab043762c045d73839c`.
The frozen contract is: both executions are semantic twins of shipped `ResultSetAggregateMinMax`, using `irstream symbol,min(volume),max(volume),min(distinct volume),max(distinct volume)` over `SupportMarketDataBean#length(3)` filtered to DELL/IBM/GE and grouped by symbol. OM additionally builds/copies `EPStatementObjectModel` and asserts its exact EPL before deployment; ViewCompile uses `eplToModelCompileDeploy`. Both expose String symbol and boxed Long aggregate fields, emit the same 12 listener callbacks for the exact DELL/IBM sequence, preserve global length eviction, duplicate extrema, null skipping/all-null output and IBM-then-DELL multi-row order. No virtual time, iterator, error phase or join is in scope.
Read-only scouts `JavaContract` and `GoSurface` froze the contract and confirmed the existing typed Min/Max/DistinctAggregate/length-window surface. The implementation adds a separate two-runtime parity scenario/oracle so the shipped MinMax/HAVING evidence remains immutable; primary owns runner dispatch/tests, generated traces/evidence, manifest, roadmap, CHANGELOG, README, formatting, validation, commit and push.
Independent reviewer `ReviewDraft4277` confirmed 24 Java/Go records, exact callback cardinality, metadata and trace mutation coverage, and required documenting the representation boundary. The manifest records the case, static IDs `java-8ef85c99fae01e5e4431`/`java-a340ca15a11cf67e3170`, runtime IDs, evidence paths and mapping. The approved difference is explicit: Java exercises SODA copy/toEPL and EPL-to-model compilation; Go uses one equivalent typed immutable Query/Plan builder because it intentionally exposes no EPL parser or second mutable SODA hierarchy. The persisted zero-difference comparison is limited to normalized listener rows; the Java trace proves the fixed Java paths executed.
## Delivery
Validation complete before commit: fixed Java oracle replay plus Go replay and differential evidence for both runtime IDs report 24 Java/Go listener records and zero differences; trace mutations (value, multi-row value, order, time, missing record) are rejected. `go test ./internal/app/parity -run 'TestRunResultSetAggregateMinMaxGroupByOMViewCompile|TestRunHelpSucceeds' -count=1 -timeout 240s`, `go test ./internal/compat/... -count=1`, `go vet ./...`, `go test ./... -count=1 -timeout 240s`, `make check`, `./scripts/check-layout.sh`, `git diff --check`, manifest validation and parity JSON validation all passed. Independent reviewer `ReviewDraft4277` re-reviewed after central fact repair and returned PASS.
Central facts record the new case/evidence/mapping, static IDs `java-8ef85c99fae01e5e4431`/`java-a340ca15a11cf67e3170`, runtime IDs, approved Java SODA-versus-Go typed-plan representation difference, and summary 569 cases / 182 differential-verified cases / 703 differential runtimes / 3324 associations. Create one semantic commit and push only after these gates pass; do not add a checkpoint-only hash update.
## Handoff
