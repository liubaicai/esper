...
- Completed: Draft 4.311 (`resultset-output-limit-row-limit-variable`) is committed and pushed at `bdc968996`; Java/Go each produced 69 records with zero-difference evidence, and all local gates plus independent parity review passed.
- Completed: Draft 4.311 (`resultset-orderby-aggregate-grouped` ordinals 0-4) is committed and pushed at `bd2327e40`; Java/Go each produced five listener records with zero-difference evidence, and all local gates plus independent parity review passed.
- Completed: Draft 4.312 (`resultset-orderby-aggregate-grouped` ordinals 5-7, all 8 executions differential-verified, 11 records per side, 0 differences, gates and independent review PASS) is committed and pushed to master; Git owns the commit identity.
- Current target: Draft 4.317 implemented — `case.resultset-output-limit-insert-into` (InsertFirst + InsertSnapshot) differential-verified (9 records per side, 0 differences); gates and independent review PASS; commit and push remain. Deferred from this prefetch: ChangeSetOpt (white-box changeset counter + hint/order-by compile rejection = engine work), MicrosecondResolution (needs fractional-rate parse verification), ParameterizedByContext (per-partition context-property cron verification).
## Current work unit
Shipped: Draft 4.315 (`case.resultset-orderby-self-join`) — all items below are complete, including gates, independent review `SelfJoinReview` PASS (3 P3 notes: verification-paragraph placeholders fixed before commit; the "review PASS" target line accurate as of commit; a pre-existing double `evaluateAggregateGroup` call, observation only), and the commit/push (Git owns identity).
- Engine fix (single writer, primary): `internal/esper/runtime.go` `snapshotJoinAggregateBatch` now emits one row per retained join tuple for UNGROUPED join aggregates that read non-key scalars (Java AGGREGATED_UNGROUPED RowPerEvent iterator shape); previously all ungrouped join aggregates collapsed to one group-representative row. Probe-verified, then full `internal/esper`, `internal/app/parity`, and `internal/compat` suites pass with no collateral change.
- Asset writer `agent_9a01e1c0-5f50-4c15-bb8f-cfea8cedbea8` authored the oracle/script/scenario; primary repaired two asset defects found by the pinned oracle run: (1) EXPECTED_FIELDS sorted order (`prio` sorts before `priority`) and (2) expectation tuples written in (cnt,ecid,priority,prio) order while validation indexes positionally against sorted fields — retupled to (cnt,ecid,prio,priority); an empirical debug run confirmed the actual Java values first (callbacks prio1-first, snapshot prio1-first — consistent with the Java regression's ordered assertion).
- Go runner uses flat `JoinMany(JoinSource...)` (JoinChain requires per-edge conditions; EPL has none), `AggregateStream.Where` with `In[int]`/`InOf` (SQL 3VL null parent candidate), `Cast[int64,int](CountAll())`, `GroupWindow(Field, LastEvent())`, and the no-subscribe iterator replay; all plain fields in aggregate where/select must be JoinField (engine validation).
- [x] Freeze self-join contract (prefetch).
- [x] Implement join-iterator snapshot shape engine fix.
- [x] Implement resultset-orderby-self-join runner, modes, and tests.
- [x] Author self-join oracle, script, and scenario via asset writer.
- [x] Generate traces and zero-difference evidence.
- [x] Update manifest, roadmap, and CHANGELOG.
- [x] Run gates and independent parity review.
- [x] Commit and push resultset-orderby-self-join unit.
Self-join verification: `go test ./internal/app/parity -run 'TestRunResultsetOrderbySelfJoin' -count=1` passed (direct replay, passing evidence, 10 mutations reject, checked-in evidence matches both traces and live replay, 8 malformed-scenario mutations reject, runtime-ID mapping). The pinned Java 9.0.0 oracle compiled, emitted four records, and regenerated the checked-in Java trace byte-identically (cmp=0); evidence passing with 4 Java records, 4 Go records, 0 differences, one frozen runtime ID (Java trace SHA-256 `5f4f9d69be7470698cc394bd268ba816f4445384ea56067400cc1f01fd0bd008`, Go trace SHA-256 `b53391f78ab85c0367e43afaefc6a2f5b469ec037e64306ff7cd6985902dea3d`). The runtime was newly referenced (SelfJoin file had no prior case inventory), so referenced grows 3166 -> 3167 and unreferenced drops to 969. Full gates: `gofmt` clean, `go vet ./...`, complete `go test ./... -count=1`, layout equivalents (tracked-generated, batched `gofmt -l`, facade `cmp`, apidump `cmp`), `git diff --check`, JSON parses, and `bash -n` on the new runner script all pass; the recorded race-gate host limitation still applies. Final parity review `SelfJoinReview` (independent read-only reviewer, agent `agent_fd9b62fe-f1e5-4943-be41-fe1c60902b00`) returned PASS with zero P1/P2 findings and three P3 notes: (1) placeholder defects in this paragraph — fixed before commit; (2) the premature "review PASS" target line — accurate as of commit; (3) a pre-existing double `evaluateAggregateGroup` call in the group-representative path (observation only, no action).
Prefetch (Draft 4.315, contract organized, no writes yet): both read-only scouts completed. `SelfJoinJavaContract` (`agent_ea4f5d5e-4d3e-47ae-9c46-022b2d4c51a7`) adjudicated the EPL (3-way self-join: c1 `#lastevent`, c2/p `#groupwin(event_criteria_id)#lastevent`, where `c2.event_criteria_id in (c1.event_criteria_id,2,1)` and `p.event_criteria_id in (c1.parent_event_criteria_id, c1.event_criteria_id)`, select `ecid/priority/prio/cast(count(*),int) as cnt`, order by `c2.priority asc`), bean `SupportHierarchyEvent(Integer event_criteria_id, Integer priority, Integer parent_event_criteria_id)` (parent nullable), processor AGGREGATED_UNGROUPED row-per-event (one output row per new join row, batch-level count), events (1,1,null) → (3,2,2) → (3,2,2) with expected callbacks: R1 `{ecid=1, priority=1, prio=1, cnt=1}`; R2/R3 two rows `[{prio=1,cnt=2},{prio=2,cnt=2}]` (count stays 2 via net deltas −1+2, −2+2); final iterator snapshot `{{prio 1,cnt 2},{prio 2,cnt 2}}` is Java-asserted ground truth (strict positional, prio-asc). The oracle run must empirically pin R1-R3.
`SelfJoinGoSurface` (`agent_e590ed0e-3fc3-4a45-9757-9cdbb0cd7674`) confirmed NO engine gaps: flat `JoinMany(JoinSource(c1), JoinSource(c2), JoinSource(p))` (JoinChain requires ≥1 condition per edge, so no ON predicates → JoinMany), `AggregateStream.Where` for the post-join `And(In[0-arg...], InOf(*int...))` conjunction (typed `In` for the literal mix, `InOf` for the `*int` parent candidate; SQL 3VL null handling verified in expr.go), `Cast[int64, int](CountAll())` for cnt (aggregate detection sees through Cast), `GroupWindow(Field, LastEvent())` views, count-over-join removals decrement via `removeAggregateScopeEvents`, and the ungrouped row-for-event snapshot path emits one row per retained join tuple (`snapshotAggregateStateBatchInternal`). Iterator replay skeleton: `runResultsetOrderbyJoinIteratorReplay`. Scenario: one case `selfjoin`, observation listener, iteratorSnapshots 1, 5 steps (case + 3 sends + snapshot), payloads `{event_criteria_id, priority, parent_event_criteria_id}` = (1,1,null),(3,2,2),(3,2,2).
Plan: primary owns the Go runner (`internal/app/parity/resultset_orderby_self_join.go`, modes `resultset-orderby-self-join`/-diff) + tests; asset writer owns the new Java oracle/script/scenario. Watch-items: cnt must render as Go int (not int64/float); milestone(0) not modeled; run the oracle first to pin R1-R3 empirically.
Shipped: Draft 4.314 (`case.resultset-orderby-multi-delivery`) — patternBatch match-row recount + pattern order-by engine support with two regression tests; asset writer `agent_df37b522-41e8-4db1-952f-22f8bd93811d` authored oracle/script/scenario (primary corrected the contract-draft groupwin A2 int to the Java source value 1 and restored over-broad pins); Java/Go 3 records each, 0 differences, byte-identical oracle replay, all gates green, review `MultiDeliveryReview` (`agent_8507c95d-32f8-4934-b27f-1a9d929b58e3`) PASS with 3 accepted P3 notes; committed and pushed (Git owns identity).
- Completed: Draft 4.313 (`case.resultset-orderby-join`, `ResultSetOrderBySimple` ordinals 1-2) shipped: new join scenario/oracle/script by asset writer `agent_f96cdcf8-d81d-4eda-af6b-569c0467e65f`; primary fixed the oracle's shared-execution-index OOB and used `esper.SelectFrom` JoinSelection projections; Java/Go 4 records each, 0 differences, byte-identical oracle replay, all gates green, review `OrderbyJoinReview` PASS (1 accepted P3), committed and pushed (Git owns identity). Asset files: `testdata/parity/resultset-orderby-join.*`, `tools/java-oracle/ResultSetOrderByJoinScenarioOracle.java`, `tools/java-oracle/run-resultset-orderby-join.sh`.
Deferred with rationale (from this prefetch): `ResultSetOrderBySimpleSortCollator` (java-runtime-97855caf4970534092ae) requires Java-Collator-FR semantics plus a `sortUsingCollator=true` compiler-config knob — zero Go hits for collation; needs a dedicated engine feature unit. `ResultSetOrderBySelfJoin` (java-runtime-74b0c1ce48febfe83007) is a 3-way self-join with `groupwin`+`lastevent`, correlated `in` with null, `cast(count(*), int)`, and row-per-event ungrouped-aggregate semantics — viable but its own unit.
Draft 4.316 unit 1 status (all checked):
- Runner `internal/app/parity/output_after_events.go` (new file; the DV'd `output_after.go`/`output-after-last` family left untouched): strict pinned loader, 2 cases, `OutputAfterEvents(3)` plain + composed with `OutputWhen`/`SetOutputVariable`, `ReplayWithStatementsAndHandlers` with set-variable (`engine.SetVariable`) and read-variable (`engine.GetVariable` → variable records) handlers; run.go modes `output-after-events`/-diff.
- Asset writer `agent_c1b544ba-1554-4933-b7be-092923262945` authored the oracle/script/scenario and correctly deviated from the contract where the contract self-conflicted: step count is 14 (the enumerated list sums to 14, not the stated 11) and concrete E1..E5 payloads replace the "EX" placeholder. Primary repaired one oracle defect the pinned run exposed: the row-count check read the flat per-field values array (yielding 2) instead of the contract's 1-row callbacks — fixed to expect one row per callback; and made the Go payload decode strict (extra payload keys rejected) after the payload-extra mutation test caught the loose Unmarshal.
- [x] Freeze after-events contract (prefetch).
- [x] Implement output-after-events runner, modes, and tests.
- [x] Author after-events oracle, script, and scenario via asset writer.
- [x] Generate traces and zero-difference evidence.
- [x] Update manifest, roadmap, and CHANGELOG.
- [x] Run gates and independent parity review.
- [x] Commit and push output-after-events unit.
Shipped: Draft 4.316 unit 1 (`case.output-after-events`) committed and pushed (Git owns identity).
Draft 4.316 unit 2 (time slice: ords 4 DirectTimePeriod `java-runtime-77a5ebb703ccb5fdbf63` + 1 EveryPolicy `java-runtime-88c88732359bcd33f9cd`): the existing 4.316 prefetch reports already pin both timelines (ord 4: t=1/6000/19999 suppressed, 20000 boundary-inclusive callback {E4}, 21000 {E5}; ord 1: callbacks at 25000 [E4,E5] and 30000 [E6] with every-anchoring at the first post-gate batch) and the Go composition (`OutputAfterTime` plain and composed with `OutputEveryTime`; engine precedent output_test.go:221). Extend `output_after_events.go`/`output-after-events.json` with the two new cases (advance-time steps; keepall+Select theString), or follow the file-per-slice convention if cleaner — decide at implementation start and record the choice.
After-events verification: `go test ./internal/app/parity -run 'TestRunOutputAfterEvents' -count=1` passed (direct replay, passing evidence, 9 mutations reject, checked-in evidence matches both traces and live replay, 8 malformed-scenario mutations reject, runtime-ID mapping). The pinned Java 9.0.0 oracle compiled, emitted five records, and regenerated the checked-in Java trace byte-identically (cmp=0); evidence passing with 5 Java records, 5 Go records, 0 differences, two frozen runtime IDs (Java trace SHA-256 `49498c3a8c77795b427ce8c699014ae86d5c869be2e6f9d83c5a61f90ee2f8c5`, Go trace SHA-256 `551261745a11b6c06d9433f7c1af85f0459bcbd8a85191a22b94a4d7102b8716`). Both runtimes were already referenced by `case.output-after-basic`, so referenced/unreferenced stay 3167/969.
Draft 4.316 unit 2 status (all checked):
- Extended `output_after_events.go` and `output-after-events.json` with cases `after-20-seconds` (OutputAfterTime(20s); t=1/6000/19999 suppressed, 20000 boundary-inclusive {E4}, 21000 {E5}) and `after-20-seconds-every-5` (OutputAfterTime(20s, OutputEveryTime(5s)); every schedule anchored while gated — 25000 delivers [E4,E5], 30000 delivers [E6]); advance-time absolute steps cover both virtual-clock sequences. Loader now accepts the advance-time op; validator uses step-timing tables; assert rewritten for 9 records with explicit record indices (variables interleave at 3-4 within the when-then case).
- Asset writer `agent_469f45a0-ad14-4ddd-9ade-8645f2351b3e` extended the oracle/script/scenario (correctly deviating where the contract's step arithmetic slipped: sends 20, advance-times 13, CASES length 4, output case sequence 9). Primary repaired two oracle defects the pinned run exposed: (1) the every-5 validation loop paired all 6 sends with advances although E6 follows TWO advances (25000 fire + 27000 clock) — loop bound reduced to 5 with the explicit tail retained; (2) validateRows indexed the flat expectation array per-field for multi-row callbacks — switched to row-major (rowIndex * width + fieldIndex). Empirically confirmed the Java engine's actual deliveries ([E4,E5] at 25000 and [E6] at 30000) before freezing.
- [x] Extend oracle/script/scenario via asset writer.
- [x] Extend runner, loader, and tests with the time slice.
- [x] Generate traces and zero-difference evidence.
- [x] Update manifest, roadmap, and CHANGELOG.
- [x] Run gates and independent parity review.
- [ ] Commit and push the time-slice extension.
Time-slice verification: `go test ./internal/app/parity -run 'TestRunOutputAfterEvents' -count=1` passed (direct replay, passing evidence, mutations reject incl. the new gate-leak/batch-order/anchor-time mutations, malformed-scenario mutations reject incl. advance-time drift, runtime-ID mapping). The pinned Java 9.0.0 oracle compiled, emitted nine records, and regenerated the checked-in Java trace byte-identically (cmp=0); evidence passing with 9 Java records, 9 Go records, 0 differences (Java trace SHA-256 `2398ae1d64d0861ce90f5ac897398879320961ac8f0a8459e1bc86bd01dc2ca4`, Go trace SHA-256 `ffcb4ef9bf89e0559ec8d89dd97d43a320dd2498f6876dab48e82ad5b625ae44`). The two runtimes were already referenced by `case.output-after-basic`, so referenced/unreferenced stay 3167/969.
Draft 4.316 unit 3 status (all checked):
- Extended `output_after_events.go` with cases `after-1-month` (`OutputAfterCalendar(0,1,0)` over a window-less stream with `select *` default projection; engine clock anchored at 2002-02-01T09:00:00.000 via `WithStartTime`; 2002-02-28T08:59:59.999 advance suppressed, boundary advance delivers one select-* row E3/intPrimitive 3) and `after-20-seconds-snapshot-variable` (`RegisterVariable` int myvar_local=1; `OutputAfterTime(20s, OutputWhenWith(OutputSnapshot(), Equal[int](VariableRef, Literal(1))))`; suppressed events reappear in window snapshots [E1..E4] at 20000 and [E1..E5] at 21000). Loader/validator extended to 6 cases / 56 steps.
- Go engine fix (single writer, primary): `applyOutput` snapshot-when pending rebuild now requires a non-empty update batch — a bare time advance neither refreshes nor delivers the snapshot (Java evaluates output-when only on updates and variable changes). Empirically driven by the pinned oracle: Java delivered 2 callbacks for the snapshot case while Go delivered 4 (extra bare-advance fires at 20000/21000).
- Asset writer `agent_07fc7611-cfc6-4e55-9359-48e9dad8aa78` extended the oracle/script/scenario (56 steps, 28 sends, 19 advances, 12 records) and correctly resolved contract contradictions (anchor epoch via Instant.parse; CASE_START_TIMES placement; listener count 10; time format without .000 fraction; sends 28 / advances 19 totals).
- [x] Extend oracle/script/scenario via asset writer.
- [x] Extend runner with calendar + snapshot cases and the engine guard.
- [x] Generate traces and zero-difference evidence.
- [x] Update manifest, roadmap, and CHANGELOG.
- [x] Run gates and independent parity review.
- [ ] Commit and push the calendar+snapshot extension.
After-events review: independent reviewer `agent_c7b389ce-5eaa-4f73-aebe-5d85cf09dc31` returned PASS with zero P1 findings, one P2 fixed before commit, and accepted P3 notes. P2 fix: the month-case E2 advance was pinned as 2002-02-28T08:59:59.999Z (a first-scout mischaracterization of the calendar boundary); the Java source advances to 2002-03-01T09:00:00.000 minus 1ms = 2002-03-01T08:59:59.999Z — corrected across scenario/oracle/Go validator and the evidence regenerated (record checksums unchanged because advances are not recorded). P3 accepted: outputWhenMatches still evaluates on empty batches (a time/count-sensitive when-expression flipping true on a bare advance could deliver a stale whenPending — not exercised today); the oracle deploys variable+statement as one module versus Java's two-deploy @public path (semantically equivalent, path-visibility facet unexercised).
Schedule-unit verification (pre-commit): `go test ./internal/app/parity -run 'TestRunResultsetOutputLimitMicrosecond|TestRunResultsetOutputLimitParameterizedContext' -count=1` passed (direct replays, passing evidence, 7+3 mutations reject, checked-in evidence matches both traces and live replay, malformed-scenario mutations reject, runtime-ID mapping). Both pinned Java 9.0.0 oracles compiled and emitted traces; the microsecond oracle regenerated its checked-in trace byte-identically (cmp=0) and the parameterized oracle likewise (cmp=0 via the script gate). Evidence passing with 4 Java records and 4 Go records (microsecond, Java trace SHA-256 `f55c7902dc4a56e9baf6f23b802b22778d2079dd84ba72333ef0d0831dd8253b`, Go trace `00144cf2509c081f3de8248b5fea2d2cf1cd2261ae9d3eeaa8e2a5fb98c41c7f`) and 1 Java record and 1 Go record (parameterized, Java trace `121e3e90acd2cade455447fac01b27543b47035074b83aaf9b234679c9543271`, Go trace `64eeca69e1c3ed115a2781dac16a3356c1080fd4b52372255636158807dea312`), 0 differences. Both runtimes were newly referenced (neither file had prior case inventory), so referenced grows to 3172 and unreferenced drops to 964.
Independent review `SelfScheduleReview` (agent `agent_65036810-5647-45bc-af8b-617691c02bf7`) returned PASS with zero P1/P2 findings and three P3 notes: microsecond runtime-ID helper returns the same constant in both branches (intentional per frozen contract); `contextDef` assigned-then-discarded (cosmetic inconsistency vs `if _, err :=`); this verification paragraph records the SHA-256s (added).
Calendar+snapshot verification: `go test ./internal/app/parity -run 'TestRunOutputAfterEvents' -count=1` passed (direct replay, passing evidence, mutations reject incl. month-boundary-value/time, month-gate-leak, snapshot-window-value/count, checked-in evidence matches both traces and live replay, malformed-scenario mutations reject, runtime-ID mapping). The pinned Java 9.0.0 oracle compiled, emitted twelve records, and regenerated the checked-in Java trace byte-identically (cmp=0); evidence passing with 12 Java records, 12 Go records, 0 differences (Java trace SHA-256 `0d69ed3057dcbcc7e4bde087d9d6b5a7be0af0b1345be20d9ed68b27665352b5`, Go trace SHA-256 3899eaac346dd41e4cbe6a65f74fd691329a737bdfbc5f574ad84fcab7ea25ec). The MonthScoped runtime was already referenced (case.output-after-basic); SnapshotVariable was newly referenced, so referenced grows 3167 -> 3168 and unreferenced drops to 968.
Draft 4.317 status (all checked):
(4.317 shipped as commit 61effe8f1; the 4.318 prefetch scouts were relaunched after the session pause killed the first pair — new IDs below.)
- Frozen contract (from both prefetch scouts): `ResultSetOutputLimitInsertFirst` (java-runtime-e57a7555b3a0303c0303, static java-00bb626878ef4282b5d2) and `ResultSetOutputLimitInsertSnapshot` (java-runtime-cd524991d69bc898c061, static java-460f4166b6f5e1099aae) as cases insert-first and insert-snapshot; both deploy a two-statement module (producer s0 insert-into MyStream with an output policy + consumer s1 select * from MyStream); 10 steps; 9 records per side.
- Deferred with rationale (from this prefetch): ChangeSetOpt is white-box (OutputProcessView.getNumChangesetRows internal counter + ENABLE_OUTPUTLIMIT_OPT-with-order-by compile rejection) — needs dedicated engine work; MicrosecondResolution requires verifying the fractional-rate literal parse and canonicalizing the plain-suite (millis) parameters; ParameterizedByContext requires verifying per-context-partition cron resolution of context tag properties.
- [x] Freeze insert-into contract (prefetch).
- [x] Implement resultset-output-limit-insert-into runner, modes, and tests.
- [x] Author insert-into oracle, script, and scenario via asset writer.
- [x] Generate traces and zero-difference evidence.
- [x] Update manifest, roadmap, and CHANGELOG.
- [x] Run gates and independent parity review.
- [ ] Commit and push the insert-into unit.
Insert-into verification: `go test ./internal/app/parity -run 'TestRunResultsetOutputLimitInsertInto' -count=1` passed (direct replay, passing evidence, 8 mutations reject, checked-in evidence matches both traces and live replay, 8 malformed-scenario mutations reject, runtime-ID mapping). The pinned Java 9.0.0 oracle compiled, emitted nine records, and regenerated the checked-in Java trace byte-identically (cmp=0); evidence passing with 9 Java records, 9 Go records, 0 differences, two frozen runtime IDs (Java trace SHA-256 `3246b7836684a026b1f1379de1f95a7300566eb347e7c34becf0bb9701ac4ce5`, Go trace SHA-256 `f88bdd2828fd85c90bd34ab9bf667fb0125e22eeabcbb6585749d431f382e4dd`). Both runtimes were newly referenced (InsertInto file had no prior case inventory), so referenced grows 3168 -> 3170 and unreferenced drops to 966.
- Asset writer `agent_28a522bc-4c00-4e3c-b90b-9f23d7f55224` authored the oracle/script/scenario; primary repaired three oracle defects found by the pinned run: (1) nonexistent `compileModule(Module, args)` API replaced with `compile(module, args)`; (2) the per-callback row expectation flat array needed a per-listener cursor (rowCursor) since EXPECTED_ROW_STRINGS is a flat per-case list consumed across callbacks; (3) the insert-snapshot routed consumer (s1) actually receives THREE deliveries [E1]@1s, [E1]@2s, [E2]@2s (rows route individually) — script jq and main() count corrected to 9. Go runner/asset-writer joint verification: both engines agree on the routed delivery granularity.
Draft 4.318 contract (FROZEN — both scouts complete):
- `ScheduleJavaContract` (`agent_dd354bc7-8e0b-41b8-b260-aff9f793a33e`) adjudicated: MicrosecondResolution canonical registration is the PLAIN suite (millis unit) with two parameter sets run sequentially on one shared runtime — ordinal A (startTime=0, size="1", flip=1000, repeat=1000): deploy at 0, E1@0, 999 no-fire, 1000 FIRE [E1], E2@1000, 1999 no-fire, 2000 FIRE [E2]; ordinal B (startTime=789123456789, size="0.1" = 100ms, flip=+100): deploy, E1, flip-1 no-fire, flip FIRE [E1], E2, flip+100-1 no-fire, flip+100 FIRE [E2]. Boundary INCLUSIVE (schedule==now fires via headMap(t+1)); every-timer anchored at deploy-time reference. "0.1 seconds" parses via deltaForSecondsDouble → 100ms (millis abacus). ParameterizedByContext: context MyCtx start SupportScheduleSimpleEvent as sse (ctor (athour, atminute) — (10,15) means hour=10 minute=15), s0 = context MyCtx select count(*) as c from SupportBean_S0 output last at(minute=context.sse.atminute, hour=context.sse.athour, *,*,*) and when terminated; timeline: advance 2002-05-01T09:00:00.000, deploy, SSE(10,15) starts partition, S0(0) count=1 silent, advance 10:14:59 silent, advance 10:15:00 → ONE callback, row {c: 1L} (count long); "when terminated" never fires (start-only context).
- `ScheduleGoSurface` (`agent_31693a45-1f40-4292-93d4-874502a2de60`) confirmed ZERO engine gaps: OutputEveryTime interval arms at initializeAt (deploy reference) with inclusive tick (flipTime−1 no, flip yes, runtime.go:9291); CreatePatternInitiatedContext + PatternFrom captures sse tags per partition (context.go:1614, runtime.go:6970/7022/7024); cron resolves via CronValuesExpr(ContextPatternField[int]("sse","atminute"/"athour")) through partition variables (runtime.go:7024→11346→schedule.go:444-451, frozen once per partition via ensureCronSchedule runtime.go:11358); OutputAndWhenTerminated tolerates start-only contexts (end-pattern loop skips, runtime.go:7751); Aggregate(Alias("c", CountAll())) precedent context_output.go:62-68; WithStartTime pins the engine clock (context_output.go:73); compat advance-time parses RFC3339Nano (scenario.go:91).
- Plan: TWO scenarios in one unit — `resultset-output-limit-microsecond-resolution` (2 cases: one-second 0/1000/2000 and fractional 789123456789 + 100ms steps) and `resultset-output-limit-parameterized-context` (1 case: advance 2002-05-01T09:00:00Z, SSE, S0, advance 10:14:59Z, advance 10:15:00Z, record {c:1} @10:15). Go runner files mirror context_init_term_correlated.go (initialAdvanceTime helper, tagged PatternFrom start, ReplayWithStatements) with WithStartTime pinning the clock and UTC TZ.
Draft 4.318 prefetch (in flight, no writes yet): `ScheduleJavaContract` (`agent_dd354bc7-8e0b-41b8-b260-aff9f793a33e`) and `ScheduleGoSurface` (`agent_31693a45-1f40-4292-93d4-874502a2de60`) are adjudicating the two queued executions — ResultSetOutputLimitMicrosecondResolution (java-runtime-fb9601d7d5288b0b7ba5, plain-suite millis params 0/"1"/1000/1000 canonical) and ResultSetOutputLimitParameterizedByContext (java-runtime-3fc747eca88c34b6ab3a, context-property cron fields) — as Draft 4.318's two-case unit. Slicing, engine-work flags, and the frozen contract will be recorded here when the scouts return. (An earlier scout pair for this unit was killed by a session pause before delivering.)
Draft 4.319 prefetch (in flight, no writes yet): ChangeSetOpt white-box unit — `ChangeSetJavaContract` (`agent_d36697ea-d0bb-4ddb-ab69-de39f4c048a9`) is enumerating the full 36-round matrix (hint variants x select/group-by/having/last-all-first/order-by, getNumChangesetRows before/after timer, the UnsupportedOperationException conditions, and the ENABLE_OUTPUTLIMIT_OPT+order-by compile rejection) and `ChangeSetGoEngine` (`agent_49b53070-0193-4869-8cb8-ed6262f90a71`) is adjudicating the Go engine buffering locations and the minimal changeset-counter/hint-rejection design. Engine work is expected (single writer: primary). The frozen contract and design will be recorded here when both scouts return.
Draft 4.317 prefetch (COMPLETED — see the shipped status above): `SmallTailsJavaContract` (`agent_203ca19e-15a5-42b3-8eac-2d547ac79c94`) and `SmallTailsGoSurface` (`agent_e92f9d8c-4cde-4c6e-a7e2-28f57aa6ab3f`) adjudicated the four single-execution OutputLimit files plus InsertInto's two executions — hints (ENABLE/DISABLE_OUTPUTLIMIT_OPT), microsecond-resolution output rates vs the Go millis clock, context-parameterized output rates, and insert-into routing observation. Slicing decision (one unit with four mini-scenarios vs regrouping) and any engine-work flags will be recorded here when the scouts return.
Draft 4.316 prefetch (frozen, no writes yet): both read-only scouts completed for the `ResultSetOutputLimitAfter` after-gate family. `AfterJavaContract` (`agent_e5b8a798-d0d3-4a74-ac08-089bedd675ff`) adjudicated the engine semantics: event-count after counts INPUT events and satisfies at cumulative > N (Nth event still suppressed); time/calendar after is boundary-inclusive from view-creation clock; the gate runs BEFORE every/snapshot/when so every-counter/schedules anchor at the first post-gate batch; unaggregated-ungrouped gated events are dropped except snapshot reads the whole window. Per-execution timelines pinned (ord 1 EveryPolicy: callbacks at 25000 [E4,E5] and 30000 [E6]; ord 2 MonthScoped: Feb-2002 28-day boundary minus-1ms suppressed, exact boundary outputs E3; ord 3: after 3 events → E4/E5 per-event; ord 4: 20 seconds boundary-inclusive at 20000; ord 5 SnapshotVariable: snapshot [E1..E4] at 20000 and [E1..E5] at 21000 with @public int variable; ord 6 WhenThen: gate at 3 events + when myvar0=true then set myvar1/myvar2). `AfterGoSurface` (`agent_9ca504b7-f797-4ca4-a91e-f531e5b8d71c`) confirmed the API is fully present (`OutputAfterEvents/Time/Calendar` with base-policy composition, `OutputWhenWith`, `SetOutputVariable`, `WithStartTime` for the month anchor) — zero engine work expected — and recommends extending the existing `output-after` scenario/runner (currently single-case `after-last`, mode `output-after`/-diff at run.go:684) to the per-case pattern, adding `set-variable` handling via `ReplayWithStatementsAndHandlers` where needed. Slicing: three units — events (ords 3,6), time (ords 4,1), calendar+snapshot (ords 2,5); ord 0 stays with the DV'd `case.output-after-last`. Risks: month-anchor clock pinning via `WithStartTime` (2002-02-01T09:00:00.000), `select a.*` projection shape, variable naming/handler plumbing, strict-vs-loose validator convention in output_after.go.
## Delegation checkpoint
Scouts for this unit (both read-only, ran concurrently before implementation): Java oracle scout `agent_3439c4cd-50ed-4112-b5d0-12cbdcc25ee0` (froze ordinals 5-7 event vectors, output-last row-per-group batches, iterator join multiplicity, oracle extension mechanics, sequence-0 snapshot convention) and Go surface scout `agent_9ab3c16b-c71c-4670-90cc-574a9ef6cae2` (confirmed `OutputLastEveryEvents`, snapshot op with `mode:"any"`, zero internal/esper or internal/compat changes, no run.go change, lockstep extension plan). Primary is the shared-core writer (runner + scenario + tests); the Java oracle extension is parity-asset work that may be delegated to an isolated asset writer. Planned record count is 11 (5 existing listeners + 2 last-join + 2 iterator snapshots + 2 last) and 84 steps (40 + 18 + 13 + 13); the Go scout's "9 records" note was corrected by the Java scout's arithmetic and both must be pinned by the actual oracle run.
Asset writer `agent_d706670f-72ba-4add-9cb4-03da11af74d9` (isolated parity-asset writer) authored the Java oracle, runner-script jq pins, and scenario JSON on the frozen contract while the primary wrote the Go runner and tests on disjoint files; primary review found the patches contract-exact. Integration repairs owned by the primary: (1) snapshot steps switched from `mode:"any"` to strict positional order (matching the row-per-group precedent — the differ's any-sort canonicalization made evidence rows diverge from the raw pinned Java trace because `{"sumPrice":"149"}` sorts before `{"sumPrice":"65"}` lexicographically); (2) the iterator case needs a no-subscribe custom replay path (`runResultsetOrderbyAggregateGroupedIteratorReplay`) because `ReplayWithStatements` auto-subscribes and would record the continuous grouped-join listener rows Java never records; (3) runner-script portability: quoted `$java_bin` version probe (unquoted expansion broke on spaced Windows paths) and platform-aware classpath separator (`;` + `cygpath -m` on Windows, `:` on Linux).
- [x] Freeze ordinals 5-7 contract.
- [x] Extend aggregate-grouped Go runner and iterator replay.
- [x] Extend aggregate-grouped semantic tests and mutations.
- [x] Generate aggregate-grouped traces and evidence.
- [x] Update aggregate-grouped manifest and documentation.
- [x] Run gates and independent parity review.
- [x] Commit and push aggregate-grouped completion unit.
Prefetch delegation checkpoint: `OrderByNextJavaContract` (java-oracle scout, `agent_b1e667aa-bcf6-41b5-9fd6-72c8f7b5c3c7`) and `OrderByNextGoSurface` (Go surface scout, `agent_579c3c40-50e4-4ffb-a270-8e2c1cc2b8e6`) completed concurrently as read-only prefetch for Draft 4.313; their frozen candidate and deferrals are recorded in the Current work unit section. No Draft 4.313 writes have started; implementation must begin with this checkpoint's contract update and its own delegation plan (asset writer for the new Java oracle + scenario, primary for the Go runner, tests, traces, evidence, and central facts).
Aggregate-grouped verification: `go test ./internal/app/parity -run 'TestRunResultsetOrderbyAggregateGrouped|TestRunHelpSucceeds' -count=1` passed; the pinned Java 9.0.0 oracle replay compiled, emitted eleven records, and regenerated the checked-in Java trace byte-identically (`cmp=0`); Java/Go traces and evidence contain 11 records per side with zero differences (Java trace SHA-256 `2fdf1ec670bdc078f78840be98a6502da4948cacebd746f42af89758e64afdd5`, Go trace SHA-256 `fe2dcfdf4d128aa50df755f8256abbe5775d6ea2f134e5f4f07467b972245930`). Manifest JSON validates (`TestCapabilityManifestArtifactValidates`). Full gates: `gofmt` clean on changed files, `go vet ./...`, complete `go test ./... -count=1`, layout equivalents (tracked-generated, batched `gofmt -l`, facade `cmp`, apidump `cmp`, `go list`), `git diff --check`, scenario/trace/evidence/manifest JSON parses, and `bash -n` on the runner script all pass. Gate exception (recorded): `go test -race` cannot run on this Windows host — cgo requires a C compiler and none exists in PATH (`exec: "gcc": not found`), and WSL has no Go toolchain; this unit changes zero `internal/esper`/`internal/compat` production code, and the complete non-race suite passes. `make` itself is unavailable on this host; the identical checks were run manually. Final parity review `AggregateGroupedCompletionReview` (independent read-only reviewer, agent `agent_ff934d71-3fa3-4415-ba98-e8a257139255`) returned PASS with zero P1/P2 findings and three P3 notes, all accepted without action: (1) the oracle's iterator-case listener swallows hypothetical spurious callbacks (symmetric with Go's no-subscribe replay; impossible for a no-output-clause statement); (2) Java milestones are not modeled as scenario steps (output-neutral, matches the committed ordinals 0-4 convention); (3) the capability-manifest rewrite normalizes pre-existing inconsistent indentation (cosmetic; `git diff -w` confirms no substantive change outside this unit's edits).
