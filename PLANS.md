...
- Shipped: Draft 4.318 (microsecond-resolution + parameterized-context) committed at `2fa5b7f54`; Git owns identity.
- Shipped: Draft 4.318 groundwork (changeset counter, hint gate, order-by compile rejection, `Statement.NumChangesetRows`) committed at `cb0539923`.
- Current target: Draft 4.319 ChangeSetOpt white-box differential — Go runner wired, 72/72 records zero-difference vs Java, evidence passing, engine double-count fix in working tree; reviewer dispatch and commit remain.

## Current work unit
Active: Draft 4.326 ('infra-nwtable-start-stop') — mid-lifecycle undeploy/redeploy of insert and consumer statements over NW/table; contract frozen from both prefetch scouts; asset writer dispatched, primary implementing the runner.

Frozen contract (scouts 'JavaContract4326' + 'GoSurface4326'): 4 executions of InfraNWTableStartStop: Consumer{namedWindow=true} java-runtime-e14796f7892a57096988 ordinal 0 (flags [OBSERVEROPS]); Consumer{namedWindow=false} java-runtime-7ef0dbf9f8ea7e311720 ordinal 1 (flags [OBSERVEROPS]); Inserter{namedWindow=true} java-runtime-6c0691c9d5549b937cfc ordinal 2 (flags []); Inserter{namedWindow=false} java-runtime-683b282c41f311ed8429 ordinal 3 (flags []). Static ids: Consumer java-2fe58125cb5f265d5fd1 (both consumer cases), Inserter java-c4a5793bec169880d651 (both inserter cases). Infra: keepall NW projected (a=theString string, b=intPrimitive int) or composite-PK table (a string pk, b int pk); unnamed insert into MyInfra select theString as a, intPrimitive as b from SupportBean; consumers select a, b from MyInfra. Listener behavior: NW create+select listeners fire per insert; table variant produces ZERO listener records (table emits no istream to listeners). Inserter variant timeline: E1 fires create+select; stop insert (undeploy); E2 silent; redeploy insert; E3 fires create+select NW (ordered select iterator [E1,E3]); stop insert; E4 silent. Consumer variant: E1 fires create+select; stop select; E2 → create fires, old select listener silent; RESTART select → new consumer immediately sees current window [E1,E2] via iterator (both variants, unguarded); E3 fires create+select (ordered select iterator [E1,E2,E3]); stop select; E4 → create fires, select silent.

Go surface (scout adjudication): ALL READY, zero engine work. Per-statement undeploy via one-deployment-per-statement + engine.Undeploy(ctx, id) (variables_use.go:693-709 precedent mirroring undeployModuleContaining); redeploy against the same env path works (env catalog survives engine undeploy); new consumer sees current window rows immediately (infra_nwtable_subquery.go:137 precedent); iterator observations via Statement.Snapshot (NW) and FAF-over-FromTable (table) — snapshot op supports mode any for order-insensitive multi-row checks. Scenario ops: case/deploy/undeploy(statement)/send/snapshot/undeploy-all. Mode 'infra-nwtable-start-stop'/-diff; file internal/app/parity/infra_nwtable_start_stop.go; iteratorSnapshots records added (first unit in the family to use snapshot records).

- [x] Freeze start-stop contract (parallel prefetch scouts).
- [x] Author oracle, script, and scenario via asset writer (24-record Java trace).
- [x] Implement infra-nwtable-start-stop runner, modes, and 6-test family.
- [x] Generate traces and zero-difference evidence (24/24, evidence passing).
- [x] Update manifest (617/230/816/3439), roadmap, and CHANGELOG; also repaired the committed correl-join evidence javaExecutions fragmentation (comma-naive CLI metadata splitting had shattered execution names containing commas; regenerated evidence now carries the 3 whole names and correl-join tests stay green).
- [x] Run gates and independent parity review: StartStopParityReview PASS (agent_d2b3179a).
- [x] Commit and push the unit (Git owns identity; no hash recorded here).

Start-stop review: parity review `StartStopParityReview` (independent read-only reviewer, agent `agent_d2b3179a-3a0c-46e1-930d-417ec16e963f`) returned PASS on all five dimensions with no P1/P2 findings. Fidelity verified against the checked-in scenario oracle + Java trace (fixed Java tree absent in environment): @Name/@name casing split, per-statement undeploy/redeploy, NW-vs-table listener gating, redeployed-consumer iterator visibility, E1-E4 timelines, PK/keepall infra split. All manifest summary numbers recomputed and matched; correl-join evidence repair confirmed as the only touched committed artifact, with a 223-file scan finding no other comma-fragmentation signatures. P3 notes: (1) Go validator pins step counts/EPLs/payloads/snapshot totals per case but not exact positional interleaving of undeploy/snapshot steps (mitigated by upstream-pinned scenario + trace diff); (2) create-table constant renamed to infraNWTableStartStopCreateTbl in unit; (3) latent: compat.TraceRecord.New omitempty vs Java oracle always emitting "new" for snapshots — a future empty-infra snapshot unit would diverge on key presence; keep in mind for snapshot-heavy units.

## Delegation checkpoint

Draft 4.326 unit:
- Prefetch scouts (read-only, concurrent, before implementation): Java contract scout 'JavaContract4326' ('agent_31eba2d1-2f6b-4257-83a4-71ae727035cf') pinned all 4 execution timelines incl. mid-case undeploys of specific statements (insert/select), the listener reset semantics (assertPropsNew resets, assertListenerNotInvoked does not), the NW-vs-table listener gating (all listener asserts inside if(namedWindow)), static ids per class, OBSERVEROPS flags for Consumer executions, and the snapshot-op mapping (mode any for multi-row).
- Go surface scout 'GoSurface4326' ('agent_d0b156be-478b-4e5c-86a5-694cee174254') adjudicated all items READY with precedents: per-statement undeploy via one-deployment-per-statement + engine.Undeploy (variables_use.go:693-709); redeploy against env path; Statement.Snapshot for NW iterators; FAF-over-FromTable for table iterator reads; no engine work.
- Asset writer (isolated parity-asset writer, concurrent with primary runner work): owns 'tools/java-oracle/InfraNWTableStartStopScenarioOracle.java', 'tools/java-oracle/run-infra-nwtable-start-stop.sh', 'testdata/parity/infra-nwtable-start-stop.json'.
- Primary owns the Go runner ('internal/app/parity/infra_nwtable_start_stop.go'), run.go wiring, run_test.go family, traces/evidence, manifest, roadmap, CHANGELOG, PLANS.md, gates, review, commit, and push.
