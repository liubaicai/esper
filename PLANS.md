...
- Current target: Draft 4.364 'dataflow-doc-samples' — NEW differential chain for EPLDataflowDocSamples's two uncovered executions (EPLDataflowDocSamplesRun java-runtime-b41da4e41f347dcd49a8 ord 0, EPLDataflowDocSamples java-runtime-aaf36f532374aca5a232 ord 1), continuing the dataflow DV upgrade. Follow-up queue: database slices (DB harness), select-state/representation leftovers.
...
## Current work unit
Active: Draft 4.364 ('dataflow-doc-samples').

- [x] Parallel read-only scouts dispatched. Go surface scout agent_6c75b0f5 COMPLETE — NAME CORRECTION: ord 1 is `EPLDataflowSODA` (java-runtime-aaf36f532374aca5a232), NOT "EPLDataflowDocSamples" (that name belongs to a different inner class in EPLDataflowOpSelect, INVALIDITY-flagged); the PLANS target line must be corrected before freeze. Exec 0 EPLDataflowDocSamplesRun (java-runtime-b41da4e41f347dcd49a8) is GREEN: hello-world BeaconSourceWithOptions{Iterations:1, text:"hello world"} -> LogSink graph, blocking Run to DataflowComplete, zero engine change; record shape = state record {value:"complete"} plus optionally a LogSink capture record (needs Java-side LogSink-capture decision: capturing appender vs provider substitution vs state-only fallback). Exec 1 EPLDataflowSODA is RED — pure parse→SODA→EPStatementFormatter round-trip with ZERO runtime observables (no Go parser/SODA surface per AGENTS EPL-not-primary) — disposition: compile-path-only case difference, zero records (4.358 replay-shape-drop precedent). Six named no-Go-surface dimensions (parse grammar, SODA round-trip, eventbean<> grammar, systemProperties, class: params, var parameter) all parse-only, no differential loss. Java contract scout agent_349235e5 in flight.
- [ ] Freeze contract; oracle/scenario/trace assets (writer) + Go runner + mode + tests (primary).
- [x] Manifest (NEW case), evidence, roadmap, CHANGELOG. (Writer agent_eaef00cf delivered oracle/scenario/script/Java trace: 8 records matching the frozen contract. Evidence passing 0 differences, 2 IDs/2 executions; BOTH trace artifacts committed; manifest NEW case.dataflow-doc-samples DV mapped to dataflow.graph; summary 636 cases / 251 DV cases / 923 DV runtime IDs / 3556 assoc / referenced 3291 / unreferenced 845; CHANGELOG entry. Roadmap count updated separately: epl 253→251 for 2 newly-referenced dataflow runtime IDs.)
- [x] Gates: affected packages + make check + git diff --check (green; recorded before commit).
- [x] Independent parity review. Reviewer agent_b679dc27-0ee1-40e3-aa8b-d3dcb6ddbc07 initially OVERALL FAIL on registry/prose integrity: (1) the case's javaNames[1]/static pairing and prose misattributed the OpSelect DocSamples execution as SODA; (2) the roadmap epl counter was not decremented; (3) the CHANGELOG repeated the wrong runtime attribution. All fixed in-unit and re-verified: manifest case corrected (name → EPLDataflowDocSamples, static IDs paired per-execution, javaSourceFiles +EPLDataflowOpSelect.java, difference populated for the SODA compile-path-only execution), roadmap epl 253→251 applied at all four sites with correct names/IDs in the supplement, PLANS scout derivation corrected. Reviewer's P3s also fixed: "five-Select" → six Select operators noted, IllegalStateException → UnsupportedOperationException corrected in comments, state records now read from the instance API (stateName helper). Four probes re-derived from the Java sources (BeaconSourceOp iterations=1 single-event, multi-source run() UnsupportedOperationException, LogSink stdout non-asserted, EPDataFlowState enum names matching).
- [x] Commit and push (single semantic commit with all checkpoint edits folded in).
