...
- Current target: Draft 4.336 'filter-rebool-context-pattern-value' — ExprFilterOptimizableBooleanLimitedExpr ords 6/7/8/12/13; contract frozen from ReboolJavaContract (agent_1a89be3a) + ReboolGoSurface (agent_f2bc15ed, all five zero engine work).
...
## Current work unit
Active: Draft 4.336 ('filter-rebool-context-pattern-value') — extends the shipped filter-rebool-optimizable chain (case.expr-filter-optimizable-boolean-rebool, ords 0/3/5/9/10 DV) with ords 6/7/8/12/13.

Frozen contract (from the Java source plus both scouts):
- ord 6 context-value-deep `java-runtime-273669bcae26e6ec054d` static `java-cfeb9cb387bbcf07e580`: `create context MyContext start SupportBean_S0 as s0;` + `@name('s0') context MyContext select * from SupportBean_S1(p10 regexp p11 || context.s0.p00)`; S0(1,".*X") start, S1(1,"gardenX","abc") F, S1(1,"garden","gard") F, S1(1,"gardenX","gard") T.
- ord 7 context-value-with-const `java-runtime-1da34c09fa675957b063` static `java-6f1b75c3a4b2d081eef2`: `p10 || 'abc' regexp context.s0.p00`; S0(1,"x.*abc"), S1(1,"ydotabc",null) F, S1(1,"xdotabc",null) T.
- ord 8 pattern-value-with-const `java-runtime-11f60c98a851b822d97f` static `java-c68e1b5ed06f5dbaa49a`: `pattern[s0=SupportBean_S0 -> SupportBean_S1(p10 || 'abc' regexp s0.p00)]`; same S0/S1 timeline as ord 7.
- ord 12 duplicate-like `java-runtime-0f83ba904d5457bbc6d2` static `java-6cafa38b41cc5bb01851`: `theString like '%' and theString like '%'`; SB("x",0) T.
- ord 13 duplicate-regexp `java-runtime-5253f42525fa48e8784c` static `java-636dd440a66dbb237561`: `theString regexp "test.*" and theString not regexp ".*\\.gov" and theString not regexp ".*\\.org"` (EPL text with embedded newline/4-space continuation, double-quoted literals, `\\.` escapes); test.com T, test.gov F, test.org F, x.com F.
- Approved adaptation: Java filter-started context (start stream SupportBean_S0, consumed by an S1 statement) maps to Go CreatePatternInitiatedContext(PatternFrom(From[S0],"s0",true)) + ContextPatternField[string]("s0","p00") in the filter, because Go filter-start evaluation reads only the consuming statement's stream events (runtime.go:7889 initiated-context start eval) while pattern-start state is fed all events (context_start_end_correlated precedent, DV). Observable partition lifecycle and value reads are identical; the engine-level filter-start gap goes into the manifest note as backlog.
- Deferrals: ord 1+4 → N+2 (MixedValueRegexpRHS 6-statement block; NoValueExprRegexpSelf trivial); ord 2 excluded (wall-clock delta<1000 primary assertion, EXCLUDEWHENINSTRUMENTED; behavioral core duplicates verified ord 0); ord 11 intentionally-different (compile-only SupportFilterPlanHook forge assertions, precedent case.expr-filter-optimizable-value-limited-disqualify).

- [x] Freeze contract (parallel prefetch scouts complete).
- [x] Asset writer: oracle SupportBean_S1 registration + 5 scenario cases + Java trace regen (end-to-end ×2, byte-identical; @name('ctx') adaptation empirically required — Java auto-names the unnamed create-context stmt-0).
- [x] Primary: 5 new parity tests in expr_filter_optimizable_boolean_limited_parity_test.go (10/10 rebool tests PASS on first run) + evidence extension (29 records, perCase 10 entries).
- [x] Manifest (extend case.expr-filter-optimizable-boolean-rebool: +5 runtime IDs, +5 goTests, notes), summary 856→861 DV runtime IDs / associations 3479→3484 / referenced 3238→3243 / unreferenced 898→893.
- [x] Roadmap section-0 supplement + domain tables recomputed to ground truth (epl 286, infra 156, event 151, expr 97, resultset 41, context 45, multithread 56, rowrecog 34, view 27 — sum 893; the prior table values were stale and the decrement initially landed on the wrong domain per review F1); CHANGELOG 4.336 entry.
- [x] Gates + independent parity review (agent_c5554d83-8df3-4c91-8129-8f1fa55254ba): initial FAIL on one P1 — the roadmap 5.2/6.1 decrement landed on the wrong domain (epl instead of expr; the table values were also stale) — plus three P3s (duplicate PLANS 4.335 paragraph, unchecked completed boxes, runtime.go citation drift). All four fixed in the same work unit (tables recomputed to ground truth epl 286/infra 156/event 151/expr 97/resultset 41/context 45/multithread 56/rowrecog 34/view 27, sum 893; 6.2 986→893) and re-confirmed OVERALL PASS by the same reviewer with an independent oracle rerun byte-identical to the checked-in trace. Residual P3 observation (section-4 prose staleness) also fixed pre-commit.
