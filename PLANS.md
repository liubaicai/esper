...
- Current target: Draft 4.356 'dataflow-types' — NEW differential chain for EPLDataflowTypes (java-runtime-a277455fea9aac5b640b EPLDataflowBeanType, java-runtime-8006afc99e90b53a008c EPLDataflowMapType; both DATAFLOW-flagged), upgrading the dataflow domain (many implemented-not-DV chains, one DV connector chain) with its second DV chain. Go dataflow differential harness exists (internal/app/parity/dataflow_connector.go + testdata/parity/dataflow-connector-output.*). Follow-up queue: dataflow lifecycle/select DV upgrades, DV-list consolidation, invalidity dispositions, database slices.
...
## Current work unit
Active: Draft 4.356 ('dataflow-types').

- [x] Parallel read-only scouts dispatched and complete. Java contract scout agent_5b9920a1: both executions are EPL-text create-dataflow graphs (DefaultSupportSourceOp with one canned event fanned out to two sinks; bean case outstream<SupportBean> → MySupportBeanOutputOp + SupportGenericOutputOpWPort with ports [0]; map case @public create map schema MyMap(p0 String,p1 int) on a path → MyMapOutputOp + DefaultSupportCaptureOp), instantiated via operatorProvider instances and drained synchronously by blocking run() — fully deterministic, no threads; 4 records recommended (sink-per-record, operation "capture", statement "flow", per-case sequence restart; WPort record carries ports:[0]); map case needs path semantics (schema deploy then flow deploy with path); op classes resolvable by installing regression-lib onto the script classpath (precedent run-dt-between.sh:62) rather than mirrors. Go surface scout agent_66751d46 COMPLETE (both executions GREEN over the dataflow_connector.go precedent; port-name→index adaptation identified as the one contract decision).
- [x] Freeze contract; oracle/scenario/trace assets (writer agent_74e0a45d) + Go runner + mode + tests (primary). (Both scouts complete: Java contract agent_5b9920a1, Go surface agent_66751d46. Runner: DefineDataflow + literal BeaconSource + Custom capture operators + synchronous Run; probe produced exactly the 4 frozen records; run.go modes + 3-test family + 5 mutations.)
- [x] Manifest (NEW case), evidence, roadmap, CHANGELOG. (compat: NormalizeEvents([]esper.Event) added at internal/compat/scenario.go:562 — NEW this unit, required by the runner to render raw captured operator events; identical row logic to NormalizeResults. Writer agent_74e0a45d delivered oracle via regression-lib install with real op classes, scenario, script (exit 0, deterministic rerun), Java trace 4 records matching the frozen contract and the Go probe exactly. Evidence passing 0 differences; BOTH .trace.json and .go.trace.json committed (4.355 P1 process fix applied); manifest NEW case.dataflow-types DV mapped to dataflow.graph (+2 capability DV runtime IDs on top of the connector chain's existing DV entry, static IDs from the static manifest); summary 631 cases / 244 DV cases / 906 DV runtime IDs / 3536 assoc / referenced 3278 / unreferenced 858; roadmap epl 260→258 (two newly-referenced epl runtimes — genuine change this time) + supplement; CHANGELOG entry.)
