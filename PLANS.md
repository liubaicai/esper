...
- Updated: 2026-08-28
- Baseline: `HEAD` == `origin/master` at Draft 4.274 (`b54566134799b76c438ebbd4174ed068c826a038`), pushed. Draft 4.275 RowForAll ordinals 4-8 is complete pending the semantic commit.
- Closed unit (Draft 4.274, commit `b54566134`): `ResultSetAggregationMethodSorted.java` execution `ResultSetAggregateSortedMultiCriteria` was differential-verified with 1 Java/Go record and zero differences; strict table sorted-key and trace mutation validation passed independent parity review.
- Closed unit (Draft 4.273, commit `6e721eac6`): `ResultSetAggregateSortedMinMaxBy.java` execution `ResultSetAggregateMultipleCriteriaSimple` was differential-verified with 4 Java/Go records and zero differences.

## Current work unit
Draft 4.275: `ResultSetQueryTypeRowForAll.java`, executions `ResultSetQueryTypeRowForAllSumOneView`, `ResultSetQueryTypeRowForAllSumJoin`, `ResultSetQueryTypeRowForAllAvgPerSym`, `ResultSetQueryTypeRowForAllSelectStarStdGroupBy`, and `ResultSetQueryTypeRowForAllSelectExprGroupWin` (ordinals 4-8, shared inventory/static source `java-00cfc4061ba3a5f163ff`; static candidates `java-bc11e2f6050e55363879`, `java-00cfc4061ba3a5f163ff`, `java-37e0c1f5635eeb35eb80`, `java-8365ed29927b20c08486`, `java-106ea4427ad8de418588`). The fixed Java oracle is `/root/app/esper` at commit `9e1b9f1cc9117fea4bf33ab043762c045d73839c`. Runtime IDs in execution order are `java-runtime-1d0f29a53fbb381563ec`, `java-runtime-061f312bd81e3cb1c340`, `java-runtime-422d65bd284a78278d56`, `java-runtime-9a7f951d363bb58d506e`, `java-runtime-780e3dede1e586fde738`.
The frozen contract is: SumOneView and SumJoin use `irstream sum(longBoxed)` over a 10-second time window, emit initial-null iterator state, events 10/15/-5 at t=0/5/8 seconds, and expiry IR pairs at t=10/15/18 seconds with post-expiry iterator values 10/-5/null; SumJoin first seeds keepall `SupportBeanString(KEY)` and only matching `SupportBean` rows contribute. AvgPerSym uses `groupwin(sym)#length(2)` with `irstream avg(price),sym` and intentionally preserves Java's observed rows A/1, B/1.5, A/4, B/7.5, then final A old/new both 8.5. Star uses `istream *` over grouped length(2), preserving a typed underlying market event; Expr uses `istream price` over the same view and emits scalar price 1. The Go representation uses typed `GroupWindow` retention, explicit selections, and fixed epoch virtual time.
Contract is frozen from `RowForAllJava275` and `RowForAllGo275`; implementation, direct fixed-Java replay, evidence regeneration, focused tests, repository gates, and independent review are complete. `RowForAllReview` verdict PASS: regenerated Java/Go traces contain 33 records each and zero differences; mutations cover aggregate values, old-row loss, grouped averages, wildcard type, and listener time/sequence. The bounded runtime repair only handles empty non-grouped join aggregate iterator snapshots and reuses `evaluateEmptyAggregateGroup`. Primary owns manifest, roadmap, CHANGELOG, PLANS, validation, commit, and push.
Required gates passed: focused RowForAll tests; direct fixed-Java/Go replay with 33/33 records and zero differences; trace mutations; evidence consistency; `go vet ./...`; `go test ./... -count=1 -timeout 240s`; `go test ./internal/compat/... -count=1`; `make check`; and `git diff --check`. No target runtime is currently associated in the manifest; the next step is central fact updates followed by one semantic commit and push.

## Delivery
Required gates: focused typed row-all tests, direct fixed-Java/Go replay for all five runtimes, zero-difference evidence and trace mutations, manifest/evidence consistency, independent parity review, `go vet ./...`, `go test ./... -count=1 -timeout 240s`, `go test ./internal/compat/... -count=1`, `make check`, layout, and diff checks. Create one semantic commit and push only after all pass; do not add a checkpoint-only hash update.
## Handoff
