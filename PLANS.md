...
activity or a single coverage percentage.
 ...
 - Updated: 2026-08-29
 - Baseline: `HEAD` == `origin/master` at Draft 4.279 (`d2f514984c2de51e2e200d207a16a34f55e9df02`), pushed; worktree was clean before Draft 4.280.
 - Closed unit (Draft 4.280; validation and independent review complete, final semantic commit/push is the delivery action): `ResultSetAggregateMinMax.java` ordinal 0, `ResultSetAggregateMinMaxNoDataWindowSubquery` was differentially verified with 4 Java/Go records and zero differences. The typed unbounded `Max`/`Min` plus `SubqueryValue` over `LastEvent` contract covers silent S0-only updates, empty scalar Null state, exact event order, fixed metadata, strict malformed/duplicate/payload validation, and value/order/time/count mutations; independent parity reviewer `ReviewDraft4280` returned PASS after repairing Java recursive strict validation and manifest boundary-test coverage.
 - Closed unit (Draft 4.279, commit `d2f514984`): `ResultSetAggregateMedianAndDeviation.java` ordinals 0-2 were differential-verified with 21 Java/Go records and zero differences; persistent StdDev enter/leave/clone repair, NaN poisoning coverage, strict metadata loading, malformed and raw mutation tests, and independent reviewer `ReviewDraft4279-2` returned PASS.
...
## Current work unit
Draft 4.280 delivery is complete and ready for the single semantic commit/push: `ResultSetAggregateMinMax.java` ordinal 0, `ResultSetAggregateMinMaxNoDataWindowSubquery` (static candidate `java-347fe4c3739663bf45c4`, execution inventory ID `java-09115f6e876ce88a42f9`, runtime `java-runtime-f1fedff9a25cca57fefa`, flag `EXCLUDEWHENINSTRUMENTED`). The fixed Java oracle is `/root/app/esper` at commit `9e1b9f1cc9117fea4bf33ab043762c045d73839c`.
Java contract is `@name('s0') select max(intPrimitive) as maxi, min(intPrimitive) as mini, (select max(id) from SupportBean_S0#lastevent) as max0, (select min(id) from SupportBean_S0#lastevent) as min0 from SupportBean`. The deterministic finite sequence sends `SupportBean E1/3`, `SupportBean E2/4`, `SupportBean_S0(2)`, `SupportBean E3/4`, `SupportBean_S0(1)`, and `SupportBean E4/5`; it yields four new-only callbacks `[3,3,null,null]`, `[4,3,null,null]`, `[4,3,2,2]`, `[5,3,1,1]`. S0 last-event updates are silent; outer SupportBean min/max is unbounded; empty scalar subquery is null and event order is observable.
Go contract uses typed `SupportBean` and `SupportBean_S0`, unbounded `From` plus `Aggregate(Alias("maxi", Max(...)), Alias("mini", Min(...)), Alias("max0", SubqueryValue(...)), Alias("min0", SubqueryValue(...)))`, inner `From[SupportBean_S0].Window(LastEvent()).AsRecord()`, and `ReplayWithStatements`. Fixed-Java replay, Go replay, zero-difference checked-in evidence, focused mutation/malformed/boundary tests, manifest/evidence consistency, layout, `go vet ./...`, `go test ./... -count=1 -timeout 240s`, `make check`, and `go test -race ./... -count=1 -timeout 600s` passed.
Delegation checkpoint: read-only Java scout `MinMaxSubqueryJava` and Go scout `MinMaxSubqueryGo` matched source/runtime/static identity and observable behavior; no shared `internal/esper` gap. Disjoint writes were assigned to `MinMaxSubqueryGo-2` (runner/dispatch/focused tests) and `MinMaxSubqueryAssets` (Java oracle/launcher/scenario); primary owns generated traces/evidence, manifest, roadmap, CHANGELOG, README, validation, review, commit, and push.
## Handoff
After Draft 4.280 delivery, verify the pushed ref read-only and select the next manifest/roadmap work unit; never add a checkpoint-only hash update.
## Handoff
