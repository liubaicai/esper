...
- Updated: 2026-08-29
- Baseline: `HEAD` == `origin/master` at Draft 4.281 (`18c58cd644ac5507c29200ae57f0f3ef9e6555fb`), pushed; worktree clean.
- Closed unit (Draft 4.281; validation and independent review complete): `ResultSetAggregateMinMax.java` ordinals 2-3, `ResultSetAggregateMinMaxNamedWindowWEver{soda=false}` and `{soda=true}`, were differentially verified with 8 Java/Go records and zero differences; named-window current/ever min/max, FIFO eviction, strict metadata/payload/trace mutation validation, full local gates, and reviewer `ReviewDraft4281-3` returned PASS.
- Closed unit (Draft 4.280): `ResultSetAggregateMinMax.java` ordinal 0 `ResultSetAggregateMinMaxNoDataWindowSubquery` was differentially verified with 4 Java/Go records and zero differences; reviewer PASS.
- Closed unit (Draft 4.279): `ResultSetAggregateMedianAndDeviation.java` ordinals 0-2 were differentially verified with 21 Java/Go records and zero differences; reviewer PASS.

## Current work unit
Draft 4.282: port the smallest safe slice of fixed `ResultSetQueryTypeRowForAllHaving.java`, ordinal 0 `ResultSetQueryTypeRowForAllWHavingSumOneView`. Runtime `java-runtime-65e4ba1499cb12f04d8a`, shared execution-inventory ID `java-45a56190252ed051c35f`, static candidate `java-52f40599a0c7dc213c2b`, inventory/static flags `[]`, source order ordinal 0. The fixed Java oracle is `/root/app/esper` at commit `9e1b9f1cc9117fea4bf33ab043762c045d73839c`.
Java contract is `@name('s0') select irstream sum(longBoxed) as mySum from SupportBean#time(10 seconds) having sum(longBoxed) > 10`; projected `mySum` type is `Long`. Fresh virtual-time execution sends `SupportBean(KEY,10)`, then at 5 seconds `15`, at 8 seconds `-5`, and advances to 10 seconds: the first send is suppressed, then new rows 25 and 20, then an old-only row 20 with null new data when the 10-second event expires. No iterator, object-model, compile-only, or malformed-input path is in this execution.
Go contract is a typed `SupportBean` with nullable `LongBoxed`, `TimeWindow(10*time.Second)`, `Sum[int64]`, `Greater(sum, Literal(int64(10)))`, `WithOldStream`, and statement `s0`; use the pinned runtime URI and strict replay metadata/payload/trace mutation validation. Do not bundle ordinals 1-2: SumJoin still needs sliding join old/new parity closure, and unique-window average needs an independent HAVING visibility probe.
Delegation checkpoint: read-only scouts `RowForAllHavingJava` and `RowForAllHavingGo` froze the source/runtime/static identity and selected this one-runtime slice; they found no shared `internal/esper` semantic gap. A Go slice writer owns only the new parity runner/dispatch/tests; a parity asset writer owns only the Java oracle/launcher/scenario. Primary owns generated traces/evidence, manifest, roadmap, CHANGELOG, validation, review, commit, and push.
Implementation, focused tests, fixed Java oracle replay, Go replay, and differential evidence are complete: both traces produce 3 callbacks (t5 new 25, t8 new 20/old 25, t10 old 20), evidence status is passing with zero differences, and malformed/duplicate/extra/order/time/type/range and trace-mutation checks pass. The runtime repair remains intentionally limited to ungrouped IRStream aggregate HAVING removal rows; grouped and row-per-event branches remain unchanged. Independent parity reviewer `ReviewDraft4282` returned PASS with no P0/P1/P2 findings. `./scripts/check-layout.sh`, `go vet ./...`, `go test ./... -count=1`, and `make check` all pass; central manifest, roadmap, CHANGELOG, and README facts are updated. The unit is ready for one semantic commit and push.

 ## Handoff
After Draft 4.282 delivery, verify the pushed ref read-only and select the next manifest/roadmap work unit; never add a checkpoint-only hash update.
