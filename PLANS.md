...
- Updated: 2026-08-28
- Baseline: `HEAD` == `origin/master` at Draft 4.269 (`d75597bff`), pushed. The Prev/Nth indexed-access unit is committed; the next indexed first/last unit is now being implemented.
...
## Current work unit
Draft 4.270: `ResultSetAggregateFirstLastWindow.java` indexed first/last slice, source execution `ResultSetAggregateFirstLastIndexed` (ordinal 5, runtime `java-runtime-30f62dc2e86a7a1e80a0`). The fixed Java oracle is `/root/app/esper` at commit `9e1b9f1cc9117fea4bf33ab043762c045d73839c`. Contract is the isolated primary statement `@name('s0') select first(intPrimitive, 0..3) as f0..f3, last(intPrimitive, 0..3) as l0..l3 from SupportBean#length(3)`; send 10, 11, 12, 13 and assert four new-only rows, null out-of-range index 3, and FIFO shifts after eviction. It excludes the execution's join, dynamic-variable, Math.abs, wildcard, invalid, old-stream, iterator, FAF, output-rate, time, and lifecycle variants.
Delegation checkpoint: `NextIndexedFirstLastJava` and `NextIndexedFirstLastGo` completed read-only scouts before this unit; they froze the exact source/inventory/runtime contract and confirmed existing typed First/Last APIs. No shared `internal/esper` production writer is needed. `IndexedFirstLastAssets` owns only the Java oracle, shell runner, scenario input, and dedicated internal/esper parity test; primary owns the parity runner, central mode/tests, generated trace/evidence, manifest, roadmap, CHANGELOG, validation, review, commit, and push. A singleton asset task is intentional: all remaining central runner work must stay with the primary and no independent shared-core slice exists.
## Progress
Candidate contract is frozen from fixed Java source lines 244-266 and inventory line 3324; existing typed indexed First/Last APIs and tests cover the required aggregate operators. The disjoint oracle, scenario, and typed parity test assets are complete; central runner wiring, generated trace, and differential evidence are complete.
Implementation complete: indexed `First` and `Last` are wired through a fresh typed Go aggregate query; the generated pinned Java trace and Go replay each contain 4 listener records and the differential evidence is passing with zero differences. Focused aggregate, runner, compat, shell syntax, JSON, and diff checks pass.
Manifest and migration documentation are updated for one new differential-verified case/runtime (563 cases, 175 differential-verified cases, 688 differential runtime IDs, 3311 associations, 99/102 representative scenarios). Independent `IndexedFirstLastReview` re-review returned strict PASS with no concrete P0-P2 findings.
N+1 selection remains deferred until this unit is committed; the join and dynamic-variable phases of `ResultSetAggregateFirstLastIndexed` remain separate follow-up boundaries.
## Delivery
