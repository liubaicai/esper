...
- Current target: Draft 4.338 'insert-into-populate-ctor' — EPLInsertIntoPopulateUnderlying ords 0/1/2/11; contract frozen from PopulateUnderlyingJavaContract (agent_fae76b58) + PopulateUnderlyingGoSurface (agent_8cd5b2e7, all four READY zero engine work).
...
## Current work unit
Active: Draft 4.338 ('insert-into-populate-ctor') — extends the shipped epl-insert-into-populate-underlying chain (case.epl-insert-into-populate-underlying, ords 3/4/5/6 DV; harness: tools/java-oracle/EPLInsertIntoPopulateUnderlyingScenarioOracle.java + testdata/parity/epl-insert-into-populate-underlying.{json,trace.json,evidence.json} + internal/esper/epl_insert_into_populate_underlying_parity_test.go) with ords 0/1/2/11.

Frozen contract (from the Java source plus both scouts; zero engine work):
- ord 0 EPLInsertIntoCtor `java-runtime-706d3ed19b9a9b680430` static `java-a12a755abb439b882d37`: 5 stages — (1) 4-col projection into ctor(String,Integer,int,bool): E1/2/true/100, E2/3/false/101, null/4/true/null; (2) 3-col `theString, null, intBoxed` → theString=E1, intBoxed=null, intPrimitive=100 (boxed→primitive positional); (3) join wildcard ST0#lastevent×ST1#lastevent → ctor(ST0,ST1); (4) `insert into SupportBeanCtorOne(theString, intPrimitive) select 'E1', 5` — Java ignores the column list for ctor targets, 2 select cols → 2-arg ctor, intPrimitive keeps ctor default 99 (Go: alias projection + WithJSONDefaults(intPrimitive=99), column-list-ignored is an approved API-surface difference); (5) same-type ctor `select c1,c2` from two same-schema lastevent streams.
- ord 1 EPLInsertIntoCtorWithPattern `java-runtime-42b687e10ab403b319bd` static `java-6842f27f1803589c55ed`: `pattern[every s=ST0 -> [2] e=ST1]` → ctor(ST0, ST1[]); Go: MatchUntil(2,2) + TagEvents("e") into []Event field; ST0("E0",1), ST1("E1",2), ST1("E2",3) → st0.id=E0, st1 len 2 ids E1/E2.
- ord 2 EPLInsertIntoBeanJoin `java-runtime-43b50e297c1593da8ec3` static `java-f4cea06a7d5a2310de97`: SupportBean_N×SupportBean_S0 lastevent join → SupportBeanObject select * / select one,two (identity preserved through routes; assertSame → Go identity), stage 3 is a comment/text artifact (same observable as stage 2, not separately pinned), stage 4 MyLocalTarget value=1.
- ord 11 EPLInsertIntoWindowAggregationAtEventBean `java-runtime-5c663b3a17e6e4aac880` static `java-92ef442cc9f141361514`: `insert into SupportBeanArrayEvent select window(*) @eventbean from SupportBean#keepall` → Go `From[SB].Window(KeepAll()).Aggregate(Alias("array", WindowEvents())).InsertInto("SupportBeanArrayEvent")` into struct with []Event field, insertion order E1,E2.
- Approved adaptations (documented in evidence/manifest notes): Java ctor/factory targets → Go RegisterStruct targets populated by named projection; ctor defaults via WithJSONDefaults; column-list-ignored semantics is an API-surface difference; ord 2 stage 3 fully-qualified-name target is unobservable text.
- Deferrals: ords 9+10 (until-pattern array population, per-representation) → next unit; ord 7 (CharSequenceCompat, zero-event deploy smoke) and ord 8 (factory-observable half) → small unit after; ord 12 (invalidity) → implemented-only Build-error unit restricted to reproducible sub-cases.

- [x] Freeze contract (parallel prefetch scouts complete).
- [x] Asset writer: 4 executions → 10 fresh-runtime sub-cases appended (agent_1794953f; scenario +29 steps, oracle +10 registrations +bean-property expansion whitelist; trace 24 records = 11 prior byte-identical + 13 new, triple-run byte-identical, MyLocalTarget registered as public static nested class by FQN).
- [x] Primary: 4 parity tests (5 ctor subtests + pattern + bean-join 3 subtests + window aggregation; all PASS — one build fix: Stream has no .Select method, one deref fix via iipuBoxed) + evidence extension (24 records, 14 goTests, 8 runtime IDs).
- [x] Manifest (+4 runtime IDs/names/goTests, notes), summary 863→867 DV runtime IDs / associations 3486→3490 / referenced 3245→3249 / unreferenced 891→887.
- [x] Roadmap supplement + epl 286→282 (5.2/6.1 and section-4 prose; the class is suite/epl/insertinto → epl domain); CHANGELOG 4.338 entry.
- [x] Gates + independent parity review (agent_a74a6926-78a3-4882-a9ad-1efc82ae7594): OVERALL PASS on all five dimensions with an independent oracle rerun byte-identical to the checked-in trace and manifest/table arithmetic recomputed-exact (epl 282/infra 156/event 151/expr 95/multithread 56/context 45/resultset 41/rowrecog 34/view 27, sum 887); two P2s (evidence note misattributed the deduped stage to ord 0 instead of ord 2; stale caseMatchSummary missing the 10 new sub-cases) and three P3s (weak row0 window assertion, dead if/else in bean-join producer, PLANS placeholder fragment) all fixed in the same work unit.
