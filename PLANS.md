...
- Current target: Draft 4.312 extends `resultset-orderby-aggregate-grouped` with `ResultSetOrderByAggregateGrouped.java` ordinals 5-7 (`ResultSetLastJoin`, `ResultSetIterator`, `ResultSetLast`); contract frozen, scouts launching.
## Current work unit
`ResultSetOrderByAggregateGrouped.java` ordinals 5-7 complete the file's eight executions inside the existing `case.resultset-orderby-aggregate-grouped` scenario. Fixed Java commit is `9e1b9f1cc9117fea4bf33ab043762c045d73839c`; runtime IDs in ordinal order are `java-runtime-12d689fc781af934b76d` (`ResultSetLastJoin`, static `java-663c2d183bbd4dc98fa2`), `java-runtime-680e7b6016501fed42cb` (`ResultSetIterator`, static `java-0de948d5133afc56e9fe`), and `java-runtime-18eacf385e5ac00e8a80` (`ResultSetLast`, static `java-726632af45ffc7989e27`); all flags are empty; shared inventory ID is `java-015649f9c0449597e55a`.
Case `last` (ordinal 7) deploys `@name('s0') select symbol, volume, sum(price) from SupportMarketDataBean#length(20) group by symbol output last every 6 events order by sum(price)` and listens new-only. Batch one sends IBM/101/3, IBM/102/4, CMU/103/1, CMU/104/2, CAT/105/5, CAT/106/6 (milestones after events 2 and 4) and emits CMU/104/3.0, IBM/102/7.0, CAT/106/11.0; batch two sends IBM/201/3, IBM/202/4, CMU/203/5, CMU/204/5, DOG/205/0, DOG/206/1 (milestone after event 8) and emits DOG/206/1.0, CMU/204/13.0, IBM/202/14.0; six listener rows total, row-per-group because the order key is the selected aggregate.
Case `last-join` (ordinal 5) uses the same statement as a join `SupportMarketDataBean#length(20) as one, SupportBeanString#length(100) as two where one.symbol = two.theString`, seeds CAT/IBM/CMU/KGB/DOG `SupportBeanString` rows before the twelve market events, and produces the same two three-row batches new-only.
Case `iterator` (ordinal 6) deploys `@name('s0') select symbol, theString, sum(price) as sumPrice from SupportMarketDataBean#length(10) as one, SupportBeanString#length(100) as two where one.symbol = two.theString group by symbol order by symbol`, seeds the five join strings, sends CAT/50, IBM/49, CAT/15, IBM/100, then asserts a 4-row iterator snapshot any-order ({CAT,CAT,65}x2, {IBM,IBM,149}x2), sends KGB/75, and asserts a 5-row iterator snapshot (+{KGB,KGB,75}); two iterator-snapshot records, no listener records.
Also fix the ordinal-1 static ID to the full `java-015649f9c0449597e55a` everywhere (scenario, runner, manifest case, evidence) — static-manifest.json is the machine fact; the current files carry a truncated `java-015649f9c0449597e55`.
...
- [ ] Commit and push aggregate-grouped completion unit.
