...
- Updated: 2026-08-31
- Baseline: `HEAD` == `origin/master` at Draft 4.285 (`313d44f917bcf11c6d3edc92ac05c15acbba2bee`), pushed; Draft 4.286 is validated and ready for its semantic commit/push.
...
## Current work unit
Draft 4.286 ports the smallest safe next slice: fixed Java `ResultSetQueryTypeRowForAll.java` ordinal 10 `ResultSetQueryTypeRowForAllSelectAvgStdGroupByUni` (shared inventory `java-00cfc4061ba3a5f163ff`, runtime `java-runtime-de3a01d12a22bb33ee43`, static candidate `java-7ba8b4893d30071e2fa5`, no flags). The Java contract is `@name('s0') select istream average as aprice from SupportMarketDataBean#groupwin(symbol)#length(2)#uni(price)` with A/1, B/3, A/3, A/10, A/20; asserted new averages are 1.0, 3.0, 2.0, and 15.0 (A/10 callback is reset and intentionally unasserted). Typed Go uses `GroupWindow(symbol, LengthWindow(2))`, `UnivariateStatistics(price).Average()`, and new-only listener replay. Initial integration exposed a runtime distinction: pure `Avg` remains row-for-all, while `#uni` must materialize the `GroupWindow` key as an implicit aggregate dimension.
Integration finding: the ordinal-ten direct replay failed at callback 2 (`average = 2`, want `3`) because `aggregateBatch` only implicit-grouped non-key projections; the frozen scouts identify recursive `univariate-statistics*` detection as the narrow fix. This shared runtime change must preserve ordinal-nine plain `Avg` and the existing ungrouped/named-window `#uni` contracts.
Validation checkpoint: pinned Java oracle replay regenerated `testdata/parity/resultset-querytype-row-for-all-select-avg-std-group-by-uni.trace.json` from `/root/app/esper` at `9e1b9f1cc9117fea4bf33ab043762c045d73839c`; Go replay is persisted in the matching `.go.trace.json`, and differential evidence reports `passing` with 4 records per side and zero differences. Focused Go replay, mutation, malformed-scenario, aggregate regression, and manifest-against-inventory checks pass. Reviewer `ReviewDraft4286` returned PASS with no actionable P0-P2 findings after checking the source/runtime/static IDs, strict validators, traces/evidence, manifest, README, roadmap, CHANGELOG, and PLANS; `make check`, `go test -race ./... -count=1 -timeout 600s`, layout, and `git diff --check` pass. The manifest now includes this case/runtime and derives 578 cases, 191 differential-verified cases, 716 differential runtimes, 3337 runtime associations, and 3124 referenced runtimes.
## Handoff
The ordinal-ten contract is frozen by concurrent read-only scouts `NextRowForAllJava4` (`java-oracle-scout`) and `NextRowForAllGo4` (`scout`). Asset writer `RowForAllOrdinal10Assets` was canceled before writing after repeated steering did not produce files; the primary recovered its three assigned oracle/scenario/launcher files under the documented serial fallback. The primary owns the Go runner, dispatch/tests, generated traces/evidence, manifest, roadmap, CHANGELOG, validation, review, commit, and push. Do not alter ordinal-nine files or the fixed Java oracle.
