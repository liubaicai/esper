...
- Shipped: Draft 4.331 ('resultset-aggregate-filter-named-parameter-linear-join') committed; Git owns identity.
- Current target: Draft 4.332 candidate 'resultset-aggregate-filter-named-parameter-sorted-join' — ordinals 15/17/18 (sorted join + two-key multicriteria); prefetch scout dispatched concurrently with the 4.331 review.

## Current work unit
Active: Draft 4.331 ('resultset-aggregate-filter-named-parameter-linear-join') — SHIPPED this commit. Closing summary: contract frozen from AggregateFilterNamedParameterScout with both spikes resolved (join group rows are joinTuples; FilterAggregate predicates bind via JoinField(1,...); first(*,filter).theString navigation not needed for this slicing); asset writer agent_34f6437a authored oracle/script/scenario (30 steps, 19-record Java trace validated end-to-end; corrected step enumeration and confirmed no-space EPL concatenation and Integer[]/Map[] rendering); primary wrote resultset_aggregate_filter_named_parameter_linear_join.go (JoinMany(...).Aggregate(Alias(FilterAggregate(First/Last/WindowValues/FirstEver/LastEver/CountEver(JoinField)))) for ord 9/11; From(...).Window(KeepAll).AsRecord().Aggregate(Alias(FilterAggregate(WindowEvents()))) event-array rendering for ord 13; CountEver non-generic), run.go wiring, 6-test family; evidence 19/19 zero differences; manifest 622/235/838/3461 (referenced 3220, unreferenced 916); gates green. Deferred to follow-up units: ord 15/17/18 (sorted join, two-key multicriteria with doublePrimitive), ord 19 (audit/reuse), ord 20 (invalid compiles — approved difference).

- [x] Resolve join FilterAggregate predicate-binding spike.
- [x] Freeze linear-join contract.
- [x] Author oracle, script, and scenario via asset writer (19-record Java trace).
- [x] Implement runner, modes, and 6-test family.
- [x] Generate traces and zero-difference evidence (19/19).
...
- Post-validation concurrent batch: independent parity reviewer 'LinearJoinParityReview' (`agent_26a8205b-e13a-45c9-ac3d-755063d766ce`) returned OVERALL PASS (one P2 mutation-count prose fix + P3s, fix confirmation in flight); N+1 prefetch scout 'SortedJoinScout' (`agent_4ab62aa5-5513-449a-b98c-d7d0a53070ec`) COMPLETE — 4.332 candidate 'resultset-aggregate-filter-named-parameter-sorted-join' (ord 15/17/18) adjudicated ZERO ENGINE WORK with one compositional requirement: the join-case sorted column must build as SortedEventsBy[esper.Event](JoinEventValue[esper.Event](1), key, false) (bare SortedEvents returns joinTuple rows whose empty schema renders field-less); maxby/minby(.theString) compose as FilterAggregate[string](MaxBy(JoinField(1,"theString"), beanInt), filter); ord 18 two-key via SortedEvents(Ascending(beanInt), Ascending(beanDouble)) over a 3-field bean (doublePrimitive float64, renders integral); labels sorted-join-bound/sorted-join-unbound/sorted-multicriteria; 29 steps (13/9/7), 18 records; schema decision (a) 3-field bean in ALL cases with doublePrimitive:-1 pinned in join-case sends for symmetry.
