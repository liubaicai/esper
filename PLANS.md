...
- Shipped: Draft 4.328 ('infra-named-window-on-update') committed; Git owns identity.
- Current target: Draft 4.329 candidate 'on-update variants' (InfraNamedWindowOnUpdate ordinals 0/3/4/5) — Java contract already pinned by OnUpdateJavaContract; OnUpdateVariantsGoSurface prefetch scout adjudication in progress.

## Current work unit
Active: Draft 4.328 ('infra-named-window-on-update') — on-update statements over named windows, 4 of 8 executions of InfraNamedWindowOnUpdate.java as ONE differential case (unit size 4, within the 1-5 norm; the 4 share one file, one harness, one runtime surface). Contract frozen from prefetch scouts 'OnUpdateJavaContract' (agent_480e7832) + 'OnUpdateGoSurface' (agent_79ed2dad, ALL READY, zero engine work).

Frozen contract: ordinals are the Java executions() ordinals 1/2/6/7 — Intersect (java-runtime-f921cf2543cbb10b4150, static java-b0afef60c0bc90fc51d8), Union (java-runtime-0947ec873ea298b60209, java-a5f1d32f26cb39f39266), MultikeyWArrayPrimitiveArray (java-runtime-9870ee394d3099ef85e8, java-0b7af7284d24833e1753), MultikeyWArrayTwoFields (java-runtime-a25a18f2754aeae08015, java-cdea340e2ff2cabfd0f3); flags [] on all four (runtime inventory authoritative; static-manifest over-approximates SERDEREQUIRED on all 8 — noted, do not copy). Case labels: intersect/union/multikey-array/multikey-two-fields. Verbatim EPL per case (create/insert/update split into three deploy steps per family convention; Java compiles one multi-statement deployment): intersect create `@name('create') create window MyWindowMDW#unique(theString)#length(2) as select * from SupportBean`, insert `insert into MyWindowMDW select * from SupportBean`, update `on SupportBean_A update MyWindowMDW set intPrimitive=intPrimitive*100 where theString=id`; union create `@name('create') create window MyWindowMU#unique(theString)#length(2) retain-union as select * from SupportBean`, insert `insert into MyWindowMU select * from SupportBean`, update `on SupportBean_A update MyWindowMU mw set mw.intPrimitive=intPrimitive*100 where theString=id`; multikey create `@name('create') create window MyWindow#keepall as SupportEventWithManyArray`, insert `insert into MyWindow select * from SupportEventWithManyArray`, updates `on SupportEventWithIntArray as sewia update MyWindow as mw set value = sewia.value where mw.intOne = sewia.array` (array case) / `... where mw.id = sewia.id and mw.intOne = sewia.array` (two-fields case). Timelines: intersect/union send SupportBean E1/2, E2/3 then SupportBean_A E2 — create listener records ALL invocations (Java test asserts only the last): seq1 new [{theString E1,intPrimitive 2}] old nil, seq2 new [{E2,3}] old nil, seq3 new [{E2,300}] old [{E2,3}], then one create snapshot: intersect [[E1,2],[E2,300]] mode any (Java AnyOrder), union same rows but ORDERED by theString (Java sorts before comparing — pin ordered, no mode any). multikey-array: ManyArray sends E1[1,2] E2[3,4] E3[1] E4[] (payloads {"id","intOne"}, value defaults 0), then IntArray triggers U1{3,4}=10 U2{1}=11 U3{}=12 U4{1,2}=13 (payloads {"id","array","value"}), no listeners, snapshot [[E1,13],[E2,10],[E3,11],[E4,12]] mode any (empty-array equality matches E4). two-fields: ManyArray ID1[1,2] ID2[3,4] ID3[1], triggers ("ID2",{3,4},10)("ID3",{1},11)("ID1",{1,2},12)("IDX",{1},14)✗("ID1",{1,2,3},15)✗, snapshot [[ID1,12],[ID2,10],[ID3,11]] mode any (no-match rows untouched). milestone(0) is a harness no-op. Trace projection convention: snapshot rows render ONLY the Java-asserted fields (intersect/union theString,intPrimitive; multikey id,value) symmetrically in oracle and Go runner — the 27-property ManyArray full-row rendering is NOT the pinned contract and avoids a schema-equivalence quagmire. Totals: 9+9+14+14 = 46 steps, 10 records (3+3 listener + 1+1 snapshot). Update old/new semantics (Java OnExprViewNamedWindowUpdate): one dispatch with New=post-update rows, Old=pre-update rows, same lengths; zero matches → no listener invocation at all. Go builds: CreateNamedWindow with NamedWindowRetention(IntersectWindows(Unique(Field,Literal-order per Java),LengthWindow(2))) / UnionWindows(...)/KeepAll; insert via OnEvent(...).InsertIntoNamedWindow(window, CopyMatchingFields()); update via OnEvent(...).UpdateNamedWindow(window, predicate, SetColumn(...)) with NamedWindowField (window row) vs Field (trigger); array equality via EqualOf(NamedWindowField[[]int]("intOne"), Field[[]int]("array")); create consumer FromNamedWindow(...).Query(StatementName("create"), WithOldStream()) subscribed only for intersect/union; snapshot via statement.Snapshot (sequence 0, epoch time) per start-stop precedent. Deferred to a later unit: ord 0 (method-call set clauses, approved-difference SetColumn literals), ord 3 (subclass typing adaptation), ord 4 (copy-method config), ord 5 (wrapper select + extra property).

- [x] Freeze on-update contract (parallel prefetch scouts).
- [x] Author oracle, script, and scenario via asset writer (agent_c0087c09; 46-step/10-record artifacts, end-to-end oracle run validated; writer correctly caught and fixed the primary's missed multikey-array trigger sends and added @public per family convention).
- [x] Implement infra-named-window-on-update runner, modes, and 6-test family.
- [x] Generate traces and zero-difference evidence (10/10, evidence passing).
- [x] Update manifest (619/232/827/3450), roadmap, and CHANGELOG.
- [x] Run gates and independent parity review: make check PASS, git diff --check clean; OnUpdateParityReview OVERALL PASS (single P2 stale-totals line fixed and re-confirmed by the same reviewer).
- [x] Commit and push the unit (Git owns identity; no hash recorded here).

Integration notes: (1) The asset writer caught two primary contract bugs during authoring — the pinned multikey-array timeline omitted the four SupportEventWithIntArray trigger sends (without them the update never fires and the trace gate contradicts Java), and the create EPLs need @public for the oracle's separate-module deploys (InfraNamedWindowJoinScenarioOracle convention); both fixed in scenario+oracle and mirrored back into the runner pins. (2) Mode-any snapshots are now emitted in the differential canonical row order (ascending marshaled-fields string) by BOTH the oracle and the Go runner, so checked-in traces equal canonical traces and stay stable against engine iteration order; the CheckedIn evidence test therefore compares raw DiffTraces like the sibling families. (3) Go union-update composite-retention branch (updateCompositeWhereState union path) exercised differentially for the first time — zero divergence.

Filtered-correl closing summary (shipped this commit): contract frozen from scouts 'FilteredCorrelJavaContract' (agent_9cd7d0cb) + 'FilteredCorrelGoSurface' (agent_57aa3160); isolated asset writer agent_3667f273 authored oracle/script/scenario (94 steps, 28 records); primary wrote infra_nwtable_subq_filtered_correl.go, run.go wiring, 6-test family, traces/evidence, manifest (618 cases/231 DV/823 DV runtimes/3446 associations), roadmap, CHANGELOG; one primary harness fix (S0 sends delivered under "S0" type name — pinned EPL consumes `from S0 s0`, Esper routes sends by type name); scratch engine probe (deleted after use) confirmed NW/table folded-filter subquery + late CreateIndex semantics before the Java trace landed; runner validator pins the exact per-case step interleaving (create/insert/[index]/consume + 8 sends), closing the positional-interleaving P3 pattern flagged in the 4.326 review; gates all green (make check, git diff --check, gofmt). Independent parity review `FilteredCorrelReview` (agent_6761c0e0-3c0e-4c49-9eab-69cd92229352) returned OVERALL PASS on all five dimensions; single P2 (stale "80 steps" in PLANS.md) fixed to 94 with arithmetic and re-confirmed PASS by the same reviewer.
