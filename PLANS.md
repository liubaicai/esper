...
## Current work unit
Shipped: Draft 4.319 (`case.resultset-output-limit-changeset-opt`) — all items below are complete through gates; independent review and commit/push remain.

Frozen contract (from the Draft 4.319 prefetch scouts, prior session): single execution `ResultSetOutputLimitChangeSetOpt` (java-runtime-9b0f27f829d9820e3792, static java-9f9a094322f3c6687f00). 36 rounds × 2 records: one shared runtime with cumulative virtual time; each round deploys s0 over SupportBean#length(2), sends E0..E4 (theString Ei, intPrimitive i), reads the white-box changeset counter BEFORE the one-second advance, advances, re-reads, undeploys. Counter reads 5 exactly on the nine DISABLE_OUTPUTLIMIT_OPT rounds (indices 4,8,11,16,19,24,27,31,34 = plain-all/count-last/count-all/string-count-last/string-count-all/grouped-{count,key-count}-{last,all}-disable); every other round reads 0 because Java maps default/ENABLE statements to the unordered last-all view (rule: !agg && !grouped && LAST, or opt && (ALL|LAST) && !orderBy), FIRST to the inline-clear ConditionFirst (ungrouped) or the per-update-drained null condition (grouped), and aggregated order-by LAST/ALL without DISABLE to NONFIRST (not present in the matrix). ENABLE_OUTPUTLIMIT_OPT+order-by compile rejection is asserted in code on both sides, emitting no record. Config default outputLimitOpt=true is what makes no-hint rounds read 0.
- [x] Freeze ChangeSetOpt contract (prefetch scouts, prior session).
- [x] Engine groundwork: changesetRows counter + DISABLE-only gate + order-by compile rejection + Statement.NumChangesetRows (committed cb0539923).
- [x] Author oracle/script/scenario via asset writer (prior session; committed with cb0539923 except the Java trace).
- [x] Implement resultset-output-limit-changeset-opt Go runner, modes, and tests.
- [x] Fix grouped double-count: applyLastEveryTimeGrouped second increment removed (applyOutput counts once per update pair).
- [x] Repair run-script jq corruption (working-tree copy had parameterized-context pins; restored committed changeset pins).
- [x] Generate traces and zero-difference evidence (72 records per side, 0 differences; oracle replay byte-identical, cmp=0).
- [x] Update manifest (610 cases, 223 DV, 791 DV runtime IDs, 3414 associations, referenced 3173, unreferenced 963), roadmap, and CHANGELOG.
- [x] Run gates (package tests, gofmt, vet, manifest validation, git diff --check, bash -n, facade/api cmp, go list).
Changeset review: final parity review `ChangesetParityReview` (independent read-only reviewer, agent `agent_358f143c-225e-488b-9699-3a4485f5a7c0`) returned PASS across contract/runner/evidence/tests/manifest/docs/never-change with zero P1 findings. Accepted before commit: P2 — deleted untracked `tools/java-oracle/classes/` build artifacts; P3 — added explicit time.Parse error check at the advance step; P3 — removed an orphaned duplicate NumChangesetRows doc-comment block in runtime.go. Accepted P3 observations without action (all pre-existing or outside this matrix): the DISABLE-only counter gate diverges from Java for DISABLE+ungrouped-LAST, DISABLE+FIRST, DISABLE+grouped-first (Java reads 0 via LASTALL_UNORD / inline-clear / null-condition paths), and for no-hint/ENABLE+aggregated LAST/ALL WITH order-by (Java NONFIRST would count; Go gate reads 0); non-default config outputLimitOpt=false is not modeled; the rejection-assert query shape differs from the oracle INVALID_EPL while both are rejected by the same rule.
- [x] Independent parity review: ChangesetParityReview PASS.
- [x] Commit and push the changeset unit (Git owns identity; no hash recorded here).
## Delegation checkpoint

Draft 4.319 ChangeSetOpt unit (agents from the prior session, recorded here as the active checkpoint):
- Prefetch scouts (read-only, concurrent): `ChangeSetJavaContract` (`agent_d36697ea-d0bb-4ddb-ab69-de39f4c048a9`) pinned the 36-round matrix semantics and `ChangeSetGoEngine` (`agent_49b53070-0193-4869-8cb8-ed6262f90a71`) adjudicated the Go buffering surface; the engine contract (counter + DISABLE-only gate + order-by rejection) was frozen from both.
- Asset writer (prior session) authored `tools/java-oracle/ResultSetOutputLimitChangeSetOptScenarioOracle.java`, `run-resultset-output-limit-changeset-opt.sh`, and `testdata/parity/resultset-output-limit-changeset-opt.json` on the frozen contract; committed with the cb0539923 groundwork.
- Primary is the single writer for `internal/esper` semantics and owns the Go runner (`internal/app/parity/resultset_output_limit_changeset_opt.go`), run.go wiring, run_test.go family, compat FormatTraceTime export, traces/evidence, manifest, roadmap, CHANGELOG, PLANS.md, gates, review, commit, and push.
- Primary repairs this session: removed the grouped last-every-time double increment of the changeset counter; restored the run-script jq pins (the working-tree copy had been clobbered with parameterized-context pins); added the record `case` label to match the Java trace shape.

Exception note: this unit's implementation resumed after a session break; no new scouts were dispatched for the integration-only remainder (runner wiring, test family, evidence). The prior scout pair plus the committed oracle/scenario pins cover the contract; re-scouting had no independent question left to answer.
