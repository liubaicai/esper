...
- Current target: Draft 4.341 'insert-into-invalidity' — EPLInsertIntoPopulateUnderlying ord 12 (implemented-only, closes the class 13/13 dispositioned); contract pinned by PopulateUnderlyingJavaContract (agent_fae76b58) + PopulateUnderlyingGoSurface (agent_8cd5b2e7).
...
## Current work unit
Active: Draft 4.341 ('insert-into-invalidity') — EPLInsertIntoPopulateUnderlying ord 12, implemented-only (closes the class at 13/13 dispositioned). No oracle/scenario/trace changes: Java ord 12's primary observable is compile rejection with exact messages that have no Go rejection-message surface (established invalidity policy: Go implemented status with Build/send-error unit tests, like epl_insert_into_transpose_stream_invalid_test.go).

Frozen contract (from the 4.338 scouts' pinned case list):
- Reproducible Build-rejection sub-cases (Go messages): unknown column `select 3 as dummyField` / bare `select 3` / `(dummy)` column into a struct target; interface-hierarchy mismatch (fixture declares interface-typed fields, e.g. FieldDef with an interface type, routing an incompatible impl); `select 1` into a ctor-shape target with non-matching arity (route projection mismatch).
- Reproducible send-error sub-cases: setter-throw (target registered WithPropertySetter returning an error — Go stand-in for Java's throwing setter; Send must return the error); wrong-type map value (string into int field — either-behavior accepted per Java, Go surfaces an error).
- Positive: same-schema cast `insert into MapOneA select * from MapTwoA` (identical schemas) deploys OK.
- Approved differences (documented, not fabricated as rejections): long→int and null→int "types mismatch"/"nullable mismatch" (Go converts numerics, null → zero value); constructor-not-found and ctor-throws concepts; ABCStream/xmltype auto-declared insert-into types (Go requires pre-registered targets); `c0 null` null-typed column; xmltype XMLDOM precondition.

- [x] Freeze contract (4.338 scouts; no oracle/scenario asset changes needed and all remaining files are primary-owned — delegation-gate serial exception recorded).
- [x] Primary: invalidity Go tests in epl_insert_into_populate_underlying_invalid_test.go (6 tests/8 leaves all PASS — fixes: package-level Select form, unnamed column via empty alias, local MyMap with anint).
- [x] Manifest: ord 12 runtime id added to javaRuntimeIds (not DV list), goTests + note (class 13/13 dispositioned: 12 DV + 1 implemented-not-DV); summary associations 3494→3495 / referenced 3253→3254 / unreferenced 883→882 (DV runtime IDs stay 871); per-domain recount verified epl 277.
- [x] Evidence note extension; Roadmap supplement + epl 278→277 (5.2/6.1 and section-4 prose); CHANGELOG 4.341 entry.
- [x] Gates + independent parity review (agent_9d9cf34c-6019-42a4-ae19-886045e140e0): OVERALL PASS on all five dimensions with every Go rejection verified as real engine behavior and all manifest/domain numbers recomputed-exact; four P3s fixed in the same work unit — the MyMap(dummy) sub-case dispositioned as an approved difference in all four lists, the transpose cross-list dropped from the case's goTests (evidence+manifest realigned to 24), the wrong-type wording made phase-honest ("either-behavior observed as a Go-side error in the run"), and the PLANS box/count corrected; the reviewer's two residuals from the fix round (evidence transpose entry, manifest "pinned at the Go error" phrase) landed verbatim per the reviewer's no-further-cycle instruction.
