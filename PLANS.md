...
- Updated: 2026-09-03; Draft 4.305 (`resultset-querytype-local-group-by`) is complete and ready for the semantic commit. It targets fixed Java `ResultSetQueryTypeLocalGroupBy.java` ordinal 13 and grouped multi-level `window(*)` local-group access.
- Baseline: `HEAD` == `origin/master` at `6d2823002`; the worktree was clean after Draft 4.304. Draft 4.304 (`resultset-aggregate-filtered-w-math-context`) was committed and pushed at that baseline.
- Closed predecessor (Draft 4.304): `ResultSetAggregateFilteredWMathContext.java` ordinal 0 was differentially verified with zero differences, all local gates, race, and independent parity review passing.
- Draft 4.305 result: one Java/Go listener snapshot with three ordered rows and full SupportBean event arrays; Java and Go traces compare with zero differences, mutation tests reject value/order/time/record-count changes, the manifest/evidence validators pass, and independent review found no correctness defects after central documentation was reconciled.
## Current work unit
Draft 4.305 covers ordinal 13 of `ResultSetQueryTypeLocalGroupBy.java`: `ResultSetLocalGroupedMultiLevelAccess`; Java runtime `java-runtime-87b3bcfdf164bc939fe5`; static ID `java-b0b3444c9ac5a1297cd0`; Java commit `9e1b9f1cc9117fea4bf33ab043762c045d73839c`; flags empty.
The fixed Java source is `/root/app/esper/regression-lib/src/main/java/com/espertech/esper/regressionlib/suite/resultset/querytype/ResultSetQueryTypeLocalGroupBy.java`, lines 400-438. Exact EPL uses `SupportBean#keepall`, outer `group by theString,intPrimitive`, local access parameters for `(intPrimitive,theString)`, `theString`, `intPrimitive`, and `()`, snapshot output every 10 seconds, and order by `theString,intPrimitive`.
The observable contract is one snapshot after five sends `(E1,10,100)`, `(E1,20,202)`, `(E2,10,303)`, `(E1,10,404)`, `(E2,10,505)` and virtual time 10000ms: exactly three new rows ordered E1/10, E1/20, E2/10; c0-c4 are full SupportBean event arrays preserving keepall arrival order and filtered by the corresponding local key. No old rows, removals, null/missing input, or empty-window behavior is asserted.
## Delegation checkpoint
Draft 4.305 scouts `LocalGroupJavaContract` (`java-oracle-scout`) and `LocalGroupGoSurface` (`scout`) completed concurrently. Java froze ordinal 13, its IDs, EPL, timer, row and event-array contract; Go confirmed existing typed `LocalGroupBy`, `WindowEvents`/`WindowValues`, grouping, snapshot, and scope surfaces express the unit with no facade change. File-disjoint asset writer `LocalGroupAssets` (`parity-asset-worker`, isolated) owned only the new oracle, shell wrapper, scenario, and parity runner files. Primary owned shared runtime/API integration, generated traces/evidence, manifest, roadmap, CHANGELOG, validation, review, commit, and push.
- Draft 4.306 prefetch scouts `OutputLimitJavaPrefetch-2` (`java-oracle-scout`) and `OutputLimitGoPrefetch-2` (`scout`) completed concurrently, read-only. Java froze `ResultSetOutputLimitRowLimit.java` ordinals 1/3 and the paired batch old/new contract; Go confirmed existing typed `LengthBatch`, `Limit`, `OutputEvery`, `OrderBy`, iterator, and `WithOldStream` surfaces, with no facade redesign for this bounded candidate. Writes remain deferred until Draft 4.305 is committed and pushed.
- Independent reviewer `LocalGroupReview-2` (`parity-reviewer`) reviewed the integrated diff: PASS after the roadmap and CHANGELOG entries were added; no P1/P2 correctness findings.
- Validation before commit: `go test ./internal/app/parity -run 'TestRunResultSetQueryTypeLocalGroupBy' -count=1` passed; the pinned Java oracle replay matched the checked-in trace; `go test ./internal/compat -run 'TestCapabilityManifestArtifactValidates' -count=1` passed after summary repair; evidence reports status `passing`, zero differences, one record per side, and three new rows per side.
## Next work unit
After this semantic commit, begin Draft 4.306 with the prefetched OutputLimitRowLimit ordinals 1 and 3; keep ordinal 0 context termination, ordinal 4 grouped iterator, ordinal 7 negative rowcount, and ordinals 8/9 invalid/variable semantics as separate candidates.
## Prior outcomes
