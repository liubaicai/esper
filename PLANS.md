...
- Current target: Draft 4.387 'infra-namedwindow-views-ext-time' - slice S6 of the InfraNamedWindowViews inventory (event-time, NO advance-time steps): ord 6 `InfraExtTimeWindow` (`java-runtime-c960628819cddf06d0bd`), ord 7 `InfraExtTimeWindowSceneTwo` (`java-runtime-aa7fe5003096f5bf1cf8`), ord 53 `InfraExternallyTimedBatch` (`java-runtime-3af75eefff14ce16ae81`). Scout findings: the Java executions are purely event-timestamp driven and register no timer, and the Go engine mirrors that (named-window ext_timed slides expire on insert via expiresAt/maxExtTimestamp; `expireNamedWindowState` intentionally has no external-timed arm so `AdvanceTime` is a no-op for these windows - Java parity, not a gap), so S6 is chain-only. Carry-in details: model ord 53's `0L` reference explicitly (`ExternallyTimedBatchWithReference(..., 0)`) because the named-window batch path always anchors at the epoch; the sliding arm keys off the monotone max timestamp where Java uses the current event's timestamp (identical for ascending input); the oracle must pin the clock to the epoch or record times would be wall-clock.

## Current work unit
Active: Draft 4.386 ('infra-named-window-time-views') - chain, evidence and facts complete; gates and independent review in flight.

- Scope: slice S5 of the InfraNamedWindowViews inventory - ord 4 `InfraTimeWindowSceneTwo` (`java-runtime-879d6ad8aee378657a02`), ord 5 `InfraTimeFirstWindow` (`java-runtime-5f5a65dd5f72c8e24619`), ord 8 `InfraExtTimeWindowSceneThree` (`java-runtime-a6bfdd3389c077127861`); 57 records (30/15/12).
- New protocol element: `advance-time` scenario steps (RFC3339 `at`); the Go runner moves the engine clock first and stamps each record with the clock value at delivery, so boundary expiry deliveries carry the boundary instant.
- Pinned semantics: sliding time(10 sec) expiry at t=10000/36000 (old-only to the window statement and its separately deployed consume consumer, 35999 silent), equal-time advances accepted, firsttime's deploy-anchored silent close at 11000 with retained rows surviving until deleted, bean-backed win:time(10 sec) with a SupportBean_A trigger and a theString-only projection.
- Contract correction during the unit: ord 8 has 6 snapshots / 12 records (the deep report's header said 13); the asset scout verified this against the Java source and the slice total is 57.
- [x] Scouts in hand; contract frozen; asset lane delivered oracle/script/scenario/trace (deterministic).
- [x] Go chain `internal/app/parity/infra_named_window_time_views.go` + modes + 6-test family (mutation indices cross-checked against the trace layout); Go trace and evidence passing with 0 differences.
- [x] Manifest (`case.infra-namedwindow-views-time`, capability DV +3, summary 642/265/987/3585), CHANGELOG 4.386, roadmap 4.386, README stats, PLANS.
