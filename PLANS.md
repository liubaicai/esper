...
- Current target: Draft 4.389 'infra-namedwindow-views-time-batch' - slice S8 of the InfraNamedWindowViews inventory (virtual time): ord 16 `InfraTimeBatch` (`java-runtime-1ad42a8ed8025c4a0730`), ord 17 `InfraTimeBatchSceneTwo` (`java-runtime-1eb11f5cf275069ef6b5`), ord 18 `InfraTimeBatchLateConsumer` (`java-runtime-bb926d092e7110db203f`), ord 23 `InfraTimeLengthBatch` (`java-runtime-22bf6b3644a24862df7d`), ord 24 `InfraTimeLengthBatchSceneTwo` (`java-runtime-dd924e1b7e500df135f8`). Scouts in hand: all five already have Java-faithful Go unit tests, and the named-window engine models the three batch retentions under AdvanceTime with new/old flush semantics, the inclusive boundary, the silent-delete rule and the late-consumer preload skip, so no engine change is expected. Carry-in details: the flush carries the accumulated rows as new data with the previous batch as old data; time_length_batch's time-or-size dual trigger; ord 18's late consumer must not preload from a batch window. Latent divergences NOT exercised by this slice (do not assume covered): time_length_batch does not re-anchor its time boundary after a size flush (Java does), batch retention ignores ForceUpdate/StartEager, and calendar batch specs can spin.

## Current work unit
Active: Draft 4.388 ('infra-named-window-time-order-accum-views') - chain, evidence and facts complete; gates and independent review in flight.

- Scope: slice S7 of the InfraNamedWindowViews inventory (virtual time) - ord 9 `InfraTimeOrderWindow` (`java-runtime-320f1b03eddb24520308`), ord 10 `InfraTimeOrderSceneTwo` (`java-runtime-ed5bfc247d088a603224`), ord 14 `InfraTimeAccum` (`java-runtime-d74b2c9c614b26973637`), ord 15 `InfraTimeAccumSceneTwo` (`java-runtime-8b4e2e7d81bb990c7d72`); 101 records (16/21/34/30).
- Pinned semantics: sliding time_order retention with old-only boundary releases and ascending iterators; the stale-row pass-through as ONE callback carrying both streams (ord 10 G3); time_accum's immediate delivery with re-arm to the arrival plus ten seconds and multi-row bursts released in ONE callback in insertion order (3 rows, 2 rows, 1 row), newest-row deletes re-arming to the stored arrival, last-row deletes cancelling the timer, and silent deletes of already-flushed keys. `internal/esper` unchanged - the existing TimeOrder/TimeAccum insert and AdvanceTime expiry arms reproduce the Java timelines exactly.
- Notables: ord 10's suite asserts only the key column, so its trace rows project `key` alone (found by the first diff run); the asset scout corrected the prefetch report's ord-15 snapshot instant (t=20000, not 29999) against the Java source.
- [x] Scouts in hand; contract frozen; asset lane delivered oracle/script/scenario/trace (deterministic, 101 records).
- [x] Go chain `internal/app/parity/infra_named_window_time_order_accum_views.go` + modes + 6-test family (mutation indices cross-checked against the trace layout); Go trace and evidence passing with 0 differences.
- [x] Manifest (`case.infra-namedwindow-views-time-order-accum`, capability DV +4, summary 644/267/994/3592), CHANGELOG 4.388, roadmap 4.388, README stats, PLANS.
