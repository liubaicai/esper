...
- Current target: Draft 4.345 'viewgroup-merge-view' — ENGINE unit closing the groupwin union-aggregate divergence + differential-verifying ViewGroup ords 0/14; contracts pinned by fresh parallel scouts ViewGroupJavaContract2 (agent_ac68b9d7) + ViewGroupGoEngineScout (agent_153be8e5).
...
## Current work unit
Active: Draft 4.345 ('viewgroup-merge-view') — ENGINE unit + differential chain for ViewGroup ords 0/14 (ord 6 deferred: the #uni derived-value view's irstream old rows have no Go counterpart).

Root cause (verified in code by ViewGroupGoEngineScout agent_153be8e5): runtime.go:18098-18109 injects the groupwin keys as an implicit aggregate group-by whenever `aggregateDefinitionReadsNonKeyEvent(definition)` (scalar projection like `p1`), so `select p1,sum(p2) from ...#groupwin(p1)#length(2)` sums per-group (10/11/22/25). Java has no group-by clause → one ungrouped union aggregate over the merged cross-group deltas (10/21/33/36 — the groupwin parent is a merge/union point; per-group length(2) subviews post their delta pairs to the shared downstream; ord 0 exact: (A,10)→10, (B,11)→21, (A,12)→33, (A,13)→36 with in-group eviction 13−10). Java's `#uni/#correl/#linest` chained on a groupwin ARE per-group child views — the split is ordinary-aggregate=union vs derived-statistics-view=per-group.

Engine changes (frozen design from the scout):
1. runtime.go:18098 — drop `aggregateDefinitionReadsNonKeyEvent(definition)` from the implicit group-by injection gate (keep `tableTarget != ""` and the univariate trigger); rewrite the stale comment.
2. runtime.go:19550 — extend `expressionTreeContainsUnivariateStatistics` to match `correlation` and `linear-regression` node kinds (Java makes #correl/#linest per-group like #uni).
3. Test re-model: view_group_intersect_parity_test.go:159 `Alias("average", Avg(x))` → `UnivariateStatistics[float64](x).Average()` (the mirrored Java statements use #uni(price); the pinned 2.0 and correlation pins stay, now carried by the derived-statistics trigger).

Blast radius (scout-verified): tableTarget shapes keep per-key rows; #uni DV keeps passing (uni trigger); avg-per-sym DV unchanged (explicit literal GroupBy); non-GroupWindow statements never hit the injection; only item 3 breaks, by design.

Differential contract (Java ViewGroup.java, pinned by ViewGroupJavaContract2 agent_ac68b9d7; ord 6 deferred — see deferrals):
- ord 0 `java-runtime-a3b6bef89e22a122cc7a` static (verify at write time): `@name('s0') select p1,sum(p2) as sp2 from OAEventStringInt#groupwin(p1)#length(2)`; sends {A,10}→{A,10}, {B,11}→{B,21}, {A,12}→{A,33}, {A,13}→{A,36} (assertPropsNew last new row; fields p1,sp2; sum(int) yields Long).
- ord 14 `java-runtime-639bc9b69621f3b6a417`: `select irstream theString as c0,intPrimitive as c1 from SupportBean#groupwin(theString)#length(3)`; IR pairs with in-group eviction; iterator AnyOrder (cross-group order unpinned).
- Deferrals: ord 7 (compile messages — future invalid sweep), ord 8 (PERFORMANCE), ord 19 (SERDEREQUIRED), ords 2/3/9 (reclaim + schedule-count introspection — separate reclaim unit), ords 1/4/5 (derived-value stats views with Java-double-exact math — natural follow-up), ords 10-13/16/18 (virtual-time batch/time-window machinery — own subdomains), ord 17 (expression groupwin + datetime).

- [x] Freeze contract (fresh parallel scouts; single writer on internal/esper per AGENTS.md).
- [x] Asset writer: 2 scenario cases + oracle + run script + Java trace regen (agent_d10fafe3; ord 6 deferred during the unit — the #uni derived-value view's irstream old rows {size:N-1} have no Go counterpart over a keepall group window, same gap class as ords 1/4/5; trace 12 records, double-run byte-identical).
- [x] Primary: engine fixes (no view_group_intersect re-model needed — the statement's correlation nodes keep the univariate injection alive, so the scout's item-3 prediction did not materialize) + app/parity runner (view_group_merge_view.go, Aggregate form) + run.go wiring + diff/mutation test family.
- [x] Manifest: NEW case entry case-viewgroup-merge-view (2 DV runtime IDs, statics java-4c1562348946dab6dc09 (ord 0)/java-48fb0b51c8d6abc3e858 (ord 14), mapping under view.window-core) + refreshed case.view-group-matrix difference note; summary 877→879 DV runtime IDs / associations 3502→3504 / referenced 3261→3262 / unreferenced 875→874 (recomputed-exact via the validator).
- [x] Evidence; Roadmap supplement + view 27→26 (5.2/6.1; the two runtimes are suite/view) with the 4.285 bounded-injection note superseded by the 4.345 supplement; CHANGELOG 4.345 entry.
