...
- Current target: Draft 4.355 'fromclausemethod-variable' — NEW differential chain for EPLFromClauseMethodVariable (6 inventory rows / 5 logical executions: java-runtime-b6134319ddad78ee7888 ConstantVariable, 7fa38fe9688391d3f9da NonConstantVariable{soda=true} + 1c125ff69ef065d76608 {soda=false} as one logical execution in two compile variants, 26c863d3cc9d5aca7cc7 ContextVariable, 2989d0cd737e30ae5633 VariableMapAndOA, 4448f4d53e5bf0e7688d VariableInvalid), upgrading the `epl.fromclausemethod` capability (implemented, zero DV) with its first differential chain. Go surface exists: FromMethod/FromMethodOn + MethodProvider. Rationale: database and dataflow lack any DV chain but need heavier harnesses; this suite is the tightest deterministic zero-coverage closed loop.
...
## Current work unit
Active: Draft 4.355 ('fromclausemethod-variable').

- [x] Parallel read-only scouts dispatched. Go surface scout agent_93be56ff COMPLETE — headline: an engine-level Go parity test for this exact suite already exists (internal/esper/fromclausemethod_variable_parity_test.go: ConstantVariable :57, NonConstantVariable :70 with both soda variants as one chain, ContextVariable :118 incl. invalid-context builds, VariableMapAndOA :214), so 4 of 5 logical executions are GREEN with ZERO engine work; MethodProvider polls per trigger row with a fresh visible-variable snapshot (runtime.go:12434-12466, faf.go:2653), constant-vs-nonconstant contrast is provider-side, context variables proven per-partition (RegisterContextVariable + WithContext + Invocation.ContextPartitionID), Map/OA schemas render like Java; NEW-chain wiring enumerated from the 4.350/4.345 precedents (runner file, two run.go branches, three run_test families, 4 testdata files, oracle+script). VariableInvalid disposition plan: sub-shape 3 (context-variable scoping, ErrorInvalidRule plan.go:1341) GREEN in-trace; sub-shape 1 (reflective footprint mismatch) and sub-shape 2 (null variable + instance method) have no Go equivalent — intentionally-different dispositions with named Go nearest-invalids (ErrorDependency nil-provider plan.go:2264, ErrorUnknownName unknown-trigger plan.go:2279). Java contract scout agent_d1ca34cf in flight.
- [x] Contract frozen; scope decision: the chain covers the 5 behavioral executions (constant-variable, nonconstant-soda-true/false, context-variable listener records, map-and-oa snapshots) — 14 records; the invalid compiles (VariableInvalid's three reflective-resolution failures + the context execution's two context-scoping compiles) follow the 4.341/4.344 invalidity policy: Go Build-rejection coverage (already in fromclausemethod_variable_parity_test.go:190-206) + manifest disposition, no trace rows. Writer agent_b1b0f7cb creating oracle/scenario/script/Java trace. Primary: Go runner DONE (internal/app/parity/epl_from_clause_method_variable.go — 5 case builders mirroring the committed engine-test shapes; probe produced exactly the 14 frozen records), run.go modes wired, test family added (Diff/DirectReplay-14/7 mutations).
- [x] Manifest (NEW case), evidence, roadmap, CHANGELOG. (Writer agent_b1b0f7cb delivered oracle (all service classes mirrored, W-Config variables, per-case fresh runtime, pinned metadata validation), scenario (5 cases/32 steps), script (exit 0, deterministic), Java trace 14 records matching the frozen contract and the Go probe record-for-record. Evidence passing 0 differences, 5 IDs/5 executions. Manifest: NEW case.epl-fromclausemethod-variable DV (5 IDs, per-case DV list, mapping to epl.fromclausemethod + capability verification/dvIds upgraded — 38 DV capabilities); umbrella basic case carries the EPLFromClauseMethodVariableInvalid name-level disposition note (all three reflective-resolution invalids have no Go equivalent; portable context-scoping invalids pinned by the committed engine test); summary 630 cases / 243 DV cases / 904 DV runtime IDs / 3534 assoc / referenced 3276 (IDs pre-associated by the umbrella) / unreferenced 860; roadmap epl count unchanged at 260 (all six suite IDs were pre-associated by the umbrella basic case; referenced 3276 / unreferenced 860 unchanged) + supplement; CHANGELOG entry with the scoped Build-rejection qualifier.)
- [x] Gates: affected packages + make check + git diff --check (green; recorded before commit).
- [x] Independent parity review. Reviewer agent_542ea2f6 OVERALL FAIL on two P1s — both fixed and re-verified: (1) the chain's committed .trace.json/.go.trace.json artifacts were missing (writer wrote to /tmp per the as-keyword-embedded-evidence assumption; this chain commits both artifacts — copied from the verified runs and the .go.trace.json added to the manifest evidence array per the 4.354 convention); (2) the roadmap epl decrement 260→254 was arithmetically wrong — all six suite IDs were pre-associated by the umbrella basic case, so referenced 3276 / unreferenced 860 and the roadmap count are UNCHANGED at 260 (reverted at all four sites; PLANS claim corrected). P3s fixed: CHANGELOG Build-rejection qualifier scoped to the context-scoping invalids; stale Current-target tail trimmed. All substantive dimensions verified clean (contract fidelity, builder fidelity, evidence content, manifest arithmetic recomputed, test adequacy, ownership); four probes re-derived from the Java sources (per-invocation variable resampling, per-partition isolation, constant format, triggerless snapshot channel).
- [x] Commit and push (single semantic commit with all checkpoint edits folded in).
