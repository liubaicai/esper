...
- Current target: Draft 4.386 'infra-namedwindow-views-time' - slice S5 of the InfraNamedWindowViews inventory: ord 4 `InfraTimeWindowSceneTwo` (`java-runtime-879d6ad8aee378657a02`), ord 5 `InfraTimeFirstWindow` (`java-runtime-5f5a65dd5f72c8e24619`), ord 8 `InfraExtTimeWindowSceneThree` (`java-runtime-a6bfdd3389c077127861`); ord 3 is already verified via `case.time-window-long-running`. Scout findings to carry in: the chain needs virtual-time (`advance-time`) step support and per-record clock times; the engine's named-window firsttime gate is arrival-anchored where Java anchors at deploy (latent for ord 5), and slipping/external-timed expiry on virtual time needs verification for ord 8 (which is `win:time`, not `ext_timed`). Follow-ups: remaining InfraNamedWindowViews slices (S6-S14), the mixed-mutation-wave ordering investigation, perf items, multithread/rowrecog/EsperIO domains.

## Current work unit
Active: Draft 4.385 ('infra-named-window-lengthbatch-sort-views') - chain, evidence and facts complete; gates and independent review in flight.

- Scope: slice S4 of the InfraNamedWindowViews inventory - ord 19 `InfraLengthBatch` (`java-runtime-81f4d92e62dbafdd7f2f`), ord 20 `InfraLengthBatchSceneTwo` (`java-runtime-ce4a95ec7941743a7bfe`), ord 21 `InfraSortWindow` (`java-runtime-7edfd8382e4eee6892dc`), ord 22 `InfraSortWindowSceneTwo` (`java-runtime-5fb3e9a4d20bc3964f5f`); 80 records (11/26/30/13).
- New semantics pinned: length_batch releases only at the size boundary (first flush carries no old rows; later flushes carry the replaced batch as old), unflushed deletes stay silent, sort keeps the N smallest with newest-first ties, expels the maximum on overflow and self-evicts a non-surviving arrival; length_batch/sort consumers are istream-only so delete waves reach the window statement alone.
- [x] Scouts in hand (`ViewsBatchSortJavaContract`, `ViewsBatchSortGoSurface`); contract frozen from the Java report.
- [x] Go chain `internal/app/parity/infra_named_window_lengthbatch_sort_views.go` + modes + 6-test family; probe replay reproduced the frozen 80 records before the authored scenario landed.
- [x] Asset lane delivered oracle/script/scenario/trace (80 records, deterministic; verified my chain constants against the scenario); Go trace and evidence passing with 0 differences.
- [x] Manifest (`case.infra-namedwindow-views-lengthbatch-sort`, capability DV +4, summary 641/264/984/3582), CHANGELOG 4.385, roadmap 4.385, README stats, PLANS.
