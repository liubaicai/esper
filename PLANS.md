...
## Current work unit
Shipped: Draft 4.322 ('case.epl-subselect-order-of-eval-index') — all items below are complete through gates; independent review and commit/push remain.

Frozen contract (scouts 'JavaContract4322' + 'GoSurface4322'): 4 executions / 170 steps / 45 records, all times epoch 0, no advance-time, istream-only. Cases: correlated-subquery-order (java-runtime-3dbb926e23a4521d64d5, static java-2ddf3ed58c64fe3674f7) — 2-statement module (seed #lastevent + s0 with window(tl.*)/window(ts.*) over #time(20 minutes) correlated by securityID=1000), wall-clock bean times pinned to 1000/1010 (outcome invariant: securityID-only correlation, length-only Java assertions), 2 sends → 2 records whose columns render as arrays of rows {price,securityID,time,volume}; order-of-eval-subselect-first (java-runtime-9a68943733f98a1ea1dd, static java-eaa8e4f13676cc852f92) — 2 deploy cycles with preeval-on (default selfSubselectPreeval=true) making 'not in' filters silent: 2 sends → 0 records; index-choices-overdefined-where (java-runtime-f220d864166a5650e6f7, static java-29ec8e7d3c6e96c8d5aa) — 19 deploy/sends/undeploy cycles over scalar correlated subqueries with #unique field sets, where-shape variants (incl. between, sorted ranges, DISABLE_UNIQUE_IMPLICIT_IDX hint on cycle 14), 36 records {c0,c1} with null for no-match/multi-match/uncorrelated; unique-index-correlated (java-runtime-0fbd10b1080bb8e7afa3, static java-77258a6e645d2449b31c) — 4 cycles (#unique, #firstunique, #time(1)#unique, #groupwin#unique), 7 records. @Hook(INDEX_CALLBACK_HOOK) dropped from all EPLs (regression-lib hook class off classpath; plan-observation only, behavior-neutral).

Go surface (scout adjudication): ALL READY, zero engine work. WindowEvents() for window(x.*) aggregates (subselect_multirow.go SubqueryValueWithOptions[[]Event] precedent); Not(SubqueryIn) with 3VL empty-set semantics (subselect_in.go); Unique/FirstUnique windows (view_unique.go); implicit subquery indexing is plan-only and trace-invisible (subquery.go subqueryIndexSelection); registry freshness for #unique/#time subquery sources confirmed (subquery.go:40-63, runtime.go:6651). Runner-local structs: SupportTradeEventTwo{time,securityID,price,volume}, SupportSimpleBeanOne/Two (infra_named_window_join.go precedents). Multi-cycle cases need generalized undeploy-all handling (undeploy active deployments and continue the loop, per-case sequence numbering continuous). Mode 'epl-subselect-order-of-eval-index'/-diff; file internal/app/parity/epl_subselect_order_of_eval_index.go; no compat op additions.

- [x] Freeze order-of-eval/index contract (parallel prefetch scouts).
- [x] Author oracle, script, and scenario via asset writer.
- [x] Implement epl-subselect-order-of-eval-index runner, modes, and tests.
- [x] Generate traces and zero-difference evidence.
- [x] Update manifest, roadmap, and CHANGELOG.
- [x] Run gates and independent parity review: SubselectOrderOfEvalIndexParityReview PASS.
Order-of-eval/index review: final parity review `SubselectOrderOfEvalIndexParityReview` (independent read-only reviewer, agent `agent_2d0f1f97-296e-42ff-a97a-b01fc9dcbb05`) returned PASS across contract/engine/runner/evidence/tests/manifest/docs/never-change with zero P1 findings, including an independent Java-oracle reproduction and fresh Go replay with zero differences. Fixed before commit: P2 — raw-mutation count corrected to 7 in CHANGELOG/roadmap. Accepted P3 observations: dead wantIndexC1 residue removed before commit; nested row-array key order (Java kind-first vs Go map-sorted) semantically neutral under canonical comparison; differentialVerifiedCapabilities=37 vs reviewer jq-count 40 is a pre-existing definition difference at HEAD; check-layout gofmt arg-list Windows limitation (pre-existing).
- [x] Commit and push the unit (Git owns identity; no hash recorded here).

## Delegation checkpoint

Draft 4.322 unit:
- Prefetch scouts (read-only, concurrent, before implementation): Java contract scout 'JavaContract4322' ('agent_e9fa9d41-114d-4c69-a10e-773cd7632a6a') pinned all four execution timelines (170 steps / 45 records), the preeval-on zero-record semantics of case B, the 19-cycle index-choice matrix, the wall-clock pinning justification (1000/1010), the @Hook drop rationale, and the array-of-rows trace rendering; Go surface scout 'GoSurface4322' ('agent_358b8b73-fccb-4a7b-a0bf-dca71989e7da') adjudicated all items READY with cited precedents and named the runner-local struct needs.
- Asset writer (isolated parity-asset writer, concurrent with primary runner work): owns 'tools/java-oracle/EPLSubselectOrderOfEvalIndexScenarioOracle.java', 'tools/java-oracle/run-epl-subselect-order-of-eval-index.sh', 'testdata/parity/epl-subselect-order-of-eval-index.json'. The oracle must add array normalization copied from EPLSubselectMultirowScenarioOracle (the within-filter-having oracle normalize() does not handle Object[]), pin the 45 records, and leave selfSubselectPreeval at its default.
- Primary owns the Go runner ('internal/app/parity/epl_subselect_order_of_eval_index.go'), run.go wiring, run_test.go family, traces/evidence, manifest, roadmap, CHANGELOG, PLANS.md, gates, review, commit, and push.
- Serial-fallback exception (recorded): the asset-writer agent was cancelled by a session interruption mid-unit with no resumable agent ID; it had completed the scenario JSON (verified against the frozen contract: 170 steps, 19-cycle matrix, hint on cycle 14, no hooks, doubled @name pinned). The primary authored the remaining oracle and run script serially rather than relaunching a context-free replacement; per-unit ownership and review unchanged.
