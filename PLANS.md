...
- Current target: Draft 4.313 implements `resultset-orderby-join` — `ResultSetOrderBySimple` ordinals 1-2 (`ResultSetIterator`, `ResultSetAcrossJoin` as two statement cases) in a new join-based differential scenario; contract frozen below, asset writer dispatching, no other writes yet.
## Current work unit
Draft 4.313 creates `case.resultset-orderby-join` for `ResultSetOrderBySimple.java` ordinals 1-2 at fixed Java commit `9e1b9f1cc9117fea4bf33ab043762c045d73839c`: ordinal 1 `ResultSetIterator` = `java-runtime-53aea47cdd71b80fdbb9` (static `java-3d5138ecf75168eb575d`), ordinal 2 `ResultSetAcrossJoin` = `java-runtime-4a0adaff4741914b9ad1` (static `java-61243b98a3005c75950a`); flags empty. The primary independently re-read the Java source (lines 126-204 and helpers createAndSend/sendJoinEvents/sendEvent/orderValuesByPriceJoin/orderValuesBySymbolPrice) and confirms the scouts' contract:
Case `iterator` (ordinal 1) deploys `@name('s0') select symbol, theString, price from SupportMarketDataBean#length(10) as one, SupportBeanString#length(100) as two where one.symbol = two.theString order by price`, listener attached but unrecorded. Steps: seed five SupportBeanString rows CAT/IBM/CMU/KGB/DOG, send MD CAT@50, then MD IBM@49, CAT@15, IBM@100, snapshot 1 (4 rows ordered by price: {CAT,CAT,15},{IBM,IBM,49},{CAT,CAT,50},{IBM,IBM,100}), send MD KGB@75 (a market event; the KGB string was already seeded), snapshot 2 (5 rows: same four plus {KGB,KGB,75} at rank 4). Two snapshot records, sequence 0, no listener records, all volumes 0.
Case `across-join-price` (ordinal 2) deploys `@name('s0') select symbol, theString from SupportMarketDataBean#length(10) as one, SupportBeanString#length(100) as two where one.symbol = two.theString output every 6 events order by price`. Steps: send six MD (IBM/0/2, KGB/0/1, CMU/0/3, IBM/0/6, CAT/0/6, CAT/0/5) with the string window empty (no join rows), then seed the five strings. The output limiter counts join delta rows: CAT +2, IBM +2, CMU +1, KGB +1 = fires on the KGB string; one new-only listener callback of 6 rows ordered by price {KGB,IBM,CMU,CAT,CAT,IBM} with prices {1,2,3,5,6,6} (CAT-before-IBM at price 6 is contractual via orderValuesByPriceJoin), fields exactly {symbol, theString}.
Case `across-join-symbol-price` (ordinal 2) deploys `@name('s0') select symbol from SupportMarketDataBean#length(10) as one, SupportBeanString#length(100) as two where one.symbol = two.theString output every 6 events order by theString, price` (fresh deployment, fresh windows). Same step order (six MD then five strings), one callback of 6 rows ordered by (theString, price): {CAT@5, CAT@6, CMU@3, IBM@2, IBM@6, KGB@1}, fields exactly {symbol}.
Scenario `testdata/parity/resultset-orderby-join.json`: 3 cases (observation iterator/2, listener/0, listener/0), 37 steps (13 + 12 + 12), ops case/send/snapshot only. Evidence javaRuntimeIds/javaExecutions list the two unique executions; the AcrossJoin runtime ID appears in two cases. Go runner `internal/app/parity/resultset_orderby_join.go` registers modes `resultset-orderby-join`/`-diff` in run.go; the iterator case reuses the no-subscribe snapshot replay convention (sequence 0). Manifest case maps to capability `resultset.orderby-simple` ("ORDER BY on join result sets" remaining item).
Delegation: asset writer `agent_f96cdcf8-d81d-4eda-af6b-569c0467e65f` (isolated) authored the new Java oracle, runner script, and scenario JSON while the primary wrote the Go runner, run.go modes, and the six-test family on disjoint files. Integration repairs owned by the primary: the oracle's case-metadata validator indexed the length-2 RUNTIME_IDS/EXECUTION_NAMES arrays with the case index (0-2), throwing ArrayIndexOutOfBounds for the shared-ordinal AcrossJoin cases — fixed with an execution-index mapping (case 0 -> 0, cases 1-2 -> 1). The Go side needed `esper.SelectFrom(source, name, expr)` (JoinSelection) rather than `Alias` for plain join projections without GroupBy; engine semantics were probe-verified first (iterator ordering, output-limit counting join deltas, order-by on unselected join columns).
- [x] Freeze ordinals 1-2 contract.
- [x] Implement resultset-orderby-join Go runner, modes, and tests.
- [x] Author join oracle, script, and scenario via asset writer.
- [x] Generate join traces and zero-difference evidence.
- [x] Update manifest, capability remaining, roadmap, and CHANGELOG.
- [x] Run gates and independent parity review.
- [x] Commit and push resultset-orderby-join unit.
Join verification: `go test ./internal/app/parity -run 'TestRunResultsetOrderbyJoin' -count=1` passed (direct replay, passing evidence, 10 mutations reject, checked-in evidence matches both traces and live replay, 11 malformed-scenario mutations reject, runtime-ID mapping). The pinned Java 9.0.0 oracle compiled, emitted four records, and regenerated the checked-in Java trace byte-identically (`cmp=0`); evidence is passing with 4 Java records, 4 Go records, 0 differences, and two frozen runtime IDs (Java trace SHA-256 `0cd2e9400023fdf41ac0c4dbff6d0c60d70889d40b002f9fd64a00b9d13086d3`, Go trace SHA-256 `05c32de220850747d0076f9ae70a295e5675a7ef0b76b5d21bf5e69c04519a92`). Both runtime IDs were already referenced by the umbrella `case.resultset-orderby-simple` inventory, so referenced/unreferenced counts stay 3166/970 while associations grow 3399 -> 3401. Full gates: `gofmt` clean, `go vet ./...`, complete `go test ./... -count=1`, layout equivalents (tracked-generated, batched `gofmt -l`, facade `cmp`, apidump `cmp`, `go list`), `git diff --check`, JSON parses, and `bash -n` on the new runner script all pass; the recorded race-gate host limitation from Draft 4.312 still applies (this unit also changes zero `internal/esper`/`internal/compat` production code). Final parity review `OrderbyJoinReview` (independent read-only reviewer, agent `agent_2d29f17f-0f01-4b5e-be3e-613949a0e4ec`) returned PASS with zero P1/P2 findings and one accepted P3 note (the oracle's iterator-case listener silently ignores hypothetical callbacks, mirroring the Go no-subscribe convention; divergence cannot be masked because zero listener records are expected on both sides and snapshots are independently validated).
Deferred with rationale (from this prefetch): `ResultSetOrderBySimpleSortCollator` (java-runtime-97855caf4970534092ae) requires Java-Collator-FR semantics plus a `sortUsingCollator=true` compiler-config knob — zero Go hits for collation; needs a dedicated engine feature unit. `ResultSetOrderBySelfJoin` (java-runtime-74b0c1ce48febfe83007) is a 3-way self-join with `groupwin`+`lastevent`, correlated `in` with null, `cast(count(*), int)`, and row-per-event ungrouped-aggregate semantics — viable but its own unit. `ResultSetOrderByMultiDelivery` (java-runtime-1c3d57ae9e8d4bca739f) needs pattern single-batch delivery verification first.
