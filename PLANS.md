...
- Updated: 2026-08-31; Draft 4.290 is validated, independently reviewed, and ready for semantic commit/push.
- Baseline: `HEAD` == `origin/master` at Draft 4.289 (`d7d47ddc988df0d938fadb6f2ff3c787c7a3b3eb`), pushed; Draft 4.290 `ResultSetQueryTypeRollupGroupingFuncs` is complete. Focused replay, pinned Java regeneration, byte-identical trace checks, differential evidence, mutation rejection, layout, vet, full tests, and race all pass.
...
## Completed work unit
Draft 4.290 is frozen as one execution from fixed `ResultSetQueryTypeRollupGroupingFuncs.java`: `ResultSetQueryTypeDocSampleCarEventAndGroupingFunc` ordinal 0, runtime `java-runtime-52a4f853b6052fb47437`, shared/static ID `java-1ae66c9985dc53282af0`, all flags empty. The contract is three sequential fresh deployments—plain `compileDeploy`, `@Audit` EPL, and `eplToModelCompileDeploy`—each subscribing `s0`, sending `SupportCarEvent(skoda, france, 100)` then `(skoda, germany, 75)`, and emitting listener new-only rows in the fixed eight-row order per phase: `[skoda,france,100,0,0,0]`, `[skoda,null,100,0,1,1]`, `[null,france,100,1,0,2]`, `[null,null,100,1,1,3]`, then `[skoda,germany,75,0,0,0]`, `[skoda,null,175,0,1,1]`, `[null,germany,75,1,0,2]`, `[null,null,175,1,1,3]`; fields are `name`, `place`, `sum(count)`, `grouping(name)`, `grouping(place)`, `gid`, for 6 listener records and 24 rows at epoch zero.
Ownership is frozen: primary owns new `internal/app/parity/rollup_grouping_funcs.go`, `internal/app/parity/run.go`, `internal/app/parity/run_test.go`, scenario/trace/evidence, manifest and documents; one isolated `parity-asset-worker` owns only new `tools/java-oracle/ResultSetQueryTypeRollupGroupingFuncsScenarioOracle.java` and `tools/java-oracle/run-resultset-querytype-rollup-grouping-funcs.sh`. Fixed Java remains read-only. Read-only scouts `ScoutNextResultset` and `ScoutNextGoSurface` completed concurrently and confirmed the exact contract and reusable typed `GroupByGroupingSets`/`Grouping`/`GroupingID` surfaces; no `internal/esper` change is expected.
Delegation checkpoint: `ScoutNextResultset` (`java-oracle-scout`) and `ScoutNextGoSurface` (`scout`) ran concurrently for the N+1 FAF candidate while the active unit was reviewed; `ReviewDraft4290` (`parity-reviewer`) independently re-audited the latest unit after fixes and returned PASS with no actionable P0-P2 findings. The review findings—pinned launcher commit override, missing checked-in evidence replay test, and stale docs—were fixed before final gates.
...
## Recent outcome
Draft 4.290 added `resultset.aggregate-dimensional` case `rollup-grouping-funcs-dedicated` for `ResultSetQueryTypeRollupGroupingFuncs.java` ordinal 0. Three fresh Java compilation/deployment paths and typed Go `GroupByGroupingSets` replay produced six matching listener records (24 grouping rows) with zero differences; checked-in evidence, manifest totals, documentation, full local gates, and independent review are consistent.

## Delivery
- Draft 4.290 is complete: implementation, scenario, Java/Go traces, evidence, manifest, and documentation are validated; independent reviewer `ReviewDraft4290` returned PASS with no actionable P0-P2 findings. All final pre-commit checks passed. The next action is one semantic commit and push to `master`; after commit only read-only remote verification remains.
