...
- Current target: Draft 4.358 'dataflow-create-start-stop-destroy' — NEW differential chain for EPLDataflowAPICreateStartStopDestroy (java-runtime-1a8426a71e3b5252a181 EPLDataflowCreateStartStop, java-runtime-2c3c90d3cdb64596c225 EPLDataflowDeploymentAdmin), continuing the dataflow DV upgrade on the 4.356/4.357 graph differential pattern. Follow-up queue: dataflow OpSelect/EPStatementSource chains, DV-list consolidation, invalidity dispositions, database slices.
...
## Current work unit
Active: Draft 4.358 ('dataflow-create-start-stop-destroy').

- [x] Parallel read-only scouts dispatched. Java contract scout agent_005aa560 COMPLETE — headline: NEITHER execution ever starts/runs a dataflow instance; they are service-registry/administration differentials (deploy → getDataFlows listing → descriptor fields → instantiate discarded at INSTANTIATED → undeploy → three byte-exact instantiate-error probes → getDataFlows empty → re-deploy with a random UUID deployment id, which must NOT be pinned). Zero rows; fully deterministic; single-threaded. Proposed record protocol: service records (dataflows listing, dataFlowName, statementName, instantiateError messages, descriptor null) + state records (instantiated ×2) + a module item-count record — 12 total (10+2), reducible to 11 if the Go module-parse surface is absent. Open dependency for the Go verdict: the Go engine appears to lack a public registry-listing/descriptor API (only internal notifyDataflowStatementDeployed/Undeployed + InstantiateDataflow) — a small surface addition or frozen adaptation may be required. Go surface scout agent_b109cc6b in flight.
- [ ] Freeze contract; oracle/scenario/trace assets (writer) + Go runner + mode + tests (primary).
- [x] Manifest (NEW case), evidence, roadmap, CHANGELOG. (Writer agent_4aff8dce delivered oracle with internal byte-exact message assertions, scenario, script (exit 0, deterministic), Java trace 8 records matching the frozen contract and the Go probe exactly. Evidence passing 0 differences, 2 IDs/2 executions; BOTH trace artifacts committed; manifest NEW case with per-execution static IDs mapped to dataflow.graph; summary 633 cases / 246 DV cases / 911 DV runtime IDs / 3541 assoc / referenced 3283 / unreferenced 853; roadmap epl 255→253 (two newly-referenced IDs) + supplement; CHANGELOG entry. Protocol completion: DiffTraces gained Count-field comparison — count records were previously not discriminated; full parity suite re-run green under the stricter differ.)
- [x] Gates: affected packages + make check + git diff --check (green; recorded before commit).
- [x] Independent parity review. Reviewer agent_bf1f82bf-df08-4462-b71f-dcda5c6f3e8d OVERALL PASS with canonical-formula manifest recomputation, differ-completeness verification (Count was the last uncompared TraceRecord field; all 9 count-bearing records in committed chains agree Java/Go), record-content checks, and four probes re-derived from the Java sources (identical message template, post-undeploy removal mechanism, re-deploy equivalence, evidence values). P2 fixed in-unit: per-execution static-ID pairing was swapped (java-32675e64a2d679c673e4 is DeploymentAdmin, java-d21cf72eab5cb8f58a0c is CreateStartStop) — manifest array swapped and roadmap prose corrected. P3 informational: the committed .trace.json carries an explicit value:null (raw minimaljson) while the evidence embed omits the key — differ-equal, noted for future embed-vs-file checks.
- [x] Commit and push (single semantic commit with all checkpoint edits folded in).
