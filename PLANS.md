...
## Current work unit
Active: Draft 4.324 ('infra-nwtable-subq-at-eventbean') — @eventbean whole-event subselects over named window/table; contract frozen from both prefetch scouts; asset writer dispatched, primary implementing the runner.

Frozen contract (scouts 'JavaContract4324' + 'GoSurface4324'): 2 executions of InfraSubSelStar (java-runtime-59604d7dbcef79bacde1 namedWindow=true; java-runtime-d8e8832ac46005459ffd namedWindow=false; shared static java-0d99efae7b23e8d8b346; flags []). Identical observable timelines: create unnamed `@public create window MyInfra#keepall as (c0 string, c1 int)` (NW) / `@public create table MyInfra(c0 string primary key, c1 int)` (table) → unnamed `insert into MyInfra select theString as c0, intPrimitive as c1 from SupportBean` → s0 `@name('s0') select p00, (select * from MyInfra) @eventbean as detail from SupportBean_S0` (listener on s0 only). Sends: S0(id=0) → callback {p00:null, detail:null} (empty infra; Java assertNull(getFragment)); SupportBean E1/1 → no output; S0 → {p00:null, detail:[row(E1,1)]}; SupportBean E2/2; S0 → {p00:null, detail:[row(E1,1), row(E2,2)]}. 10 steps per case (case marker + 3 deploys + 5 sends + undeploy-all), 6 records total. detail renders as row array {c0,c1}; null fragment renders {'state':'null'} with key present. Table-case detail row order is AnyOrder in Java — canonicalize by sorting rows in both oracle and comparator.

Go surface (scout adjudication): ALL READY, zero engine work. Key decision: use SubqueryValueWithOptions[[]Event](inner, WindowEvents()) — the aggregate path accumulates named-window/table candidates into the final group so values[0] is the full snapshot; empty infra → WindowEvents() Null() → matches Java getFragment null. Do NOT use SubqueryEvents (renders present-empty [], would fail the diff). Inner source = FromNamedWindow/FromTable (live snapshot per S0 evaluation, never registry-owned). Precedents: subselect_multirow.go:215, epl_subselect_order_of_eval_index.go:452. Infra creation before engine snapshot (subq-uncorrel precedent). Mode 'infra-nwtable-subq-at-eventbean'/-diff; file internal/app/parity/infra_nwtable_subq_at_eventbean.go; no compat op additions.

- [x] Freeze @eventbean subselect contract (parallel prefetch scouts).
- [x] Author oracle, script, and scenario via asset writer.
- [x] Implement infra-nwtable-subq-at-eventbean runner, modes, and tests.
- [x] Generate traces and zero-difference evidence (6 records per side, 0 differences; oracle replay byte-identical).
- [x] Update manifest, roadmap, and CHANGELOG.
- [x] Run gates (full suite 14 pkgs, gofmt, vet, git diff --check, facade cmp, manifest validation).
- [x] Commit and push the unit (Git owns identity; no hash recorded here).

## Delegation checkpoint

Draft 4.324 unit:
- Prefetch scouts (read-only, concurrent, before implementation): Java contract scout 'JavaContract4324' ('agent_d4432653-e1cf-4164-9584-a396ea4b5f6b') pinned both timelines (6 records), the null-fragment render ({'state':'null'} key present, proven by assertNull(getFragment) + subselect-multirow precedent), the shared static id, and the AnyOrder table-row caveat; Go surface scout 'GoSurface4324' ('agent_7e90822b-6299-47f8-a482-e250e2151254') adjudicated all items READY and made the load-bearing WindowEvents() vs SubqueryEvents decision (present-empty [] would fail the diff).
- Asset writer (isolated parity-asset writer, concurrent with primary runner work): owns 'tools/java-oracle/InfraNWTableSubqueryAtEventBeanScenarioOracle.java', 'tools/java-oracle/run-infra-nwtable-subq-at-eventbean.sh', 'testdata/parity/infra-nwtable-subq-at-eventbean.json'. The oracle must sort detail row arrays canonically (Java AnyOrder) and render the null fragment as {'state':'null'}.
- Primary owns the Go runner ('internal/app/parity/infra_nwtable_subq_at_eventbean.go'), run.go wiring, run_test.go family, traces/evidence, manifest, roadmap, CHANGELOG, PLANS.md, gates, review, commit, and push.
