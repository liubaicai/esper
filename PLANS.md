...
- Current target: Draft 4.331 'resultset-aggregate-filter-named-parameter-linear-join' (candidate) — ResultSetAggregateFilterNamedParameter ordinals 9/11/13; contract from AggregateFilterNamedParameterScout; two pre-implementation spikes required (FilterAggregate join predicate binding; first(*,filter).theString property navigation).

## Current work unit
Active: Draft 4.330 ('infra-named-window-insert-from') — 4 of 7 executions of InfraNamedWindowInsertFrom.java as ONE differential case. Contract frozen from 'InsertFromScout' (agent_fdcfb9e3, Java contract + Go surface, zero engine work pending three runner-side probes).

Frozen contract (ordinals are executions() ordinals; flags [] all; javaFlags []):
- ord 0 create-after-named: InfraCreateNamedAfterNamed, java-runtime-b3f6cb7b36c5211c8822, static java-b0917219c462cba9d770. Java EPL (single module, no @public): `@name('windowOne') create window MyWindow#keepall as SupportBean`; `@name('windowTwo')create window MyWindowTwo#keepall as MyWindow` (NO space after windowTwo' in source); `insert into MyWindow select * from SupportBean`; `@name('selectOne') select theString from MyWindow`. Listeners windowOne+selectOne; send SupportBean E1/1 → 2 records: windowOne new [{intPrimitive:1,theString:"E1"}], selectOne new [{theString:"E1"}]. Scenario adds @public to windowOne/windowTwo creates (oracle separate-module convention; asset writer pins the exact resulting strings incl. the no-space quirk handling). Steps: 7 (4 deploys + 1 send + undeploy-all + marker), iteratorSnapshots 0.
- ord 1 insert-where-type-filter: InfraInsertWhereTypeAndFilter, java-runtime-e601b3cc7f827d578185, static java-cff4a3193da2b063a351. EPL (source verbatim): `@name('window') @public create window MyWindowIWT#keepall as SupportBean`; `insert into MyWindowIWT select * from SupportBean(intPrimitive > 0)` (filtered stream insert); seeded: `@name('windowTwo') @public create window MyWindowTwo#keepall as MyWindowIWT insert`, `@name('windowThree') @public create window MyWindowThree#keepall as MyWindowIWT insert where theString like 'A%'`, `@name('windowFour') @public create window MyWindowFour#unique(intPrimitive) as MyWindowIWT insert`; routing: `insert into MyWindowIWT select * from SupportBean(theString like 'A%')`, `insert into MyWindowTwo select * from SupportBean(theString like 'B%')`, `insert into MyWindowThree select * from SupportBean(theString like 'C%')`, `insert into MyWindowFour select * from SupportBean(theString like 'D%')`. Timeline: deploy window+insert; sends A1/1 B2/1 C3/1 A4/4 C5/4 (5 window listener records); deploy windowTwo → snapshot [A1,B2,C3,A4,C5] ordered (projected [theString]); deploy windowThree → snapshot [A1,A4]; deploy windowFour → snapshot [C3,C5] mode any (unique dedup-at-seed keeps last per id); deploy 4 routing inserts; send B9/-9 → windowTwo record [B9]; A8/-8 → window record [A8]; C7/-7 → windowThree record [C7]; D6/-6 → windowFour record [D6]; undeploy-all. Seed deliveries never reach create-statement listeners (attached post-deploy). StatementProperty/assertSame/count checks dropped as approved differences (counts derivable from pinned snapshots). Steps: 23 (marker + 9 deploys + 9 sends + 3 snapshots + undeploy-all), 12 records, iteratorSnapshots 3.
- ord 5 lenient-map / ord 6 lenient-objectarray: InfraNamedWindowInsertLenientPropCount{rep=MAP} java-runtime-46011542d6e9d34a87f5 / {rep=OBJECTARRAY} java-runtime-8138dd777290d00417d1, shared static java-282d7b64878866ea4709. EPL: `@public create <REP> schema MyTwoColEvent(c0 string, c1 int)` (REP = MAP / OBJECTARRAY verbatim); `@public @name('window') create window MyWindow#keepall as MyTwoColEvent`; `insert into MyWindow select theString as c0 from SupportBean`; `insert into MyWindow select id as c1 from SupportBean_S0`. Go: ord5 NewMapSchema, ord6 NewObjectArraySchema (same fields c0 string/c1 *int — pointer for null-exact c1); lenient partial inserts via single SetColumn (c1 null-filled). Sends SupportBean E1/0 → snapshot [{c0:"E1",c1:null}]; send SupportBean_S0 id=10 → snapshot [{E1,null},{null,10}] ordered. Steps: 10 each (schema deploy is a no-op arm in the runner; env-level registration), 2 records each, iteratorSnapshots 1.
Route-payload correction (asset-writer, per Java line 171): the B-trigger send is B9/-9, not B2/-9 (the B2 row already lives in the window; the record table itself pinned B9). Step-totals correction: ord 1 is 23 steps (marker + 9 deploys + 9 sends + 3 snapshots + undeploy) and the lenient cases are 10 each, so Totals: 7+23+10+10 = 50 steps, 18 records (11 listener + 7 snapshots). Ordered snapshots pin explicit "ordered" mode (engine iteration order); windowFour pins mode any with canonical row order. Mode 'infra-named-window-insert-from'/-diff; new file internal/app/parity/infra_named_window_insert_from.go; run.go two dispatch arms; 6-test family in run_test.go. Deferred: ord 2 (OM toEPL round-trip + 4-representation matrix — the execution's substance is unportable), ord 3 (invalid compiles — no Go construct rejects them), ord 4 (variant window + id? projection unproven).

- [x] Freeze insert-from contract (InsertFromScout prefetch).
- [x] Author oracle, script, and scenario via asset writer (agent_12f75aa2; end-to-end oracle run; corrected the primary's step totals to 7/23/10/10 = 50 and the B9 routing payload).
- [x] Implement infra-named-window-insert-from runner, modes, and 6-test family.
- [x] Generate traces and zero-difference evidence (18/18, evidence passing).
- [x] Update manifest (621/234/835/3458), roadmap, and CHANGELOG.
- [x] Run gates and independent parity review: make check PASS, git diff --check clean; InsertFromParityReview OVERALL PASS (one P2 seed-build prose claim corrected in three tracked files + P3s fixed, re-confirmed by the same reviewer; residual filtered-seed-leak mutation retargeted to the where-filtered seed snapshot per the reviewer's minimal-fix suggestion).
- [x] Commit and push the unit (Git owns identity; no hash recorded here).

Integration notes: primary synced the runner to the authored scenario (explicit ordered/any snapshot modes; B9 route payload; case[].epl pins) and fixed two first-replay bugs (lenient window statements were wrongly subscribed — Java's lenient executions carry no listener, snapshots only; ord-1 spans recounted to 23). The seed path (Snapshot + Underlying + engine.InsertNamedWindow) was validated differentially including the unique-retention dedup-at-seed and the where-filtered seed.
...
- Post-validation concurrent batch (running now): independent parity reviewer 'OnUpdateMiscParityReview' (`agent_aa44ac67-aa99-4891-8062-c0b4cf5835f2`) + N+1 prefetch scout 'InsertFromScout' (`agent_fdcfb9e3-d4c8-47ac-942d-05016acdee45`, read-only, Java contract + Go surface for InfraNamedWindowInsertFrom.java).
...
- Post-validation concurrent batch (running now): independent parity reviewer 'InsertFromParityReview' (`agent_4c006b9c-f24e-4a38-b982-6be9405b2caf`) + N+1 prefetch scout 'AggregateFilterNamedParameterScout' (`agent_a78845dd-992b-4741-9b91-eb88622ac976`, read-only; COMPLETE).
- 4.331 prefetch result (AggregateFilterNamedParameterScout): the real gap in ResultSetAggregateFilterNamedParameter.java is 8 unreferenced executions — ordinals 9, 11, 13, 15, 17, 18, 19, 20 (the older "26 unreferenced" snapshot was the file total; the file also already has case.resultset-aggregate-filter-named-parameter differential-verified covering ordinals 4-7, so new units need new case ids). Recommended slicing: unit A = ord 9/11/13 (join linear-bound/unbound + mixed-filter window), unit B = ord 15/17/18 (sorted-bound join, sorted-unbound join, two-key sorted multicriteria — needs doublePrimitive on the bean struct), ord 19 (audit/reuse) alone or paired; ord 20 (5 invalid compiles incl. create-table filter rejection and correlated-subselect aggregation) has no Go rejection surface — approved difference via go-unit tests unless the parity framework grows a compile-invalid op. Zero engine work except two pre-implementation spikes: (1) FilterAggregate predicate binding under Join (does ctx.Group hold per-side events or composite pairs — decides JoinField-bound predicates vs side-extraction shim, internal/esper/expr.go:3338); (2) first(*,filter:...).theString property navigation over an Event-valued aggregate (FirstEventValue + Property, expr.go:4438 / facade 5334). The existing translator lacks doublePrimitive and SupportBean_S1 — new unit files define their own.
