...
- Current target: Draft 4.357 'dataflow-api-op-lifecycle' — NEW differential chain for EPLDataflowAPIOpLifecycle (3 executions: EPLDataflowTypeEvent, EPLDataflowFlowGraphSource, EPLDataflowFlowGraphOperator), continuing the dataflow domain's DV upgrade using the 4.356-established graph differential pattern (DefineDataflow + Custom/CustomSource + OperatorProvider/SourceProvider + lifecycle Start/Join/Cancel). Follow-up queue: EPLDataflowAPICreateStartStopDestroy (2), DV-list consolidation, invalidity dispositions, database slices.
...
## Current work unit
Active: Draft 4.357 ('dataflow-api-op-lifecycle').

- [ ] Parallel read-only scouts dispatched (Java contract pin for the three executions; Go surface adjudication of the lifecycle/typed-event/graph-factory surfaces).
- [ ] Freeze contract; oracle/scenario/trace assets (writer) + Go runner + mode + tests (primary).
- [x] Manifest (NEW case), evidence, roadmap, CHANGELOG. (Writer agent_f16d6ec8 delivered oracle importing the real regression-lib nested classes, scenario, script (exit 0, deterministic), Java trace 21 records matching the frozen contract and the Go probe. Engine-mechanism corrections documented: the operator provider carries only the capture op (provider-supplied instances bypass factory.realize, so records 2-7 require factory-realized SupportGraphSource); the capture readout uses getCurrentAndReset since the forge lacks @DataFlowOpProvideSignal. Evidence passing 0 differences, 3 IDs/3 executions; BOTH trace artifacts committed; manifest NEW case with per-execution static IDs; summary 632 cases / 245 DV cases / 909 DV runtime IDs / 3539 assoc / referenced 3281 / unreferenced 855; roadmap epl 258→255 + supplement; CHANGELOG entry.)
- [x] Gates: affected packages + make check + git diff --check (green; recorded before commit).
- [x] Independent parity review. Reviewer agent_72e2df5c-700a-4827-9aa6-9791bac227a7 OVERALL PASS: Java contract, builder fidelity (recording order matches Java drain order record-for-record; provider carries only the capture op since provider-supplied instances bypass factory realization; forge lacks @DataFlowOpProvideSignal so capture readout uses getCurrentAndReset — both documented corrections sound), trace/evidence integrity (record-by-record equality incl. times), manifest recomputed under the canonical formulas (corrections noted: dataflow.graph DV list went 3→6 not "extended to 5"; referenced 3278→3281 relative to 4.356's 3278), test adequacy (all 7 mutation indices verified), prose, ownership. Four probes re-derived from the Java sources (forge/factory drain counts with nothing Go-pinnable lost; next(numrows=3) impossible; Object[] wrap faithful; state complete after blocking run). P3s: case-2 injection is a name-keyed factory closure rather than the engine's instance-level SourceProvider hook (Build requires a definition factory; prose corrected — observable parity unaffected); case-0 reflect check pins plumbing not the schema name (frozen adaptation, noted); dead statement removed; checkpoint hygiene fixed.
- [x] Commit and push (single semantic commit with all checkpoint edits folded in).
