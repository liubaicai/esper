...
## Current work unit
Shipped: Draft 4.321 ('case.epl-subselect-within-filter-having') — all items below are complete through gates; independent review and commit/push remain.

Frozen contract (scouts 'JavaContract4321' + 'GoSurface4321'): 4 executions / 14 records total. Cases: within-filter-exists (java-runtime-844ff5e51e235151dccc, static java-e0811e7acc03cb5daab2) — 'exists (select * from SupportBean_S1#keepall where MyUtil.compareIt(s0.p00,p10))' filter, 6 sends, 2 records {id,p00,p01:null,p02:null,p03:null} at ids 3/4 (select * exposes exactly five S0 properties: the Java value field has no getter and never becomes an event property — pinned empirically by the oracle); within-filter-row (java-runtime-1405e55b4e78331c4ec7, static java-09e87f283cdd09c8434f) — 'abc'= (select p11 ...) scalar filter, 7 sends, 2 records ids 3/4; within-having-named-window (java-runtime-5249f7c19ee8d26d8f30) and within-having-table (java-runtime-4b4ca324ee2a41b8ccd6) share static java-d69f172401e51113c81d — create MyInfra (#unique(key) window or PK table) + insert + grouped groupwin#length(2) having sum > correlated subselect on MyInfra, 17 sends, 5 records each {c0,c1} = G2/21, G3/31, G3/33, G1/105, G1/100; strict > boundary (equal sums never fire). Flags []. All times epoch 0; no milestones; istream-only listeners; no old rows.

Go surface (scout adjudication): ALL READY, zero engine work. Precedents: infra_nwtable_subquery.go (dual named-window/table cases, SubqueryValue/SubqueryExists with OuterField, FromTable), subselect_filtered.go (filter-clause subqueries), GroupWindow+LengthWindow+Having(Sum,…) chains. Runner-local approved differences: inlined_class UDF compareIt replaced by string Equal (Go closures replace inlined classes), table-variant routing via OnEvent(...).InsertIntoTable+SetColumn (routeTargetSchema is named-window/plain-schema only, plan.go:194), runner-local SupportMaxAmountEvent{key,maxAmount float64} struct. Mode 'epl-subselect-within-filter-having'/-diff; file internal/app/parity/epl_subselect_within_filter_having.go; no compat op additions.

- [x] Freeze subselect-within contract (parallel prefetch scouts).
- [x] Author oracle, script, and scenario via asset writer.
- [x] Implement epl-subselect-within-filter-having runner, modes, and tests.
- [x] Generate traces and zero-difference evidence.
- [x] Update manifest, roadmap, and CHANGELOG.
- [x] Run gates and independent parity review: SubselectWithinFilterHavingParityReview PASS.
Subselect-within review: final parity review `SubselectWithinFilterHavingParityReview` (independent read-only reviewer, agent `agent_7cf97a99-e61a-4015-ba67-d94912a4cc0e`) returned PASS across contract/engine/runner/evidence/tests/manifest/docs/never-change with zero P1 findings. Fixed before commit: P2 - the frozen-contract line above still carried the scout-stage value:0 expectation; corrected to the validated 5-property shape. Accepted P3 observations without action: check-layout gofmt arg-list Windows limitation (pre-existing); Java oracle replays all four cases on one runtime vs Go per-case engines (unobservable; undeployAll tears down infra); manifest case lists both javaSourceFiles while the scenario pins the WithinFilter path in javaSource (defensible asymmetry, noted in the scenario description); one record per batch vs per row (equivalent for single-row batches).
- [x] Commit and push the unit (Git owns identity; no hash recorded here).

## Delegation checkpoint

Draft 4.321 unit:
- Prefetch scouts (read-only, concurrent, before implementation): Java contract scout 'JavaContract4321' ('agent_7d1bac70-c368-4723-b7b8-13fc8c0f9826') pinned all four execution timelines (14 records), static IDs, the multi-statement deployment order (create → insert → s0), and scenario encoding; Go surface scout 'GoSurface4321' ('agent_16c2dc18-de91-4e42-9651-9ee0244f1081') adjudicated all items READY with runner-local approved differences and named the precedents (infra_nwtable_subquery.go, subselect_filtered.go, epl_variables_output_rate.go runner family).
- Asset writer (isolated parity-asset writer, concurrent with primary runner work): owns 'tools/java-oracle/EPLSubselectWithinFilterHavingScenarioOracle.java', 'tools/java-oracle/run-epl-subselect-within-filter-having.sh', 'testdata/parity/epl-subselect-within-filter-having.json'. Local mirror classes for SupportBean_S0/S1 and SupportMaxAmountEvent on the oracle classpath; 14 records pinned (2+2+5+5).
- Primary owns the Go runner ('internal/app/parity/epl_subselect_within_filter_having.go'), run.go wiring, run_test.go family, traces/evidence, manifest, roadmap, CHANGELOG, PLANS.md, gates, review, commit, and push.
