...
- Current target: Draft 4.388 'infra-namedwindow-views-time-order-accum' - slice S7 of the InfraNamedWindowViews inventory (virtual time): ord 9 `InfraTimeOrderWindow` (`java-runtime-320f1b03eddb24520308`), ord 10 `InfraTimeOrderSceneTwo` (`java-runtime-ed5bfc247d088a603224`), ord 14 `InfraTimeAccum` (`java-runtime-d74b2c9c614b26973637`), ord 15 `InfraTimeAccumSceneTwo` (`java-runtime-8b4e2e7d81bb990c7d72`). Scouts in hand: the Java executions advance the clock (timer-driven ordered and accumulating expiry) and the Go engine already has complete TimeOrder/TimeAccum named-window support on both the insert and the AdvanceTime expiry paths (state.go insert arms 3759-3763/3829-3858, expiry arms 4189-4200/4236-4249), with old-data ordering matching Java (insertion order for time_accum multi-row release, external-timestamp order for time_order); no engine change expected. Two unexercised latent divergences to avoid assuming covered: mergeWhere has no TimeOrder arm and the TimeAccum expiry arm ignores TimeAccumCalendar's calendar fields.

## Current work unit
Active: Draft 4.387 ('infra-named-window-ext-time-views') - chain, evidence and facts complete; gates and independent review in flight.

- Scope: slice S6 of the InfraNamedWindowViews inventory (event time, no advance-time steps) - ord 6 `InfraExtTimeWindow` (`java-runtime-c960628819cddf06d0bd`), ord 7 `InfraExtTimeWindowSceneTwo` (`java-runtime-aa7fe5003096f5bf1cf8`), ord 53 `InfraExternallyTimedBatch` (`java-runtime-3af75eefff14ce16ae81`); 49 records (16/19/14).
- Pinned semantics: sliding `ext_timed(value, 10 sec)` releases expired rows as old data together with the arriving row in one invocation (threshold newest-9999), deleted rows emit old-only and swept tombstones stay silent, and the epoch-referenced `ext_timed_batch(value, 10 sec, 0L)` stays silent until the boundary with the first flush carrying no old data. All records at the epoch; `internal/esper` unchanged (the named-window expiry switch intentionally has no external-timed arm, matching Java's insert-driven views).
- Contract correction during the unit: ord 7 has 12 iterator assertion sites (the frozen contract said 7; the 5 milestone re-checks are real assertions), so the slice total is 49.
- [x] Scouts in hand; contract frozen; asset lane delivered oracle/script/scenario/trace (deterministic, 49 records).
- [x] Go chain `internal/app/parity/infra_named_window_ext_time_views.go` + modes + 6-test family (mutation indices cross-checked against the trace layout); Go trace and evidence passing with 0 differences.
- [x] Manifest (`case.infra-namedwindow-views-ext-time`, capability DV +3, summary 643/266/990/3588), CHANGELOG 4.387, roadmap 4.387, README stats, PLANS.
