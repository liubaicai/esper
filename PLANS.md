...
- Shipped by commit 40d13962b: Draft 4.393 ('database-restart-statement'); Git owns identity. The restart output replay and in-process lifecycle guard passed, while the MySQL/JDBC connection-lifecycle boundary remains explicitly non-DV.
## Current work unit
Active: Draft 4.394 ('infra-named-window-insert-shape').

- [x] Contract frozen from the N+1 prefetch scouts `PrefetchInfraInsertShapeJava-2` and `PrefetchInfraInsertShapeGo-2`: fixed Java source `InfraNamedWindowViews.java`, commit `9e1b9f1cc9117fea4bf33ab043762c045d73839c`; slice S12 covers ord 28 `InfraDoubleInsertSameWindow` (`java-runtime-1b9f5b6ccf6b49cdba09`), ord 36 `InfraIntersection` (`java-runtime-f739aa91028d27aa572f`), ord 54 `InfraSelectStreamDotStarInsert` (`java-runtime-9740441818d84a3ee94d`), and ord 56 `InfraOnInsertPremptiveTwoWindow` (`java-runtime-71402af94b27b4255ab8`).
- [x] Observable contract frozen: ord 28 deploys one keepall window, two SupportBean insert statements and `s0`, then sends E1/10; the window receives two separate new callbacks (11 then 12) and `s0` flattens both rows in that order. Ord 36 deploys `#length(2)#unique(intPrimitive)` plus wildcard insert and irstream `s0`; E1 and E2 are new-only, E3/int=2 emits new E3 with old E1/E2 in any order. Ord 54 is deployment-only: Java accepts `p0` plus `sb.* as c0` against an object-array `(p0 int)` target and emits no records; typed Go rejects undeclared `c0` at build time as the approved representation difference, then exercises the expressible p0-only shape. Ord 56 declares TypeOne/TypeTwo/TypeTrigger and WinOne/WinTwo, routes both trigger inserts preemptively, an…
- [x] Asset writer and Go runner implementation dispatched with disjoint files; no shared `internal/esper` change is assumed unless replay proves a semantic gap.
- [x] Initial replay generated deterministic Java and Go artifacts; ord 28 exposed a real engine gap, not a deployment artifact: Go coalesces the two front named-window routes into `create/create/s0(11,12)`, while Java dispatches `create/s0/create/s0` at each front-route boundary.
- [x] Java runtime contract for the fix is frozen from `EPEventServiceImpl.processThreadWorkQueueFront` and `NamedWindowDispatchServiceImpl`: one front item is processed, named-window consumer dispatch is drained, then ordinary statement dispatch occurs before the next front item; nested front routes preempt the older front queue item, and callbacks run outside the engine lock.
- [x] Implemented the narrow per-insert callback boundary in `internal/esper/runtime.go` (primary, superseding the `FrontRouteBoundary` sketch): `namedWindowInsertBoundary` captures one direct insert trigger's direct+consumer+window work; `send` dispatches inbound statement outputs first under an isolated pending state, then one boundary at a time, then routes. Removed the unused broad `dispatchPendingNamedWindowBoundaryLocked` helper and all stderr diagnostics. `TestNamedWindowDoubleInsertDispatchesPerInsertBoundary` fails pre-fix ([create create s0 s0]) and passes post-fix.
- [x] Replayed Java/Go differential evidence: fresh Java trace regenerated epoch-pinned (oracle harness fix: `setInternalTimerEnabled(false)`, drop `EPRuntimeSPI.initialize(0L)`); evidence `passing`, 8/8 records, 0 differences. Focused suite `TestRunInfraNamedWindowInsertShape*` (6 tests incl. 6 mutations) green; full `internal/esper` suite green.
- [x] Manifest (`case.infra-namedwindow-views-insert-shape`, capability DV +4, summary 649/272/1016/3614), roadmap 4.394, CHANGELOG 4.394 facts recorded.
- [x] Independent parity review `ReviewInsertShape`: PASS, no P0/P1/P2 (two unitemized P3 polish notes, nothing actionable). Full local gates green: `go test ./...` all packages ok, `go vet ./...` clean, gofmt clean, `git diff --check` clean, manifest machine checks pass. Ready to commit and push (single semantic commit).
- Prefetch N+1 (read-only, no writes started): final `InfraNamedWindowViews` slice = ord 46 `InfraInvalid`, ord 47 `InfraNamedWindowInvalidAlreadyExists`, ord 48 `InfraNamedWindowInvalidConsumerDataWindow`, ord 52 `InfraPattern`, ord 57 `InfraNamedWindowTimeToLiveDelete` (Java contract in hand from `ScoutFinalNWJava`; Go surface consolidated by primary after two `scout` infra 503s: `TestInfraNWViewsTimeToLiveDeleteParity`, `TestInfraNWViewsInvalidParity`, `TestInfraNWViewsPatternParity` already cover TTL/invalid trio/pattern in `internal/esper/infra_namedwindow_views_parity_test.go` — likely a thin asset-only close-out).
