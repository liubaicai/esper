...
- Shipped: Draft 4.352 ('subselect-range-coercion-joins') committed and pushed; Git owns identity. EPLSubselectAggregatedSingleValue.java now 20/21 executions DV (EPLSubselectAggregatedInvalid remains for the invalidity-policy unit). 
- Current target: next work unit selection. Subselect domain remainder: EPLSubselectInWildcard, EPLSubselectOrderOfEvalNoPreeval (behavioral pair, different suites), EPLSubselectAggregatedInvalid + the exists-suite SODA/OM manifest-surface disposition (invalidity/intentionally-different policy work), and the four policy-gated performance suites. Other roadmap priorities: database join chains, dataflow DV upgrades, method sources, infra tables/named-window mutation/transaction.
