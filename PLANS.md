...
- Current target: Draft 4.359 'dataflow-select-flows' — NEW differential chain for EPLDataflowOpSelect's iterate and join flows (EPLDataflowIterateFinalMarker java-runtime to pin, EPLDataflowFromClauseJoinOrder, EPLDataflowOuterJoinMultirow), upgrading case.dataflow-select-iterate and case.dataflow-select-join from implemented-not-DV on the 4.356/4.357/4.358 graph differential pattern (Go Select/SelectIterate/SelectIterateOnFinalMarker/SelectJoin operators exist). Follow-up queue: dataflow select-state (time-based) and select-representation DV upgrades, DV-list consolidation, invalidity dispositions, database slices.
...
## Current work unit
Active: Draft 4.359 ('dataflow-select-flows').

- [x] Parallel read-only scouts dispatched. Go surface scout agent_176452ab COMPLETE: all three flows GREEN — IterateFinalMarker via SelectIterate (grouped, rows emitted only on FinalMarker, groups in first-arrival order with stable orderBy; Java capture.getCurrent() before marker is 0 rows → 1 trace record with 3 ordered rows {E1,6},{E2,5},{E3,4}); FromClauseJoinOrder via SelectJoin Inner/LastEvent with the three ConnectInput port permutations {0,1,2},{2,1,0},{1,2,0} (1 row each {s0id:1,s1id:10,s2id:100}); OuterJoinMultirow via SelectJoin FullOuter/KeepAll (1 row {p00:"S0_1", p10:null-marker}). Record protocol: all sinks DefaultSupportCaptureOp → operation "capture", statement "flow:DefaultSupportCaptureOp", 5 records total. Named gaps: (a) compat lacks a Row normalizer — add NormalizeRows([]esper.Row) or convert locally; (b) Java runtime IDs pinned by the Java scout/oracle run; (c) case.dataflow-select-iterate / case.dataflow-select-join upgrade implemented→DV (5 executions already name-associated to the umbrella cases — NO new referenced IDs expected; roadmap count unchanged). Java contract scout agent_04e32a5f in flight.
- [ ] Freeze contract; oracle/scenario/trace assets (writer) + Go runner + mode + tests (primary).
- [x] Manifest (NEW case.dataflow-select-flows DV upgrading case.dataflow-select-iterate / case.dataflow-select-join coverage), evidence, roadmap, CHANGELOG. (Writer agent_4e959aa5 delivered oracle/scenario/script/Java trace: 12 records matching the frozen contract and the Go probe exactly. Evidence passing 0 differences, 3 IDs/3 executions; BOTH trace artifacts committed; summary 634 cases / 247 DV cases / 914 DV runtime IDs (+3) / 3544 assoc / referenced 3283 & unreferenced 853 unchanged (IDs pre-associated by the umbrellas — roadmap count stays 253); compat gained NormalizeRows (Row rendering mirror of NormalizeEvents); roadmap supplement; CHANGELOG entry.)
- [x] Gates: affected packages + make check + git diff --check (green; recorded before commit).
- [x] Independent parity review. Reviewer agent_ee668c6a-a54c-4255-84c4-187e23d37242 OVERALL PASS: Java contract (byte-exact graphs, iterate-on-final-marker semantics, three from-clause permutations, outer-join projection incl. the suite's p11 positional quirk documented), builder fidelity (permutation wiring maps aliases to the tuple sides; capture reset mirrors getCurrentAndReset; zero internal/esper changes), compat NormalizeRows mirror verified, traces/evidence record-by-record, manifest recomputed (pre-association of the 3 IDs at HEAD proven — referenced/unreferenced unchanged), tests (Count mutation discriminates via the 4.358 comparison), prose (roadmap count stays 253 correctly). P2 fixed in-unit: javaStaticIds reordered from lexicographic to execution order (positional-pairing convention); P3 embed-omitempty artifact and capability-DV wording noted, no action.
- [x] Commit and push (single semantic commit with all checkpoint edits folded in).
