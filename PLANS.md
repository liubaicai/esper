...
## Current work unit
Active: Draft 4.323 ('infra-nwtable-subq-uncorrel') — InfraNWTableSubqUncorrel uncorrelated infra subqueries; contract frozen from both prefetch scouts; asset writer dispatched, primary implementing the runner.

Frozen contract (scouts 'JavaContract4323' + 'GoSurface4323'): 4 executions (all static java-5af40542812749ae30e4, flags []): (T,F,F) java-runtime-e175cfaf55ce541ca088, (T,T,F) java-runtime-f03db66c82987b5cef54, (T,T,T) java-runtime-d521ebd5c45c89c8e842, (F,F,F) java-runtime-1a498d1407159a6d8c90. One lifecycle per case: deploy create (projection NW #keepall as select theString as a, longPrimitive as b, longBoxed as c from SupportBean — table variant: create table MyInfra(a string primary key, b long, c long); exec 1/2 create prefixed @Hint('enable_window_subquery_indexshare'); exec 2 select/selectTwo prefixed @Hint('disable_window_subquery_indexshare')) → unnamed insert into MyInfra select ... → select (irstream (select a from MyInfra) as value, symbol from SupportMarketDataBean) → selectTwo → delete (@name('delete') on SupportBean_A delete from MyInfra where id = a). Listeners: create/select/selectTwo/delete all attached (create silent in table case). Timeline: M1→select{M1,null}; S1→NW create new{S1,1,2}; M1→select{M1,S1}+selectTwo{M1,S1}; S2→create new{S2,10,20}; M2→select{M2,null}+selectTwo{M2,null} (multi-row scalar → null); S1-A→NW create old{S1,1,2} (delete new unasserted but engine-pinned); M3→select+selectTwo{M3,S2}; S2-A→create old{S2,10,20}; M4→select+selectTwo{M4,null}; S3→create new{S3,100,200}; M5→select+selectTwo{M5,S3}; undeployAll. Record counts if all 4 listeners captured: NW 18 (select 6 + selectTwo 5 + create 5 (3 new + 2 old) + delete 2), table 13. Hints are plan-only (behavior-neutral, verified in Java engine source). Listener order pinned by Java (resolved empirically by the asset writer, contradicting the scout's predicted deferred-dispatch order): Market send → select then selectTwo; SupportBean_A send → create old FIRST, then delete new. The Go engine's deferred trigger dispatch reproduces this order.

Go surface (scout adjudication): ALL READY, zero engine work. Projection named window via NewMapSchema+RegisterNamedWindow+CreateNamedWindowQuery (infra_named_window_join.go:1239-1267 — closest single template incl. SetColumn rename and SupportBean_A delete where id = a); table via CreateTable+PrimaryKeyColumn/ TableColumnOf[int64]; subselect via SubqueryValueWithOptions[string](FromNamedWindow/FromTable, NamedWindowField/TableField, SubqueryCardinalityMode(SubqueryNullOnMultiple)) — SubqueryFirst default would wrongly return S1 at the 2-row point, NullOnMultiple mandatory; delete via OnEvent(...).DeleteFromNamedWindow/DeleteFromTableWhere(Equal(NamedWindowField/TableField("a"), Field("id"))); hints as WithStatementHints metadata (constants exist; behaviorally neutral — Go index sharing opt-in and uncorrelated no-predicate subselects never index). Runner-local trap: Go listener callback order for SupportBean_A sends must be checked empirically against Java (delete-new before create-old); if reversed, per-statement sequence counters keep sequences stable but the record-array order must match Java (assert-pinned).

- [x] Freeze subq-uncorrel contract (parallel prefetch scouts).
- [x] Author oracle, script, and scenario via asset writer.
- [x] Implement infra-nwtable-subq-uncorrel runner, modes, and tests.
- [x] Generate traces and zero-difference evidence (67 records per side, 0 differences; oracle replay byte-identical, cmp=0).
- [x] Update manifest (614 cases, 227 DV, 807 DV runtime IDs, 3430 associations, referenced 3189, unreferenced 947), roadmap, and CHANGELOG.
- [x] Run gates (package tests, gofmt, vet, git diff --check, facade cmp, full suite 14 pkgs).
- [x] Commit and push the unit (Git owns identity; no hash recorded here).

## Delegation checkpoint

Draft 4.323 unit:
- Prefetch scouts (read-only, concurrent, before implementation): Java contract scout 'JavaContract4323' ('agent_f4aa087d-ed9a-4783-a0ab-a831ec4928c6') pinned all 4 execution timelines, statement deployment order, listener semantics (create old-stream on delete), the intra-send listener ordering, and the hints behavior-neutrality; Go surface scout 'GoSurface4323' ('agent_853eb9c4-160b-4f01-bd0b-7ad92669b73a') adjudicated all items READY with the SubqueryNullOnMultiple trap and the infra_named_window_join.go:1239-1267 template.
- Asset writer (isolated parity-asset writer, concurrent with primary runner work): owns 'tools/java-oracle/InfraNWTableSubqUncorrelScenarioOracle.java', 'tools/java-oracle/run-infra-nwtable-subq-uncorrel.sh', 'testdata/parity/infra-nwtable-subq-uncorrel.json'. The oracle must attach listeners to create/select/selectTwo/delete, emit new AND old arrays (create delivers old rows on delete), pin the Java listener order, and pin record counts (NW 18, table 13).
- Primary owns the Go runner ('internal/app/parity/infra_nwtable_subq_uncorrel.go'), run.go wiring, run_test.go family, traces/evidence, manifest, roadmap, CHANGELOG, PLANS.md, gates, review, commit, and push.
