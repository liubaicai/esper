...
- Current target: Draft 4.360 'dataflow-select-state' — NEW differential chain upgrading case.dataflow-select-state (implemented-not-DV) to DV via EPLDataflowOpSelect's time-based Select executions: EPLDataflowOutputRateLimit (java-runtime-64f78048eb114d0e9cf9, ord 4) and EPLDataflowTimeWindowTriggered (java-runtime-553841d9498fc8d6d1c7, ord 5), exercising SelectTimeWindow/virtual-clock snapshot semantics. Go builder surface exists per the 4.359 scout. Follow-up queue: select-representation wrapper variant, EPStatementSource chain, DV-list consolidation, invalidity dispositions, database slices.
...
## Current work unit
Active: Draft 4.360 ('dataflow-select-state').

- [x] Parallel read-only scouts dispatched. Java contract scout agent_a3a1c834 COMPLETE — 8 records (output-rate-limit 5, time-window-triggered 3): case A output snapshot every 1 minute with cumulative unbounded sum (submissions at t=5000 emit nothing; the 5000→65000 jump fires only the t=60000 snapshot {sumInt:14}; 125000 fires {23}=14+9 cumulative; cancel suppresses later snapshots); case B #time(1 minute) insert-time output ({2}, {7} cumulative) with boundary-inclusive expiry exactly at 65000 ({5}). Virtual-time schedule: absolute jumps 0/5000/65000/125000/245000 (A) and 0/5000/10000/65000 (B); fully synchronous inside advanceTime. Records carry the live clock time so phase boundaries are observable; capture reads via getCurrentAndReset; post-cancel submit asserted empty. Go surface scout agent_c8d79e74 in flight.
- [ ] Freeze contract; oracle/scenario/trace assets (writer) + Go runner + mode + tests (primary).
- [x] Manifest (case.dataflow-select-state DV upgrade: status→DV, dv list +2, file-path evidence, notes), evidence, roadmap, CHANGELOG. (Writer agent_66059ce1 delivered oracle with live-clock record times, scenario (2 cases/11 advance-time steps), script (exit 0, deterministic), Java trace 8 records matching the frozen contract and the Go probe exactly. Evidence passing 0 differences, 2 IDs/2 executions; BOTH trace artifacts committed; summary dvIds 916 (+2), assoc 3544, referenced 3283 & unreferenced 853 unchanged; roadmap count stays 253 + supplement; CHANGELOG entry.)
- [x] Gates: affected packages + make check + git diff --check (green; recorded before commit).
- [x] Independent parity review. Reviewer agent_ce0e6c14-3cee-49ee-bac2-f8d575784c22 OVERALL PASS: engine surface (SelectSnapshotEvery snapshot anchoring at instance Start, boundary-inclusive expireAt, unwindowed cumulative currentEvents), Java contract, traces/evidence record-by-record incl. live-clock times, manifest recomputation, test adequacy (8 mutations), prose, ownership. Two P2s fixed in-unit: prose association count corrected 3546→3544 in all three files (the case pre-carried both javaRuntimeIds — zero new association pairs) and the dataflow.graph capability DV list extended +2 (11→13, matching the roadmap claim and unit convention). P3 hardened in-unit: a live-clock time mutation added to the mutation family (Records[1].Time 00:01:05→00:01:00 catches a static-epoch regression). P3 noted: Go runner uses TheString "E" for all case-0 submissions vs Java E1-E5 — unobservable (theString unprojected).
- [x] Commit and push (single semantic commit with all checkpoint edits folded in).
