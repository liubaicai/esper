...
- Updated: 2026-09-01; Draft 4.292 is the active work unit on the pushed Draft 4.291 baseline.
- Baseline: `HEAD` == `origin/master` at `d816984ce` (`feat(resultset): port rollup grouping FAF`); the worktree is clean before Draft 4.292 implementation.
- Closed predecessor (Draft 4.291): `ResultSetQueryTypeRollupGroupingFuncs.java` ordinal 2 was differentially verified through `rollup-grouping-funcs-faf-dedicated` with one FAF record containing 12 rows and zero differences.

## Current work unit
Draft 4.292 is frozen as fixed `ResultSetQueryTypeRollupHavingAndOrderBy.java` ordinals 2 and 3, `ResultSetQueryTypeIteratorWindow{join=false}` and `{join=true}`. Runtime IDs are `java-runtime-1ce0d3fc4b6fed76d38d` and `java-runtime-7730d794daee6dac92c7`; both use shared/static ID `java-efb06d5c38363ea3a5d0`, inventory flags are empty, and the Java source is read-only. The exact EPL is `@Name('s0') select theString as c0, sum(intPrimitive) as c1 from SupportBean#length(3) [, SupportBean_S0#keepall] group by rollup(theString)`. Java seeds one `SupportBean_S0(1)`, sends `SupportBean` E1/1, E2/2, E1/3, E2/4, and asserts any-order iterator snapshots `{E1,1}/{null,1}`, `{E1,1}/{E2,2}/{null,3}`, `{E1,4}/{E2,2}/{null,6}`, then after FIFO eviction `{E2,6}/{E1,3}/{null,9}`. The join twin receives the same single seed and otherwise identical rows.
Ownership is frozen: primary owns `internal/app/parity/resultset_query_type_rollup_having_iterator.go`, `internal/app/parity/run.go`, `internal/app/parity/run_test.go`, the scenario/traces/evidence, manifest, roadmap, CHANGELOG, and `PLANS.md`; one isolated parity-asset worker owns only `tools/java-oracle/ResultSetQueryTypeRollupHavingIteratorScenarioOracle.java` and `tools/java-oracle/run-resultset-querytype-rollup-having-iterator.sh`. No `internal/esper` semantic change is currently planned. Scenario validation must pin both runtime descriptors, exact four snapshot steps per case, event payloads, case order, and any-mode iterator observation; Go must use typed `LengthWindow(3)`, optional `SupportBean_S0#keepall` equivalent join, `GroupByRollup`, `Sum`, and `Statement.Snapshot`.
Delegation checkpoint: `NextJavaCandidate` (`java-oracle-scout`) and `NextGoSurface-2` (`scout`) completed concurrently. They confirmed the exact Java contract, runtime/static IDs, no existing manifest coverage, and reusable typed rollup/iterator APIs. The contract excludes AggregateGroupedHaving ordinals 2/3 because their observable sum/where behavior is already covered by existing `ResultSetQueryTypeAggregateGrouped` ordinals 5/6; no duplicate runtime evidence will be added. `RollupIteratorOracleAsset` is the isolated asset writer for the Java oracle and launcher; it must not run formatters, tests, builds, commit, or push.

 - Draft 4.292 implementation and replay integration are complete: strict scenario/runner/test wiring and isolated Java oracle assets are present; the pinned Java oracle regenerated the iterator trace byte-identically (SHA-256 `d61eb122df7258704d3906e1b474d48de4672b915fd372ca3e4f71526983c46a`) and Go replay regenerated its trace byte-identically (`7052165e0c46fb7940500853bd0f6d3650d441c0401caa40e706ced5a915afc4`).
 - Draft 4.292 focused iterator replay, differential, checked-in evidence, trace-mutation, malformed-scenario, manifest, and documentation checks pass; checked-in evidence is passing with 8 Java/Go records per side and zero differences. `make check`, the full `go test -race ./... -count=1 -timeout 600s` gate, focused parity/manifest/compat tests, JSON/schema/layout checks, and `git diff --check` pass. Independent reviewer `ReviewDraft4292` re-reviewed after the documentation repair and returned PASS with no actionable P0-P3 findings; the unit is ready for one semantic commit and push.

## Delivery
- Do not commit or push until Java and Go traces, zero-difference evidence, mutation rejection, manifest consistency, full local gates, and independent parity review pass. After the semantic commit, verify the pushed ref read-only and do not edit `PLANS.md` only to add its hash.
