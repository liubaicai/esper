...
## Current work unit
Active: Draft 4.320 (`epl-variable-output-rate`) — EPLVariablesOutputRate, 4 executions, contract frozen from both prefetch scouts; engine work starting, asset writer dispatched.

Frozen contract: preconfigured non-constant variable `var_output_limit` (long, initial 3, host-reset per execution via runtimeSetVariable). Cases: events / events-om / events-compile (java-runtime-0b028b6fe8bddbbb6480 / 0539523182c81174c7f6 / 303a9b5ee74d52e62125; static java-b8a2d4466ca4d002fa69 / java-b08aeecf9371278c24f6 / java-9ee028b9ce2729673e8e) share ONE observable timeline — `@name('s0') select count(*) as cnt from SupportBean output last every var_output_limit events`, 15 sends (E1..E13 SupportBean + 2 SupportMarketDataBean setters via deployed on-set stmt) and 6 outputs {cnt=3},{8},{10},{11},{12},{13} with rate changes 3→5→2→1→null(keeps 1); the om/compile variants differ only in deployment/EPL-round-trip form (no listener difference). Case time-snapshot (java-runtime-4909a80e9612b815310e, static java-b3199a3fee2738477788): `output snapshot every var_output_limit seconds` — outputs {cnt=2}@3000, {cnt=4}@4000, {cnt=4}@5000, {cnt=4}@8000, {cnt=6}@12000 with the anchored-multiple reschedule rule (next = ref + ceil((now-ref)/interval)*interval, ref 0; rate change takes effect at next event-arrival reschedule or trigger, setter beans never reach s0); at 13999 setter sets null, and advanceTime(14000) throws with message `Unexpected exception in statement 's0': Failed to evaluate time period, received a null value for 'Received null value evaluating time period'` while the queued {cnt=8} snapshot is never delivered. Flags [] for all four; milestones are no-ops.

Engine gaps (single writer: primary): (1) expression-capable output-rate policies — CountExpr/IntervalExpr on OutputPolicy + OutputLastEveryEventsExpr/OutputSnapshotEveryExpr constructors, plan.go positivity-validation exemption, outputDescription, facade re-export; (2) per-fire variable rate evaluation with null-keeps-last-rate for events kind; (3) anchored-multiple reschedule arithmetic for time kind (replaces incremental stepping from previous nextOutputAt); (4) null rate at time trigger must abort delivery and propagate an error out of engine.AdvanceTime (Java throws before dispatch). READY (reuse): OnEvent(...).SetVariables(SetVariableExpr(...)) on-set statement (row-limit-variable precedent), snapshot-every new-only delivery shape, compat listener records, runner/test family conventions, mode epl-variable-output-rate/-diff with runner-owned error record (send-error precedent).

- [x] Freeze variable-output-rate contract (parallel prefetch scouts).
- [x] Implement OutputPolicy expression fields, constructors, validation, facade re-export.
- [x] Implement per-fire rate evaluation, anchored reschedule, null semantics, AdvanceTime error plumbing.
- [x] Implement epl-variable-output-rate runner, modes, and tests.
...
Variable-output-rate review: final parity review `VariableOutputRateParityReview` (independent read-only reviewer, agent `agent_4ee2a45d-a587-4b9b-9674-cb484e2ddba3`) returned PASS across contract/engine/runner/evidence/tests/manifest/docs/never-change with zero P1/P2 findings. Accepted P3 observations without action: (1) event-path schedule-error deferral - a null-rate event arrival stores scheduleError surfaced at the next AdvanceTime instead of throwing synchronously out of Send like Java (unreachable in this scenario; latent note for future time-expr units); (2) initial-rate-unknown edge - Go suppresses output until the first non-null rate read while Java's degenerate compile-time rate 0 would fire every update (all four executions pre-set the variable); (3) 4 unchecked advanceOutputSchedule bool returns in last/all-time pending-nil paths are unreachable because plan validation restricts IntervalExpr to every-time policies; (4) doc mutation-count drift (8 vs 10 raw mutations) corrected before commit.
- [x] Commit and push the unit (Git owns identity; no hash recorded here).

## Delegation checkpoint

Draft 4.320 unit:
- Prefetch scouts (read-only, concurrent, before implementation): Java contract scout `JavaContract4320` (`agent_52692018-9e6c-40ba-8167-8796e9731b79`) pinned all four execution timelines, the error-phase message/ordering, static IDs, and scenario encoding; Go surface scout `GoSurface4320` (`agent_cddc035c-42a9-4726-90e1-ba906743ee2e`) adjudicated READY/GAP per item and proposed file ownership. Gap list frozen (see Current work unit).
- Asset writer (isolated parity-asset writer, concurrent with primary engine work): owns `tools/java-oracle/EPLVariablesOutputRateScenarioOracle.java`, `tools/java-oracle/run-epl-variable-output-rate.sh`, `testdata/parity/epl-variable-output-rate.json`. Must install the rethrow exception-handler factory and disabled internal timer in the oracle config, hard-assert expected output counts, and encode the 14000 timer as an advance-time-error step.
- Primary owns all `internal/esper` semantics (stream.go, plan.go, runtime.go, facade regeneration), the Go runner (`internal/app/parity/epl_variables_output_rate.go`), run.go wiring, run_test.go family, traces/evidence, manifest, roadmap, CHANGELOG, PLANS.md, gates, review, commit, and push.
