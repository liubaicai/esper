...
- Current target: Draft 4.363 'subselect-invalid-dispositions' — invalidity-policy completion for the subselect domain: register the compile-only/invalid executions that the flat capture protocol cannot pin (EPLSubselectInvalid, EPLSubselectAggregatedInvalid, and the exists-suite's four SODA/OM compile-variant executions) as properly dispositioned with Go Build-rejection coverage verified; also record that the DV-list consolidation concern is RESOLVED (all 250 DV cases carry per-case differentialVerifiedRuntimeIds; union 922 == summary), closing the quality-strategy line 40 tension. Follow-up queue: dataflow DocSamples/AllTypes, database slices.
...
## Current work unit
Active: Draft 4.363 ('subselect-invalid-dispositions').

- [x] Parallel read-only scouts dispatched. Java contract scout agent_3cb60360 COMPLETE: EPLSubselectIn.EPLSubselectInvalid = 1 compile rejection (intArr array-typed left operand in IN against a scalar subquery — "Collection or array comparison and null-type values are not allowed for the IN, ANY, SOME or ALL keywords"); EPLSubselectAggregatedInvalid = 6 compile rejections (correlated properties inside subselect aggregation ×2, non-aggregated inner property alongside an aggregate, having-clause String-to-numeric sum coercion, bare having property with first-function hint, non-boolean having) — all tryInvalidCompile PREFIX matches, all compile-time; the four exists OM/Compile executions confirmed as compile-path variants whose runtime observables are exactly the text counterparts' (SODA round-trip equality is the extra observable). Runtime IDs pinned: In-Invalid java-runtime-b19d939e1d67b9257320 (ord 15), AggregatedInvalid java-runtime-4a313ed4ff15caa50489 (ord 17), ExistsInSelectOM 734eaed8189b7faedd98, ExistsInSelectCompile 25e3ac3f68aefee11293, NotExistsOM ec3ea2a5c59a7c9dbbe5, NotExistsCompile 9ada75189af485e306c1. Name-collision caution: five suites share the execution name EPLSubselectInvalid — match on runtimeId. Go coverage scout agent_f5808553 in flight.
- [x] Freeze dispositions; manifest registration + Go Build-rejection tests. (Java contract scout agent_3cb60360 pinned 7 compile-time prefix rejections + 4 SODA/OM compile-path variants with runtime IDs; Go coverage scout agent_f5808553 adjudicated: 4 silently-accepted gaps now closed by NEW Build validation (ungroupedSubselectViolations / ungroupedSubselectAggregateTouchesOuterField in validateSubquery), non-boolean having already rejected but untested (now tested), In-array shape inexpressible in the typed API (any-typed hole documented), exists OM/Compile variants compile-path-only (difference disposition registered). Manifest: subselect-in +1 runtime/name with disposition note; aggregated-single-value +1 runtime/name + disposition note + goTest; subselect-exists difference populated + 4 OM/Compile names/IDs registered; summary assoc 3554 / referenced 3289 / unreferenced 847, dvIds 922 unchanged. Roadmap epl 253→247 (the 6 newly-registered runtime IDs were not pre-associated: 4 exists OM/Compile + In-Invalid + AggregatedInvalid).)
