...
- [x] Evidence pipeline and implementation facts corrected: the 100-record Java/Go output replay remains a non-DV implementation artifact because the Go provider does not model JDBC connection acquisition, release, or pool exhaustion. The restart runtime was removed from `differentialVerifiedRuntimeIds`; `testdata/parity/epl-database-restart.evidence.json` is regenerated from the label/count-pinned scenario; manifest summary DV runtime IDs are corrected to 1012; README/roadmap/CHANGELOG retain the non-DV boundary and JDBC lifecycle in remaining.
- [x] Gates + independent parity review. Go formatting, generation/API drift, vet, full tests and race tests passed; focused restart replay, manifest/evidence checks, and argument-safe layout checks passed. Both independent parity reviewers returned PASS; the corrected lifecycle and output-only JDBC boundary are verified.
