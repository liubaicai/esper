...
- Current target: Draft 4.382 'infra-namedwindow-views-lastevent-firstevent' - slice S1 of the InfraNamedWindowViews inventory: ord 0 `InfraKeepAllSimple` (`java-runtime-26c44410c8018a34d696`), ord 29 `InfraLastEvent` (`java-runtime-b85cc831b5c23a570cf7`), ord 30 `InfraLastEventSceneTwo` (`java-runtime-59100403b3c6affd0729`), ord 31 `InfraFirstEvent` (`java-runtime-c2f54d9eb104d061950c`). Follow-ups: remaining 13 InfraNamedWindowViews slices, ExprFilterOptimizableBooleanLimitedExpr leftovers, EPLOtherPlanInKeywordQuery, perf items, multithread/rowrecog/EsperIO domains.

## Current work unit
Active: Draft 4.381 ('infra-namedwindow-views-keepall-delete') - implementation, evidence, facts, gates and independent re-verification complete; commit and push pending.

- Scope: `InfraNamedWindowViews.java` ord 1 + 39-42 (see the frozen contract `local://infra-named-window-keepall-delete-contract.md`). New chain `infra-named-window-keepall-delete` (scenario/trace/Go-trace/evidence under `testdata/parity/`), oracle `tools/java-oracle/InfraNamedWindowKeepAllDeleteScenarioOracle.java`, script `tools/java-oracle/run-infra-named-window-keepall-delete.sh`, Go runner `internal/app/parity/infra_named_window_keepall_delete.go` + `run.go` modes + 6-test family in `run_test.go`.
- Engine change (shared core, primary-owned): `flushNamedWindowConsumerWaveLocked` now drains pending direct named-window dispatches before the consumer wave when no statement output queued by mutation preprocessing is waiting, so the window statement's output precedes the tail-view consumers on the delete/update mutation path as it already did on the insert path (Java `NamedWindowTailViewInstance.update` order) while preserving the routed loop's documented statement-before-direct invariant. Regression test `internal/esper/namedwindow_statement_order_test.go`; runtime.go +12/-0 and the pinned traces/evidence are byte-identical with and without the guard.
- [x] Parallel read-only scouts complete: Java contract (`JavaContractViewsSlice`), Go surface (`GoSurfaceViewsSlice`), N+1 slice inventory (`ViewsFileInventory`, 14 future slices over the remaining 53 executions).
- [x] Contract frozen; asset lane (`ViewsOracleAssets`) delivered oracle + scenario + script + deterministic Java trace (sha256 081cb444...; 144 records, 20/31/31/31/31).
- [x] Go chain, modes and test family; evidence passing with 0 differences (independent python cross-check of Java vs Go traces: 144/144 records identical).
- [x] Manifest (new `case.infra-namedwindow-views-keepall-delete`, mapping, capability DV +5, summary 637/635/260/969/3567/3293/843), CHANGELOG 4.381, roadmap 4.381, README stats refresh, stale delete-channel comment corrected in `infra_namedwindow_views_parity_test.go`.
- [x] Full local gates: `make check` green (layout/vet/tests; parity 180s, esper 58s, compat 0.2s); Java trace re-generated independently by the primary (sha256 081cb444..., byte-identical); Go replay byte-identical on re-run.
- [x] Independent parity review (`ViewsSliceReview`): everything observable reproduced byte-exactly and manifest arithmetic verified; verdict FAIL on one P2 — the unconditional direct drain could precede queued mutation-preprocessing statement output. Fixed in-unit with the reviewer's verified guard (`len(e.pendingStatementDispatches) == 0`), artifacts unchanged; P3s fixed (prose "1/58 previously differential", runtime.go +12/-0, engine test recorded in the manifest goTests/goRefs). Re-verification by the same reviewer returned PASS with no remaining P1/P2 on the guarded drain, corrected prose and manifest facts.
- Follow-up (recorded, not this slice): mixed mutation waves (update-istream + on-delete trigger listener + window statement + consumers) order the trigger's own output differently from Java's main dispatch; needs its own Java oracle scenario before an engine change.
- [ ] Commit and push.
