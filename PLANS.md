...
- Updated: 2026-08-28
- Baseline: `HEAD` == `origin/master` at Draft 4.275 (`77b01e8d1047e019160a67feaf9c44aa318e43d8`), pushed. Draft 4.275 RowForAll passed focused/full gates and independent parity review.
- Closed unit (Draft 4.275, commit `77b01e8d1`): `ResultSetQueryTypeRowForAll.java` ordinals 4-8 were differential-verified with 33 Java/Go records and zero differences; reviewer `ReviewRowForAll` returned PASS.
- Closed unit (Draft 4.274, commit `b54566134`): `ResultSetAggregationMethodSorted.java` execution `ResultSetAggregateSortedMultiCriteria` was differential-verified with 1 Java/Go record and zero differences.

## Current work unit
Draft 4.276: `ResultSetAggregateMaxMinGroupBy.java`, executions `ResultSetAggregateMinMax` and `ResultSetAggregateMinNoGroupHaving` (suite ordinals 0 and 4; static candidate `java-0ca3164935a71ca95033`; runtime IDs `java-runtime-6ee286d6f857ddbbd091` and `java-runtime-cd645170c5defa3996da`). The fixed Java oracle is `/root/app/esper` at commit `9e1b9f1cc9117fea4bf33ab043762c045d73839c`.
The frozen contract is: MinMax uses `irstream symbol,min(all volume),max(all volume),min(distinct volume),max(distinct volume)` over filtered `SupportMarketDataBean#length(3)` grouped by symbol, preserving Long aggregate types, one old/new row per accepted event, global length eviction, duplicate extrema stability, null skipping and all-null Null. The exact DELL/IBM sequence is 50,30,30,90,100 then 20,5,15,18,null,null,null. MinNoGroupHaving uses new-only `select symbol from SupportMarketDataBean#time(5 sec) having volume > min(volume) * 1.3`; DELL volumes 100,105,100 are silent, 131 and 132 emit DELL, and 129 is silent; no time advance or expiry assertion is in oracle scope. No compile/runtime error path is exercised.
The read-only scouts `MinMaxJavaContract` and `MinMaxGoSurface` froze the contract and identified existing typed Min/Max/DistinctAggregate/Multiply APIs plus the missing replayable scenario/assets. Allowed core files are the bounded aggregate/HAVING runtime and a dedicated focused test; allowed asset files are the new Java oracle, shell wrapper and scenario. Manifest, generated trace/evidence, central runner registration/tests, roadmap, CHANGELOG, README, formatting, validation, commit and push remain primary-owned. No CountSum writes are pending.
The typed Go runner is integrated in internal/app/parity/resultset_aggregate_minmax_groupby.go with strict 20-step case/payload validation and runtime-specific fresh environments; run.go dispatch and focused runner/mutation tests are present. The Java oracle/wrapper and scenario assets are integrated, the fixed Java trace and passing differential evidence are generated under testdata/parity, and direct replay/cmp plus focused MinMax and runner tests pass. The Java oracle initially exposed grouped multi-row callbacks; the oracle and wrapper now preserve the authoritative callback batch arrays [1,1,1,1,1,2,2,2,1,1,1,1]. Reviewer ReviewMinMaxParity first found unasserted IBM multi-row focused-test callbacks and missing help metadata; both fixes are applied and the re-review is PASS. make check, go vet, full go test, compatibility tests, layout, and direct Java/Go replay all pass. Central manifest is ready for final documentation consistency verification.

## Delivery
Required gates: focused MinMax/HAVING tests, direct fixed-Java/Go replay for both runtimes, zero-difference evidence and trace mutations, manifest/evidence consistency, independent parity review, `go vet ./...`, `go test ./... -count=1 -timeout 240s`, `go test ./internal/compat/... -count=1`, `make check`, layout, and diff checks. Create one semantic commit and push only after all pass; do not add a checkpoint-only hash update.
## Handoff
