...
oracle_java="$script_root/EPLInsertIntoEventColSubqueryBeanScenarioOracle.java"
oracle_class="$script_root/EPLInsertIntoEventColSubqueryBeanScenarioOracle.class"
...
(cd "$esper_root" && java -cp "$(cat "$classpath_file"):/tmp/oracle-classes" \
    EPLInsertIntoEventColSubqueryBeanScenarioOracle "$scenario" > "$output")
