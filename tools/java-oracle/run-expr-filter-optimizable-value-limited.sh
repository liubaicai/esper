...
usage() {
    cat >&2 <<'EOF'
usage: run-expr-filter-optimizable-value-limited.sh --esper-root PATH --scenario PATH --output PATH [--skip-build]
...
actual_commit=$(git -C "$esper_root" rev-parse HEAD 2>/dev/null) || {
    echo "unable to resolve the Esper checkout commit" >&2
    exit 1
}
if [ "$actual_commit" != "$expected_commit" ]; then
    echo "Esper checkout is at $actual_commit; the pinned oracle commit is $expected_commit" >&2
    exit 1
fi
...
work=$(mktemp -d "${TMPDIR:-/tmp}/esper-expr-filter-optimizable-value-limited.XXXXXX")
cleanup() {
...
"$javac_bin" -encoding UTF-8 -cp "$classpath" -d "$classes" \
    "$script_root/ExprFilterOptimizableValueLimitedScenarioOracle.java"

parent=$(dirname "$output")
mkdir -p "$parent"
"$java_bin" -Dfile.encoding=UTF-8 -Duser.timezone=UTC -Duser.language=en \
    -Duser.country=US -Duser.variant= -cp "$classpath" \
    ExprFilterOptimizableValueLimitedScenarioOracle "$scenario" > "$output"
