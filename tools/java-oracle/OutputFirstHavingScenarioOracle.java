...
/**
 * Direct Esper 9.0.0 oracle for the shared output-first-having parity
 * scenario. Mirrors ResultSetOutputLimitFirstHaving: ungrouped HAVING with
 * output first every N events, and virtual-time aggregate HAVING with output
 * first every interval.
 */
public final class OutputFirstHavingScenarioOracle {
    private static final String VERSION = "esper-parity/v1";

    private OutputFirstHavingScenarioOracle() {
    }

    public static void main(String[] args) throws Exception {
        if (args.length != 1) {
            throw new IllegalArgumentException("usage: OutputFirstHavingScenarioOracle <scenario.json>");
        }
    ...
    private static void runCase(JsonArray allSteps, String caseName, JsonArray records) throws Exception {
        ...
        EPRuntime runtime = EPRuntimeProvider.getRuntime("parity-output-first-having-" + caseName, configuration);
        ((EPRuntimeSPI) runtime).initialize(0L);
        String epl;
        if ("events".equals(caseName)) {
            epl = "@name('s0') select doublePrimitive from SupportBean " +
                    "having doublePrimitive > 1 output first every 2 events";
        } else if ("time".equals(caseName)) {
            epl = "@name('s0') select sum(doublePrimitive) as val0 from SupportBean#length(5) " +
                    "having sum(doublePrimitive) > 100 output first every 2 seconds";
        } else {
        ...
        try {
            ...
            EPDeployment deployment = runtime.getDeploymentService().deploy(compiled,
                    new DeploymentOptions().setDeploymentId("parity-output-first-having-" + caseName));
