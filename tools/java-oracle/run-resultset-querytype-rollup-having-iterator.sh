...
jq -e '.id == "resultset-querytype-rollup-having-iterator" and .javaCommit == "9e1b9f1cc9117fea4bf33ab043762c045d73839c" and (.cases|type=="array") and (.steps|type=="array")' "$scenario" >/dev/null
if [ "$skip_build" -eq 0 ]; then
  "$mvn_bin" -q -f "$esper_root/pom.xml" -pl compiler,runtime -am test-compile -DskipTests=true -Dcheckstyle.skip=true -Dgpg.skip=true -Dfile.encoding=UTF-8 -Duser.timezone=UTC
fi
work=$(mktemp -d "${TMPDIR:-/tmp}/esper-rollup-having-iterator.XXXXXX"); cleanup(){ rm -rf "$work"; }; trap cleanup EXIT HUP INT TERM
...
"$javac_bin" -encoding UTF-8 -cp "$classpath" -d "$classes" "$script_root/ResultSetQueryTypeRollupHavingIteratorScenarioOracle.java"
mkdir -p "$(dirname "$output")"
"$java_bin" -Dfile.encoding=UTF-8 -Duser.timezone=UTC -Duser.language=en -Duser.country=US -cp "$classpath" ResultSetQueryTypeRollupHavingIteratorScenarioOracle "$scenario" > "$output"
jq -e '.version == "esper-parity/v1" and .id == "resultset-querytype-rollup-having-iterator" and .javaCommit == "9e1b9f1cc9117fea4bf33ab043762c045d73839c" and (.records|type=="array")' "$output" >/dev/null
