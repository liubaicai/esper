...
work=$(mktemp -d "${TMPDIR:-/tmp}/esper-epl-database-join.XXXXXX")
cleanup() { rm -rf "$work"; }
...
"$javac_bin" -encoding UTF-8 -cp "$classpath" -d "$classes" \
    "$script_root/EPLDatabaseJoinScenarioOracle.java"

parent=$(dirname "$output")
mkdir -p "$parent"
"$java_bin" -Dfile.encoding=UTF-8 -Duser.timezone=UTC -Duser.language=en \
    -Duser.country=US -Duser.variant= -cp "$classpath" \
    EPLDatabaseJoinScenarioOracle "$scenario" > "$output"

if ! jq -e '.version == "esper-parity/v1" and .id == "epl-database-join" and (.records | type == "array") and (.records | length == 9) and
    ([.records[] | select(.case == "simple-join-left" and .operation == "listener" and .statement == "s0" and (.new | length == 1) and .new[0].kind == "row" and (.new[0].fields | length == 9))] | length) == 1 and
    ([.records[] | select(.case == "property-resolution")][0].new[0].fields.mynumeric == {"state": "null"}) and
    ([.records[] | select(.case == "2historical-star" and .operation == "count" and .statement == "flow")] | length) == 3' "$output" >/dev/null 2>&1; then
