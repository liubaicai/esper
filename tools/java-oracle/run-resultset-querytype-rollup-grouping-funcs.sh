...
work=$(mktemp -d "${TMPDIR:-/tmp}/esper-rollup-grouping-funcs.XXXXXX"); cleanup(){ rm -rf "$work"; }; trap cleanup EXIT HUP INT TERM
...
"$javac_bin" -encoding UTF-8 -cp "$classpath" -d "$classes" "$script_root/ResultSetQueryTypeRollupGroupingFuncsScenarioOracle.java"
mkdir -p "$(dirname "$output")"
"$java_bin" -Dfile.encoding=UTF-8 -Duser.timezone=UTC -Duser.language=en -Duser.country=US -Duser.variant= -cp "$classpath" ResultSetQueryTypeRollupGroupingFuncsScenarioOracle "$scenario" > "$output"
jq -e '.version == "esper-parity/v1" and .id == "rollup-grouping-funcs-dedicated" and (.records | type == "array")' "$output" >/dev/null
