...
work=$(mktemp -d "${TMPDIR:-/tmp}/esper-dataflow-eventbus-sink.XXXXXX")
cleanup() { rm -rf "$work"; }
...
"$javac_bin" -encoding UTF-8 -cp "$classpath" -d "$classes" \
    "$script_root/DataflowEventbusSinkScenarioOracle.java"
...
"$java_bin" -Dfile.encoding=UTF-8 -Duser.timezone=UTC -Duser.language=en \
    ...
    DataflowEventbusSinkScenarioOracle "$scenario" > "$output"

if ! jq -e '.version == "esper-parity/v1" and (.id | length > 0) and (.records | type == "array") and (.records | length == 13)' "$output" >/dev/null 2>&1; then
