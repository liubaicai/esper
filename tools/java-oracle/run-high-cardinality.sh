...
usage() {
    cat >&2 <<'EOF'
usage: run-high-cardinality.sh --esper-root PATH --scenario PATH --output PATH [--skip-build]
...
work=$(mktemp -d "${TMPDIR:-/tmp}/esper-high-cardinality.XXXXXX")
cleanup() {
...
"$javac_bin" -encoding UTF-8 -cp "$classpath" -d "$classes"     "$script_root/HighCardinalityScenarioOracle.java"

parent=$(dirname "$output")
mkdir -p "$parent"
"$java_bin" -Dfile.encoding=UTF-8 -Duser.timezone=UTC -Duser.language=en     -Duser.country=US -Duser.variant= -cp "$classpath"     HighCardinalityScenarioOracle "$scenario" > "$output"
