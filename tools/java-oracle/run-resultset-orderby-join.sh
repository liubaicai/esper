...
usage() {
    cat >&2 <<'EOF'
usage: run-resultset-orderby-join.sh --esper-root PATH --scenario PATH --output PATH [--skip-build]
...
if ! jq -e '
    ...
    (.steps | type == "array" and length == 37)
' "$scenario" >/dev/null 2>&1; then
    echo "scenario is not a valid resultset-orderby-join replay: $scenario" >&2
    exit 1
fi
...
work=$(mktemp -d "${TMPDIR:-/tmp}/esper-resultset-orderby-join.XXXXXX")
cleanup() {
    status=$?
    rm -rf "$work"
    exit $status
...
"$javac_bin" -encoding UTF-8 -cp "$classpath" -d "$(native_path "$classes")" \
    "$script_root/ResultSetOrderByJoinScenarioOracle.java"

mkdir -p "$(dirname "$output")"
"$java_bin" -Dfile.encoding=UTF-8 -Duser.timezone=UTC -Duser.language=en \
    -Duser.country=US -Duser.variant= -cp "$classpath" \
    ResultSetOrderByJoinScenarioOracle "$scenario" > "$output"
...
if ! jq -e '
    ...
    ([.records[].case] == ["iterator", "iterator", "across-join-price", "across-join-symbol-price"]) and
    ([.records[].operation] == ["snapshot", "snapshot", "listener", "listener"]) and
    ([.records[].statement] | all(. == "s0")) and
    ([.records[].sequence] == [0, 0, 1, 1]) and
    ([.records[].time] | all(. == "1970-01-01T00:00:00Z")) and
    ([.records[] | select(.old != null)] | length == 0) and
    ([.records[].new] | all(type == "array")) and
    ([.records[].new | length] == [4, 5, 6, 6]) and
    ([.records[].new[] | .kind] | all(. == "row"))
' "$output" >/dev/null 2>&1; then
    echo "Java oracle produced an invalid resultset-orderby-join trace: $output" >&2
    exit 1
fi
