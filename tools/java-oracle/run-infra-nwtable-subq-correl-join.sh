...
usage() {
    cat >&2 <<'EOF'
usage: run-infra-nwtable-subq-correl-join.sh --esper-root PATH --scenario PATH --output PATH [--skip-build]
...
if ! jq -e '
    ...
    .id == "infra-nwtable-subq-correl-join" and
    .javaCommit == "9e1b9f1cc9117fea4bf33ab043762c045d73839c" and
    .javaSource == "regression-lib/src/main/java/com/espertech/esper/regressionlib/suite/infra/nwtable/InfraNWTableSubqCorrelJoin.java" and
    (.javaRuntimes | type == "array" and length == 3) and
    ([.javaRuntimes[] | startswith("java-runtime-")] | all) and
    (.javaNames | type == "array" and length == 3) and
    ([.javaNames[] | startswith("InfraNWTableSubqCorrelJoinAssertion{")] | all) and
    (.javaStaticIds | type == "array" and length == 3) and
    ([.javaStaticIds[] | . == "java-0ce6cd8b5317076e9a5a"] | all) and
    (.javaFlags | type == "array" and length == 0) and
    (.cases | type == "array" and length == 3) and
    ([.cases[] | select(.observation == "listener" and .iteratorSnapshots == 0)] | length == 3) and
    ([.cases[].case] == ["nw-no-share", "nw-index-share", "table"]) and
    ...
    ([.steps[] | select(.op == "send" and .eventType == "SupportBean_S1")] | length == 9) and
    ...
    (.steps | type == "array" and length == 39)
' "$scenario" >/dev/null 2>&1; then
    echo "scenario is not a valid infra-nwtable-subq-correl-join replay: $scenario" >&2
    exit 1
fi
...
work=$(mktemp -d "${TMPDIR:-/tmp}/esper-infra-nwtable-subq-correl-join.XXXXXX")
cleanup() {
    status=$?
    rm -rf "$work"
    exit $status
...
"$javac_bin" -encoding UTF-8 -cp "$classpath" -d "$(native_path "$classes")" \
    "$script_root/InfraNWTableSubqCorrelJoinScenarioOracle.java"

mkdir -p "$(dirname "$output")"
"$java_bin" -Dfile.encoding=UTF-8 -Duser.timezone=UTC -Duser.language=en \
    -Duser.country=US -Duser.variant= -cp "$classpath" \
    InfraNWTableSubqCorrelJoinScenarioOracle "$scenario" > "$output"

if ! jq -e '
    def correlcase($off; $name):
        ([.records[$off:$off+4][].case] | all(. == $name)) and
        ([.records[$off:$off+4][].operation] | all(. == "listener")) and
        ([.records[$off:$off+4][].statement] | all(. == "s0")) and
        ...
        ([.records[$off:$off+4][] | .new[0].fields.val] == [20, 20, 10, 30]);
    .version == "esper-parity/v1" and
    .id == "infra-nwtable-subq-correl-join" and
    .javaCommit == "9e1b9f1cc9117fea4bf33ab043762c045d73839c" and
    ...
    # nw-no-share: named window without index share; 4 records, one per
    # completed join evaluation.
    correlcase(0; "nw-no-share") and
    # nw-index-share: enable_window_subquery_indexshare create hint; same listener trace.
    correlcase(4; "nw-index-share") and
    # table: create table correlates identically through the primary-key index.
    correlcase(8; "table")
' "$output" >/dev/null 2>&1; then
    echo "Java oracle produced an invalid infra-nwtable-subq-correl-join trace: $output" >&2
    exit 1
fi
