...
usage() {
    cat >&2 <<'EOF'
usage: run-epl-insert-into-transpose-stream.sh --esper-root PATH --scenario PATH --output PATH [--skip-build]
...
oracle_java="$script_root/EPLInsertIntoTransposeStreamScenarioOracle.java"
oracle_class="$script_root/EPLInsertIntoTransposeStreamScenarioOracle.class"
...
(cd "$esper_root" && java -cp "$(cat "$classpath_file"):/tmp/oracle-classes" \
    EPLInsertIntoTransposeStreamScenarioOracle "$scenario" > "$output")
