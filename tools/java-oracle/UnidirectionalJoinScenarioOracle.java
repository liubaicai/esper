...
/**
 * Direct Esper 9.0.0 oracle for the shared unidirectional aggregate-join
 * parity scenario. Mirrors EPLJoin2TableJoinGrouped: a transient
 * SupportMarketDataBean driver joins against retained SupportBean rows and
 * emits irstream grouped aggregate rows on each trigger.
 */
public final class UnidirectionalJoinScenarioOracle {
    private static final String VERSION = "esper-parity/v1";

    private UnidirectionalJoinScenarioOracle() {
    }

    public static void main(String[] args) throws Exception {
        if (args.length != 1) {
            throw new IllegalArgumentException("usage: UnidirectionalJoinScenarioOracle <scenario.json>");
        }
    ...
    private static void runCase(JsonArray allSteps, String caseName, JsonArray records) throws Exception {
        ...
        EPRuntime runtime = EPRuntimeProvider.getRuntime("parity-unidirectional-join-" + caseName, configuration);
        ((EPRuntimeSPI) runtime).initialize(0L);
        String epl = "@name('s0') select irstream symbol, count(*) as cnt " +
        ...
        try {
            ...
            EPDeployment deployment = runtime.getDeploymentService().deploy(compiled,
                    new DeploymentOptions().setDeploymentId("parity-unidirectional-join-" + caseName));
