...
usage() {
    cat >&2 <<'EOF'
usage: run-expr-core-logical.sh --esper-root PATH --scenario PATH --output PATH [--skip-build]
...
if ! jq -e '
    ...
    .id == "expr-core-logical" and
    (.steps | type == "array") and
    ([.steps[] | select(.op == "case") | .case]) ==
        ["and-or-not-combined", "not-with-variable", "and-or-not-null"] and
    ([.steps[] | select(.op == "send")] | length) == 13 and
    ([.steps[] | select(.op == "send" and .eventType == "SupportBean")] | length) == 13 and
    ([.steps[] | select(.op == "set-variable" and .name == "thing" and
        .payload == "5 x 5")] | length) == 1
' "$scenario" >/dev/null 2>&1; then
    echo "scenario is not a valid expr-core-logical replay: $scenario" >&2
    exit 1
fi
...
work=$(mktemp -d "${TMPDIR:-/tmp}/esper-expr-core-logical.XXXXXX")
cleanup() {
...
"$javac_bin" -encoding UTF-8 -cp "$classpath" -d "$classes" \
    "$script_root/ExprCoreLogicalScenarioOracle.java"

parent=$(dirname "$output")
mkdir -p "$parent"
"$java_bin" -Dfile.encoding=UTF-8 -Duser.timezone=UTC -Duser.language=en \
    -Duser.country=US -Duser.variant= -cp "$classpath" \
    ExprCoreLogicalScenarioOracle "$scenario" > "$output"

if ! jq -e '
    .version == "esper-parity/v1" and
    .id == "expr-core-logical" and
    (.records | type == "array") and
    ((.records | length) == 13) and
    ([.records[] | select(.operation == "listener" and .statement == "s0" and (.new | type == "array"))] | length) == 13
