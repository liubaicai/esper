...
/**
 * Direct Esper 9.0.0 oracle for the shared pattern + timer + captured-property
 * parity scenario. Mirrors PatternObserverTimerInterval.
 * PatternIntervalSpecExpressionWithProperty: every captured SupportBean arms a
 * timer:interval(intPrimitive seconds) timer that fires carrying the captured
 * event.
 */
public final class PatternTimerScenarioOracle {
    private static final String VERSION = "esper-parity/v1";

    private PatternTimerScenarioOracle() {
    }

    public static void main(String[] args) throws Exception {
        if (args.length != 1) {
            throw new IllegalArgumentException("usage: PatternTimerScenarioOracle <scenario.json>");
        }
        ...
        String[] cases = {"interval-property"};
    ...
    private static void runCase(JsonArray allSteps, String caseName, JsonArray records) throws Exception {
        ...
        EPRuntime runtime = EPRuntimeProvider.getRuntime("parity-pattern-timer-" + caseName, configuration);
        ((EPRuntimeSPI) runtime).initialize(0L);
        String epl = "@name('s0') select a.theString as id from pattern " +
                "[every a=SupportBean -> timer:interval(intPrimitive seconds)]";
        if (!"interval-property".equals(caseName)) {
        ...
        try {
            ...
            EPDeployment deployment = runtime.getDeploymentService().deploy(compiled,
                    new DeploymentOptions().setDeploymentId("parity-pattern-timer-" + caseName));
