...
usage() {
    cat >&2 <<'EOF'
usage: run-expr-core-instanceof.sh --esper-root PATH --scenario PATH --output PATH [--skip-build]
...
if ! jq -e '
    ...
    ([.steps[] | select(.op == "send")] | length) == 17 and
    ([.steps[] | select(.op == "send" and .eventType == "SupportBean")] | length) == 6 and
    ([.steps[] | select(.op == "send" and .eventType == "SupportBeanDynRoot")] | length) == 11
    ' "$scenario" >/dev/null 2>&1; then
    echo "scenario is not a valid expr-core-instanceof replay: $scenario" >&2
    exit 1
fi
...
work=$(mktemp -d "${TMPDIR:-/tmp}/esper-expr-core-instanceof.XXXXXX")
cleanup() {
...
"$javac_bin" -encoding UTF-8 -cp "$classpath" -d "$classes" \
    "$script_root/ExprCoreInstanceOfScenarioOracle.java"

parent=$(dirname "$output")
mkdir -p "$parent"
"$java_bin" -Dfile.encoding=UTF-8 -Duser.timezone=UTC -Duser.language=en \
    -Duser.country=US -Duser.variant= -cp "$classpath" \
    ExprCoreInstanceOfScenarioOracle "$scenario" > "$output"

if ! jq -e '
    .version == "esper-parity/v1" and
    .id == "expr-core-instanceof" and
    (.records | type == "array") and
    ((.records | length) == 17) and
    ([.records[] | select(.operation == "listener" and .statement == "s0" and (.new | type == "array"))] | length) == 17 and
    ([.records[] | select(.case == "instanceof-simple")] | length) == 2 and
    ([.records[] | select(.case == "instanceof-string-om")] | length) == 2 and
    ([.records[] | select(.case == "instanceof-string-compile")] | length) == 2 and
    ([.records[] | select(.case == "instanceof-dynamic-types")] | length) == 5 and
    ([.records[] | select(.case == "instanceof-dynamic-hierarchy")] | length) == 6
