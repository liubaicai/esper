...
work=$(mktemp -d "${TMPDIR:-/tmp}/esper-dataflow-ports-feedback.XXXXXX")
cleanup() { rm -rf "$work"; }
...
# regression-lib/target/classes carries EPLDataflowInputOutputVariations whose
# nested MyCustomOp/MyFactorialOp classes the session imports.
...
"$javac_bin" -encoding UTF-8 -cp "$classpath" -d "$classes" \
    "$script_root/DataflowPortsFeedbackScenarioOracle.java"

parent=$(dirname "$output")
mkdir -p "$parent"
"$java_bin" -Dfile.encoding=UTF-8 -Duser.timezone=UTC -Duser.language=en \
    -Duser.country=US -Duser.variant= -cp "$classpath" \
    DataflowPortsFeedbackScenarioOracle "$scenario" > "$output"

if ! jq -e '.version == "esper-parity/v1" and (.id | length > 0) and (.records | type == "array") and (.records | length == 6)' "$output" >/dev/null 2>&1; then
