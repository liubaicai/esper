...
usage() {
    cat >&2 <<'EOF'
usage: run-infra-table-into-table.sh --esper-root PATH --scenario PATH --output PATH [--skip-build]
...
actual_commit=$(git -C "$esper_root" rev-parse HEAD 2>/dev/null) || { echo "Could not determine commit of $esper_root" >&2; exit 1; }
if [ "$actual_commit" != "$expected_commit" ]; then
    echo "Esper checkout commit $actual_commit differs from pinned $expected_commit" >&2
    exit 1
fi
...
if [ "$skip_build" -eq 0 ]; then
    for module in common compiler runtime common-avro regression-lib; do
        (cd "$esper_root" && mvn -q -pl "$module" -am package -DskipTests -Dmaven.javadoc.skip=true \
            -Dgpg.skip=true -Dspotless.check.skip=true -Dspotless.apply.skip=true \
            -Dfile.encoding=UTF-8 -Duser.timezone=UTC)
    done
fi

work=$(mktemp -d "${TMPDIR:-/tmp}/esper-infra-table-into-table.XXXXXX")
cleanup() {
    status=$?
    rm -rf "$work"
    exit $status
...
"$javac_bin" -encoding UTF-8 -cp "$classpath" -d "$classes" \
    "$script_root/InfraTableIntoTableScenarioOracle.java"

parent=$(dirname "$output")
mkdir -p "$parent"
"$java_bin" -Dfile.encoding=UTF-8 -Duser.timezone=UTC -Duser.language=en \
    -Duser.country=US -Duser.variant= -cp "$classpath" \
    InfraTableIntoTableScenarioOracle "$scenario" > "$output"

if ! jq -e '.version == "esper-parity/v1" and (.id | length > 0) and (.records | type == "array")' "$output" >/dev/null 2>&1; then
    echo "Oracle trace did not pass validation: $output" >&2
    exit 1
fi
