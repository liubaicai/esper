...
/**
 * Direct Esper 9.0.0 oracle for the shared long-running virtual-clock parity
 * scenario. Mirrors InfraNamedWindowViews.InfraTimeWindow: a time(10 sec)
 * named window receives inserts, a consumer sees irstream rows, a delete
 * trigger removes rows, and virtual-clock boundaries expire old rows.
 */
public final class TimeWindowScenarioOracle {
    private static final String VERSION = "esper-parity/v1";

    private TimeWindowScenarioOracle() {
    }

    public static void main(String[] args) throws Exception {
        if (args.length != 1) {
            throw new IllegalArgumentException("usage: TimeWindowScenarioOracle <scenario.json>");
        }
    ...
    private static void runCase(JsonArray allSteps, String caseName, JsonArray records) throws Exception {
        ...
        EPRuntime runtime = EPRuntimeProvider.getRuntime("parity-time-window-" + caseName, configuration);
        ((EPRuntimeSPI) runtime).initialize(0L);
        String epl = "@name('create') @public create window MyWindowTW#time(10 sec) as (key string, value long);\n" +
                "insert into MyWindowTW select theString as key, longBoxed as value from SupportBean;\n" +
                "@name('s0') select irstream key, value from MyWindowTW;\n" +
                "@name('delete') on SupportMarketDataBean as s0 delete from MyWindowTW as s1 where s0.symbol = s1.key;";
        if (!"time-window".equals(caseName)) {
        ...
        try {
            ...
            EPDeployment deployment = runtime.getDeploymentService().deploy(compiled,
                    new DeploymentOptions().setDeploymentId("parity-time-window-" + caseName));
