...
usage() {
    cat >&2 <<'EOF'
usage: run-infra-nwtable-subquery-delete-aggregate.sh --esper-root PATH --scenario PATH --output PATH [--skip-build]
...
work=$(mktemp -d "${TMPDIR:-/tmp}/esper-infra-nwtable-subquery-delete-aggregate.XXXXXX")
cleanup() {
...
"$javac_bin" -encoding UTF-8 -cp "$classpath" -d "$classes" \
    "$script_root/InfraNWTableSubqueryDeleteAggregateScenarioOracle.java"

parent=$(dirname "$output")
mkdir -p "$parent"
"$java_bin" -Dfile.encoding=UTF-8 -Duser.timezone=UTC -Duser.language=en \
    -Duser.country=US -Duser.variant= -cp "$classpath" \
    InfraNWTableSubqueryDeleteAggregateScenarioOracle "$scenario" > "$output"

if ! jq -e '.version == "esper-parity/v1" and .id == "infra-nwtable-subquery-delete-aggregate" and (.records | type == "array") and ((.records | length) > 0)' "$output" >/dev/null 2>&1; then
