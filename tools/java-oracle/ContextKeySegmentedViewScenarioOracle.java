...
/**
 * Direct Esper 9.0.0 oracle for the context-key-segmented-view parity
 ...
 */
public final class ContextKeySegmentedViewScenarioOracle {
    private static final String VERSION = "esper-parity/v1";

    private ContextKeySegmentedViewScenarioOracle() {
    }

    public static void main(String[] args) throws Exception {
        if (args.length != 1) {
            throw new IllegalArgumentException("usage: ContextKeySegmentedViewScenarioOracle <scenario.json>");
        }
    ...
    private static void runCase(JsonArray allSteps, int caseIndex, String caseName, JsonArray records) throws Exception {
        ...
        EPRuntime runtime = EPRuntimeProvider.getRuntime("parity-context-keyed-view", configuration);
        ...
        try {
            ...
            switch (caseName) {
                case "view-scene-one":
                    epl = "@Name('context') @public create context SegmentedByString as partition by theString from SupportBean;\n" +
                    ...
                    break;
                case "view-scene-two":
                    epl = "@Name('CTX') @public create context SegmentedByString partition by theString from SupportBean;\n" +
                            "@Name('S1') context SegmentedByString select irstream * from SupportBean#lastevent()";
    ...
    private static final class TraceWriter implements UpdateListener {
        ...
        private JsonArray partitions() {
            ContextPartitionCollection collection = runtime.getContextPartitionService().getContextPartitions(
                    statement.getDeploymentId(), "SegmentedByString", ContextPartitionSelectorAll.INSTANCE);
