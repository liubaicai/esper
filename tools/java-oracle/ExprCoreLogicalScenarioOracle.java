...
/**
 * Direct Esper 9.0.0 oracle for the observable logical-expression executions.
 * The Java source's builder, variable, and boxed-expression executions are
 * represented by their shared EPL result contracts in this deterministic
 * replay.
 */
public final class ExprCoreLogicalScenarioOracle {
    private static final String VERSION = "esper-parity/v1";
    private static final String COMBINED_CASE = "and-or-not-combined";
    private static final String VARIABLE_CASE = "not-with-variable";
    private static final String NULL_CASE = "and-or-not-null";

    private static final String COMBINED_EPL =
            "select ((intPrimitive=1) or (intPrimitive=2)) as c0, " +
                    "((intPrimitive>0) and (intPrimitive<3)) as c1, " +
                    "(not(intPrimitive=2)) as c2 from SupportBean";
    private static final String VARIABLE_EPL =
            "create variable string thing = \"Hello World\";" +
                    "@name('s0') select not thing.contains(theString) as c0 from SupportBean";
    private static final String NULL_EPL =
            "select (cast(null, boolean) and cast(null, boolean)) as c0, " +
                    "(boolPrimitive and boolBoxed) as c1, " +
                    "(boolBoxed and boolPrimitive) as c2, " +
                    "(boolPrimitive or boolBoxed) as c3, " +
                    "(boolBoxed or boolPrimitive) as c4, " +
                    "(not boolBoxed) as c5 from SupportBean";

    private ExprCoreLogicalScenarioOracle() {
    }

    public static void main(String[] args) throws Exception {
        String scenarioFile = args.length > 0 ? args[0] : "testdata/parity/expr-core-logical.json";
        ...
        String[] cases = {COMBINED_CASE, VARIABLE_CASE, NULL_CASE};
    ...
    private static void runCase(JsonArray allSteps, String caseName, JsonArray records) throws Exception {
        ...
        String runtimeName = "parity-expr-core-logical-" + caseName;
        String deploymentID = runtimeName;
        ...
        try {
            ...
            EPCompiled compiled = EPCompilerProvider.getCompiler().compile(
                    (VARIABLE_CASE.equals(caseName) ? epl : "@name('s0') " + epl),
            ...
            replayCase(allSteps, caseName, runtime, deploymentID);
    ...
    private static String eplFor(String caseName) {
        if (COMBINED_CASE.equals(caseName)) {
            return COMBINED_EPL;
        }
        if (VARIABLE_CASE.equals(caseName)) {
            return VARIABLE_EPL;
        }
    ...
    private static void replayCase(JsonArray allSteps, String caseName, EPRuntime runtime, String deploymentID) {
        ...
        for (JsonValue value : allSteps) {
            ...
            if ("set-variable".equals(op)) {
                if (!VARIABLE_CASE.equals(caseName)) {
                    throw new IllegalArgumentException("variable update outside variable case");
                }
                runtime.getVariableService().setVariableValue(deploymentID,
                        step.getString("name", ""), step.get("payload").asString());
                continue;
            }
            if ("send".equals(op)) {
    ...
    private static SupportBean toSupportBean(JsonObject payload) {
        ...
        bean.setIntPrimitive(integer(payload, "intPrimitive", 0));
        bean.setBoolPrimitive(booleanValue(payload, "boolPrimitive", false));
        bean.setBoolBoxed(nullableBoolean(payload, "boolBoxed"));
        return bean;
    }
    ...
    private static int integer(JsonObject payload, String name, int defaultValue) {
        JsonValue value = payload.get(name);
        return value == null || value.isNull() ? defaultValue : value.asInt();
    }

    private static boolean booleanValue(JsonObject payload, String name, boolean defaultValue) {
        JsonValue value = payload.get(name);
