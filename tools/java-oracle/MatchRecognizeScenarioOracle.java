...
/**
 * Direct Esper 9.0.0 oracle for the shared match-recognize parity scenario.
 * Mirrors RowRecogOps.RowRecogConcatenation: pattern (A B) with
 * define B as B.value > A.value.
 */
public final class MatchRecognizeScenarioOracle {
    private static final String VERSION = "esper-parity/v1";

    private MatchRecognizeScenarioOracle() {
    }

    public static void main(String[] args) throws Exception {
        if (args.length != 1) {
            throw new IllegalArgumentException("usage: MatchRecognizeScenarioOracle <scenario.json>");
        }
    ...
    private static void runCase(JsonArray allSteps, String caseName, JsonArray records) throws Exception {
        ...
        EPRuntime runtime = EPRuntimeProvider.getRuntime("parity-match-recognize-" + caseName, configuration);
        ((EPRuntimeSPI) runtime).initialize(0L);
        String epl = "@name('s0') select * from SupportRecogBean#keepall " +
                "match_recognize (" +
        ...
        try {
            ...
            EPDeployment deployment = runtime.getDeploymentService().deploy(compiled,
                    new DeploymentOptions().setDeploymentId("parity-match-recognize-" + caseName));
