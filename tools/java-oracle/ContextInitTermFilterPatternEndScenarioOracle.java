...
/**
 * Direct Esper 9.0.0 oracle for the context-init-term-filter-pattern-end
 * parity scenario. Mirrors ContextInitTermFilterAndPattern (overlapping
 * filter-initiated context whose sequence end pattern terminates the
 * matching partition) and ContextInitTermPatternAndAfter1Min (pattern
 * initiated by every S0 -> S1(id=s0.id) with one-minute duration
 * termination and start-tag projections).
 */
public final class ContextInitTermFilterPatternEndScenarioOracle {
    private static final String VERSION = "esper-parity/v1";

    private ContextInitTermFilterPatternEndScenarioOracle() {
    }

    public static void main(String[] args) throws Exception {
        if (args.length != 1) {
            throw new IllegalArgumentException("usage: ContextInitTermFilterPatternEndScenarioOracle <scenario.json>");
        }
        ...
        String[] cases = {"filter-and-pattern", "pattern-and-after-1-min"};
    ...
    private static void runCase(JsonArray allSteps, String caseName, JsonArray records) throws Exception {
        ...
        EPRuntime runtime = EPRuntimeProvider.getRuntime("parity-context-init-term-fpe-" + caseName, configuration);
