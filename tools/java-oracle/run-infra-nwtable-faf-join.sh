...
usage() {
    cat >&2 <<'EOF'
usage: run-infra-nwtable-faf-join.sh --esper-root PATH --scenario PATH --output PATH [--skip-build]
...
if [ "$skip_build" -eq 0 ]; then
    "$mvn_bin" -f "$esper_root/pom.xml" -pl compiler,runtime,common-avro -am test-compile \
        -DskipTests=true -Dcheckstyle.skip=true -Dgpg.skip=true \
        -Dfile.encoding=UTF-8 -Dproject.build.sourceEncoding=UTF-8 \
        -Dproject.reporting.outputEncoding=UTF-8 -Duser.timezone=UTC
fi

work=$(mktemp -d "${TMPDIR:-/tmp}/esper-infra-nwtable-faf-join.XXXXXX")
cleanup() {
...
# The common-avro module copies its avro/jackson runtime jars into lib/
# via dependency:copy; populate it so compile and runtime see them.
"$mvn_bin" -q -f "$esper_root/common-avro/pom.xml" dependency:copy -Dgpg.skip=true
...
classpath="$classpath:$esper_root/common-avro/target/classes:$esper_root/common-avro/lib/*:$esper_root/common-xmlxsd/target/classes"
classpath="$classpath:$(tr '\n' ':' < "$work/compiler-cp.txt"):$(tr '\n' ':' < "$work/runtime-cp.txt"):$(tr '\n' ':' < "$work/avro-cp.txt")"

"$javac_bin" -encoding UTF-8 -cp "$classpath" -d "$classes" \
    "$script_root/InfraNWTableFAFJoinScenarioOracle.java"

parent=$(dirname "$output")
mkdir -p "$parent"
"$java_bin" -Dfile.encoding=UTF-8 -Duser.timezone=UTC -Duser.language=en \
    -Duser.country=US -Duser.variant= -cp "$classpath" \
    InfraNWTableFAFJoinScenarioOracle "$scenario" > "$output"
