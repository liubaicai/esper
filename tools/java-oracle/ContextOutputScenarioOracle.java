...
/**
 * Direct Esper 9.0.0 oracle for the shared context + output-termination parity
 * scenario. Mirrors ContextInitTerm.ContextInitTermOutputSnapshotWhenTerminated:
 * every-minute cron-initiated partitions terminate after one minute and emit
 * an aggregate snapshot when terminated.
 */
public final class ContextOutputScenarioOracle {
    private static final String VERSION = "esper-parity/v1";

    private ContextOutputScenarioOracle() {
    }

    public static void main(String[] args) throws Exception {
        if (args.length != 1) {
            throw new IllegalArgumentException("usage: ContextOutputScenarioOracle <scenario.json>");
        }
    ...
    private static void runCase(JsonArray allSteps, String caseName, JsonArray records) throws Exception {
        ...
        EPRuntime runtime = EPRuntimeProvider.getRuntime("parity-context-output-" + caseName, configuration);
        ((EPRuntimeSPI) runtime).initialize(0L);
        String epl = "@public create context EveryMinute as " +
                "initiated by pattern[every timer:at(*, *, *, *, *)] " +
                "terminated after 1 min;\n" +
                "@name('s0') context EveryMinute " +
                "select sum(intPrimitive) as c1 from SupportBean output snapshot when terminated";
        if (!"termination".equals(caseName)) {
        ...
        try {
            ...
            EPDeployment deployment = runtime.getDeploymentService().deploy(compiled,
                    new DeploymentOptions().setDeploymentId("parity-context-output-" + caseName));
