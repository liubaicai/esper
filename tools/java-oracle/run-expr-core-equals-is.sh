...
usage() {
    cat >&2 <<'EOF'
usage: run-expr-core-equals-is.sh --esper-root PATH --scenario PATH --output PATH [--skip-build]
...
if ! jq -e '
    ...
    ([.steps[] | select(.op == "send" and .eventType == "SupportEventWithManyArray")] | length) == 2 and
    (reduce .steps[] as $step ({active: "", counts: {}, invalid: false};
        ...
        $shape.counts["equals-coercion"] == 2 and
        $shape.counts["equals-same-type"] == 2 and
        $shape.counts["equals-array"] == 2 and
        $shape.counts["equals-null"] == 2
    )
    ' "$scenario" >/dev/null 2>&1; then
    echo "scenario is not a valid expr-core-equals-is replay: $scenario" >&2
    exit 1
fi
...
work=$(mktemp -d "${TMPDIR:-/tmp}/esper-expr-core-equals-is.XXXXXX")
cleanup() {
...
"$javac_bin" -encoding UTF-8 -cp "$classpath" -d "$classes" \
    "$script_root/ExprCoreEqualsIsScenarioOracle.java"

parent=$(dirname "$output")
mkdir -p "$parent"
"$java_bin" -Dfile.encoding=UTF-8 -Duser.timezone=UTC -Duser.language=en \
    -Duser.country=US -Duser.variant= -cp "$classpath" \
    ExprCoreEqualsIsScenarioOracle "$scenario" > "$output"
...
if ! jq -e '
    ...
    ([.records[] | select(.operation == "listener" and .statement == "s0" and (.new | type == "array"))] | length) == 8 and
    ([.records[] | select(.case == "equals-coercion")] | length) == 2 and
    ([.records[] | select(.case == "equals-same-type")] | length) == 2 and
    ([.records[] | select(.case == "equals-array")] | length) == 2 and
    ([.records[] | select(.case == "equals-null")] | length) == 2
