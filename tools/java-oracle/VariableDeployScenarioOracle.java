...
/**
 * Direct Esper 9.0.0 oracle for the shared variable + deployment-order parity
 * scenario. Mirrors EPLVariablesOnSet.EPLVariableOnSetWDeploy: a select
 * references a variable before an on-set trigger is deployed, and later on-set
 * events update the value seen by the select.
 */
public final class VariableDeployScenarioOracle {
    private static final String VERSION = "esper-parity/v1";

    private VariableDeployScenarioOracle() {
    }

    public static void main(String[] args) throws Exception {
        if (args.length != 1) {
            throw new IllegalArgumentException("usage: VariableDeployScenarioOracle <scenario.json>");
        }
    ...
    private static void runCase(JsonArray allSteps, String caseName, JsonArray records) throws Exception {
        ...
        EPRuntime runtime = EPRuntimeProvider.getRuntime("parity-variable-deploy-" + caseName, configuration);
        ((EPRuntimeSPI) runtime).initialize(0L);
        String epl = "create variable int var1RTC = 10;\n" +
                "@name('s0') select var1RTC, theString from SupportBean(theString like 'E%');\n" +
                "@name('set') on SupportBean(theString like 'S%') set var1RTC = intPrimitive;";
        if (!"deploy-variable".equals(caseName)) {
        ...
        try {
            ...
            EPDeployment deployment = runtime.getDeploymentService().deploy(compiled,
                    new DeploymentOptions().setDeploymentId("parity-variable-deploy-" + caseName));
            for (EPStatement statement : deployment.getStatements()) {
                String name = statement.getName();
                if ("s0".equals(name) || "set".equals(name)) {
