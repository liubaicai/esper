...
/**
 * Direct Esper 9.0.0 oracle for the context-key-segmented-join-remove-stream parity
 ...
 */
public final class ContextKeySegmentedJoinRemoveStreamScenarioOracle {
    private static final String VERSION = "esper-parity/v1";

    private ContextKeySegmentedJoinRemoveStreamScenarioOracle() {
    }

    public static void main(String[] args) throws Exception {
        if (args.length != 1) {
            throw new IllegalArgumentException("usage: ContextKeySegmentedJoinRemoveStreamScenarioOracle <scenario.json>");
        }
    ...
    private static void runCase(JsonArray allSteps, int caseIndex, String caseName, JsonArray records) throws Exception {
        ...
        beanType.put("pageName", String.class);
        beanType.put("sessionId", String.class);
        configuration.getCommon().addEventType("SupportWebEvent", beanType);
        ...
        EPRuntime runtime = EPRuntimeProvider.getRuntime("parity-context-keyed-join-remove-stream", configuration);
        ((EPRuntimeSPI) runtime).initialize(0L);
        try {
            String epl;
            String listenerStatement;
            switch (caseName) {
                case "join-remove-stream":
                    epl = "@Name('CTX') @public create context SegmentedBySession partition by sessionId from SupportWebEvent;\n" +
    ...
    private static void send(EPRuntime runtime, JsonObject step) {
        String eventType = step.getString("eventType", "SupportWebEvent");
        JsonObject payload = step.get("payload").asObject();
        Map<String, Object> event = new HashMap<>();
        event.put("pageName", payload.getString("pageName", ""));
        event.put("sessionId", payload.getString("sessionId", ""));
