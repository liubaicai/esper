...
/**
 * Direct Esper 9.0.0 oracle for the context-key-segmented-term-event-select
 ...
 * key.
 */
public final class ContextKeySegmentedTermEventSelectScenarioOracle {
    private static final String VERSION = "esper-parity/v1";

    private ContextKeySegmentedTermEventSelectScenarioOracle() {
    }

    public static void main(String[] args) throws Exception {
        if (args.length != 1) {
            throw new IllegalArgumentException("usage: ContextKeySegmentedTermEventSelectScenarioOracle <scenario.json>");
        }
    ...
    private static void runCase(JsonArray allSteps, int caseIndex, String caseName, JsonArray records) throws Exception {
        ...
        beanType.put("userId", String.class);
        beanType.put("alert", String.class);
        configuration.getCommon().addEventType("UserEvent", beanType);
        EPRuntime runtime = EPRuntimeProvider.getRuntime("parity-context-keyed-term-event-select", configuration);
        ((EPRuntimeSPI) runtime).initialize(0L);
        try {
            String epl = "create context UserSessionContext partition by userId from UserEvent\n" +
    ...
    private static void send(EPRuntime runtime, JsonObject step) {
        ...
        event.put("userId", payload.getString("userId", ""));
        if (payload.get("alert").isNull()) {
            event.put("alert", null);
        } else {
            event.put("alert", payload.getString("alert", ""));
        }
        runtime.getEventService().sendEventMap(event, "UserEvent");
    }
