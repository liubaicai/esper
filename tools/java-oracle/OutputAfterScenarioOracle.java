...
/**
 * Direct Esper 9.0.0 oracle for the shared output-after parity scenario.
 * Mirrors ResultSetOutputLimitAfter.ResultSetAfterWithOutputLast: the output
 * policy activates after four events and then emits the last aggregate row
 * every two events.
 */
public final class OutputAfterScenarioOracle {
    private static final String VERSION = "esper-parity/v1";

    private OutputAfterScenarioOracle() {
    }

    public static void main(String[] args) throws Exception {
        if (args.length != 1) {
            throw new IllegalArgumentException("usage: OutputAfterScenarioOracle <scenario.json>");
        }
    ...
    private static void runCase(JsonArray allSteps, String caseName, JsonArray records) throws Exception {
        ...
        EPRuntime runtime = EPRuntimeProvider.getRuntime("parity-output-after-" + caseName, configuration);
        ((EPRuntimeSPI) runtime).initialize(0L);
        String epl = "@name('s0') select sum(intPrimitive) as thesum from SupportBean#keepall " +
                "output after 4 events last every 2 events";
        if (!"after-last".equals(caseName)) {
        ...
        try {
            ...
            EPDeployment deployment = runtime.getDeploymentService().deploy(compiled,
                    new DeploymentOptions().setDeploymentId("parity-output-after-" + caseName));
