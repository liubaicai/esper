...
usage() {
    cat >&2 <<'EOF'
usage: run-row-for-all.sh --esper-root PATH --scenario PATH --output PATH [--skip-build]
...
work=$(mktemp -d "${TMPDIR:-/tmp}/esper-row-for-all.XXXXXX")
cleanup() {
...
"$javac_bin" -encoding UTF-8 -cp "$classpath" -d "$classes" \
    "$script_root/ResultSetRowForAllScenarioOracle.java"

parent=$(dirname "$output")
mkdir -p "$parent"
"$java_bin" -Dfile.encoding=UTF-8 -Duser.timezone=UTC -Duser.language=en \
    -Duser.country=US -Duser.variant= -cp "$classpath" \
    ResultSetRowForAllScenarioOracle "$scenario" > "$output"
