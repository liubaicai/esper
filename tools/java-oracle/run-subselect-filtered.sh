...
usage() {
    cat >&2 <<'EOF'
usage: run-subselect-filtered.sh --esper-root PATH --scenario PATH --output PATH [--skip-build]
...
work=$(mktemp -d "${TMPDIR:-/tmp}/esper-subselect-filtered.XXXXXX")
cleanup() {
...
"$java_bin" -Dfile.encoding=UTF-8 -Duser.timezone=UTC -Duser.language=en \
    ...
    EPLSubselectFilteredScenarioOracle "$scenario" > "$output"
