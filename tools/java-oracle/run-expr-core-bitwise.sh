...
usage() {
    cat >&2 <<'EOF'
usage: run-expr-core-bitwise.sh --esper-root PATH --scenario PATH --output PATH [--skip-build]
...
if ! jq -e '
    ...
    .id == "expr-core-bitwise" and
    (.steps | type == "array") and
    ([.steps[] | select(.op == "case" and .case == "bitwise-op")] | length) == 1 and
    ([.steps[] | select(.op == "case" and .case == "bitwise-op-om")] | length) == 1 and
    ([.steps[] | select(.op == "send")] | length) == 2
' "$scenario" >/dev/null 2>&1; then
    echo "scenario is not a valid expr-core-bitwise replay: $scenario" >&2
    exit 1
fi
...
work=$(mktemp -d "${TMPDIR:-/tmp}/esper-expr-core-bitwise.XXXXXX")
cleanup() {
...
"$javac_bin" -encoding UTF-8 -cp "$classpath" -d "$classes" \
    "$script_root/ExprCoreBitwiseScenarioOracle.java"

parent=$(dirname "$output")
mkdir -p "$parent"
"$java_bin" -Dfile.encoding=UTF-8 -Duser.timezone=UTC -Duser.language=en \
    -Duser.country=US -Duser.variant= -cp "$classpath" \
    ExprCoreBitwiseScenarioOracle "$scenario" > "$output"
