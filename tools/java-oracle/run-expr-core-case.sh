...
usage() {
    cat >&2 <<'EOF'
usage: run-expr-core-case.sh --esper-root PATH --scenario PATH --output PATH [--skip-build]
...
if ! jq -e '
    ...
    (reduce .steps[] as $step ({active: "", counts: {}, invalid: false};
        ...
        $shape.counts["case-with-else"] == 2 and
        $shape.counts["case-branches3"] == 3 and
        $shape.counts["case-simple-numeric"] == 4
    )
    ' "$scenario" >/dev/null 2>&1; then
    echo "scenario is not a valid expr-core-case replay: $scenario" >&2
    exit 1
fi
...
work=$(mktemp -d "${TMPDIR:-/tmp}/esper-expr-core-case.XXXXXX")
cleanup() {
...
"$javac_bin" -encoding UTF-8 -cp "$classpath" -d "$classes" \
    "$script_root/ExprCoreCaseScenarioOracle.java"

parent=$(dirname "$output")
mkdir -p "$parent"
"$java_bin" -Dfile.encoding=UTF-8 -Duser.timezone=UTC -Duser.language=en \
    -Duser.country=US -Duser.variant= -cp "$classpath" \
    ExprCoreCaseScenarioOracle "$scenario" > "$output"
...
if ! jq -e '
    ...
    ([.records[] | select(.operation == "listener" and .statement == "s0" and (.new | type == "array"))] | length) == 9 and
    ([.records[] | select(.case == "case-with-else")] | length) == 2 and
    ([.records[] | select(.case == "case-branches3")] | length) == 3 and
    ([.records[] | select(.case == "case-simple-numeric")] | length) == 4
