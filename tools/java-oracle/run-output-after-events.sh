...
usage() {
    cat >&2 <<'EOF'
usage: run-output-after-events.sh --esper-root PATH --scenario PATH --output PATH [--skip-build]
...
work=$(mktemp -d "${TMPDIR:-/tmp}/esper-output-after-events.XXXXXX")
cleanup() {
    status=$?
    rm -rf "$work"
    exit $status
...
"$javac_bin" -encoding UTF-8 -cp "$classpath" -d "$(native_path "$classes")" \
    "$script_root/ResultSetOutputLimitAfterEventsScenarioOracle.java"

mkdir -p "$(dirname "$output")"
"$java_bin" -Dfile.encoding=UTF-8 -Duser.timezone=UTC -Duser.language=en \
    -Duser.country=US -Duser.variant= -cp "$classpath" \
    ResultSetOutputLimitAfterEventsScenarioOracle "$scenario" > "$output"
...
if ! jq -e '
    ...
    ([.records[].case] == ["after-3-events", "after-3-events", "after-3-events-when-then", "after-3-events-when-then", "after-3-events-when-then"]) and
    ([.records[].operation] == ["listener", "listener", "listener", "variable", "variable"]) and
    ([.records[] | select(.operation == "listener") | .statement] | all(. == "s0")) and
    ([.records[] | select(.operation == "listener") | .sequence] == [1, 2, 1]) and
    ([.records[] | select(.operation == "listener") | .time] | all(. == "1970-01-01T00:00:00Z")) and
    ([.records[] | select(.old != null)] | length == 0) and
    ([.records[] | select(.operation == "listener") | .new] | map(length) == [1, 1, 1]) and
...
' "$output" >/dev/null 2>&1; then
    echo "Java oracle produced an invalid output-after-events trace: $output" >&2
    exit 1
fi
