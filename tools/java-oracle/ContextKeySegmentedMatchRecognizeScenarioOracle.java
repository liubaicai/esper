...
/**
 * Direct Esper 9.0.0 oracle for the context-key-segmented-match-recognize
 ...
 * key.
 */
public final class ContextKeySegmentedMatchRecognizeScenarioOracle {
    private static final String VERSION = "esper-parity/v1";

    private ContextKeySegmentedMatchRecognizeScenarioOracle() {
    }

    public static void main(String[] args) throws Exception {
        if (args.length != 1) {
            throw new IllegalArgumentException("usage: ContextKeySegmentedMatchRecognizeScenarioOracle <scenario.json>");
        }
    ...
    private static void runCase(JsonArray allSteps, int caseIndex, String caseName, JsonArray records) throws Exception {
        ...
        EPRuntime runtime = EPRuntimeProvider.getRuntime("parity-context-keyed-match-recognize", configuration);
        ((EPRuntimeSPI) runtime).initialize(0L);
        try {
            String epl = "create context SegmentedByString partition by theString from SupportBean;\n" +
                    "@name('s0') context SegmentedByString select * from SupportBean\n" +
                    "match_recognize ( \n" +
