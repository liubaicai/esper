...
usage() {
    cat >&2 <<'EOF'
usage: run-infra-faf-scene-two.sh --esper-root PATH --scenario PATH --output PATH [--skip-build]
...
work=$(mktemp -d "${TMPDIR:-/tmp}/esper-infra-faf-scene-two.XXXXXX")
cleanup() {
...
"$javac_bin" -encoding UTF-8 -cp "$classpath" -d "$classes" \
    "$script_root/InfraFAFSceneTwoScenarioOracle.java"

parent=$(dirname "$output")
mkdir -p "$parent"
"$java_bin" -Dfile.encoding=UTF-8 -Duser.timezone=UTC -Duser.language=en \
    -Duser.country=US -Duser.variant= -cp "$classpath" \
    InfraFAFSceneTwoScenarioOracle "$scenario" > "$output"

if ! jq -e '.version == "esper-parity/v1" and .id == "infra-faf-scene-two" and (.records | type == "array") and ((.records | length) == 4)' "$output" >/dev/null 2>&1; then
