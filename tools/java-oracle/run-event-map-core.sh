...
usage() {
    cat >&2 <<'EOF'
usage: run-event-map-core.sh --esper-root PATH --scenario PATH --output PATH [--skip-build]
...
work=$(mktemp -d "${TMPDIR:-/tmp}/esper-event-map-core.XXXXXX")
cleanup() {
    status=$?
    rm -rf "$work"
    exit $status
...
"$javac_bin" -encoding UTF-8 -cp "$classpath" -d "$classes" \
    "$script_root/EventMapCoreScenarioOracle.java"

parent=$(dirname "$output")
mkdir -p "$parent"
"$java_bin" -Dfile.encoding=UTF-8 -Duser.timezone=UTC -Duser.language=en \
    -Duser.country=US -Duser.variant= -cp "$classpath" \
    EventMapCoreScenarioOracle "$scenario" > "$output"
