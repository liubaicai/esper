...
/**
 * Direct Esper 9.0.0 oracle for grouped-aggregate output-policy iterator
 * parity: "output all every N events", "output first every N events" and
 * "output last every N events".
 */
public final class OutputPolicyScenarioOracle {
    private static final String VERSION = "esper-parity/v1";

    private OutputPolicyScenarioOracle() {
    }

    public static void main(String[] args) throws Exception {
        if (args.length != 1) {
            throw new IllegalArgumentException("usage: OutputPolicyScenarioOracle <scenario.json>");
        }
        ...
        String[] cases = {"all-every-3", "first-every-3", "last-every-3"};
    ...
    private static void runCase(JsonArray allSteps, String caseName, JsonArray records) throws Exception {
        ...
        Map<String, Object> tradeType = new HashMap<>();
        tradeType.put("symbol", String.class);
        tradeType.put("price", Double.class);
        configuration.getCommon().addEventType("Trade", tradeType);

        EPRuntime runtime = EPRuntimeProvider.getRuntime("parity-output-policy-" + caseName, configuration);
        ((EPRuntimeSPI) runtime).initialize(0L);
        String epl = "@name('s0') select symbol, sum(price) as total from Trade#length(2) where price > 10 group by symbol";
        if ("all-every-3".equals(caseName)) {
            epl += " output all every 3 events";
        } else if ("first-every-3".equals(caseName)) {
            epl += " output first every 3 events";
        } else if ("last-every-3".equals(caseName)) {
            epl += " output last every 3 events";
        } else {
        ...
        try {
            ...
            EPDeployment deployment = runtime.getDeploymentService().deploy(compiled,
                    new DeploymentOptions().setDeploymentId("parity-output-policy-" + caseName));
