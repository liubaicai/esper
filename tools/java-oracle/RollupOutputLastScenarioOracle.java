 ...
 * ResultSet7DefaultHavingNoJoin; case "last-output" mirrors
 * ResultSet13LastNoHavingNoJoin; case "last-having" mirrors
 * ResultSet15LastHavingNoJoin; case "first-output" mirrors
 * ResultSet17FirstNoHavingNoJoin; case "snapshot-output" mirrors
 * ResultSet18SnapshotNoHavingNoJoin; case "all-output" mirrors
 * ResultSetNoJoinAll; case "having-output" mirrors
 * ResultSetHaving; case "max-output" mirrors
 * ResultSetMaxTimeWindow; cases "none-join", "none-having-join",
 * "default-join" and "default-having-join" mirror ResultSet2NoneNoHavingJoin,
 * ResultSet4NoneHavingJoin, ResultSet6DefaultNoHavingJoin and
 * ResultSet8DefaultHavingJoin; cases "last-join", "last-having-join" and
 * "first-join" mirror ResultSet14LastNoHavingJoin, ResultSet16LastHavingJoin
 * and ResultSet17FirstNoHavingJoin; case "all-time" mirrors
 * ResultSet9AllNoHavingNoJoin; case "all-join" mirrors
 * ResultSet10AllNoHavingJoin; cases "all-having" and "all-having-join"
 ...
 * ResultSetFirstSimpleHavingAndNoHaving; cases "limit-snapshot" and
 * "limit-snapshot-join" mirror ResultSetLimitSnapshot and
...
public final class RollupOutputLastScenarioOracle {
    ...
    private static void runCase(JsonArray allSteps, String caseName, JsonArray records) throws Exception {
        ...
        if ("snapshot".equals(caseName) || "last-market".equals(caseName) || "first-market".equals(caseName) || "no-limit-market".equals(caseName) || "default-market".equals(caseName) || "default".equals(caseName) || "last-aggregate".equals(caseName) || "no-output".equals(caseName) || "default-output".equals(caseName) || "default-having".equals(caseName) || "last-output".equals(caseName) || "last-having".equals(caseName) || "first-output".equals(caseName) || "snapshot-output".equals(caseName) || "all-output".equals(caseName) || "having-output".equals(caseName) || "max-output".equals(caseName) || "none-join".equals(caseName) || "none-having-join".equals(caseName) || "default-join".equals(caseName) || "default-having-join".equals(caseName) || "last-join".equals(caseName) || "last-having-join".equals(caseName) || "first-join".equals(caseName) || "all-time".equals(caseName) || "all-join".equals(caseName) || "all-having".equals(caseName) || "all-having-join".equals(caseName) || "default-join2".equals(caseName) || "all-join2".equals(caseName) || "last-join2".equals(caseName) || "having-join".equals(caseName) || "join-sort-window".equals(caseName) || "limit-snapshot".equals(caseName) || "limit-snapshot-join".equals(caseName)) {
        ...
        } else if ("last-output".equals(caseName)) {
            epl = "@Name('s0') select irstream symbol, volume, sum(price) " +
                    "from SupportMarketDataBean#time(5.5 sec) " +
                    "group by symbol output last every 1 seconds order by symbol";
        } else if ("last-having".equals(caseName)) {
            epl = "@Name('s0') select irstream symbol, volume, sum(price) " +
                    "from SupportMarketDataBean#time(5.5 sec) " +
                    "group by symbol having sum(price) > 50 output last every 1 seconds";
        } else if ("first-output".equals(caseName)) {
            epl = "@Name('s0') select irstream symbol, volume, sum(price) " +
                    "from SupportMarketDataBean#time(5.5 sec) " +
                    "group by symbol output first every 1 seconds";
        } else if ("snapshot-output".equals(caseName)) {
            epl = "@Name('s0') select symbol, volume, sum(price) " +
                    "from SupportMarketDataBean#time(5.5 sec) " +
                    "group by symbol output snapshot every 1 seconds";
        } else if ("all-output".equals(caseName)) {
            epl = "@Name('s0') select symbol, volume, sum(price) as mySum " +
                    "from SupportMarketDataBean#length(5) " +
                    "where symbol in ('DELL', 'IBM', 'GE') group by symbol " +
                    "output all every 2 events";
        } else if ("having-output".equals(caseName)) {
            epl = "@Name('s0') select irstream symbol, volume, sum(price) as sumprice " +
                    "from SupportMarketDataBean#time(10 sec) " +
                    "group by symbol having sum(price) >= 10 output every 3 events";
        } else if ("max-output".equals(caseName)) {
            epl = "@Name('s0') select irstream symbol, volume, max(price) as maxVol " +
                    "from SupportMarketDataBean#time(1 sec) " +
                    "group by symbol output every 1 seconds";
        } else if ("none-join".equals(caseName)) {
            epl = "@Name('s0') select irstream symbol, volume, sum(price) " +
                    "from SupportMarketDataBean#time(5.5 sec), " +
                    "SupportBean#keepall where theString=symbol group by symbol";
        } else if ("none-having-join".equals(caseName)) {
            epl = "@Name('s0') select irstream symbol, volume, sum(price) " +
                    "from SupportMarketDataBean#time(5.5 sec), " +
                    "SupportBean#keepall where theString=symbol " +
                    "group by symbol having sum(price) > 50";
        } else if ("default-join".equals(caseName)) {
            epl = "@Name('s0') select irstream symbol, volume, sum(price) " +
                    "from SupportMarketDataBean#time(5.5 sec), " +
                    "SupportBean#keepall where theString=symbol " +
                    "group by symbol output every 1 seconds";
        } else if ("default-having-join".equals(caseName)) {
            epl = "@Name('s0') select irstream symbol, volume, sum(price) " +
                    "from SupportMarketDataBean#time(5.5 sec), " +
                    "SupportBean#keepall where theString=symbol " +
                    "group by symbol having sum(price) > 50 output every 1 seconds";
        } else if ("last-join".equals(caseName)) {
            epl = "@Name('s0') select irstream symbol, volume, sum(price) " +
                    "from SupportMarketDataBean#time(5.5 sec), " +
                    "SupportBean#keepall where theString=symbol " +
                    "group by symbol output last every 1 seconds order by symbol";
        } else if ("last-having-join".equals(caseName)) {
            epl = "@Name('s0') select irstream symbol, volume, sum(price) " +
                    "from SupportMarketDataBean#time(5.5 sec), " +
                    "SupportBean#keepall where theString=symbol " +
                    "group by symbol having sum(price) > 50 output last every 1 seconds";
        } else if ("first-join".equals(caseName)) {
            epl = "@Name('s0') select irstream symbol, volume, sum(price) " +
                    "from SupportMarketDataBean#time(5.5 sec), " +
                    "SupportBean#keepall where theString=symbol " +
                    "group by symbol output first every 1 seconds";
        } else if ("all-time".equals(caseName)) {
            epl = "@Name('s0') select irstream symbol, volume, sum(price) " +
                    "from SupportMarketDataBean#time(5.5 sec) " +
                    "group by symbol output all every 1 seconds order by symbol";
        } else if ("all-join".equals(caseName)) {
            epl = "@Name('s0') select irstream symbol, volume, sum(price) " +
                    "from SupportMarketDataBean#time(5.5 sec), " +
                    "SupportBean#keepall where theString=symbol " +
                    "group by symbol output all every 1 seconds order by symbol";
        } else if ("all-having".equals(caseName)) {
            epl = "@Name('s0') select irstream symbol, volume, sum(price) " +
                    "from SupportMarketDataBean#time(5.5 sec) " +
                    "group by symbol having sum(price) > 50 output all every 1 seconds";
        } else if ("all-having-join".equals(caseName)) {
            epl = "@Name('s0') select irstream symbol, volume, sum(price) " +
                    "from SupportMarketDataBean#time(5.5 sec), " +
                    "SupportBean#keepall where theString=symbol " +
                    "group by symbol having sum(price) > 50 output all every 1 seconds";
        } else if ("default-join2".equals(caseName)) {
            epl = "@Name('s0') select symbol, volume, sum(price) as mySum " +
                    "from SupportBeanString#length(100) as one, " +
                    "SupportMarketDataBean#length(5) as two " +
                    "where (symbol='DELL' or symbol='IBM' or symbol='GE') " +
                    "and one.theString = two.symbol group by symbol output every 2 events";
        } else if ("all-join2".equals(caseName)) {
            epl = "@Name('s0') select symbol, volume, sum(price) as mySum " +
                    "from SupportBeanString#length(100) as one, " +
                    "SupportMarketDataBean#length(5) as two " +
                    "where (symbol='DELL' or symbol='IBM' or symbol='GE') " +
                    "and one.theString = two.symbol group by symbol output all every 2 events";
        } else if ("last-join2".equals(caseName)) {
            epl = "@Name('s0') select symbol, volume, sum(price) as mySum " +
                    "from SupportBeanString#length(100) as one, " +
                    "SupportMarketDataBean#length(5) as two " +
                    "where (symbol='DELL' or symbol='IBM' or symbol='GE') " +
                    "and one.theString = two.symbol group by symbol output last every 2 events";
        } else if ("having-join".equals(caseName)) {
            epl = "@Name('s0') select irstream symbol, volume, sum(price) as sumprice " +
        ...
        } else if ("limit-snapshot".equals(caseName)) {
            epl = "@Name('s0') select symbol, volume, sum(price) as sumprice " +
                    "from SupportMarketDataBean#time(10 seconds) group by symbol output snapshot every 1 seconds";
        } else if ("limit-snapshot-join".equals(caseName)) {
            epl = "@Name('s0') select symbol, volume, sum(price) as sumprice " +
                    "from SupportMarketDataBean#time(10 seconds) as m, SupportBean#keepall as s " +
                    "where s.theString = m.symbol group by symbol output snapshot every 1 seconds " +
                    "order by symbol, volume asc";
