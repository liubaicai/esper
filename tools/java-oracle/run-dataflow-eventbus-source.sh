...
work=$(mktemp -d "${TMPDIR:-/tmp}/esper-dataflow-eventbus-source.XXXXXX")
cleanup() { rm -rf "$work"; }
...
"$javac_bin" -encoding UTF-8 -cp "$classpath" -d "$classes" \
    "$script_root/DataflowEventbusSourceScenarioOracle.java"
...
"$java_bin" -Dfile.encoding=UTF-8 -Duser.timezone=UTC -Duser.language=en \
    ...
    DataflowEventbusSourceScenarioOracle "$scenario" > "$output"

if ! jq -e '.version == "esper-parity/v1" and (.id | length > 0) and (.records | type == "array") and (.records | length == 22)' "$output" >/dev/null 2>&1; then
