...
/**
 * Direct Esper 9.0.0 oracle for the shared filter + length-window +
 * grouped-aggregate + output-policy parity scenario.
 *
 ...
 * assertions.
 */
public final class FilterWindowAggregateScenarioOracle {
    private static final String VERSION = "esper-parity/v1";

    private FilterWindowAggregateScenarioOracle() {
    }

    public static void main(String[] args) throws Exception {
        if (args.length != 1) {
            throw new IllegalArgumentException("usage: FilterWindowAggregateScenarioOracle <scenario.json>");
        }
    ...
    private static void runCase(JsonArray allSteps, String caseName, JsonArray records) throws Exception {
        ...
        java.util.Map<String, Object> tradeType = new java.util.HashMap<>();
        tradeType.put("symbol", String.class);
        tradeType.put("price", Double.class);
        tradeType.put("volume", Long.class);
        configuration.getCommon().addEventType("Trade", tradeType);

        EPRuntime runtime = EPRuntimeProvider.getRuntime("parity-fwa-" + caseName, configuration);
        ((EPRuntimeSPI) runtime).initialize(0L);
        String epl;
        if ("plain".equals(caseName)) {
            epl = "@name('s0') select symbol, sum(price) as total from Trade#length(2) where price > 10 group by symbol";
        } else if ("every-3".equals(caseName)) {
            epl = "@name('s0') select symbol, sum(price) as total from Trade#length(2) where price > 10 group by symbol output every 3 events";
        } else {
        ...
        try {
            ...
            EPDeployment deployment = runtime.getDeploymentService().deploy(compiled,
                    new DeploymentOptions().setDeploymentId("parity-fwa-" + caseName));
    ...
    private static void send(EPRuntime runtime, JsonObject step) {
        ...
        if (payload.get("volume") != null) {
            event.put("volume", payload.get("volume").asLong());
        }
        runtime.getEventService().sendEventMap(event, "Trade");
    }
