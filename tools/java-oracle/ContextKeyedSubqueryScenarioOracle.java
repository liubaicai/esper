...
/**
 * Direct Esper 9.0.0 oracle for the shared context-keyed-subquery parity
 * scenario. Mirrors ContextKeySegmentedSubqueryFiltered: a keyed context
 * keeps partition-local #lastevent subquery state that starts empty for a
 * newly created partition.
 */
public final class ContextKeyedSubqueryScenarioOracle {
    private static final String VERSION = "esper-parity/v1";

    private ContextKeyedSubqueryScenarioOracle() {
    }

    public static void main(String[] args) throws Exception {
        if (args.length != 1) {
            throw new IllegalArgumentException("usage: ContextKeyedSubqueryScenarioOracle <scenario.json>");
        }
    ...
    private static void runCase(JsonArray allSteps, String caseName, JsonArray records) throws Exception {
        ...
        EPRuntime runtime = EPRuntimeProvider.getRuntime("parity-context-keyed-subquery-" + caseName, configuration);
        ...
        String epl = "@Name('context') @public create context SegmentedByString " +
                ...
                "(select p00 from SupportBean_S0#lastevent as s0 where sb.intPrimitive = s0.id) as val0 " +
                "from SupportBean as sb";
        if (!"keyed-subquery".equals(caseName)) {
        ...
        try {
            ...
            EPDeployment deployment = runtime.getDeploymentService().deploy(compiled,
                    new DeploymentOptions().setDeploymentId("parity-context-keyed-subquery-" + caseName));
