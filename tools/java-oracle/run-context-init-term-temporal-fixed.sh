...
usage() {
    cat >&2 <<'EOF'
usage: run-context-init-term-temporal-fixed.sh --esper-root PATH --scenario PATH --output PATH [--skip-build]
...
"$javac_bin" -encoding UTF-8 -cp "$classpath" -d "$classes" \
    "$script_root/ContextInitTermTemporalFixedScenarioOracle.java"

parent=$(dirname "$output")
mkdir -p "$parent"
"$java_bin" -Dfile.encoding=UTF-8 -Duser.timezone=UTC -Duser.language=en \
    -Duser.country=US -Duser.variant= -cp "$classpath" \
    ContextInitTermTemporalFixedScenarioOracle "$scenario" > "$output"
