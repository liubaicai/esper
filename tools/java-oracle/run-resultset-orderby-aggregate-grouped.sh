...
usage() {
    cat >&2 <<'EOF'
usage: run-resultset-orderby-aggregate-grouped.sh --esper-root PATH --scenario PATH --output PATH [--skip-build]
...
work=$(mktemp -d "${TMPDIR:-/tmp}/esper-resultset-orderby-aggregate-grouped.XXXXXX")
cleanup() {
    status=$?
    rm -rf "$work"
    exit $status
...
"$java_bin" -Dfile.encoding=UTF-8 -Duser.timezone=UTC -Duser.language=en \
    ...
    ResultSetOrderByAggregateGroupedScenarioOracle "$scenario" > "$output"
