...
usage() {
    cat >&2 <<'EOF'
usage: run-context-hash.sh --esper-root PATH --scenario PATH --output PATH [--skip-build]
...
work=$(mktemp -d "${TMPDIR:-/tmp}/esper-context-hash.XXXXXX")
cleanup() {
...
"$java_bin" -Dfile.encoding=UTF-8 -Duser.timezone=UTC -Duser.language=en \
    ...
    ContextHashScenarioOracle "$scenario" > "$output"
