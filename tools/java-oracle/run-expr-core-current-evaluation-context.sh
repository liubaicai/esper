...
usage() {
    cat >&2 <<'EOF'
usage: run-expr-core-current-evaluation-context.sh --esper-root PATH --scenario PATH --output PATH [--skip-build]
...
if ! jq -e '
    ...
    .id == "expr-core-current-evaluation-context" and
    (.steps | type == "array") and
    ([.steps[] | select(.op == "case" and .case == "current-evaluation-context-epl")] | length) == 1 and
    ([.steps[] | select(.op == "case" and .case == "current-evaluation-context-soda")] | length) == 1 and
    ([.steps[] | select(.op == "advance-time")] | length) == 2 and
    ([.steps[] | select(.op == "send")] | length) == 2
' "$scenario" >/dev/null 2>&1; then
    echo "scenario is not a valid expr-core-current-evaluation-context replay: $scenario" >&2
    exit 1
fi
...
work=$(mktemp -d "${TMPDIR:-/tmp}/esper-expr-core-current-evaluation-context.XXXXXX")
cleanup() {
...
"$javac_bin" -encoding UTF-8 -cp "$classpath" -d "$classes" \
    "$script_root/ExprCoreCurrentEvaluationContextScenarioOracle.java"

parent=$(dirname "$output")
mkdir -p "$parent"
"$java_bin" -Dfile.encoding=UTF-8 -Duser.timezone=UTC -Duser.language=en \
    -Duser.country=US -Duser.variant= -cp "$classpath" \
    ExprCoreCurrentEvaluationContextScenarioOracle "$scenario" > "$output"
...
if ! jq -e '
    ...
    ([.records[] | select(.operation == "listener" and .statement == "s0" and (.new | type == "array"))] | length) == 2
