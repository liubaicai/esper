...
usage() {
    cat >&2 <<'EOF'
usage: run-subselect-aggregated-in-exists-any-all.sh --esper-root PATH --scenario PATH --output PATH [--skip-build]
...
work=$(mktemp -d "${TMPDIR:-/tmp}/esper-resultset-row-per-group-simple.XXXXXX")
cleanup() {
...
"$javac_bin" -encoding UTF-8 -cp "$classpath" -d "$classes" \
    "$script_root/EPLSubselectAggregatedInExistsAnyAllScenarioOracle.java"

parent=$(dirname "$output")
mkdir -p "$parent"
"$java_bin" -Dfile.encoding=UTF-8 -Duser.timezone=UTC -Duser.language=en \
    -Duser.country=US -Duser.variant= -cp "$classpath" \
    EPLSubselectAggregatedInExistsAnyAllScenarioOracle "$scenario" > "$output"
