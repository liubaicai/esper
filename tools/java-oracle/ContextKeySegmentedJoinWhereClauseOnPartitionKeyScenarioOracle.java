...
/**
 * Direct Esper 9.0.0 oracle for the context-key-segmented-join-where-clause-on-partition-key parity
 ...
 */
public final class ContextKeySegmentedJoinWhereClauseOnPartitionKeyScenarioOracle {
    private static final String VERSION = "esper-parity/v1";

    private ContextKeySegmentedJoinWhereClauseOnPartitionKeyScenarioOracle() {
    }

    public static void main(String[] args) throws Exception {
        if (args.length != 1) {
            throw new IllegalArgumentException("usage: ContextKeySegmentedJoinWhereClauseOnPartitionKeyScenarioOracle <scenario.json>");
        }
    ...
    private static void runCase(JsonArray allSteps, int caseIndex, String caseName, JsonArray records) throws Exception {
        ...
        EPRuntime runtime = EPRuntimeProvider.getRuntime("parity-context-keyed-join-where", configuration);
        ((EPRuntimeSPI) runtime).initialize(0L);
        try {
            String epl;
            String listenerStatement;
            switch (caseName) {
                case "join-where-partition-key":
                    epl = "create context MyCtx partition by theString from SupportBean;\n" +
                            "@name('select') context MyCtx select sb.theString, sb.intPrimitive, s0.id, s0.p00 " +
                            "from SupportBean#lastevent as sb, SupportBean_S0#lastevent as s0 " +
                            "where theString is 'Test'";
                    listenerStatement = "select";
                    break;
                default:
