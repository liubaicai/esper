...
/**
 * Direct Esper 9.0.0 oracle for the observable current-timestamp executions.
 * The SODA and compile executions share their resulting t0 projection with
 * the EPL execution; the trace records the common runtime contract.
 */
public final class ExprCoreCurrentTimestampScenarioOracle {
    private static final String VERSION = "esper-parity/v1";
    private static final String GET_EPL = "select current_timestamp(), " +
            "current_timestamp as t0, " +
            "current_timestamp() as t1, " +
            "current_timestamp + 1 as t2 from SupportBean";
    private static final String SIMPLE_EPL = "select current_timestamp() as t0 from SupportBean";

    private ExprCoreCurrentTimestampScenarioOracle() {
    }

    public static void main(String[] args) throws Exception {
        String scenarioFile = args.length > 0 ? args[0] : "testdata/parity/expr-core-current-timestamp.json";
        ...
        if (hasCase(steps, "current-timestamp-get")) {
            runCase(steps, "current-timestamp-get", records);
        }
        if (hasCase(steps, "current-timestamp-om")) {
            runCase(steps, "current-timestamp-om", records);
        }
        if (hasCase(steps, "current-timestamp-compile")) {
            runCase(steps, "current-timestamp-compile", records);
        }
    ...
    private static void runCase(JsonArray allSteps, String caseName, JsonArray records) throws Exception {
        ...
        EPRuntime runtime = EPRuntimeProvider.getRuntime("parity-expr-core-current-timestamp-" + caseName, configuration);
        try {
            ((EPRuntimeSPI) runtime).initialize(0L);
            String epl = "current-timestamp-get".equals(caseName) ? GET_EPL : SIMPLE_EPL;
