...
usage() {
    cat >&2 <<'EOF'
usage: run-epl-as-keyword-backtick.sh --esper-root PATH --scenario PATH --output PATH [--skip-build]
...
work=$(mktemp -d "${TMPDIR:-/tmp}/esper-as-keyword.XXXXXX")
cleanup() {
...
"$javac_bin" -encoding UTF-8 -cp "$classpath" -d "$classes" \
    "$script_root/EPLAsKeywordBacktickScenarioOracle.java"
