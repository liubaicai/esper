...
usage() {
    cat >&2 <<'EOF'
usage: run-rollup-dimensionality.sh --esper-root PATH --scenario PATH --output PATH [--skip-build]
...
work=$(mktemp -d "${TMPDIR:-/tmp}/esper-rollup-dimensionality.XXXXXX")
cleanup() {
...
"$javac_bin" -encoding UTF-8 -cp "$classpath" -d "$classes" \
    "$script_root/EPLResultSetQueryTypeRollupDimensionalityScenarioOracle.java"
