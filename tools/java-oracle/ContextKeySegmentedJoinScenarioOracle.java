...
/**
 * Direct Esper 9.0.0 oracle for the context-key-segmented-join parity
 ...
 */
public final class ContextKeySegmentedJoinScenarioOracle {
    private static final String VERSION = "esper-parity/v1";

    private ContextKeySegmentedJoinScenarioOracle() {
    }

    public static void main(String[] args) throws Exception {
        if (args.length != 1) {
            throw new IllegalArgumentException("usage: ContextKeySegmentedJoinScenarioOracle <scenario.json>");
        }
    ...
    private static void runCase(JsonArray allSteps, int caseIndex, String caseName, JsonArray records) throws Exception {
        ...
        EPRuntime runtime = EPRuntimeProvider.getRuntime("parity-context-keyed-join", configuration);
        ((EPRuntimeSPI) runtime).initialize(0L);
        try {
            String epl;
            String listenerStatement;
            switch (caseName) {
                case "join-per-partition":
                    epl = "@Name('context') @public create context SegmentedByString partition by theString from SupportBean;\n" +
                            "@Name('s0') context SegmentedByString " +
                            "select sb.theString, sb.intPrimitive, s0.id " +
                            "from SupportBean#keepall as sb, SupportBean_S0#keepall as s0 " +
                            "where sb.intPrimitive = s0.id";
