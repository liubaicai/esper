...
work=$(mktemp -d "${TMPDIR:-/tmp}/esper-dataflow-lifecycle-blocking.XXXXXX")
cleanup() { rm -rf "$work"; }
...
"$javac_bin" -encoding UTF-8 -cp "$classpath" -d "$classes" \
    "$script_root/DataflowLifecycleBlockingScenarioOracle.java"

parent=$(dirname "$output")
mkdir -p "$parent"
"$java_bin" -Dfile.encoding=UTF-8 -Duser.timezone=UTC -Duser.language=en \
    -Duser.country=US -Duser.variant= -cp "$classpath" \
    DataflowLifecycleBlockingScenarioOracle "$scenario" > "$output"

if ! jq -e '.version == "esper-parity/v1" and (.id | length > 0) and (.records | type == "array") and (.records | length == 10)' "$output" >/dev/null 2>&1; then
