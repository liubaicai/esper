...
/**
 * Direct Esper 9.0.0 oracle for the context-key-segmented-pattern-filter parity
 ...
 */
public final class ContextKeySegmentedPatternFilterScenarioOracle {
    private static final String VERSION = "esper-parity/v1";

    /** Stand-in for the regression's static stringContainsX UDF. */
    public static final class StringFunctions {
        private StringFunctions() {
        }

        public static boolean stringContainsX(String value) {
            return value != null && value.contains("X");
        }
    }

    private ContextKeySegmentedPatternFilterScenarioOracle() {
    }

    public static void main(String[] args) throws Exception {
        if (args.length != 1) {
            throw new IllegalArgumentException("usage: ContextKeySegmentedPatternFilterScenarioOracle <scenario.json>");
        }
    ...
    private static void runCase(JsonArray allSteps, int caseIndex, String caseName, JsonArray records) throws Exception {
        ...
        EPRuntime runtime = EPRuntimeProvider.getRuntime("parity-context-keyed-pattern-filter", configuration);
        ...
        try {
            ...
            switch (caseName) {
                case "pattern-filter":
                    epl = "@Name('context') @public create context IndividualBean partition by theString from SupportBean;\n" +
                            "@Name('s0') context IndividualBean " +
                            "select event1.theString as a_theString, event2.theString as b_theString from pattern [" +
                            "every (event1=SupportBean(theString not like '%X%') -> event2=SupportBean(theString like '%X%'))]";
