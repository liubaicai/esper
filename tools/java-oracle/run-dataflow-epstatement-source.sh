...
work=$(mktemp -d "${TMPDIR:-/tmp}/esper-dataflow-epstatement-source.XXXXXX")
cleanup() { rm -rf "$work"; }
...
"$mvn_bin" -q -f "$esper_root/runtime/pom.xml" dependency:build-classpath \
    ...
    -Dgpg.skip=true -Dfile.encoding=UTF-8 -Duser.timezone=UTC
# common-xmlxsd carries the XSD handler used by the MyXMLEvent representation;
# its runtime classpath supplies the Xerces XS-model dependency.
"$mvn_bin" -q -f "$esper_root/common-xmlxsd/pom.xml" dependency:build-classpath \
    -Dmdep.outputFile="$work/xmlxsd-cp.txt" -Dmdep.includeScope=runtime \
    -Dgpg.skip=true -Dfile.encoding=UTF-8 -Duser.timezone=UTC
...
# regression-run/etc carries the regression/threeProperties.xsd resource that
# DefaultSupportGraphEventUtil loads for the MyXMLEvent representation; the
# regression-run surefire config adds the same directory as an additional
# classpath element.
classpath="$classpath:$esper_root/regression-run/etc"
classpath="$classpath:$(tr '\n' ':' < "$work/compiler-cp.txt"):$(tr '\n' ':' < "$work/runtime-cp.txt")"
classpath="$classpath:$(tr '\n' ':' < "$work/xmlxsd-cp.txt")"

"$javac_bin" -encoding UTF-8 -cp "$classpath" -d "$classes" \
    "$script_root/DataflowEPStatementSourceScenarioOracle.java"

parent=$(dirname "$output")
mkdir -p "$parent"
# regression-run/etc carries a console log4j.xml that would otherwise be
# auto-detected and pollute the JSON trace on stdout; override it with a
# silent configuration.
cat > "$work/log4j-off.properties" <<'EOF'
log4j.rootLogger=OFF
EOF
"$java_bin" -Dfile.encoding=UTF-8 -Duser.timezone=UTC -Duser.language=en \
    -Duser.country=US -Duser.variant= \
    -Dlog4j.configuration="file:$work/log4j-off.properties" -cp "$classpath" \
    DataflowEPStatementSourceScenarioOracle "$scenario" > "$output"

if ! jq -e '.version == "esper-parity/v1" and (.id | length > 0) and (.records | type == "array") and (.records | length == 17)' "$output" >/dev/null 2>&1; then
