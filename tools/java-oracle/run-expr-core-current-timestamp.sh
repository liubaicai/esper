...
usage() {
    cat >&2 <<'EOF'
usage: run-expr-core-current-timestamp.sh --esper-root PATH --scenario PATH --output PATH [--skip-build]
...
if ! jq -e '
    ...
    .id == "expr-core-current-timestamp" and
    (.steps | type == "array") and
    ([.steps[] | select(.op == "case" and .case == "current-timestamp-get")] | length) == 1 and
    ([.steps[] | select(.op == "case" and .case == "current-timestamp-om")] | length) == 1 and
    ([.steps[] | select(.op == "case" and .case == "current-timestamp-compile")] | length) == 1 and
    ([.steps[] | select(.op == "advance-time")] | length) == 4 and
    ([.steps[] | select(.op == "send")] | length) == 4
' "$scenario" >/dev/null 2>&1; then
    echo "scenario is not a valid expr-core-current-timestamp replay: $scenario" >&2
    exit 1
fi
...
work=$(mktemp -d "${TMPDIR:-/tmp}/esper-expr-core-current-timestamp.XXXXXX")
cleanup() {
...
"$javac_bin" -encoding UTF-8 -cp "$classpath" -d "$classes" \
    "$script_root/ExprCoreCurrentTimestampScenarioOracle.java"

parent=$(dirname "$output")
mkdir -p "$parent"
"$java_bin" -Dfile.encoding=UTF-8 -Duser.timezone=UTC -Duser.language=en \
    -Duser.country=US -Duser.variant= -cp "$classpath" \
    ExprCoreCurrentTimestampScenarioOracle "$scenario" > "$output"
...
if ! jq -e '
    ...
    ([.records[] | select(.operation == "listener" and .statement == "s0" and (.new | type == "array"))] | length) == 4
