...
/**
 * Direct Esper 9.0.0 oracle for the shared named-window mutation parity
 * scenario. Mirrors InfraNamedWindowOnDelete.InfraNamedWindowSilentDeleteOnDelete:
 * a firstunique named window receives inserts and deletes are driven by
 * SupportBean_S0; create/count listeners are traced together.
 */
public final class NamedWindowMutationScenarioOracle {
    private static final String VERSION = "esper-parity/v1";

    private NamedWindowMutationScenarioOracle() {
    }

    public static void main(String[] args) throws Exception {
        if (args.length != 1) {
            throw new IllegalArgumentException("usage: NamedWindowMutationScenarioOracle <scenario.json>");
        }
    ...
    private static void runCase(JsonArray allSteps, String caseName, JsonArray records) throws Exception {
        ...
        EPRuntime runtime = EPRuntimeProvider.getRuntime("parity-named-window-" + caseName, configuration);
        ((EPRuntimeSPI) runtime).initialize(0L);
        String epl = "@name('create') create window MyWindow#firstunique(theString) as SupportBean;\n" +
                "insert into MyWindow select * from SupportBean;\n" +
                "on SupportBean_S0 s0 delete from MyWindow where p00 = theString;\n" +
        ...
        try {
            ...
            EPDeployment deployment = runtime.getDeploymentService().deploy(compiled,
                    new DeploymentOptions().setDeploymentId("parity-named-window-" + caseName));
            for (EPStatement statement : deployment.getStatements()) {
                String name = statement.getName();
                if ("create".equals(name) || "count".equals(name)) {
