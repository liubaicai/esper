...
usage() {
    cat >&2 <<'EOF'
usage: run-epl-other-wildcard-additional.sh --esper-root PATH --scenario PATH --output PATH [--skip-build]
...
actual_commit=$(git -C "$esper_root" rev-parse HEAD 2>/dev/null) || {
    echo "unable to resolve the commit of $esper_root" >&2
    exit 1
}
if [ "$actual_commit" != "$expected_commit" ]; then
    echo "Esper root must be at the pinned oracle commit $expected_commit: found $actual_commit" >&2
    exit 1
fi
...
work=$(mktemp -d "${TMPDIR:-/tmp}/esper-wildcard-additional.XXXXXX")
cleanup() {
...
"$javac_bin" -encoding UTF-8 -cp "$classpath" -d "$classes" \
    "$script_root/EPLOtherSelectWildcardWAdditionalScenarioOracle.java"

parent=$(dirname "$output")
mkdir -p "$parent"
"$java_bin" -Dfile.encoding=UTF-8 -Duser.timezone=UTC -Duser.language=en \
    -Duser.country=US -Duser.variant= -cp "$classpath" \
    EPLOtherSelectWildcardWAdditionalScenarioOracle "$scenario" > "$output"
