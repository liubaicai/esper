...
set -eu
usage() { cat >&2 <<'EOF'
usage: run-resultset-aggregate-firstlastwindow-indexed.sh --esper-root PATH --scenario PATH --output PATH [--skip-build]
EOF
exit 2
}
...
if ! jq -e '
    .version == "esper-parity/v1" and .id == "resultset-aggregate-firstlastwindow-indexed" and
    (.steps | type == "array") and (.steps | length == 5) and
    (.steps[0].op == "case" and .steps[0].case == "indexed") and
    ([.steps[1:][] | .op] == ["send", "send", "send", "send"]) and
    ([.steps[] | select(.op == "send")] | length == 4) and
    ([.steps[] | select(.op == "send" and .eventType == "SupportBean" and
      (.payload | objects | (keys == ["intPrimitive"] and (.intPrimitive | type) == "number" and
      (.intPrimitive == (.intPrimitive | floor))))) ] | length == 4) and
    ([.steps[1:][] | .payload.intPrimitive] == [10, 11, 12, 13])
' "$scenario" >/dev/null 2>&1; then
    echo "scenario is not a valid indexed replay: $scenario" >&2
    exit 1
fi
...
work=$(mktemp -d "${TMPDIR:-/tmp}/esper-resultset-aggregate-firstlastwindow-indexed.XXXXXX")
...
"$javac_bin" -encoding UTF-8 -cp "$classpath" -d "$classes" "$script_root/ResultSetAggregateFirstLastWindowIndexedScenarioOracle.java"
mkdir -p "$(dirname "$output")"
"$java_bin" -Dfile.encoding=UTF-8 -Duser.timezone=UTC -Duser.language=en -Duser.country=US -Duser.variant= -cp "$classpath" ResultSetAggregateFirstLastWindowIndexedScenarioOracle "$scenario" > "$output"
jq -e '.version == "esper-parity/v1" and .id == "resultset-aggregate-firstlastwindow-indexed" and (.records | type == "array") and ((.records | length) == 4)' "$output" >/dev/null
