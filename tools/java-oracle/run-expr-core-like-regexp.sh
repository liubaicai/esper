...
usage() {
    cat >&2 <<'EOF'
usage: run-expr-core-like-regexp.sh --esper-root PATH --scenario PATH --output PATH [--skip-build]
...
if ! jq -e '
    ...
    .id == "expr-core-like-regexp" and
    (.steps | type == "array") and
    ([.steps[] | select(.op == "case") | .case] == [
        "like-constants", "like-expressions", "regexp-constants", "regexp-expressions"
    ]) and
    ([.steps[] | select(.op == "send")] | length) == 19 and
    ([.steps[] | select(.op == "send" and .eventType == "SupportBean")] | length) == 8 and
    ([.steps[] | select(.op == "send" and .eventType == "SupportBean_S0")] | length) == 11 and
    (reduce .steps[] as $step ({active: "", counts: {}, invalid: false};
        ...
        $shape.counts["like-constants"] == 4 and
        $shape.counts["like-expressions"] == 5 and
        $shape.counts["regexp-constants"] == 4 and
        $shape.counts["regexp-expressions"] == 6)
' "$scenario" >/dev/null 2>&1; then
    echo "scenario is not a valid expr-core-like-regexp replay: $scenario" >&2
    exit 1
fi
...
work=$(mktemp -d "${TMPDIR:-/tmp}/esper-expr-core-like-regexp.XXXXXX")
cleanup() {
...
"$javac_bin" -encoding UTF-8 -cp "$classpath" -d "$classes" \
    "$script_root/ExprCoreLikeRegexpScenarioOracle.java"

parent=$(dirname "$output")
mkdir -p "$parent"
"$java_bin" -Dfile.encoding=UTF-8 -Duser.timezone=UTC -Duser.language=en \
    -Duser.country=US -Duser.variant= -cp "$classpath" \
    ExprCoreLikeRegexpScenarioOracle "$scenario" > "$output"

if ! jq -e '
    .version == "esper-parity/v1" and
    .id == "expr-core-like-regexp" and
    (.records | type == "array") and
    ((.records | length) == 19) and
    ([.records[] | select(.operation == "listener" and .statement == "s0" and (.new | type == "array"))] | length) == 19
