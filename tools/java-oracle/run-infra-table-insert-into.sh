...
usage() {
    cat >&2 <<'EOF'
usage: run-infra-table-insert-into.sh --esper-root PATH --scenario PATH --output PATH [--skip-build]
...
actual_commit=$(git -C "$esper_root" rev-parse HEAD 2>/dev/null) || {
    echo "could not read the Esper checkout commit" >&2
    exit 1
}
...
work=$(mktemp -d "${TMPDIR:-/tmp}/esper-infra-table-insert-into.XXXXXX")
cleanup() {
...
"$javac_bin" -encoding UTF-8 -cp "$classpath" -d "$classes" \
    "$script_root/InfraTableInsertIntoScenarioOracle.java"

parent=$(dirname "$output")
mkdir -p "$parent"
"$java_bin" -Dfile.encoding=UTF-8 -Duser.timezone=UTC -Duser.language=en \
    -Duser.country=US -Duser.variant= -cp "$classpath" \
    InfraTableInsertIntoScenarioOracle "$scenario" > "$output"
