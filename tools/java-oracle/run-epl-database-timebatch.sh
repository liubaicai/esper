...
work=$(mktemp -d "${TMPDIR:-/tmp}/esper-epl-database-timebatch.XXXXXX")
cleanup() { rm -rf "$work"; }
...
"$javac_bin" -encoding UTF-8 -cp "$classpath" -d "$classes" \
    "$script_root/EPLDatabaseTimeBatchScenarioOracle.java"

parent=$(dirname "$output")
mkdir -p "$parent"
"$java_bin" -Dfile.encoding=UTF-8 -Duser.timezone=UTC -Duser.language=en \
    -Duser.country=US -Duser.variant= -cp "$classpath" \
    EPLDatabaseTimeBatchScenarioOracle "$scenario" > "$output"

if ! jq -e '.version == "esper-parity/v1" and .id == "epl-database-timebatch" and (.records | type == "array") and (.records | length == 7) and
    ([.records[] | .case] | unique) == ["timebatch"] and
    ([.records[] | .time] | unique) == ["1970-01-01T00:00:00Z"] and
    ([.records[] | .sequence]) == [1, 2, 3, 4, 5, 6, 7] and
    ([.records[] | select(.operation == "count" and .statement == "flow" and .name == "iterator-rows")] | length) == 6 and
    ([.records[] | select(.operation == "count") | .count]) == [1, 2, 3, 0, 1, 2] and
    ([.records[] | select(.operation == "listener" and .statement == "s0")] | length) == 1 and
    (([.records[] | select(.operation == "listener")][0].new) | length) == 3 and
    (([.records[] | select(.operation == "listener")][0].new | map(.kind)) == ["row", "row", "row"]) and
    (([.records[] | select(.operation == "listener")][0].new | map(.fields.mybigint)) == [10, 5, 2]) and
    (([.records[] | select(.operation == "listener")][0].new | map(.fields.myint)) == [100, 50, 20]) and
    (([.records[] | select(.operation == "listener")][0].new | map(.fields.mydouble)) == [10.2, 5.2, 2.2]) and
    (([.records[] | select(.operation == "listener")][0].new | map(.fields.myreal)) == [10.3, 5.3, 2.3]) and
    ([.records[] | select(.operation == "listener")][0].new[0].fields.myvarchar == "J") and
    ([.records[] | select(.operation == "listener")][0].new[0].fields.mychar == "P") and
    ([.records[] | select(.operation == "listener")][0].new[0].fields.mybool == true) and
    ([.records[] | select(.operation == "listener")][0].new[0].fields.mynumeric == {"state": "null"}) and
    ([.records[] | select(.operation == "listener")][0].new[0].fields.mydecimal == "1000") and
    ([.records[] | select(.operation == "listener")][0].new[1].fields.myvarchar == "E") and
    ([.records[] | select(.operation == "listener")][0].new[1].fields.mychar == "V") and
    ([.records[] | select(.operation == "listener")][0].new[1].fields.mybool == false) and
    ([.records[] | select(.operation == "listener")][0].new[1].fields.mynumeric == "500") and
    ([.records[] | select(.operation == "listener")][0].new[1].fields.mydecimal == "500") and
    ([.records[] | select(.operation == "listener")][0].new[2].fields.myvarchar == "B") and
    ([.records[] | select(.operation == "listener")][0].new[2].fields.mychar == "Y") and
    ([.records[] | select(.operation == "listener")][0].new[2].fields.mybool == false) and
    ([.records[] | select(.operation == "listener")][0].new[2].fields.mynumeric == "100") and
    ([.records[] | select(.operation == "listener")][0].new[2].fields.mydecimal == "200")' "$output" >/dev/null 2>&1; then
