...
/**
 * Direct Esper 9.0.0 oracle for the context-key-segmented-null-keys
 ...
 * key.
 */
public final class ContextKeySegmentedNullKeysScenarioOracle {
    private static final String VERSION = "esper-parity/v1";

    private ContextKeySegmentedNullKeysScenarioOracle() {
    }

    public static void main(String[] args) throws Exception {
        if (args.length != 1) {
            throw new IllegalArgumentException("usage: ContextKeySegmentedNullKeysScenarioOracle <scenario.json>");
        }
    ...
    private static void runCase(JsonArray allSteps, int caseIndex, String caseName, JsonArray records) throws Exception {
        ...
        EPRuntime runtime = EPRuntimeProvider.getRuntime("parity-context-keyed-null-keys", configuration);
        ((EPRuntimeSPI) runtime).initialize(0L);
        try {
            String epl;
            if ("null-single-key".equals(caseName)) {
                epl = "@public create context MyContext partition by theString from SupportBean;\n" +
                        "@name('s0') context MyContext select count(*) as cnt from SupportBean";
            } else if ("null-key-multi-key".equals(caseName)) {
                epl = "@public create context MyContext partition by theString, intBoxed, intPrimitive from SupportBean;\n" +
                        "@name('s0') context MyContext select count(*) as cnt from SupportBean";
            } else {
    ...
    private static void send(EPRuntime runtime, JsonObject step) {
        ...
        if (payload.get("theString").isNull()) {
            event.put("theString", null);
        } else {
            event.put("theString", payload.getString("theString", ""));
        }
        JsonValue intBoxedValue = payload.get("intBoxed");
        if (intBoxedValue == null || intBoxedValue.isNull()) {
            event.put("intBoxed", null);
        } else {
            event.put("intBoxed", intBoxedValue.asInt());
        }
