...
usage() {
    cat >&2 <<'EOF'
usage: run-csv-input-adapter.sh --esper-root PATH --scenario PATH --output PATH [--skip-build]
...
esperio-csv module classes are reused from its existing Maven target/classes;
only the compiler/runtime test-compile step may run unless --skip-build.
EOF
    exit 2
}
...
[ -d "$esper_root/esperio/esperio-csv/target/classes" ] || {
    echo "esperio-csv classes are missing; building esperio-csv" >&2
    "$mvn_bin" -q -f "$esper_root/esperio/pom.xml" -pl esperio-csv -am test-compile         -DskipTests=true -Dcheckstyle.skip=true -Dgpg.skip=true         -Dfile.encoding=UTF-8 -Dproject.build.sourceEncoding=UTF-8         -Dproject.reporting.outputEncoding=UTF-8 -Duser.timezone=UTC
}

work=$(mktemp -d "${TMPDIR:-/tmp}/esper-csv-input-adapter.XXXXXX")
cleanup() {
...
if [ -f /tmp/esper-cp.txt ]; then
    base_cp=$(cat /tmp/esper-cp.txt)
else
...
classpath="$classpath:$esper_root/esperio/esperio-csv/target/classes"
classpath="$classpath:$base_cp"

"$javac_bin" -encoding UTF-8 -cp "$classpath" -d "$classes"     "$script_root/CsvInputAdapterScenarioOracle.java"

parent=$(dirname "$output")
mkdir -p "$parent"
"$java_bin" -Dfile.encoding=UTF-8 -Duser.timezone=UTC -Duser.language=en     -Duser.country=US -Duser.variant= -cp "$classpath"     CsvInputAdapterScenarioOracle "$scenario" > "$output"
