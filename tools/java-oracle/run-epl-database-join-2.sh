...
work=$(mktemp -d "${TMPDIR:-/tmp}/esper-epl-database-join-2.XXXXXX")
cleanup() { rm -rf "$work"; }
...
"$javac_bin" -encoding UTF-8 -cp "$classpath" -d "$classes" \
    "$script_root/EPLDatabaseJoin2ScenarioOracle.java"

parent=$(dirname "$output")
mkdir -p "$parent"
"$java_bin" -Dfile.encoding=UTF-8 -Duser.timezone=UTC -Duser.language=en \
    -Duser.country=US -Duser.variant= -cp "$classpath" \
    EPLDatabaseJoin2ScenarioOracle "$scenario" > "$output"

if ! jq -e '.version == "esper-parity/v1" and .id == "epl-database-join-2" and (.records | type == "array") and (.records | length == 11) and
    ([.records[] | select(.case == "2historical-star-inner" and .operation == "count" and .statement == "flow" and .name == "negative-sends" and .count == 3)] | length) == 1 and
    ([.records[] | select(.case == "2historical-star-inner" and .operation == "listener" and (.new | length == 1) and .new[0].kind == "row" and .new[0].fields.a == "B" and .new[0].fields.b == 3 and .new[0].fields.c == "B" and .new[0].fields.d == "B")] | length) == 1 and
    ([.records[] | select(.case == "2historical-star-inner" and .operation == "count" and .statement == "flow" and .name == "negative-sends" and .count == 1)] | length) == 1 and
    ([.records[] | select(.case == "join-index-null-type" and .operation == "count" and .statement == "flow" and .name == "capture-empty" and .count == 0)] | length) == 1 and
    ([.records[] | select(.case == "with-pattern" and .operation == "listener" and .new[0].fields.mychar == "Y")] | length) == 2 and
    ([.records[] | select(.case == "with-pattern" and .operation == "count" and .name == "silent-advances" and .count == 1)] | length) == 1 and
    ([.records[] | select(.case == "variables" and .operation == "listener" and .new[0].fields.myint == 50)] | length) == 1 and
    ([.records[] | select(.case == "variables" and .operation == "listener" and .new[0].fields.myint == 60)] | length) == 1 and
    ([.records[] | select(.case == "3stream" and .operation == "listener" and .new[0].fields.theString == "T2" and .new[0].fields.stringTwo == "T2" and .new[0].fields.myint == 30)] | length) == 1 and
    ([.records[] | select(.case == "3stream" and .operation == "listener" and .new[0].fields.theString == "T3" and .new[0].fields.stringTwo == "T3" and .new[0].fields.myint == 40)] | length) == 1' "$output" >/dev/null 2>&1; then
